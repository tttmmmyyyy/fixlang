...
> 補足: 既存の `ERR_LACKING_TRAIT_IMPL` は `"Lacking implementation of ..."`
> という文面 ([ast/traits.rs:63](src/ast/traits.rs#L63))。本プランでは
> hole 側を **`Missing` で書く** (英語のコンパイラエラーは慣用的に
> `missing X` を使う)。同時に既存の `Lacking implementation of ...` も
> `Missing implementation of ...` に直すと一貫する。エラーコード文字列
> `"lacking-trait-impl"` は LSP の code action 等が参照しているので
> **コード名は温存**、文面だけ差し替える方針 (別タスク扱いで OK)。

#### 文面案

**制約なしの場合**:
```
Missing expression of type `I64`.
```

**制約ありの場合**:
```
Missing expression of type `[a : Show] a`.
```

`[a : Show] a` は **`Scheme::to_string()` が返す既存の表記**
([types.rs:2048-2064](src/ast/types.rs#L2048))。Fix で多相型を出すときの
標準形式なので、ユーザーには馴染みがある。等式制約が残っていれば
`[a : Show, F a = Int] a` のように `,` で並ぶ (これも既存挙動)。
...
(メッセージ文面 `"Missing expression of type ..."` と揃える。
既存の `ERR_LACKING_TRAIT_IMPL = "lacking-trait-impl"` とは文字列キーが
ずれることになるが、`ERR_LACKING_TRAIT_IMPL` 側は LSP code action が
参照しているので別タスクで一緒に直す方が安全 §5.4 補足参照。)

#### 実装メモ

```rust
// 概念コード
fn report_hole(tc: &TypeCheckContext, hole: &Arc<ExprNode>) -> Error {
    // 1. hole の期待型 (substitution 適用済み)
    let ty = tc.substitution.substitute_type(hole.type_.as_ref().unwrap());

    // 2. hole の型が含む自由型変数集合
    let mut free = vec![];
    ty.free_vars_to_vec(&mut free);
    let free_names: Set<Name> = free.iter().map(|tv| tv.name.clone()).collect();

    // 3. 関連する predicates / equalities だけ抽出
    let preds = tc.predicates.iter()
        .filter(|p| {
            let mut buf = vec![];
            p.free_vars_to_vec(&mut buf);
            buf.iter().any(|tv| free_names.contains(&tv.name))
        })
        .cloned().collect();
    let eqs = /* 同様に tc.equalities から抽出 */;

    // 4. 合成スキームを既存の to_string で描画
    let scm = Scheme::new(/*gen_vars*/ vec![], preds, eqs, ty);
    let msg = format!("Missing expression of type `{}`.", scm.to_string());

    // 5. 型変数の出所も付加 (他のエラーで既に使われている helper)
    let extra = tc.create_tyvar_location_messages(&free, None);

    let mut err = Error::from_msg_srcs(msg, &[&hole.source]);
...
- 1 ファイルに hole が複数あれば全て個別の `ERR_HOLE` として報告
  (既存エラーは早期 return が多いが、hole 集計は post-pass なので **複数報告
  可能**)。

#### 下線範囲 (source span) の決め方

hole 自身は zero-width span なので、そのままでは下線が引けない・hover も
当たらない。エラー表示用に **「先行トークン直後 〜 後続トークン直前」** の
範囲を hole の `source` として記録する:

| 状況例                          | 先行           | 後続       | 下線範囲                            |
| ------------------------------ | -------------- | --------- | ---------------------------------- |
| `(let x = 10; )`               | `;`            | `)`       | `;` と `)` の間 (空白 1 文字)       |
| `(let x = 10;)`                | `;`            | `)`       | 空白なしなので **`)` の 1 文字**      |
| `if c { } else { 1 }`          | `{`            | `}`       | `{` と `}` の間                    |
| `if c { 1 } else `(末尾改行)    | `else`         | (EOI)     | `else` の直後〜EOL                  |
| `match x { p => , q => 1 }`    | `=>`           | `,`       | `=>` と `,` の間                   |
| `\|x\| ` + global `;`          | `\|`           | `;`       | `\|` と `;` の間                   |

実装方針:
- 各 `parse_expr_*` 関数で hole を消費する直前に **先行 Pair の `as_span().end()`**
  を保持しておく。
- hole 消費後、次の Pair を peek して **その `as_span().start()`** を取る。
  次が無い場合は親 Pair の `as_span().end()` を使う。
- `start < end` ならその範囲を hole の `source` に。
- `start == end` (隣接トークン同士の間に隙間ゼロ) のときは、後続トークンの
  先頭 1 文字に span を伸ばす (= 下線が必ず 1 文字以上引かれる)。
- 共通化のため `compute_hole_span(prev_end, next_start, fallback_end)` ヘルパー
  を作る。
...
// 期待型 = I64
expect_i64 : I64 = ;
// 期待: "Missing expression of type `I64`."

// 期待型 = String
expect_string : I64 -> String = |x| ;
// 期待: "Missing expression of type `String`."

// 多相: 残った型変数が表示される
expect_polymorphic : a -> a = |x| ;
// 期待: "Missing expression of type `a`."

// 制約付き多相
expect_with_constraint : [a : Show] a -> String = |x| ;
// 期待: "Missing expression of type `String`." (a が消えるので制約も消える)
//   または、x の使い道で制約が伝播するパターンを別途用意。
```

下線範囲の自動チェック (start/end の line/col):
- 「先行トークン直後 〜 後続トークン直前」のルールに沿った位置が出ること。
- 隙間ゼロ時に後続 1 文字に伸びていること。
...
- hole は **常にエラー** (`ERR_HOLE = "missing-expression"`)。
  ...
  warning 運用や二段運用はしない。
- メッセージ文面は **`Missing expression of type \`{scheme}\`.`** に統一
  (§5.4)。`{scheme}` は既存 `Scheme::to_string()` の出力なので、
  制約なしなら `I64`、制約ありなら `[a : Show] a` のように Fix 標準表記で
  揃う。
- hole 自身に対する hover は **非サポート**。必要な情報 (期待型 + 残存制約) は
  `ERR_HOLE` エラーメッセージに全部入っている。
- エラーメッセージの下線範囲は §5.4 のルールで「先行 〜 後続トークン間」を
  計算する (隙間ゼロのときは後続 1 文字に伸ばす)。
