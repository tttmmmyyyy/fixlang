...
### Git 依存テストのための local git repo

Git 依存が絡むケース (E, F, R, T) では実 GitHub リポジトリではなく、
**tempdir 内に `git init` → コミット → `file://` URL で参照** する local git repo を
テスト時に生成する。理由:

- コミット内容 (`preliminary_commands` の値、commit 時点の状態) を完全に制御できる
- ネットワーク非依存で CI でも安定
- 「コミットを進めて内容を変える」「同じリポジトリで複数 rev を試す」等が容易
...
fn make_git_dep_repo(
    ...
    fixproj_toml: &str,
    extra_files: &[(&str, &str)],
) -> (PathBuf /* repo dir */, String /* HEAD commit */);
```

`fix` 本体の lockfile install ロジック
([lockfile.rs:222-236](../../src/dependency/lockfile.rs#L222-L236)) は
`.fix/deps/<dir>` が正しい rev の git repo ならそのまま使うので、テストでは
`file://` URL を lockfile に書いておけば良い。
...
- (K) 対話で `y` → trust store に記録されコマンドが実行される (基本ハッピーパス)
- (L) 複数プロジェクトで `n` → 即中断、残りのプロジェクトのプロンプトは出ない
- (M) root / ローカル依存で `y` 承認後、`preliminary_commands` の内容を変更
    (path-based 承認なので再プロンプトされない)
- (N) `y` 承認後 stderr に `Recorded to <path> (edit the file to revoke).` が出る
- (O) `fix run` でも `fix build` と同等に承認フローが動く
- (P) 破損した TOML の trust store → 警告後、空扱いとして初回プロンプトが出る
- (Q) 複数プロジェクトで一部 approved / 一部 NEW の混在 → approved 一覧 + プロンプト
    の両方が表示される
- (R) プロンプト文面切り替え: Git 依存は "remember for this commit"、root/ローカルは
    "trust this path from now on" が含まれる
- (S) 入力バリデーション: `Y\n` / 空行 / EOF / 想定外文字 すべて `n` 扱い
    (`y` / `o` / `n` だけを受理)
- (T) CHANGED を対話環境で検出 → `y` を押すと新 commit で承認が更新される

(B)(C)(E)(G)(H)(I)(J)(K)(L)(N)(P)(Q)(T) は trust store の内容と stderr を
検査して確認する。
