...
3. **式に wrap 関数を挿入する**
   - 各関数（または trait member）に対して1つの `#wrap` を生成する。trait member の場合も impl ごとではなく method ごとに1つ。
   - `#wrap` の型は **元の関数の実装型全体を domain、opaque化された関数型全体を codomain** とする。
     ...
     trait member の場合も元のスキーム（opaque 以外の制約を含む）から導出する。
     例：`to_iter` なら `#wrap : [c : ToIter, x : Iterator, Item x = Elem c] (c -> x) -> (c -> ?it c)`。
   ...
   - ※ 実装時は名前衝突を避けるため、完全修飾名を使う（例：型コンストラクタは `Std::Iterator::repeat::?it`、wrap は `Std::Iterator::repeat::#wrap`、`ToIter::to_iter::#wrap` 等）。
...
// (2) #wrap の生成（method ごとに1つ。元のスキームの制約をそのまま持つ。）
//     ToIter::to_iter::#wrap : [c : ToIter, x : Iterator, Item x = Elem c] (c -> x) -> (c -> ?it c)
//     （実装は与えない。型チェック専用。）

// (3) 制約を QualPredScheme / EqualityScheme に変換（グローバルな仮定として追加）
//     QualPredScheme: ?it c : Iterator
//     EqualityScheme: Item (?it c) = ToIter::Elem c
//     （?it は to_iter 専用の TyCon なので、?it c が存在する文脈では c : ToIter は既に保証されている。
//       そのため QualPredScheme に条件は不要。）
...
型チェック後、各 impl の `#wrap(impl_expr)` で `#wrap` の domain `(c -> x)` の `x` が具体型に推論される。例えば `impl Array a` の場合、`c = Array a`, `x = ArrayIterator a` となり、これが `?it (Array a)` の具体型。

**検討**：
- 型コンストラクタ `?it c` は `c` に依存する。`Array a` に対しては `?it (Array a)`、他の型に対しては別の内部型を持つ。
- **制約はどの段階で検証されるか？** 各 impl で `#wrap(impl_expr)` の型推論が通るとき、`to_iter` の戻り値型が `Iterator` を実装し `Item` が `Elem c` と一致することが（QualPredScheme / EqualityScheme を通じて）自動的に検証される。wrap のスキームに `c : ToIter` が含まれるため、`Elem c` の解決も機能する。
...
- **型コンストラクタを生成する**
  ...
  - trait member の場合も同様：`{trait}::{member}::#wrap` 例：`ToIter::to_iter::#wrap`（method ごとに1つ）
...
- **trait member の場合も型コンストラクタを生成する**
  ...
  - wrap 関数：`{trait}::{member}::#wrap` 例：`ToIter::to_iter::#wrap`（method ごとに1つ）
  - 型引数：trait の型変数（ここでは `c`）
  - wrap の型は元のスキームから導出する。例：`ToIter::to_iter::#wrap : [c : ToIter, x : Iterator, Item x = Elem c] (c -> x) -> (c -> ?it c)`
...
1. **predicate `?it : Iterator`** → QualPredScheme として保持（条件なし）：

   ```
   QualPredScheme {
       gen_vars: [c],  // trait の型変数
       qual_pred: QualPred {
           conditions: [],  // ?it は to_iter 専用の TyCon なので、?it c が存在する文脈では c : ToIter は既に保証されている
...
2. **equality `Item ?it = ToIter::Elem c`** → EqualityScheme として保持：
   ...
   ※ QualPredScheme も EqualityScheme も条件（Qual）を持たない。`?it` は `to_iter` 専用の TyCon であり、`?it c` が出現する文脈では `to_iter` の型 `[c : ToIter] c -> ?it c` を通じて `c : ToIter` が既に要求されているため。
...
2. **式（定義側）**：実装全体を wrap に渡す。
   ...
   trait member の場合（`to_iter`、各 impl で同じ wrap を挿入）：
   ```
   // ToIter::to_iter::#wrap : [c : ToIter, x : Iterator, Item x = Elem c] (c -> x) -> (c -> ?it c)
   // before
...
trait member では wrap は method ごとに1つだが、各 impl で `#wrap(impl_expr)` が型チェックされる際、`c` が具体型に統一され、`x` が impl ごとの具体型に推論される。結果として impl ごとに異なる具体型テンプレートが得られる（type family 的な構造）：
```
