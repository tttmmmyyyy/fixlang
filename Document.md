...
- Currently, we are not able to tell the debugger the size of an array which is determined at run time. So we are always setting the array size to 32 in the debug information. You cannot show elements indexed after 32, and if the array is shorter than 32, invalid values are shown.
