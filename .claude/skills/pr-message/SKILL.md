...
- **An added item**: its **role**, written the same way, and **what reaches it** — the functions that call an added function, the code that builds an added type, the implementors of an added trait. An item arrives in the body as a loose thing until the reader is told what reaches it; saying so puts it back in the flow it was written for, and for a function it is also what says whether it runs once per program, once per symbol, or once per node of a walk.
