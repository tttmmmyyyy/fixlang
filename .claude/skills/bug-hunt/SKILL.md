...
- **The other two finders run unlensed**, told to report whatever is actually wrong. What keeps them from being one finder run twice is the area split: each owns its own part of the target. Two of them rather than one means the hunt does not rest on a single unlensed reading, and where their areas meet, the second reading measures what the first walked past.
- Comparing the lens's yield against the two unlensed sweeps says whether the lens is helping or narrowing.
...
4. **Launch the three finders in parallel** with the `Agent` tool, in a single block, each with `isolation: "worktree"`, and wait for all three. The isolation is what makes the parallelism sound: the *Evidence Bar* asks a finder to run things, so each one edits sources and builds, and three of them sharing one tree would revert each other's probes and read each other's edits as their own. It is also the hunt's largest cost — each worktree compiles its own `target/` — and the reason the fan-out stays at three.
   ...
   Brief each with: the target and its areas, its lens (or, for the two unlensed ones, the instruction to report whatever is actually wrong in the areas they own), the *Evidence Bar*, the *Techniques That Found Bugs* section, the dismissed candidates from the log, and the rule that it reverts every probe before returning, so its worktree is removed for it. A hypothesis test that passes is handed back in the report — its full body, and where in the suite it belongs — before that revert, since the report is the only thing that outlives the worktree. Each returns candidates (file, symbol, claim, failing scenario, and whether the evidence is executed or traced) and, separately, the passing hypothesis tests it wrote.
...
7. **Critique the coverage.** With all three reports in hand, name the classes of bug this wave could not have surfaced, whatever its yield — what the lens looked past, and what neither unlensed sweep had a way to reach. That answer goes in the report and becomes the strongest candidate lens for the next hunt.
...
#### Scale the input along the axis a pass is keyed on, and measure what it emits

A pass that produces one output per key — a specializer cloning a function per input summary, a monomorphizer per type instantiation, a table per distinct shape — has a key space, and the question no correctness probe asks is what bounds it. Differential runs, sanitizers and invariant assertions all read the *answer*; they are silent by construction when every answer is right and there are simply 3^n of them. So generate a family of inputs that grows along the pass's own parameter — one more argument, one more level of a call chain, one more variant — and plot what comes out: compile time, the count of items the pass emitted, the size of the artifact. A straight line on the baseline compiler and a doubling on the new one is the finding, and it needs no expected output.
...
- One line per hunt: date, target, commit, the lens, the two unlensed areas, and what each returned. A lens that has now returned nothing twice is spent — retire it, and say so on the line.
...
- Don't give the two unlensed finders an angle between them, whether by naming one or by describing the target so that only one reading is open. Their job is the reading the lens is not doing.
