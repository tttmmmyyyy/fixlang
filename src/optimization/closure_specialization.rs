...
/// How many combining copies a function is allowed before any other function has asked for one.
///
...
/// How many more combining copies each function that asks for one brings with it.
...
struct ClosureSpecializationVisitor {
    ...
    /// The symbol this walk is over, under the name it carries now. Every name this walk mints is
    /// formed from it.
    current_symbol: FullName,
    /// The global names already taken. A name minted here is one they leave free, and joins them.
