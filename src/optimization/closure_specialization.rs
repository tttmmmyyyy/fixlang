...
// `(origin of the function, argument position, origin of the lambda)`. The value is the one lifted
// lambda that key may be specialized on: meeting the same key with a different lambda is what
// stops a recursion that wraps its closure argument on every round from asking for a copy per
// round. Every specialized copy carries the table of the request that created it, so the outer
// loop walking that copy again continues the same chain rather than starting a fresh one.
type Pinned = Map<(FullName, usize, FullName), FullName>;
...
fn specializable_functions(prg: &Program) -> Map<FullName, SpecializableFunctionInfo> {
    ...
    while let Some(idx) = queue.pop_front() {
        ...
        let specializable_arg_indices = specializable_params_of(sym, &specializable_funcs);
...
impl ClosureSpecializationVisitor {
    ...
    // Commit the chain reaching here to specializing argument `idx` of the function whose origin is
    ...
    fn commit(
        ...
        let key = (func_origin.clone(), idx, origin_of(&self.origins, lam_name));
        ...
        for (i, arg) in args.iter().enumerate() {
            ...
            if arg.is_var() {
                ...
                if let Some(decap_info) = self.local_decap_lambdas.get(arg_name) {
                    ...
                    if !self.commit(&mut pinned, &func_origin, i, &decap_info.lambda_func_name) {
                        continue;
                    }
                    decaptured_args[i] = arg.set_type(decap_info.cap_list_ty.clone());
                    specialized_args.insert(i, decap_info);
                }
                ...
                let (decap_info, expr) = self.decapture_lambda(arg.clone(), state); // Visits `arg` inside this call
                                                                                    ...
                                                                                    // only records it.
                self.commit(&mut pinned, &func_origin, i, &decap_info.lambda_func_name);
                specialized_args.insert(i, decap_info);
