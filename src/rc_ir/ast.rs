...
pub enum RcExpr {
    ...
    Retain(RcVar, Path, RcState, RcExprNode),
...
/// The ownership of a single boxed leaf. `Own` receives ownership: the callee consumes it (by
...
/// The ownership of one argument, shaped like the value: each boxed leaf is `Own` or `Borrow`.
