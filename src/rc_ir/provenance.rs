...
//! owned), `Unknown` (of unknown sharing — read out of a boxed container, a global, or the result of a
...
//! uniqueness of its actual inputs (`Fresh` resolves to unique, `Unknown` to dynamic); that resolution
//! and the elimination it drives are a later pass, so this pass only computes provenance.
//!
//! `Retain` is the only operation that demotes a leaf (`Fresh`/`Arg` to `Unknown`): duplicating a
//! reference makes the value shared. Everything else preserves provenance. A boxed container's
//! contents are not tracked (reading an element yields `Unknown`); an unboxed aggregate's children are
...
use crate::rc_ir::leaf_map::{LeafKey, LeafMap};
use std::sync::Arc;

/// The origin of one boxed leaf.
// PROOF: D/A, P3, P4, P5, P6, P7, P7a, P7d, P7e, P27, P29, P30, P31 (dev-docs/proof/rc_ir/borrow-cancel)
...
// PROOF: P1, P2, P31 (dev-docs/proof/rc_ir/borrow-cancel)
...
impl Provenance {
    ...
    pub fn build_shape(
        ty: &Arc<TypeNode>,
        type_env: &TypeEnv,
        leaf: &dyn Fn(&FieldPath) -> LeafOrigins,
    ) -> Provenance {
        Provenance(LeafMap::build_shape(ty, type_env, leaf))
    }
    ...
    fn join(&self, other: &Provenance) -> Provenance {
        Provenance(
            self.0
                .zip_with(&other.0, "provenance", |a, b| a.union(b).cloned().collect()),
        )
    }
    ...
    /// `Unique`.
    // PROOF: P1, P2, P2a, P3, P4, P5, P6, P7, P7a, P7c, P7d, P7e, P7f, P8, P9, P10, P11, P12, P13, P14, P14a, P14b, P15, P16, P17, P18, P18a, P18b, P18c, P19, P20, P21, P22, P23, P24, P31 (dev-docs/proof/rc_ir/borrow-cancel)
...
fn leaf_source_to_string(origins: &LeafOrigins) -> String {
    ...
    let mut parts: Vec<String> = origins
        ...
        .map(|s| match s {
            ...
            LeafOrigin::Unknown => "unknown".to_string(),
            LeafOrigin::Arg(i, path) => {
                let p = path.iter().map(|i| format!(".{}", i)).collect::<String>();
                format!("arg{}{}", i, p)
            }
        })
...
/// against the uniqueness of a function's inputs: `Fresh` resolves to `Unique`, `Unknown` to `Dynamic`,
...
/// The resolved uniqueness of a whole value: the `SharingVerdict` of each of its boxed leaves,
/// mirroring `Provenance` with a verdict in place of each leaf source.
#[derive(Clone, PartialEq, Eq, Hash, Debug)]
...
impl Uniqueness {
    ...
    pub fn all_dynamic(ty: &Arc<TypeNode>, type_env: &TypeEnv) -> Uniqueness {
        Uniqueness(LeafKey::uniform(ty, type_env, SharingVerdict::Dynamic))
    }
}
...
pub fn resolve(prov: &Provenance, inputs: &[Uniqueness]) -> Uniqueness {
    Uniqueness(prov.0.to_key(|origins| resolve_leaf(origins, inputs)))
}
...
impl<'a> Interpreter<'a> {
    ...
    fn interpret_match(
        ...
        for arm in arms {
            ...
            let payload_prov = match arm.tag {
                Some(tag) => {
...
mod tests {
    ...
    fn agg(children: Vec<Provenance>) -> Provenance {
        ...
        for (i, child) in children.iter().enumerate() {
            for (path, origins) in child.0.iter() {
                let mut p = vec![i];
    ...
    fn to_string_renders_leaves() {
        ...
        assert_eq!(
            agg(vec![fresh(), unknown()]).to_string(),
            "{.0=fresh, .1=unknown}"
        );
    }
    ...
    fn join_envs_is_pointwise_and_keeps_one_sided_bindings() {
        ...
        b.insert(x.clone(), unknown());
        b.insert(z.clone(), unknown());

        let joined = join_envs(&a, &b);
        // A variable present on both sides is joined.
        assert_eq!(joined[&x], fresh().join(&unknown()));
    ...
    fn resolve_keys_only_the_boxed_leaves() {
        ...
        assert_eq!(
            ...
            uniq(vec![(vec![1], SharingVerdict::Unique)])
        );
    }
}
