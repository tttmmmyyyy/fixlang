...
use crate::constants::{
    ...
    DYNAMIC_OBJ_TRAVARSER_IDX, REFCNT_STATE_GLOBAL, REFCNT_STATE_LOCAL, STD_NAME, STORAGE_BUF_IDX,
...
pub fn shared_empty_array_storage<'c, 'm>(
    ...
    let global = match gc.module.get_global(&name) {
        ...
        None => {
            ...
            // The block sits at the base of its own storage, so the distance recorded for
            // `build_free_boxed` to recover that base is zero.
            let control_block = control_block_type(gc).const_named_struct(&[
                ...
                refcnt_state_type(gc.context)
                    ...
                    .into(),
                alloc_offset_type(gc.context).const_zero().into(),
            ]);
            let buf = struct_ty
