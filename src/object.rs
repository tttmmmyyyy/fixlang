...
pub fn write_alloc_offset<'c, 'm>(
    ...
    let alloc_offset = gc
        ...
        .unwrap();
    gc.build_store(MemoryRegion::AllocOffset, ptr_to_alloc_offset, alloc_offset);
}
        ...
        match field_ty {
            ObjectFieldType::ControlBlock => {
                ...
                gc.build_store(
                    MemoryRegion::Refcnt,
                    ptr_to_refcnt,
                    refcnt_type(context).const_int(1, false),
                );
...
pub fn get_dynamic_traverser_ptr<'c, 'm>(
    ...
    match create_traverser(ty, capture, gc, None, RcState::Unknown) {
        ...
        None => {
            ...
            let func = if let Some(func) = gc.module.get_function(EMPTY_TRAVERSER_NAME) {
                func
            } else {
