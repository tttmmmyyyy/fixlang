...
use super::util::{find_local_occurrences, find_member_def_src, find_trait_or_alias_def_src, find_tycon_def_src, get_current_dir, resolve_source_pos, span_to_location};
    ...
    if let Some(var_name) = var_name {
        ...
        match node {
            ...
            EndNode::Field(tc, name) | EndNode::Variant(tc, name) => {
                def_src = find_member_def_src(program, &tc, &name);
            }
            EndNode::ValueDecl(_) => {
                unreachable!()
            }
