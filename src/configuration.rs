...
impl Configuration {
    ...
    fn cache_hash_sources(&self) -> CacheHashSources {
        ...
        // and `build_assert_declared_passthrough_answers_the_operand`, and the arm a dynamic
        // traverser takes when it is called with work it does not know. The same reasoning as the
        // sanitizer's applies: an object built without those checks cannot stand in for one built
        // with them.
    ...
    fn test_object_generation_hash_separates_code_generation_settings() {
        let settings: Vec<(&str, Box<dyn FnOnce(&mut Configuration)>)> = vec![
            ...
            (
                "valgrind_tool",
                Box::new(|config: &mut Configuration| {
                    config.valgrind_tool = if config.valgrind_tool == ValgrindTool::None {
                        ValgrindTool::MemCheck
                    } else {
                        ValgrindTool::None
                    }
