...
impl LLVMGenerator {
    pub fn generate<'c, 'm, 'b>(
        ...
        let obj = match self {
            ...
            LLVMGenerator::StringLit(x) => Some(x.generate(gc, ty)),
            ...
            LLVMGenerator::ArrayModBody(x) => Some(x.generate(gc, ty)),
            ...
            LLVMGenerator::StructModBody(x) => Some(x.generate(gc, ty)),
            ...
            LLVMGenerator::UnsafeMutateBoxedDataFunctionBody(x) => Some(x.generate(gc, ty)),
            LLVMGenerator::UnsafeMutateBoxedDataIOStateFunctionBody(x) => Some(x.generate(gc, ty)),
