...
impl LLVMGenerator {
    pub fn generate<'c, 'm, 'b>(
        ...
        match self {
            ...
            LLVMGenerator::UnsafeSetArrayBody(x) => x.generate(gc, ty, rvo, bvs),
            LLVMGenerator::UnsafeGetArrayBody(x) => x.generate(gc, ty, rvo, bvs),
            LLVMGenerator::UnsafeSetSizeArrayBody(x) => x.generate(gc, ty, rvo, bvs),
            ...
            LLVMGenerator::AbortFunctionBody(x) => x.generate(gc, ty, rvo, bvs),
            ...
            LLVMGenerator::GetPtrOfBoxedValueFunctionBody(x) => x.generate(gc, ty, rvo, bvs),
