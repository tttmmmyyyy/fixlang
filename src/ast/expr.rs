...
impl ExprNode {
    ...
    // Set source for the auxiliary purpose (e.g., parameter of a lambda expression or type constructor name of a MakeStruct expression).
    #[allow(dead_code)]
    pub fn set_aux_src(&self, src: Option<Span>) -> Arc<Self> {
        let mut ret = self.clone_all();
        ret.aux_src = src;
        Arc::new(ret)
    }
    ...
    pub fn get_ffi_call_args(&self) -> Vec<Arc<ExprNode>> {
        match &*self.expr {
            Expr::FFICall(_, _, _, _, args, _) => args.clone(),
            _ => {
                panic!()
            }
        }
    ...
    pub fn set_let_value_typed(&self, value: Arc<ExprNode>) -> Arc<Self> {
        let value_ty = value.type_.as_ref().unwrap().clone();
        self.set_let_value(value).set_type(value_ty)
    }
    ...
    pub fn set_if_then_else_typed(
        ...
        let then_ty = then.type_.as_ref().unwrap().clone();
        let else_ty = else_.type_.as_ref().unwrap().clone();
        assert_eq!(then_ty.to_string(), else_ty.to_string());
        self.set_if_then_else(then, else_).set_type(then_ty)
    }
    ...
    pub fn set_match_pat_vals_typed(
        ...
        let vals_tys = pat_vals
            .iter()
            .map(|(_, val)| val.type_.as_ref().unwrap().clone())
            .collect::<Vec<_>>();
        if vals_tys.len() > 0 {
    ...
    pub fn set_ffi_call_args(&self, args: Vec<Arc<ExprNode>>) -> Arc<ExprNode> {
        let mut ret = self.clone_except_fvs();
        match &*self.expr {
            Expr::FFICall(fun_name, ret_ty, param_tys, is_va_args, _, is_io) => {
                ret.expr = Arc::new(Expr::FFICall(
                    fun_name.clone(),
                    ret_ty.clone(),
                    param_tys.clone(),
    ...
    pub fn global_to_absolute(&self) -> Arc<ExprNode> {
        ...
        node.expr = match &*self.expr {
            ...
            Expr::MakeStruct(tc, fields) => {
                ...
                for (_, _, expr) in new_fields.iter_mut() {
                    *expr = expr.global_to_absolute();
                }
            ...
            Expr::FFICall(name, ret_ty, param_tys, is_va_args, args, is_ios) => {
...
impl Expr {
    ...
    pub fn stringify(&self) -> Text {
        match self {
            ...
            Expr::FFICall(fun_name, _, _, is_va_args, args, is_ios) => {
                let mut arg_texts: Vec<Text> = vec![];
...
pub fn expr_abs_param_src(
    ...
    Arc::new(Expr::Lam(vars, val)).into_expr_node_with_aux_src(src, param_src)
}
...
pub fn expr_abs_typed(var: Arc<Var>, var_ty: Arc<TypeNode>, val: Arc<ExprNode>) -> Arc<ExprNode> {
    ...
    let abs_ty = type_fun(var_ty, val.type_.as_ref().unwrap().clone());
    expr.set_type(abs_ty)
}
...
pub fn expr_app_typed(lam: Arc<ExprNode>, args: Vec<Arc<ExprNode>>) -> Arc<ExprNode> {
    let dst_ty = lam.type_.as_ref().unwrap().get_lambda_dst();
...
pub fn expr_if_typed(
    ...
    let then_ty = then_expr.type_.as_ref().unwrap().clone();
...
pub fn expr_match_typed(
    ...
    expr_match(cond, cases, None).set_type(vals_tys[0].clone())
}
...
pub fn expr_ffi_call(
    ...
    Arc::new(Expr::FFICall(
        fun_name, ret_ty, param_tys, is_va_args, args, is_ios,
    ))
    .into_expr_node(src)
}
