...
impl PatternNode {
    ...
    pub fn resolve_namespace(
        ...
        match &self.pattern {
            ...
            Pattern::Struct(tc, field_to_pat) => {
                ...
                for (field_name, name_src, pat) in field_to_pat {
                    field_to_pat_res.push((
                        field_name.clone(),
                        name_src.clone(),
                        pat.resolve_namespace(ctx)?,
                    ));
                }
                Ok(self
            ...
            Pattern::Struct(tc, field_to_pat) => {
                ...
                for (field_name, name_src, pat) in field_to_pat {
                    field_to_pat_res.push((
                        field_name.clone(),
                        name_src.clone(),
                        pat.resolve_type_aliases(type_env)?,
                    ));
                }
        ...
        match &self.pattern {
            ...
            Pattern::Struct(tc, field_to_pat) => {
                ...
                let new_field_to_pat = field_to_pat
                    .iter()
                    .map(|(name, name_src, pat)| {
                        (name.clone(), name_src.clone(), pat.global_to_absolute())
                    })
                    .collect();
    ...
    // Construct a struct destructuring pattern with no per-field-name source
    // spans. Used by non-parser callers (built-ins, optimizations).
    pub fn make_struct(
        tycon: Arc<TyCon>,
    ...
    // Construct a struct destructuring pattern with per-field-name source
    // spans. Used by the parser.
