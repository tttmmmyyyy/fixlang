...
impl TypeEnv {
    ...
    pub fn unwrap_newtype_tycons(&mut self) {
        ...
        for (name, _tycon_info) in new_tycons.iter() {
            if self.is_unwrappable_newtype(name) {
    ...
    pub fn is_newtype_pattern(&self, tycon: &TyCon) -> bool {
        let ti = self.tycons.get(tycon).unwrap();
        ti.is_unbox && ti.variant == TyConVariant::Struct && ti.fields.len() == 1
    }

    // Is this type constructor a newtype pattern and unwrappable?
    pub fn is_unwrappable_newtype(self: &TypeEnv, tycon: &TyCon) -> bool {
        let mut visited: Set<TyCon> = Set::default();
        self.is_unwrappable_newtype_internal(tycon, &mut visited)
    }

    // Is this type constructor a newtype pattern and unwrappable?
    fn is_unwrappable_newtype_internal(&self, tc: &TyCon, visited: &mut Set<TyCon>) -> bool {
        // If this TyCon is not a newtype pattern, return false.
        if !self.is_newtype_pattern(tc) {
            return false;
        }
        ...
        let ti = self.tycons.get(tc).unwrap();
        ...
        let result = referenced_tycons.iter().all(|referenced_tycon| {
            if !self.is_newtype_pattern(referenced_tycon) {
            ...
            self.is_unwrappable_newtype_internal(referenced_tycon, visited)
        });
