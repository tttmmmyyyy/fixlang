...
/// The substitution taking each type variable of a trait member's declared scheme `defn_scm` to
/// what the implementation whose scheme is `impl_scm` puts in its place. For `trait c : ToIter`
/// with `to_iter : [?it : Iterator, Item ?it = Elem c] c -> ?it` and an implementation for
