...
```
MODE=cachegrind bash build.sh && bash cachegrind.sh   # instruction counts, load-independent
MODE=native     bash build.sh && BIN=bin_native python3 wall.py   # wall time, needs an idle machine
```
