...
## 92d503dcc51a8a816e2aa5c58267a7a7754e62de

「共有だと観測してから行動する」が原子的でない件 (#461) の修正を、fork point の `fffebab4` と並べて
測った。両方の行がこの下に在る。

**命令数の動いた 22 ケースのうち、ゆらぎでないのは 1 つだけ。** `cp_lib_scc` が **-299,840 命令
(-0.20%)**、メモリ参照も -299,227。`Std::Destructor` の解放から、減算の前に置かれていた一意性の検査
(状態の load と比較と分岐) が消えた分である。残る 21 ケースの差は最大 243 命令で、`startup` を含む
...
**サイクル列は両方の行に残ったのが 6 ケースしかない** (この行を測ったとき他の仕事が 11.27 コアを
使っていた)。その 6 ケースは最大 ±0.59% で、どれもこの変更が触れる経路を持たない。
