...
`add_offset` で与えて答えを `subtract_ptr` で読む — バイトを走査する正規表現の実装が取る形 — を
1,048,576 回回すと、命令数は 199,526,004 から 189,040,242 へ **-5.255%**、**呼び出し 1 回あたり
10.0 命令**である。サイクルも同じ向きに動く (42,005,850 -> 39,691,198、静かな機械で 5 回の最小値)。
