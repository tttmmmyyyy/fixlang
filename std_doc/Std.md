...
### `type SplitIterator = unbox struct { ...fields... }`
...
For details, see the `to_dyn` function.
...
### `impl Std::String::SplitIterator : Std::Iterator::Iterable`
...
### `from_iter : [iter : Std::Iterator::Iterable, Std::Iterator::Iterable::Item iter = a] iter -> Std::Array a`
...
### `find_last : [iter : Std::Iterator::Iterable, Std::Iterator::Iterable::Item iter = a] iter -> Std::Option a`
...
### `fold : [iter : Std::Iterator::Iterable, Std::Iterator::Iterable::Item iter = a] s -> (s -> a -> s) -> iter -> s`

Fold the elements of an iterator from left to right.

Conceptually, `[a0, a1, a2, ...].fold(s, op) = s.op'(a0).op'(a1).op'(a2)...` where `op'` is the `op` with the order of arguments reversed.

### `fold_m : [m : Std::Monad, iter : Std::Iterator::Iterable, Std::Iterator::Iterable::Item iter = a] s -> (s -> a -> m s) -> iter -> m s`
...
### `loop_iter : [iter : Std::Iterator::Iterable, Std::Iterator::Iterable::Item iter = a] s -> (s -> a -> Std::LoopState s s) -> iter -> s`
...
### `loop_iter_m : [m : Std::Monad, iter : Std::Iterator::Iterable, Std::Iterator::Iterable::Item iter = a] s -> (s -> a -> m (Std::LoopState s s)) -> iter -> m s`
...
The advantage of `DynIterator` is that it hides how the iterator is constructed and simplifies the type.
This allows, for example,
- `DynIterator` can be an instance of traits such as `Monad`, `Eq`, etc.
- In a function, it is possible to return two iterators with different construction methods depending on the branch.
...
### `singleton : a -> Std::Iterator::DynIterator a`
...
### `join : [strs : Std::Iterator::Iterable, Std::Iterator::Iterable::Item strs = Std::String] Std::String -> strs -> Std::String`
...
### `split : Std::String -> Std::String -> Std::String::SplitIterator`
