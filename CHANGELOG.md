...
- An arithmetic operation on a signed integer type (`I8`, `I16`, `I32`, `I64`) now assumes that its mathematical result falls within the range of that type, and a program that performs one whose result falls outside it has no defined behavior. The compiler passes that assumption to the code generator, which lets it fold a check whose outcome the assumption settles. Unsigned integer types are unchanged: an operation on `U8`, `U16`, `U32` or `U64` is taken modulo two to the width of the type, so use one of those where wrapping is what is wanted.
