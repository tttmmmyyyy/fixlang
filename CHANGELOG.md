...
- A loop written with monadic binds no longer consumes a stack frame per iteration at `-O basic` when its result is wide. A result that does not fit in the target's return registers used to be returned through a pointer the backend introduced on its own, and the backend then compiled the loop's tail calls as ordinary calls. The compiler now passes that pointer explicitly, so those tail calls become jumps. An `Array` (or a `String`) already fills the three integer registers x86-64 returns values in, so an ordinary result such as `(Array a, I64)` — or any `IOFail a`, whose `Result ErrMsg a` carries a `String` — crossed the limit. `-O none` deliberately keeps every call as a stack frame and is unchanged.
