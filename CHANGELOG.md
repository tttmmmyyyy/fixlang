...
- #418: A program that takes the payload out of one unboxed union value twice now runs correctly at `-O max` and `-O experimental`. Where the payload holds two or more boxed values — `|e| e.as_some.@0 == h && e.as_some.@1 == k` on an `Option (U64, Array U8, Array U8)` — the second extraction freed what the first had already freed, and the program went on to read and write memory that was no longer its own, usually aborting in `malloc()`. A payload holding one boxed value in two of its fields was freed the same way. `-O none` and `-O basic` were unaffected.
