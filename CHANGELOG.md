...
- Every help message is now headed by the compiler's version, as `fix 1.5.0 (92a3989)` — the released version, followed by the revision the binary was built from — and `fix --version` and `fix -V` report the same line, which used to be an error. The line under the header says what `fix` is.
...
- A struct literal that gives one field twice is now reported as an error, instead of aborting the compiler. `S { a : 1, a : 2 }` on `type S = struct { a : I64 };` aborted with `called 'Option::unwrap()' on a 'None' value`, and so did any literal that names every declared field and repeats one of them; a repeat that left another field out was reported as the missing field. The report points at the repeated name and carries the first occurrence of it, a struct pattern that repeats a field (`let S { a : x, a : y } = s;`) is reported the same way and now names the field, and a literal that is wrong in several ways at once reports all of them in one compilation.
