...
- #373: A program that reads an unboxed union out of a value a function only borrows now runs correctly at `-O max` and `-O experimental`, instead of freeing what the union carries twice. Handing a function a struct that carries an unboxed union with a boxed variant, reading the union out of it and looking at it — `let action = node.@action; if action.is_wait { ... }`, where the structs come out of an array — built a program that read memory it had already freed, and crashed. `-O none` and `-O basic` were unaffected.
