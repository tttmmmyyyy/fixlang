...
- A struct literal that gives one field twice is now reported as an error, instead of aborting the compiler. `S { a : 1, a : 2 }` on `type S = struct { a : I64 };` aborted with `called 'Option::unwrap()' on a 'None' value`. The report points at the repeated name and carries the first occurrence of it, a struct pattern that repeats a field (`let S { a : x, a : y } = s;`) is reported the same way and names the field, and a literal that is wrong in several ways at once reports all of them in one compilation.
