...
- LSP: The language server now keeps up with fast typing on a slow machine. It skips the requests the editor has already cancelled, such as the completion requests of earlier keystrokes, where it used to work through each half-typed state after the typing stopped and show a parse error for each.
### Fixed
