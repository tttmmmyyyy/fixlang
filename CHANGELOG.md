...
- #608: A build no longer aborts with `LLVM ERROR: Broken module found, compilation aborted!` over a global value's accessor. The initializer of a global that one compilation unit alone reads travels to that unit, and the reads it performs travel with it; the storage of a global it read stayed with the unit those reads came from, so the unit computing the initializer called an accessor no unit defined. Which unit keeps a value is now decided once the initializers are where they belong. Where the unit boundaries fall decided whether a program met this, so the same program built at one `--cu-size` and aborted at another.
