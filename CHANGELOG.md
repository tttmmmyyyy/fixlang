...
- The compiler no longer hangs on a program where a chain of global definitions naming each other comes back to where it started, such as `a = b; b = c; c = a;`. This affected `-O max` and above.
