...
- `Array::mutate_elements` documents a rule its callers already had to keep: a global value's initializer has to produce a graph of objects that nothing outside Fix holds a reference to. A callback that stores into the array a boxed value made elsewhere is one way to violate it.
