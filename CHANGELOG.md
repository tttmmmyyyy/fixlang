...
- #477: A traversal of an array's element buffer now stops at once where the count of elements it is given has come out negative. It stopped when its counter reached the count, so a negative one was never reached and the traversal walked the heap without bound. A count arrives negative only where a program has already broken a contract -- an `_unsafe_` primitive called outside its range, or an out-of-range index in a build made with `--no-runtime-check` -- so what changes is that one wrong access stays one wrong access instead of becoming an unbounded scan.
