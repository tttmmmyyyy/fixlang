...
- #211, #400: `--emit-llvm`, `--emit-rc-ir` and `--emit-symbols` now write their dumps however much of the build is cached. A build that had already been made in that directory answered from its object files and wrote nothing, silently: the exit status was 0, and a dump an earlier build had left in place was read as this build's. A dump of `-O basic` was read as the RC IR of `-O max` that way, which is the form every optimization the level runs is looked at in. A build asked for a dump now generates the code the dump is written from.
- #256, #434: `--max-cu-size`, and the `max_cu_size` field of the project file, are now honored on every build. Only the first build in a directory divided itself as asked; every later one reported `Using cached object files.` and kept the division the first had made.
- #292: A build now generates its own object files where a previous build's were made for another CPU. The CPU a build compiles for is the one the machine it runs on has, and the cache carried no record of it, so a `.fixlang` directory reaching a second machine — copied, baked into an image, shared over a network home — handed that machine object files holding instructions its CPU may lack.
- #286, #287, #478: A build no longer reuses object files generated under settings that change the code they hold. `--emit-symbols` renames the symbols of the program, which a backtrace of the built program shows, and a build made for working on the compiler carries the compiler's own consistency checks.
- #285: `fix build` and `fix run` now share their object files, so running one after the other in a directory compiles the program once. They generate the same code — the entry point of a `fix test` build is what differs between the subcommands — and each used to compile the whole program again.
