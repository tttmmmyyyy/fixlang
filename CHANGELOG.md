...
- A program that uses a type whose unboxed fields nest without end is now rejected with an error naming the types on the way, instead of exhausting the compiler's stack. The fields may lead back to the same type (`type A = unbox struct { b : B, n : I64 }; type B = unbox struct { a : A, m : I64 };`) or to an ever larger one (`type P a = unbox struct { x : P (a, a), n : I64 };`); either way a value of such a type has no size. Change one of the types on the way to boxed, which bounds the layout at a pointer.
