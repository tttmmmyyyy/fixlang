...
- `-O max` and `-O experimental` now build incrementally. They used to compile the whole program as one piece, so an edit to one line cost as much as a build from nothing; they now divide the program into compilation units, cache each unit, and regenerate only the units whose symbols changed. The units are merged into one LLVM module before the optimization runs, so the program built this way is the one whole-program compilation produces, and `--cu-size` now decides the size of a unit at these levels too.
