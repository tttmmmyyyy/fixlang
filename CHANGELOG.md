...
- #319, #379: `FFI_EXPORT` now rejects a C function name the compiler writes the body of — `main` and any name beginning with `fixruntime_` — instead of silently renaming one of the two functions that end up under the name. A program exporting `main` used to build into one that never ran its `Main::main`. `FFI_CALL` now rejects `main` for the same reason: the compiler writes that function, so naming it takes it away. Every other name stays the program's to take, the C library functions the program itself calls included; exporting one of those replaces it for the whole process, as linking an object file that defines it does, and the FFI section of `Document.md` says what that costs.
