...
- #424: A function that updates one value under a condition, over and over, now compiles at `-O max` and `-O experimental` in a time that grows with the number of updates. `let p1 = if cs.@(0) { Pair { a : p0.@b, b : p0.@a } } else { p0 };` written sixteen times in a row took thirteen seconds to build, and each further line doubled that, while `-O basic` built the same program in under a second. The same growth reached a chain of `q.mod_a(...)` under a condition, which is how a program threads a value through the conditions that update it.
