...
- #619: `F64::to_string` and `F32::to_string` now write the fewest digits that tell the number apart from the values on either side of it, where they used to write the six places `%f` gives by default. `1.0 / 3.0` was written as `0.333333`, which reads back as another number, and `1.0e-10` as `0.000000`, which carries none of the value; they are now `0.3333333333333333` and `1e-10`. The digits are written positionally where the point falls near them — `1.0`, `0.1`, `123456.789` — and as a power of ten otherwise, so `1.0e300` takes 5 bytes where it used to take 308. A program that shows a floating point number therefore shows a different text than it did. `to_string_precision` writes the places it is asked for, as before, and `to_string_exp` the exponential form.
