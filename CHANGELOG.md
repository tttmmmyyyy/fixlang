...
- #619: `F64::to_string` and `F32::to_string` now write the fewest digits that read back as the same number, where they used to write the six places `%f` gives by default: `1.0 / 3.0` is `0.3333333333333333` and `1.0e-10` is `1e-10`, where they were `0.333333` and `0.000000`. The digits are written positionally where the point falls near them — `1.0`, `0.1`, `123456.789` — and as a power of ten otherwise, so `1.0e300` is `1e300`. A program that shows a floating point number shows a different text than it did. `to_string_precision` and `to_string_exp` write what they wrote before.
