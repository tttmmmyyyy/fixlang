...
- Building with debug information (`-g`) no longer overflows the compiler's stack on a program that uses a recursive type, such as `type Tree = box union { leaf : (), node : (Tree, Tree) };`. A type's debug information is now recorded once and shared, so a reference that cycles back to the type resolves to that shared record; previously each reference expanded the type afresh, and a self-referential type expanded without end.
