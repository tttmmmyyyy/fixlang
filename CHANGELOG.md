...
- `fix run` and `fix test` now build an executable and run it, whatever `output_type` and `output` in the project file ask a build to produce. A project with `output_type = "dylib"` made both die with signal 11, and in a project naming its output file with `output`, `fix test` wrote the test binary there over the program `fix build` had produced. To keep the executable a run or a test built — to open it in a debugger, say — name it with `-o`.
