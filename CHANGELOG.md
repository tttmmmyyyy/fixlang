...
- #598: The compiler is now built against LLVM 22.1.x, where it used to be built against LLVM 17.0.x. Building it from source needs that release, with `LLVM_SYS_221_PREFIX` naming where it lives; the "Build from source" section of `Document.md` carries the commands for Linux and macOS. The programs the compiler builds change in speed either way: of the fifty LangArena benchmarks, ten run at least 3% faster and six at least 3% slower.
