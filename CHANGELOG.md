...
- #638: Added the `--check-signed-overflow` compiler option and the `check_signed_overflow` field of the project file, which stop the program at an arithmetic operation on a signed integer type whose mathematical result leaves the range of that type. The report names the operation and both operands: `Signed integer overflow: I64 addition, with 9223372036854775807 and 1`. The operations it covers are `+`, `-`, `*`, unary `-`, `/` and `%`; an unsigned operation is taken modulo two to the width of its type, so none of those is checked.
