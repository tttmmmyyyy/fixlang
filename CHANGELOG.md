...
- `FFI::boxed_from_retained_ptr` documents one more condition on its use: the initializer of a global value has to produce a graph of objects that nothing outside Fix holds a reference to, so a value restored from a retained pointer may not be brought into one. Only code that already goes through the unsafe pointer primitives can violate it; nothing expressible in pure Fix is lost.
