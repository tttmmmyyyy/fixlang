...
- #391: An absolute path now reaches a value of another module without the module being imported, whether the value's name begins with `_` or is a struct field's `@`-headed getter. `::Lib::_helper` and `::Lib::Box2::@v` were reported as `Cannot find entity named ...` although both exist, and importing `Lib` was the only way to write them.
