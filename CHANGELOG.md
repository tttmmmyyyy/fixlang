...
- #734: On AArch64 Linux, `fix test` with `memcheck = true` in `[build.test]` no longer stops a test program with `SIGILL` on a CPU that has SVE or RCpc. valgrind cannot read the instructions of those features, so the build now leaves them out when it runs the program under valgrind, as it already did for AVX-512 on x86-64.
