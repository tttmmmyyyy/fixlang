...
- A loop written with monadic binds no longer consumes a stack frame per iteration at `-O basic` when its result or its state is wide. Such a loop is a chain of tail calls, and the backend used to compile them as ordinary calls in two cases: a result that does not fit in the target's return registers, which it returned through a pointer of its own, and arguments that do not fit in the argument registers, which it could not rewrite on the stack. The compiler now passes the result pointer explicitly and gives Fix functions a calling convention that permits the rewrite, so both become jumps. Both limits are easy to cross: an `Array` (or a `String`) already fills the three integer registers x86-64 returns values in, so an ordinary result such as `(Array a, I64)` — or any `IOFail a`, whose `Result ErrMsg a` carries a `String` — crossed the first, and a state monad carrying more than about four scalars crossed the second. This applies at `-O none` as well, since whether a tail call becomes a jump is the backend's decision and it makes it there too. Building with debug information (`-g`) still keeps every call as a stack frame, so a debugger sees the whole chain.
