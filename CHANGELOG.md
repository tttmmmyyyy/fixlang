...
- LSP: The language server now keeps up with fast typing on a slow machine. It used to fall behind the completion and semantic-tokens requests the editor sends with every keystroke, and after the typing stopped it analyzed each half-typed state in turn, showing a parse error for each. A request on a document that has changed since the request was sent is now answered at once with `ContentModified`, and a cancelled one with `RequestCancelled`.
### Fixed
...
- #114, #540, #732: On Apple Silicon (AArch64 macOS), an integer narrower than 32 bits no longer crosses the FFI boundary as a wrong number. This happened when a function exported with `FFI_EXPORT` returned one to its foreign caller, and when `FFI_CALL` passed one to a foreign function that takes one. A function of type `I8 -> I8 -> I8` exported and called from C with `-100` and `30` answered 186 instead of -70. This covers `I8`, `U8`, `I16` and `U16`, and the `Std::FFI` aliases of the same widths such as `CChar` and `CShort`. Other targets were unaffected.
