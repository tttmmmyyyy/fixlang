...
- A program that uses an unboxed struct of one field whose type names the struct itself now compiles at `-O max` and `-O experimental`. `type Phantom a = unbox struct { x : I64 }; type C = unbox struct { y : Phantom C };` aborted the compiler as soon as a value of `C` was used, though the same program built at `-O none` and `-O basic`; so did `it.mod_next(f)`, which updates the one field of a `Std::DynIterator`.
