...
- LSP: The language server now keeps up with fast typing on a slow machine. It used to carry out every completion request the editor sent, even those the editor had already cancelled, so after the typing stopped it analyzed each half-typed state in turn, showing a parse error for each. A request the editor cancels before the server reaches it is now answered at once with `RequestCancelled`.
### Fixed
