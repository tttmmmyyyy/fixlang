...
- #477, #635: A count of array elements computed as a difference is now clamped at zero. A traversal of an element buffer ends when its counter reaches the count it was given, so a negative count was never reached and the traversal walked the heap until the program died; the two places that subtract -- the elements after a punched hole, and the elements a truncation drops -- now hand it the count they mean, which is none. Such a count arrives only where a program has already broken a contract (an `_unsafe_` primitive called outside its range, or an out-of-range index in a build made with `--no-runtime-check`), so what changes is what such a program does from there.
