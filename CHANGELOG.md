...
- #529, #531: A value a `match` (or an `if`) binding carries is no longer freed while it is still read. Where such a value went into a struct or a union together with a value from elsewhere, and that container was released after the last use of it, `-O max` and `-O experimental` removed a reference count the binding still needed, so a read of the binding after the container was released answered with freed memory. `-O none` and `-O basic` gave the right answer already.
- #519, #523: A struct field holding an `Option`, a `Result`, or another union with a boxed payload can now be read more than once. Where such a field was read to test the tag and read again inside the arm that test chose — `(if p.@0.is_none { 0 } else { p.@0.as_some.@x }) + p.@1` — `-O max` and `-O experimental` freed the payload before the second read, so the program answered with a value that changed from run to run and freed the same memory twice. `-O none` and `-O basic` gave the right answer already.
