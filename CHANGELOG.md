...
- A loop written with monadic binds no longer consumes a stack frame per iteration, at any optimization level, when its result or its state is wide. Such a loop is a chain of tail calls, and the backend used to compile them as ordinary calls in two cases: a result that does not fit in the target's return registers, which it returned through a pointer of its own, and arguments that do not fit in the argument registers, which it could not rewrite on the stack. The compiler now passes the result pointer explicitly and gives Fix functions a calling convention that permits the rewrite, so both become jumps. Both limits are easy to cross: an `Array` (or a `String`) already fills the three integer registers x86-64 returns values in, so an ordinary result such as `(Array a, I64)` — or any `IOFail a`, whose `Result ErrMsg a` carries a `String` — crossed the first, and a state monad carrying more than about four scalars crossed the second.
- The help text for `-O none` no longer says that tail recursion may overflow the stack. Whether a tail call becomes a jump is decided by the backend, which decides it without optimization too, so a deeply tail-recursive program runs in constant stack at every optimization level — `-O none` included, where it always had. Building with debug information (`-g`) is what keeps a stack frame for every call, tail calls included, so that a debugger sees the whole chain; a deeply tail-recursive program may overflow the stack under it, and the help text for `-g` now says so.
