...
- #319: `FFI_EXPORT` now rejects a C function name the compiler owns — `main`, a name beginning with `fixruntime_`, and the C library functions `malloc`, `realloc`, `sprintf` and `pthread_once` — instead of silently renaming one of the two functions that end up under the name. A program exporting `main` used to build into one that never ran its `Main::main`.
- #320: A program may now call the C function it exports with `FFI_CALL` of the same name; `-O max` and `-O experimental` used to fail to link it. Where a program describes one C name two ways — an `FFI_EXPORT` and an `FFI_CALL` giving different signatures, or two `FFI_CALL`s doing so — it is now rejected with an error quoting both, instead of aborting the compiler or calling the function at a signature it does not have.
