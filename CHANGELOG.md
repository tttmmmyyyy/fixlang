...
- #392, #451: The compiler now warns about an `import` of a module belonging to a project that the importing project does not declare as a dependency; an absolute path such as `::Hash::hash` reaches a module the same way and is warned about the same way. Declare the project in the `[[dependencies]]` of the project file that imports from it.
