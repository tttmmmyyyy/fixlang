...
- #461: An array shared with another thread no longer leaks its elements when a write to it meets that thread letting go of it. Every operation that writes to an array — `set`, `swap`, `push_back`, `pop_back`, `truncate`, `reserve`, `append`, `mutate_elements`, and `act` on an element — copies a shared element storage and lets go of the shared one, and where the other thread had let go of it first that release freed the storage without releasing the elements it held.
- #461: `Std::Destructor` now runs its destructor function once however many threads let go of the value at the same moment. Where two of them read the reference count before either lowered it, each left the destructor to the other and the value was freed without it ever running, so the resource it manages was never released.
