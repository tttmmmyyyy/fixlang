...
- #319: `FFI_EXPORT` now rejects a C function name the compiler has a use for — `main`, a name beginning with `fixruntime_`, and the C library functions `malloc`, `realloc`, `sprintf` and `pthread_once` — instead of silently renaming one of the two functions that end up under the name. A program exporting `main` used to build into one that never ran its `Main::main`. `FFI_CALL` now rejects `main` for the same reason: the compiler writes that function, so naming it takes it away. The rest stay available to `FFI_CALL`, which reaches the same function the compiler calls.
