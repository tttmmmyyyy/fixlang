...
- #705: A conversion of a floating-point value to an integer type now gives one value. `x.to_I64` and the other conversions to an integer type used to give a value with two answers when the value did not fit that type: at `-O max` a NaN converted to `I64` printed `0`, where `-O none` printed `-9223372036854775808`, and `I64::maximum` sent through `F64` and back returned the least value of `I64`. The value is rounded towards zero; a value beyond an end of the range now gives that end, and a NaN gives zero. Outside the range the result is unspecified, and `--check-integer-operations` stops the program.
