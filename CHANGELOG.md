...
- #170, #333, #352, #663: A type signature an implementation writes for a trait member whose declared type has an opaque type is now reported where it disagrees with that declaration, instead of aborting the compiler. Where the declaration writes an opaque type, the implementation writes one of its own: `to_iter : Odd -> RangeIterator;` written for `to_iter : [?it : Iterator, Item ?it = I64] c -> ?it;` is reported, and so is a signature that writes one opaque type for two of the declaration. The constraints a signature puts on an opaque type are compared now, where they used to be ignored: `make : [?jt : Iterator, Item ?jt = Bool] Bool -> ?jt;` written for a declaration stating `Item ?it = I64` compiled and ran, with the `Bool` it states read by nothing. A signature that writes the declaration's type another way is reported as well — `I64` where the declaration writes `Elem c`, which the implementation gives that type — since an implementation's type is the declaration's with its type variables replaced; leaving the signature out is always allowed. The report prints both signatures as they are written, where it used to print the generated types the opaque types had been replaced with.
