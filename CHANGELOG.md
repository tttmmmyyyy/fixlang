...
- On Apple Silicon (arm64) macOS, `fix build` at `-O none` — which `-g` also selects — aborted with a crash while compiling any program that merges an empty-payload union value such as `Bool` (for example one using `||`). The compiler emitted a phi node of the union's zero-sized payload, which LLVM's AArch64 GlobalISel (the instruction selector it uses at `-O none`) rejects. The code generator no longer emits phi nodes for zero-sized values.
