...
- #519: A union with a boxed payload, held in a field of a struct, is now read as many times as the program reads it. Where such a field was read once to test the tag and again inside the arm that test chose — `(if p.@0.is_none { 0 } else { p.@0.as_some.@x }) + p.@1` — `-O max` and `-O experimental` freed the payload before the second read: the program answered with a value that changed from run to run, wrote into memory it had freed, and freed it a second time. `-O none` and `-O basic` answered correctly throughout.
