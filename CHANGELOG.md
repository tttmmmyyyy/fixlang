...
- #176, #304, #305, #371: The type-check cache now sees the definitions the compiler writes itself. The implementations a tuple carries are generated for the tuple sizes the program uses, so a program that starts using a tuple of a new size gets a different `Std` — and the next build served what it had checked from the old one, leaving the debug information of the shared implementations pointing into a source of the build before, at times a file no longer on disk, with the line numbers of those functions dropped. The sizes of the C types are read the same way: editing `.fixlang/c_types.json` and building again aborted the compiler with `called 'Option::unwrap()' on a 'None' value`, naming no file.
