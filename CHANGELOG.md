...
- #335, #346: A trait method whose return type is opaque now hides a concrete type of each implementation's own, also where the member's declared type fixes the trait's type variable through a constraint alone. With `trait c : Make { make : [?it : Iterator, Item ?it = c] I64 -> ?it; }`, the implementations for `I64` and for `Bool` shared one concrete type: which of them a call reached changed from build to build of an unchanged source, an implementation returning an iterator of another element type was accepted, and a second build lost an error the first one reported.
