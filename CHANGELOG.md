...
- #356: A `U8` literal may now hold a double quote, written `'"'` or `'\"'`. Both were reported as `Expected u8_lit_char.` at the quote, and `'\x22'` was the only way to name the character, although a string literal has taken `\"` all along. The manual's lists of escape sequences now also carry `'\xHH'` and `"\uXXXX"`, which the compiler accepts.
