...
- #572, #584: A string literal that holds a null character, written `\u0000` or directly, is now reported. A `String` ends at its null terminator, so such a literal built a value whose size counted bytes that every call into C stopped short of — `println` printed `abc` for a literal of 7 characters — and whose bytes past the null were not the ones written, but whatever the compiled program held after them. The byte arrays a `String` is built from already end at their first null (`Std::FromBytes::from_bytes` truncates there), so the literal was the one way in. Where a null byte is needed, build an `Array U8`.
...
- #573, #585: A string literal now takes `\'` for a single quote, as a `U8` literal does. It was reported as `Expected string_char.`, although a single quote written on its own has always been accepted.
