...
- A global value's initializer has to produce a graph of objects that nothing outside Fix holds a reference to, and violating that is undefined behavior. Two things can bring such an object in: a value restored by `FFI::boxed_from_retained_ptr`, and a value whose raw pointer `FFI::mutate_boxed` or `Array::mutate_elements` handed to an action that gave the foreign side a reference it keeps.
