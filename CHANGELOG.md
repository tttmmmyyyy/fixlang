...
- #332: A constraint that names an opaque type variable is now required to be about that variable, instead of being accepted where it is about another type. `foo : [?s : ToString, c : Rebuildable, Elem c = I64, Rebuild c ?s = Array I64] c -> ?s;` writes `?s` in an argument of an equality about `c`, which asks a call site to relate the type `?s` stands for — the type the signature hides from it — to another type. The body of `foo` and the call site each settled `?s` on a type of their own, `fix check` reported no error, and `fix build` aborted the compiler. An opaque type variable stands in the constraints that describe it, as `?s : ToString` and `Item ?s = a` do.
