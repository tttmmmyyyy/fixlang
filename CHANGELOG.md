...
- #558, #567: LSP: A program the compiler answers with a panic — a shape a program passes through while it is being repaired — no longer stops the diagnostics of the session. The language server published nothing on any file for the rest of the session, however the program was repaired afterwards, and restarting it was the only way back; it now reports the failure on `fixproj.toml`, keeps the diagnostics of the last program it analyzed on screen, and analyzes the next program the editor writes.
