...
- #638, #689: Added the `--check-integer-operations` compiler option and the `check_integer_operations` field of the project file. They stop the program where an operation on an integer type is given, or produces, a value outside what the operation is defined on, and the report names the operation and the values: `Signed integer overflow: I64 addition, with 9223372036854775807 and 1`, `Shift amount outside the width of the type: I64 shift_left, with 64`. They cover the result of `+`, `-`, `*`, unary `-`, `/` and `%` on `I8`, `I16`, `I32` and `I64` — arithmetic on an unsigned type is taken modulo two to the width of the type, so none of it is checked — and the amount of `shift_left` and `shift_right` on every integer type, signed and unsigned alike.
...
- #689: A shift by an amount outside the width of its type now gives one value. `x.shift_left(bits)` and `x.shift_right(bits)` used to give a value with two answers where `bits` reached the width of the type or was negative: at `-O max`, `1.shift_left(n)` with `n` of -1 printed `-9223372036854775808` while answering `false` to `x < 0`. A shift is defined for an amount at least zero and less than the number of bits of the type shifted; outside that range the result is unspecified, and `--check-integer-operations` stops the program there.
