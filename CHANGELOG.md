...
- #689: Added the `--check-shift-amount` compiler option and the `check_shift_amount` field of the project file. They stop the program where the amount of a shift is outside the range the shift is defined on, and the report names the operation and the amount: `Shift amount outside the width of the type: I64 shift_left, with 64`. They cover `shift_left` and `shift_right` on every integer type, signed and unsigned alike.
- #638: Added the `--check-signed-overflow` compiler option and the `check_signed_overflow` field of the project file. They stop the program where arithmetic on a signed integer type gives a result outside the range of that type, and the report names the operation and both operands: `Signed integer overflow: I64 addition, with 9223372036854775807 and 1`. They cover `+`, `-`, `*`, unary `-`, `/` and `%`. Arithmetic on an unsigned type is taken modulo two to the width of the type, so none of it is checked.
...
- #689: A shift by an amount outside the width of its type now gives one value. `x.shift_left(bits)` and `x.shift_right(bits)` used to give a value with two answers where `bits` reached the width of the type or was negative: at `-O max`, `1.shift_left(n)` with `n` of -1 printed `-9223372036854775808` while answering `false` to `x < 0`. The amount is now taken modulo the width of the type, so an amount of the width leaves the value where it is. The amount a shift is defined on is at least zero and less than the number of bits of the type shifted; outside that range the result is unspecified, and `--check-shift-amount` stops the program there.
