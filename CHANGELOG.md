...
- #551: `Debug::assert_unique` and `Std::unsafe_is_unique` no longer answer differently at `-O max` than at the lower levels for a value that was uniquely referenced. Borrowing a parameter a function never consumes leaves its reference to be disposed of by the caller after the call rather than by the function before the observation runs, so a value that was unique read as shared: a program with `Debug::assert_unique` halted at `-O max` and ran at `-O none` and `-O basic`, and `Destructor::mutate_unique_io` copied a resource it did not need to. A function that can reach such an observation is no longer given a borrowing version. `Std::unsafe_is_unique` still documents the opposite change — a value going from shared to unique as optimization drops a use — which remains allowed.
- #552: Fixed a way the reference-count cancellation could delete a retain/release pair across a consume of the value it protected, leaving a later read of that value reading freed memory. A value a `match` produces carries its own name and the names of the values its arms produced; a release of it named all of them while a retain of it named only the first, so a consume that named an arm's value passed the retain by. No program is known to have hit this; it was found by proving the pass.
