...
- #638: Added the `--check-signed-overflow` compiler option and the `check_signed_overflow` field of the project file, which stop the program at an arithmetic operation on a signed integer type whose result leaves the range of that type, naming the operation and the values. A build without it assumes the result fits.
...
- #638: Arithmetic on a signed integer type (`I8`, `I16`, `I32`, `I64`) now assumes that its mathematical result falls within the range of that type, and a program that performs one whose result falls outside it has no defined behavior. The operations this covers are `+`, `-`, `*`, unary `-`, `/` and `%`, and `/` and `%` assume in addition that the least value of the type is not divided by -1; a shift, a bitwise operation and a conversion between integer types stay defined at every input. Unsigned integer types are unchanged: `+`, `-`, `*` and unary `-` on `U8`, `U16`, `U32` or `U64` are taken modulo two to the width of the type, so use one of those where wrapping is what is wanted. `Std::abs` of `I8::minimum` and its siblings is among the operations that now have no defined behavior.
