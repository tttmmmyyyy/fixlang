...
- #734: `fix test` with `memcheck = true` in `[build.test]` no longer stops a test program with `SIGILL` on a CPU with instructions valgrind cannot read, such as GFNI on recent x86-64 CPUs and SVE and RCpc on AArch64 Linux. Under valgrind, the program is now built with only the CPU features valgrind reads.
