...
- The compiler is now built against LLVM 22.1.x, where it used to be built against LLVM 17.0.x. Building it from source needs that release, with `LLVM_SYS_221_PREFIX` naming where it lives; the installation section of `Document.md` carries the commands for Linux and macOS. What the newer back end brings shows in the programs the compiler builds: over the fifty LangArena benchmarks ten run at least 3% faster (`Hash::CRC32` 69%, `Brainfuck::Array` 21%, `Etc::NeuralNet` 17%) and six at least 3% slower (`Sort::Merge` 10%, the worst of them).
