...
- #433: Marking the objects a value holds — what a global value's initialization does to its result, and what `Std::mark_threaded` does to the value it is given — now visits each object once. A value that reaches one object along more than one path, as nested `Tree { kids : [child, child] }` does, was walked once per path: a global value holding 29 objects took 2.3 seconds to mark, and one holding 41 objects took hours.
