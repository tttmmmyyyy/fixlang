...
- `-O max` and `-O experimental` now build incrementally. They used to compile the whole program as one piece, so an edit to one line cost as much as a build from nothing; they now divide the program into compilation units, cache each unit, and regenerate only the units whose code changed. Reference counting and the optimizations over it still run over the whole program, and a unit carries a copy of the small functions and the global accessors it reaches in another. `--cu-size` now decides the size of a unit at these levels too.
