...
- #288, #312: The compiler's caches now tell two source files of equal content apart. Building `a/main.fix` and `b/main.fix`, whose contents are equal, from one working directory made the second build reuse what was compiled for the first: a warning the build owed was reported in the other file, or dropped; a `-g` build could abort with `called 'Option::unwrap()' on a 'None' value`; and the debug information named the other file. Moving a source file without editing it left the debug information of the next `-g` build pointing at where the file used to be.
