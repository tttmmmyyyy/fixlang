...
- #638: Arithmetic on a signed integer type (`I8`, `I16`, `I32`, `I64`) now assumes that its mathematical result falls within the range of that type, and a program that performs one whose result falls outside it has no defined behavior. The operations this covers are `+`, `-`, `*`, unary `-`, `/` and `%`; a shift, a bitwise operation and a conversion between integer types stay defined at every input. Unsigned integer types are unchanged: `+`, `-`, `*` and unary `-` on `U8`, `U16`, `U32` or `U64` are taken modulo two to the width of the type, so use one of those where wrapping is what is wanted. `Std::abs` of `I8::minimum` and its siblings is among the operations that now have no defined behavior.
