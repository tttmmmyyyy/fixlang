...
- #391: An absolute path now reaches a value of another module whose name begins with `_`, and the getter of a struct field, without the module being imported. `::Lib::_helper` and `::Lib::Box2::@v` were reported as `Cannot find entity named ...` although both exist, and importing `Lib` was the only way to write them. A getter is defined for every field of every struct, so this closed the absolute path to the getters of another module's structs entirely.
