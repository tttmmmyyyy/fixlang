...
- #637, #638: Arithmetic on a signed integer type (`I8`, `I16`, `I32`, `I64`) now assumes that its mathematical result falls within the range of that type: `+`, `-`, `*`, unary `-`, `/` and `%` have no defined behavior where it does not, `/` and `%` assume in addition that the least value of the type is not divided by -1, and `Std::abs` of `I8::minimum` and its siblings is undefined for the same reason. A shift, a bitwise operation and a conversion between integer types stay defined at every input. Unsigned arithmetic still wraps, so use `U8`, `U16`, `U32` or `U64` where wrapping is wanted.
