...
- Every help message is now headed by the compiler's version, as `fix 1.5.0 (92a3989)` — the released version, followed by the revision the binary was built from — and `fix --version` and `fix -V` report the same line, which used to be an error. The header read `Fix-lang` alone before, so a help message quoted in a bug report named no version. The line under it now says what `fix` is.
