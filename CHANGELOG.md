...
- #148: A program whose global definitions name each other in a cycle now compiles, where the compiler used to exhaust its stack and abort. None of the globals in such a cycle calls itself, so every round of inlining put the whole cycle into each member again and each body doubled; a ring of three globals carrying two thousand bindings apiece aborted the build. What one round may add to a symbol is now bounded, so a program that stops on its own compiles into what it used to.
