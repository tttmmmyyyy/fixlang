...
- #587, #594: A floating point literal is now reported when the type it is written with holds no finite value that large. `3.5e38_F32` and `1.0e400` compiled as an infinity, which no literal spells, and every arithmetic reaching one carried it on. Inside the range of its type, a literal takes the value of that type nearest to what is written, so `1.0e-50_F32` is `0.0_F32`.
- #573, #585: A negative hexadecimal or binary literal is now reported when the type it is written with cannot hold it. `-0xFF_I8` compiled as `1` and `-0x1_U8` as `255`, where the same values written in decimal were reported: the check that lets a non-negative literal of those radices fill the width of its type (`0xFF_I8` is `-1`) dropped the sign before comparing. A report about a literal too wide for its type also names the type the literal is written with, where it used to name the unsigned type of that width.
- #572, #584: A string literal that holds a null character, written `\u0000` or directly, is now reported. A `String` ends at its null terminator, so such a literal built a value whose size counted bytes that every call into C stopped short of — `println` printed `abc` for a literal of 7 characters — and whose bytes past the null were whatever the compiled program held after them. Where a null byte is needed, build an `Array U8`: a `String` built from bytes already ends at the first null, since `Std::FromBytes::from_bytes` truncates there.
- #356, #578: A `U8` literal now takes an ASCII character other than a single quote, a backslash, or a line break between its quotes; those three are written `'\''`, `'\\'` and `'\n'`. `'''` used to compile as 39, reading the quote that closes the literal as its content, and a line break between the quotes compiled as 10, so a character literal could run past the end of its line. The report at a character the literal cannot hold now says what it can: ``Expected an ASCII character or an escape sequence such as `\'`.``
...
- #410, #543: An `FFI_CALL` of a variadic C function now applies C's default argument promotions to the arguments it passes past the declared parameters, and requires each of them to be a type C carries as one scalar. An `F32` is widened to a `double`, and an integer narrower than `int` — `I8`, `U8`, `I16`, `U16` — is widened to an `int` by the extension its sign asks for. That is how a C function reads its variadic arguments, so it now reads the value the call wrote: `FFI_CALL[CInt printf(Ptr, ...), fmt, -1_I8]` printed 255 and now prints -1, and `2.5_F32` reached a `va_arg(ap, double)` as 0.0 and now reaches it as 2.5. A `Bool`, a `String`, a struct, a boxed value or a closure written past the `...` used to be handed to C as the first word of its representation, and is now rejected with an error naming the way to pass it instead: a `U8` or a `CInt` for a `Bool`, and a `Ptr` from `Std::String::borrow_c_str`, `Std::FFI::boxed_to_retained_ptr` or `Std::FFI::borrow_boxed` for the rest.

#### Std

- #603, #617, #670: `F64::from_string` and `F32::from_string` now read a number the way `to_string` writes one. Two things change. The text is read with a point for its decimal mark whatever `LC_NUMERIC` the program is running under, where the point used to be whichever character that locale names — a program that had set a locale spelling it with a comma could not read back what `to_string` wrote. And a number below the smallest one its type holds in full, such as `5e-324`, is now read as that number, where it was reported as out of range; a text naming a number too small to hold at all, such as `1e-1000`, and one naming a number too large, such as `1e400`, are reported as before. `to_string_precision` and `to_string_exp` still write the decimal mark the locale names.
- #619, #670: `F64::to_string` and `F32::to_string` now write the number's shortest round-trip decimal representation, where they used to write the six places `%f` gives by default: `1.0 / 3.0` is `0.3333333333333333` and `1.0e-10` is `1e-10`, where they were `0.333333` and `0.000000`. The digits are written positionally where the point falls near them — `1.0`, `0.1`, `123456.789` — and as a power of ten otherwise, so `1.0e300` is `1e300`. A program that shows a floating point number shows a different text than it did. `to_string_precision` and `to_string_exp` write what they wrote before.
...
- #499, #517: The `--max-cu-size` option is now `--cu-size`, and its value is the average number of entries — the top-level functions and the global values whose code a compilation unit generates — rather than a bound on the number of symbols it holds. Where a unit ends is now decided by the names of those entries, so an entry a project gains moves the boundaries of at most two units instead of every unit after it, and the units before and after keep their cached object files. `--cu-size inf` puts the whole program in one unit, which gives up building it a unit at a time and lets the back end see every call the program makes. A build passing `--max-cu-size` is warned and goes on, and one passing a `--cu-size` that is neither a positive number nor `inf` is now rejected instead of aborting the compiler. The `cu_size` field of the project file takes the same values, and `--cu-size` decides over it.
...
- #588, #594: An `F32` literal now takes the `F32` value nearest the decimal written. It was read as an `F64` and then narrowed, and rounding twice can land one step past the value rounding once gives: `1.000000178813934326171874999999_F32` sits just below the midpoint of two `F32` values, so it takes the lower one, where it used to take the higher.
- #573, #585: A string literal now takes `\'` for a single quote, as a `U8` literal does. It was reported as `Expected string_char.`, although a single quote written on its own has always been accepted. An escape sequence a string literal does not have now says what one takes: ``Expected a character or an escape sequence such as `\n`.``
- #356, #578: A `U8` literal may now hold a double quote, written `'"'` or `'\"'`. Both were reported as `Expected u8_lit_char.` at the quote, and `'\x22'` was the only way to name the character, although a string literal accepts `\"`. The manual now also lists the `'\xHH'` and `"\uXXXX"` escape sequences, which the compiler accepts.
...
- #171, #381, #512: A program whose types the compiler cannot finish reducing is now rejected with an error, instead of overflowing the compiler's stack. An implementation that gives an associated type a value carrying that same associated type (`type El I64 = Array (El I64);`) and an equality constraint that does the same (`[?out : Iterator, Item ?out = (I64, Item ?out)]`) both aborted the build with `fatal runtime error: stack overflow`, under `fix check` as well as `fix build`. The report says that the type is circular, names the steps that lead from it back to itself, and points at every implementation and constraint on them. Where each step gives a larger type instead of the same one (`type El (Wrap a) = El (Wrap (Wrap a));`), it says that the type grew too large.
- #246, #276: A trait constraint that is deduced from itself is now rejected, instead of being accepted with nothing behind it. With `impl Wrap c : Holder { type Held (Wrap c) = Wrap (Wrap c); }`, the instance `impl [c : Holder, Held c = e, e : Show] Wrap c : Show` made deducing `Wrap (Wrap I64) : Show` ask for `Wrap (Wrap I64) : Show` again, and the program compiled although nothing gives `Show` to anything. Where each step asks about a larger type instead of the same one (`type Held (Wrap c) = Wrap (Wrap (Wrap c));`), the compiler used to take memory without end and report nothing; it now says that the inference is too long.
...
- #560, #570: LSP: A URI whose percent-escapes do not decode as UTF-8 no longer ends the language server. A file name on Linux is any sequence of bytes, so an editor holding one that is not UTF-8 sends such a URI, and opening one file like that took the diagnostics, the completion and the hover of the whole session down; the server now goes on, leaving that one file unanswered.
