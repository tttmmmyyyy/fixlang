...
- #332: A constraint that names an opaque type variable is now required to be about that variable, instead of being accepted where it is about another type. `foo : [?s : ToString, c : Rebuildable, Elem c = I64, Rebuild c ?s = Array I64] c -> ?s;` writes `?s` in an argument of an equality about `c`. The signature hides from the call site what `?s` stands for, and that equality asks the call site to relate the hidden type to another one. The body of `foo` and the call site each settled `?s` on a type of their own, `fix check` reported no error, and `fix build` aborted the compiler. An opaque type variable stands in the constraints that describe it, as `?s : ToString` and `Item ?s = a` do.
