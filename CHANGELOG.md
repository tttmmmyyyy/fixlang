...
- A program that uses a type with no size is now rejected with an error, instead of exhausting the compiler's stack. A type whose unboxed fields reach the type itself (`type A = unbox struct { b : B, n : I64 }; type B = unbox struct { a : A, m : I64 };`) has no size; make one of the types along the way boxed. A type that leads to itself at a larger type argument (`type P a = unbox struct { x : P (a, a), n : I64 };`) needs endlessly many types; give the recursive occurrence the same type arguments. This holds however the larger argument is reached: through a pointer, through a type argument the declaration discards (`type Phantom a = unbox struct { x : I64 }; type C a = unbox struct { p : Phantom (C (a, a)), n : I64 };`), through a field that is a function type, or through a parameter of a higher kind.
