...
- #319, #379: `FFI_EXPORT` now rejects a C function name the compiler writes the body of — `main`, when an executable is being built, and any name beginning with `fixruntime_` — instead of silently renaming one of the two functions that end up under the name. A program exporting `main` used to build into one that never ran its `Main::main`. A program may call `main`, which re-runs it, and one that calls it at another signature is reported. Every other name stays the program's to take, the C library functions the program itself calls included; exporting one of those replaces it for the whole process, as linking an object file that defines it does, and the FFI section of `Document.md` says what that costs.
