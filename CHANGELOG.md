...
- Building with debug information (`-g`) now works at every optimization level. Combining it with `-O basic` aborted the build with `function declaration may only have a unique !dbg attachment` followed by `LLVM ERROR: Broken module found`. Because `-g` selects `-O none` by default, this appeared only when an optimization level was also given explicitly.
