...
- Compiling a function of many parameters no longer aborts the compiler with a stack overflow: a global function of 13 or more parameters was enough, whatever its body. Uncurrying eta-expands each global into function pointers of every arity up to its own, and the application inlining that drives the expansion pushed each argument through the `let`s the parameters introduce, binding it to a fresh name at every level — so the intermediate expression doubled in size per parameter. An argument that is already a variable is now pushed in as it is, which makes the expansion linear in the parameter count.
