...
- #356, #578: A `U8` literal now takes an ASCII character other than a single quote, a backslash, or a newline between its quotes; those three are written `'\''`, `'\\'` and `'\n'`. `'''` used to compile as 39, and a literal newline between the quotes as 10, so one byte had two spellings and a character literal could run past the end of its line.
