...
- #460: A union of more than 256 variants is now rejected with an error, instead of compiling into a program that takes one variant for another. A union value tells which variant it holds by the index of that variant in a tag of one byte, so the 257th variant carried the tag of the first: `Many::v256(7)` answered `true` to `is_v0`, and `as_v0` handed its payload over as a value of the first variant's type — where that type was boxed, the reference counter read the payload as a pointer and the program crashed.
