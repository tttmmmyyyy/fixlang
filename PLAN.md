...
- **（調査済み）`is_var_used_later` 依存の InlineLLVM**: 約8個の `generate` が `gc.is_var_used_later(arg)` を見て RC を決める（`builtin.rs` 1855/2418/2489/2540/3873/4546＝「last-use なら引数 release」、4206/4214＝「内部 2 回使用ぶん retain」）。**いずれも RC 判断のみで計算結果・挙動は used_later に非依存**（in-place/clone はランタイム refcount で決定、used_later ではない）。RC IR では lowering の last-use 解析が明示 `Retain`/`Release` を配置し、RC-free 版 `generate` は `is_var_used_later` を呼ばない（効果は `UniqSignature` で宣言）。P1 でこれらを used_later 非依存に書き換える（grep 由来なので網羅監査する）。
