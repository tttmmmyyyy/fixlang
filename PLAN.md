...
**継続入れ子・単一 enum（終端子つき）**形式を採る。`Let`/`Retain`/`Release`/`MarkGlobal`/`MarkThreaded` は継続を持つ「文」的ノード、`Ret`/`Match` が終端子（継続の終わり）。この形は reset/reuse・borrow 等の RC 最適化を載せやすい。Fix の既存 AST（再帰的）とも同形で流用しやすい。各ノードは span を持つ（§1.1-5。下では省略）。
```rust
...
enum RcRhs {
    ...
    Closure(FuncRef, Vec<Var> /*捕捉*/),  // top-level 関数 + 捕捉変数列 → boxed closure 値（root unique、捕捉を保持）
    LLVM(InlineLLVM, Vec<Var>),           // opaque builtin: 算術/getter（射影）/リテラル(IntLit 等)/fill/empty/FFI/...。効果は UniqSignature で宣言
    Construct(Ctor, Vec<Var>),            // 集約の新規 alloc（struct/タプル/ArrayLit/union variant、rc=Static(1)・引数 move-in）。透過: 解析が型から効果を導出（getter の双対）
}
...
- **構築（集約 alloc）は透過な `Construct`、他の builtin は opaque な `LLVM`**: struct/タプル/`ArrayLit`/union variant の新規 alloc は `Construct(Ctor, args)`（rc=Static(1)・引数 move-in）。効果が型から一意に決まるので解析は宣言なしで読める（getter＝射影の双対）。**fresh=unique はこの最適化の土台なので構築の透過性は必須**。算術・getter・`fill`/`empty`・FFI など個別効果の builtin は `LLVM`（`UniqSignature` で宣言）。現コンパイラの AST は struct=`MakeStruct`・array=`ArrayLit`（専用ノード）／union=`union_new`・リテラル=`IntLit` 等（InlineLLVM）と混在するが、lowering で集約構築を `Construct` に寄せて統一する。
- **`Atom` は持たず atom 位置は `Var` のみ**: リテラル（整数/浮動小数/Bool/nullptr）は現コンパイラでも `Expr::Lit` でなく InlineLLVM（`IntLit` 等）。RC 無関係な unboxed 即値なので `LLVM` で let 束縛し、atom は `Var`（global funptr 参照を含む）に一本化（文字列リテラルは boxed `Array U8` ＝ `ArrayLit`/`Construct` 側）。
...
- **reuse**（`Release` した alloc を直後の `Construct` で再利用＝in-place 再確保）。
