...
**末尾呼び出しを壊さない（制約）**: 上の「呼び出し**後**の `Release`」が**末尾位置の呼び出しに残る**と tail call でなくなる（`let r = App(f, args); Ret(r)` が `…; Release(x); Ret(r)` になり、末尾再帰なら stack を消費）。判定は**相殺後**で見る——後続 `Release` が手前の `Retain` と §2.2 で消えるなら末尾は保たれ、その引数は `Borrow` 化してよい。相殺で消えず末尾 `App` と `Ret` の間に残るなら、その引数の `Borrow` 化を取り消し `Own`（callee が release）に留めて後続 `Release` を消す。last-use の末尾呼び出しは相殺相手の `Retain` を持たないのが普通なので、その位置は基本 `Own`。ただし同じ引数が非末尾の別サイトで `Borrow` の利得を持つ場合は per-param の所有権が競合する（`Own` 固定か特殊化かは P2）。read-only を末尾再帰で素通しする形（`Borrow` を `Borrow` 位置へ渡す）は release 自体が出ないので影響しない。
