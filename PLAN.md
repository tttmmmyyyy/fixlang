...
- **P0.7（早期・P1 と独立に入れられる）: 配列 read-modify-write を atomic builtin へ集約**（builtin `PunchedArray`＋builtin `swap`、`_unsafe_get_linear_bounds_unchecked_unretained` 系・`_unsafe_swap` を廃止）。狙い: **隠れ穴を作る composable primitive を無くして §8 (a) の DOESNT-FIT を完全消滅**させ、各 atomic op に force_unique フラグを付けて **§4 の除去対象に揃える**（swap もソートで全 unchecked 化）。std の swap 版 `PunchedArray`（std.fix:2648）の要素 move 2 回も無くす。作業:
  ...
  - **削除チェックリスト（P0.7 完了後に確実に消す。残すと再利用される）**: (i) `_unsafe_get_linear_bounds_unchecked_unretained`(+`_forceunique`)＝builtin.rs `InlineLLVMArrayUnsafeGetLinearBoundsUncheckedUnretained`＋stdlib.rs 登録＋std.fix 呼び出し元（166/174/190/366/570/571/2668/2692/2698）＋doc。(ii) `_unsafe_swap_bounds_uniqueness_unchecked`（std.fix:567 定義＋呼び出し元 439/461/466/473/527/555）。(iii) std `PunchedArray` 型（2648）＋`_plug_in`（2658）＋`_unsafe_punch_bounds_uniqueness_unchecked`（2686）。**`#[deprecated]` 等で残さず物理削除**（CLAUDE.md の dead-code 方針）。**残すもの（明示）**: `_unsafe_set_bounds_uniqueness_unchecked_unreleased`（未初期化 fill・内部専用）・`_unsafe_set_size`・`_unsafe_get_bounds_unchecked`・`_unsafe_force_unique`。
...
- **P3.5（§4 後の unsafe 掃除）**: uniqueness チェックを避けるためだけに存在する `_unsafe_*`（手動 `_unsafe_force_unique`＋続く unchecked write のパターン等）を棚卸しし、**安全版に置換 → §4 が overhead を消すことを確認 → unsafe 版を削除**（`#[deprecated]` で残さず物理削除）。§4 が unique を証明できない箇所はチェックが残る（安全側の正しい挙動＝precision は §2 で上げる）。内部専用で残さざるを得ないもの（未初期化 fill の `_unsafe_set_bounds_..._unreleased` 等）は残す。狙い: 安全版と同速のまま unsafe API 面を縮小し、「手動で uniqueness を管理する誤用しやすい関数」を無くす。
