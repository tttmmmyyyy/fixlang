...
- **P0.7（早期・P1 と独立に入れられる）: `PunchedArray` を builtin 抽象型化**（`{Array a, idx}` ＋ `idx` スキップの専用デストラクタ/clone）。std の swap 版 `PunchedArray`（std.fix:2648）と、最適化 `mod`/`act` の平タプル linear-get/set を、**穴を型に出す builtin** へ寄せる。効果: swap 除去（generic `act` も高速化）・move-out の refcount プロトコルを型で安全化・§8 (a) の DOESNT-FIT を CLEAN 化（RC IR 解析が穴を typed 値として扱える）。`mod`/`act` の force-unique 除去（§4）はこの punch/plug 上に乗る。RC IR 導入前に単独で入れられる独立変更なので早期に。検証: 共有値テスト（2 箇所格納で破壊しない）＋回帰ベンチ（`benchmark/speedtest`・`fix-bench/batch`）で swap 除去分の改善と非劣化を確認。
