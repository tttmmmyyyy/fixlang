...
<1>5. `Let(a, Llvm(MU, [t]), ..)` は `Obl` を変えない。`a` の変位は `twins` (添字 0) であり、`a` の leaf
  ...
  <2>3. `a` は `MU` が変位 0 を構築した値なので、`a` の unbox union の節のタグは 0 である。よって
        `[0, 0]` と `[0, 1]` は inhabited である。
    BY A3, D16, 3.3 の <1>2
  <2>4. QED
    BY D9, D10, <1>4, <2>1, <2>2, <2>3

<1>6. `Release(a, [], s, ..)` の後、`Obl` は `obj(x, [])` への参照 1 つを持ち、`obj(y, [])` への参照を
      持たない。
  D10 の `Release` の行は `[]` の下の inhabited な各 leaf につき参照を 1 つ取り除く。`<1>5` よりその leaf は
  `[0, 0]` と `[0, 1]` の 2 つで、指すオブジェクトは `obj(x, [])` と `obj(y, [])` である。`<1>3` の `Obl` は
  それぞれ 2 つと 1 つを持つ。
  BY D10, <1>3, <1>5
...
<1>1. `collect_consumes(B, vars, prog, own, type_env, out)` は、`own` が何であっても `out` に `(x, [])` を
      1 つだけ積む。
  <2>1. `collect_consumes_go` が積むのは、`RcExpr::Ret` の腕、`RcExpr::Destructure` の腕、`rhs_consumes` の
        `Closure`・`App`・`Llvm` の腕の 5 か所である。
    BY P7, CODE src/rc_ir/ownership.rs: collect_consumes_go, rhs_consumes
...
<1>4. 2 つの `Let` は `Obl` を変えない。
  `3.4 の <1>4` と `3.4 の <1>5` の議論は `MS` と `MU` の宣言 (`3.3`) だけに依るので、名前を付け替えた
  `f_borrow` の同じ 2 節点にもそのまま当たる。
  BY A3, D9, D10, 3.3 の <1>1, 3.3 の <1>2, 3.3 の <1>3, 3.3 の <1>4, 3.3 の <1>5, 3.4 の <1>4, 3.4 の <1>5
...
`owns_unit` が真を答えていれば、`Release` は残って借用する側の参照まで処分し、D11 の (S-a) が破れる。
どちらの答えでも破れる本体があるので、`rewrite_rc` の判定を真偽どちらへ倒しても直らない。
...
- **形 1**: 1 つの unit の下の leaf の所有が分かれないようにする。`infer_ownership` の不動点に、
  ...
  いずれかが所有されるなら、その candidates はすべて所有される」という閉包を足す。`Binding::Join` の
  candidates については `collect_consumes` がアーム本体の `Ret` を報告することで既にこの閉包が働いており
  (D9 の注、P7 の (b))、足りていないのは `origin_from_leaves_under` が作る candidates である。
