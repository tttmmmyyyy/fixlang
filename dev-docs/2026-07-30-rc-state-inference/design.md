...
抜けるので、抜けた時点で `F(S) ⊑ S`（post-fixpoint）が成り立つ。すると
...
つまり `γ(S)` は `G` の post-fixpoint であり、Knaster-Tarski（`lfp G` はすべての post-fixpoint
以下）から `lfp G ⊆ γ(S)`。すなわち `S` はプログラムの意味論を過大近似している。
