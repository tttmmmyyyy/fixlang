...
ディスパッチを全部外した上限は命令数 -13.87%（`sort`）に達する。**site** とは状態バイトを読む
箇所のことで、その一覧は「注釈する site」にある。
...
束縛 `x` とその型の **boxed leaf** パス `π` — 型を unboxed 構造に沿って展開したとき boxed 型が
現れる位置（`boxed_leaf_paths`）— について、**3 点鎖**の値を割り当てる:
...
掛かるだけで、生きている束縛のオブジェクトを巻き込まない。論証は「前提」の「P2 の論証」）。
よって性質は
時間を量化せず、値のフローを前から追うだけの may 解析で正確に計算できる。Fix にループは無く、
本体は直線とパターン分岐の木なので、1 本体の走査は前向き 1 パスで済む（分岐の合流は join、
再帰は「手続き間」の相 2 のクローンのキーが受け持つ）。

**根が非 LOCAL になる道はちょうど 3 つ（「発生源」の扉）で、それ以外の操作は根を `RootLocal` に保つ。**
...
locality の記号的サマリを不動点で求め、locality をキーに複製し、キーが証明する site に
クローンの中で印を付ける（「注釈のしかた」）。同じ骨格の、別のパスになる
...
**扉は根の話であり、内部ポインタを渡す公開 API も根を動かさない。** コンテナに何を入れても、
コンテナ自身のオブジェクトの状態バイトは動かない。`a.set(0, g)` はオペランドから見えるので
普通の転送規則が `deep` を汚し、生ポインタ経由の `mutate_boxed` / `mutate_elements` は
「要素型・payload 型に boxed leaf があれば、結果の下に非 LOCAL が居るものとして扱う」という
1 行の規則が受ける（記号的な書き方は「`result_locality`」の `build_shape` の表）。どちらも汚すのは
`deep` だけで `root` は `RootLocal` のまま — 扉ではない。`borrow_boxed` と
`Array::borrow_elements` は「借りたポインタを通して変更してはならない」と `std.fix` が定めるので
`deep` すら汚さない。`boxed_to_retained_ptr` が返すのは制御ブロックへのポインタで、payload が
要るなら `borrow_boxed` を使えとドキュメントが指している。
...
完走しているのでマーク済みであり、参照解決規則（「転送」）が `MayExt` にする。初期化子が循環して
いれば（`implement_rc_global` は評価を終えてからフラグを立てるので）再入して停止しないが、
停止しない以上マークも起きず、そこまでに実行される注釈済みの操作はすべて `LOCAL` な
オブジェクトを見ている。
...
| | 言明 | 担保する場所 | 崩す変更 |
| --- | --- | --- | --- |
| P1 | 各 op の `result_locality` が健全 — 宣言した条件が `RootLocal` 以下に解決するならその leaf のオブジェクトが `LOCAL`、`DeepLocal` なら到達先もすべて `LOCAL` | 「発生源」の扉の数え上げと、「`result_locality`」の 77 op の列挙 | 宣言を 1 つ誤る |
| P2 | 束縛の生存中にオブジェクトの `LOCAL` 性が変わらない | 「P2 の論証」（非 threaded ビルドでのみ成立し、global 初期化子への契約に寄りかかる） | threaded ビルド、初期化子が外部保持のオブジェクトを取り込む |
...
| P5 | `fresh_holding` の契約が守られている — 返すコンテナは (a) 新しく確保したもの、(b) この op が直前に unique 化したもの、(c) 呼び出し側が uniqueness を保証したうえで in-place 更新したもの、のいずれか | 「`result_locality`」（`build_branch_by_is_unique` の `global_bb` が `shared_bb` へ行くので、unique なオブジェクトは `LOCAL`） | uniqueness の裏付けなしにオペランドをそのまま返す op に `fresh_holding` を宣言する |

(c) に当たるのは（`fresh_holding` は「`result_locality`」で定義する構成子）、`assuming_unique` 版、`struct_act` が実行時 `unsafe_is_unique` の下で呼ぶ
非 force-unique な `#punch_`、そして `struct_mod` / `struct_act_tuple2` / `struct_act_identity`
が `#punch_fu_` の後で呼ぶ非 force-unique な `#plug_in_` である。これらは op の外側にある
呼び出し形の不変条件なので、そこを崩す変更は P5 を崩す。
...
- `threaded = false` のビルドでは、マークする遷移は `mark_global` だけで、その対象は初期化子の
  ...
  2. **他の global。** 読んだ時点でそのアクセサは完走しており、マーク済み。参照解決規則
     （「転送」）が `MayExt` にするので、`RootLocal` と証明された束縛ではない。
  ...
  **抜け道は unsafe な経路だけである。** 外部が保持しているハンドルが結果グラフに入る道は
  ...
  初期化子がそれを取り込む」。壊れ方は「誤りの 2 方向は対称ではない」と同じだが、ここでは retain と release の釣り合いも
  崩れる。束縛の生存中にオブジェクトが GLOBAL にマークされると、ディスパッチのまま残っている
  retain はその時点から no-op に落ちる一方、`RcState::Local` と注釈済みの `Release` は実カウントを
  減らし続ける。増える側だけが止まるので、カウントは操作のたびに一方的に減る。
  ...
  プログラムは 1 つも失われない。前提 P2（「前提」）を担保しているのは、上の数え上げと
  この契約である。
...
記号的な層（相 1 のサマリと `result_locality`。まだ呼び出し元が決まっていない）。既存の
...
enum ExtCond {
    /// 入力に関わらず成り立つ。条件の束の頂。
    /// 別ヴァリアントにすることで吸収を構造で保つ: `Always` を含む join は `Always`。
    Always,
...
`result_locality` では op のオペランドを指す。`Provenance` が `LeafOrigin::Arg(i, path)` を
2 つの索引空間で使い回しているのと同じ。

**原子は自分がどちらの事実を見るかを持つ。** 判定基準を決めるのは原子の `Aspect` であって、
条件が `root` と `deep` のどちらのフィールドに置かれているかではない。取り出し規則（「転送」）が
コンテナの `deep` 条件を結果の `root` フィールドへ**移す**ため、置き場所で判定すると移した先で
意味が変わるからである。

> 例: コンテナ leaf の `deep = IfAny({(0, [], Deep)})` のとき、そこから取り出した要素の `root` は
> この条件をそのまま受け取る。原子が `Deep` を持つので、`root` フィールドへ移っても判定は
> 「入力 0 の leaf `[]` が `DeepLocal` でないか」のままである。置き場所で判定すると移った先で
> 「入力 0 が `MayExt` か」と読まれ、`RootLocal` なコンテナでは成り立たなくなって、GLOBAL な
> 中身を取り出した値が根まで LOCAL と誤証明される。

恒等サマリは `root = IfAny({(i, σ, Root)})`、`deep = IfAny({(i, σ, Deep), (i, σ, Root)})`。
...
吸収を形で持つことは、複製のゲート（「相 2」）の精度にそのまま効く。ゲートの判定は
`IfAny(s)` の `s` が空でないことなので、原始的に「条件の集合」として持って合併すると、
`{Always, (0, σ)}` が入力への言及を含むので「依存する」と読まれ、
...
コンテナの中身については何も分かっていない。取り出した値の**根**は、コンテナの `deep`
（コンテナの下に非 LOCAL が居るか）で決まる:
...
**参照解決規則 — 変数の参照はすべて名前の局所性を見る。** 環境は局所束縛だけを持ち、**環境に
無い名前（global シンボル）は全 leaf `Always` に解決する**。これが扉 1 であり、名前が local か
どうかを見る唯一の規則である。global は `let` の右辺に限らず、Llvm/App のオペランド、`Match` の
被検査値、`Destructure` のコンテナ、`Ret` のどこにでも現れる（`lower_var` は global を束縛せず
アトムのまま渡す）ので、規則はノード種別ごとではなく**参照の解決 1 か所**に置く。

**この解決規則が `Retain`/`Release` の唯一の防御である。** 対象が局所束縛に限られるのは
`insert_rc` の直後までで、`borrow_ify` は global を名指す `Release` を作る — `borrow.rs` の
`owns_object` はパラメータ表に無い root（producer か global）を own 扱いにするので、borrow 版の
callee の借用位置に global を渡すと、呼び出しの後に release が積まれる。
...
- `let x = Llvm(op, args)`: op の `result_locality`（同名の節）。
...
impl ExtShape {
    ...
    /// 値を返さない op 用。新しく確保するだけの op は `fresh_holding` でよい（boxed leaf を持つ
...
`fresh_holding` / `always` はその写像を作る構成子であって、別扱いの分岐ではない。
...
uniqueness の裏付けなしにオペランドのオブジェクトを**そのまま返す** op には `fresh_holding` を使えない。

**boxed コンテナからの読み出しも `fresh_holding` では書けない** — 取り出した値の根はコンテナの `deep`
から来るので、`build_shape` で取り出し規則を書く。

**既定実装を置かない。** `fresh_holding` を既定にすると、オペランドから到達できない boxed オブジェクト
を作る op を将来足したときに、何も書かなくても `RootLocal` が通る — 「誤りの 2 方向は対称ではない」の「壊れる側」の誤りが
...
`always` の 7 個のうち 4 個（`InlineLLVMFixBody`、`InlineLLVMWithRetainedFunctionBody`、
`InlineLLVMArrayBorrowElementsBody`、`InlineLLVMUnionModBody`）は関数オペランドを呼ぶ op である。
呼ばれる関数は別の `RcFunc` として解析されるが、この op から見ると結果は間接呼び出しの結果で
あり、関数本体が global を読んで返すことを `fresh_holding` は捉えられない。**`fresh_holding` を既定にして
いたら、この 4 個が黙って `RootLocal` を通していた。**

**`bottom`（2）** — 発散するので結果の値が存在しない。何を主張しても空虚に真であり、束の底
（全 leaf `IfAny(∅)`）が健全かつ最も精密である。

**`fresh_holding` では駄目な理由はオペランドにある。** `undefined : String -> a` の op が受け取る
オペランドは `Array U8`（`String = unbox struct { _data : Array U8 }` の中身）なので、boxed leaf を
持つ。`fresh_holding` を当てると結果の `deep` にメッセージ文字列の条件が入り、
...
| op | |
| --- | --- |
| `InlineLLVMUndefinedInternalBody` | abort する（`--no-runtime-check` では `unreachable`） |
...
| `InlineLLVMMakeStructBody` | 結果 `[i]++σ` <- 引数 `i` の `σ`。boxed 結果は `fresh_holding` と同じ（全オペランドの join） |
| `InlineLLVMStructSetBody` | 結果 `[field_idx]++σ` <- 引数 0（値）の `σ`、他の leaf <- 引数 1（struct）の同じパス。boxed 結果は `fresh_holding` と同じ |
| `InlineLLVMStructPlugInBody` | 結果 `[field_idx]++σ` <- 引数 1（field）の `σ`、他 <- 引数 0（punched）。boxed 結果は `fresh_holding` と同じ（全オペランドの join） |
| `InlineLLVMStructPunchBody` | 結果は `(field, punched_struct)`（`PUNCHED_STRUCT_FIELD = 1`）。unboxed コンテナ: `[0]++σ` <- 引数 0 の `[field_idx]++σ`、`[1]` 以下 <- 引数 0 の残り（成分そのまま）。boxed コンテナ: punched 成分は `fresh_holding` 相当、**取り出した field 成分は取り出し規則** |
| `InlineLLVMArrayUnsafeGetBoundsUnchecked` | 取り出し規則 — 結果の全 leaf に `root = deep = 引数の配列 leaf の deep` |
| `InlineLLVMIsUniqueFunctionBody` | 結果は `(Bool, a)`。`a` 成分 <- 引数の同じ leaf（`root`/`deep` とも）。**`fresh_holding` ではない** — この op は uniqueness を*調べる*だけで unique 化しないので、共有・GLOBAL な値がそのまま出てくる |
| `InlineLLVMArrayIsStorageUniqueBody` | 同上（結果は `(Bool, Array a)`） |
| `InlineLLVMArrayPunchBody` | 結果は `(PunchedArray a, a)`（`PUNCHED_ARRAY_FIELD = 0`。`StructPunch` とは順序が逆）。punched 成分は `fresh_holding` 相当（force-unique する）。**取り出した要素成分は取り出し規則** — `read_from_array_buf_noretain` で中身を持ち出すだけなので `root = deep = 引数の配列 leaf の deep` |
| `InlineLLVMMakeUnionBody` | unboxed union: 結果 `[variant]++σ` <- 引数 0 の `σ`（成分そのまま）、他の variant は底（`IfAny(∅)`）。boxed union: `fresh_holding` 相当 |
...
| `InlineLLVMUnsafeMutateBoxedInternalFunctionBody` | `[0]`（値）の `root` は底（この op が unique 化するので `fresh_holding` と同じ論証で局所）。payload 型に boxed leaf があれば `deep` は `Always`（コールバックが生ポインタ経由で参照を書き込みうる）、無ければ底。残り（コールバックの結果）は `Always` |
| `InlineLLVMUnsafeMutateBoxedIOSInternalBody` | 同上、値の位置は `[1, 0]` |
| `InlineLLVMArrayMutateElementsInternalBody` | `[0]`（配列）の `root` は底、要素型に boxed leaf があれば `deep` は `Always`。残りは `Always` |
...
`array_set[unique]` など）。この 2 つの版の `result_locality` は同じ `fresh_holding` である。
...
その事実は素直に書ける — もっとも、扉以外の op は `root` を `IfAny(∅)` に保つので、`fresh_holding` が
既に同じ結論を出しており、uniqueness の情報を持ち込む必要は無い。

## 手続き間: 独立した specialize パス

2 相に分ける。相 1 で関数ごとの記号的サマリを不動点で求め、相 2 で locality をキーに関数を
複製して site に注釈する。

### 相 1 — 記号的サマリ

`provenance.rs` の `analyze_program` の phase 1 と同じ形。
...
2. 本体を転送規則（「転送」の節）で前から走査する。直接呼び出し `let x = App(g, args)` では
   ...
   （`Always` はそのまま）。原子が aspect を持つので代入先が一意に決まる。`result_locality` の
...
**底から始めるのは精度のため。** 健全なのは post-fixpoint（`F(S) ⊑ S` を満たす `S`。「サマリの健全性」）であることによるので、頂から
...
**この節は前提 P1-P5（「前提」の節）を仮定する。** 抽象解釈の標準形で言う。具体側の意味論を
最小不動点として定め、`γ` で抽象側とつなぎ、**局所健全性**だけを手で確かめる。あとは
Knaster-Tarski が結論を出す。
...
G(R)_f = { (a, v) | f の本体を a で走らせて v を返す導出が存在する。
                    入れ子の直接呼び出し g(b) では R_g のいずれかの対 (b, v') を使ってよい }
```
...
- **要素は関数のグラフとは限らない。** 同じ `a` に複数の `v` が対応してよく、`G` は入れ子の
  呼び出しの結果の選び方について存在量化して、ありうる `v` を全部集める（この単調性が
  `G` の単調性である）。`R` で量化するのは直接呼び出しだけで、間接呼び出しと `FFI_CALL` は
  型が許すどの値を返してもよい — 抽象側がそこに `Always` を当てるので、この自由さがそのまま
  受かる。`FFI_CALL` は同じ引数でも実行ごとに違う値を返しうるので、
  `lfp G` は実際に関係になりうる。部分性も同じ枠で済む — 発散は「対が 1 つも無い」であって、
  `Out_⊥` のような持ち上げた領域を導入しなくてよい。`γ` の条件は対ごとの全称なので、
  関係のままで通る。
...
**つなぎ。** `γ : A -> C`。条件を解決するには入力側の具体値を `LocalityKey` に読み取る必要が
あるので、それを先に定める: 具体値 `a` の各入力 leaf について、指すオブジェクトが
`REFCNT_STATE_LOCAL` でなければ `MayExt`、それ自身は LOCAL だが到達先に非 LOCAL が居れば
`RootLocal`、到達先まですべて LOCAL なら `DeepLocal` としたものを **`a` の実 locality** と呼ぶ。
成分ごとには `γ_f : ExtShape_f -> P(In_f x Out_f)` で、`S` が主張することを実際に満たす対の
集合を返す:
...
post-fixpoint であること」の 1 点である。`C` は完備（べき集合の直積）、`G` は単調（callee の振る舞いが
...
**仮定した前提のうち P3（転送の単調性）は構成で成り立つので、手で保つのは (P1)、(P2)、(P4)、
(P5) である。** とくに P1 — 77 個の手書き宣言のどれか 1 つが誤っていれば局所健全性が破れ、
結論が崩れる。だから `develop_mode` の実行時 assert（`RcState::Local` と注釈した
site で状態バイトを読んで検査する）を実装と同時に入れ、結論そのものを全テストプログラムで
直接検査する。

### 相 2 — locality をキーにした複製

キーは「パラメータごと x leaf ごとの 3 値」。
...
- クローンの実体化が本体を走査し（転送規則を呼び出し元クローンの具体的入力の下で回す。入れ子の
  ...
  キーを組んで worklist に積む。
- **ゲート**（この設計での唯一の定義）: 複製するのは「自分の RC site が入力に依存する関数」と
  「入力依存の leaf を、ゲートを通った callee へ直接呼び出しで渡す関数」。ここで数える site は
  `unique_check_elim` の後に残ったもので、site が入力に依存するとは、相 1 の収束後の走査で
  その site が参照した leaf の**いずれか**の `root` か `deep` が空でない `IfAny` であること
  （`Always` は入力に言及しないので依存に数えない）。leaf ごとに見るのは、`root` だけがキーで
  動く leaf を join が `Always` に潰してしまい、依存を見落とすからである。逆向きの誤り — 依存
  しない site を依存と数える — は余分なクローンを作るだけで、注釈は変わらない。`funcs_reaching_unique_check` と同じく直接呼び出し
...
出し元の join を 1 つ持つ）は、混合文脈の呼び出し元が 1 つあるだけで全呼び出し元の証明を
落とすので、採らない。キーの値が 2 から 3 に増えるぶんクローン数は原理上ふくらむが、
`RootLocal`（根だけ）が `DeepLocal` と分かれるのは「global 由来の値をコンテナに入れた」場合
だけで、`plan.md` の測定ではコーパスの大半が marked object を 1 つも作らない。実測で確かめる。
...
「サマリの健全性」とは別に、注釈には「クローン `(f, k)` の本体が実行されるとき、その活性化
...
であることが、単位の外から届く経路（列挙は「相 2」）を一括で受ける。
...
`reaches_unique_check`、locality のゲートは「相 2」のもので、後者はほぼ全関数
...
op を `array_set` -> `array_set[unique]` に差し替えるが、両モードの `result_locality` は同じ
`fresh_holding` なので locality が各値に与える値は変わらない。locality の注釈は `RcState` フィールドを
書くだけで、provenance はそれを読まない。

ただし**クローン集合は順序で変わる**。locality のゲートが数える site は残った実行時チェックを
含むので、`unique_check_elim` がどれだけ畳んだ後かに依存する。可換なのは
...
`array_set[unique]` になった op では宣言されたチェックが消えている。畳み込みを先に済ませて
おけば、locality パスは残った site だけを見ればよく、ゲートも「実行時チェックにまだ到達する
関数」で組める。逆順にすると、消える運命の site のためにキーを分ける。

クローンの帳簿（worklist、命名、`borrowed_units` の renaming 追従）とゲートの道具
（呼び出しグラフ上の最小不動点、retarget できる callee の判定）は 2 つのパスで同一なので、
`unique_check_elim` から `specialization.rs` へ括り出し、キー型で径数化する。本体の走査は
括り出さない — locality の走査は記号的な環境を持ち回り、uniqueness の走査は別解析が記録した
表を引くので、共有できる形が残らない。
...
| `is_unique` チェック | `build_branch_by_is_unique`。unique-check op の `generate` の中、`unique_check_operand` が宣言したもの（下記） | op インスタンスの属性フィールド |
...
**畳めるのは宣言された uniqueness チェックだけである。** `unique_check_operand` は 1 op につき
1 つのチェックを宣言するが、`generate` が出すチェックが 1 つとは限らない。
`InlineLLVMArrayAppendCapacityBoundsUnchecked` は、`dst` を unique 化するチェック
（`force_unique` のときだけ。宣言しているのはこちら）に加えて、**`force_unique` と無関係に
無条件で** `src` のストレージに対するチェックを出す — 全範囲を append するとき、要素を retain
せず memcpy で移せるのは `src` が unique なときに限るからである。したがって「対象は `LOCAL`」版が
...
性質の、手書き宣言への依存である。宣言を複数持てるようにして `src` 側も畳むのは、`unique_check_elim` にも
共通する拡張なので別途 issue にしてある（#162）。
...
ほかにもあり、それらは対象を宣言せずディスパッチのまま残す（下記の表）。
...
```rust
/// この op が `generate` の中で参照カウントする対象。結果の leaf は呼び出し側が持つ結果の
/// shape で解決するので、宣言はオペランドの型だけを見る。
fn internal_rc_targets(&self, arg_tys, type_env) -> Vec<RcTarget>;
...
**clone 経路の対象は宣言から導出する。** force-unique な op は共有時にコンテナを複製し、その中で
要素・フィールドを retain-copy して旧オブジェクトを release する。対象はどれも
「`unique_check_operand` が宣言した容器と、その中身」なので、**force-unique な op には
...
**op ごとの宣言値は、op 内部の RC を入れる段（「着地と検証」）の着手時に書く。** 先行する段の
実測を見てからこの段をやるかを決めるので、ここでは手書きが要る op の顔ぶれだけ挙げておく（`builtin.rs` を機械的に
走査して、既定合成で覆われない retain/release を `generate` が出すもの）:
...
| 上書きした旧値を release | `ArraySetBody`（`write_to_array_buf(…, true)`）、`StructSetBody`（`gc.release(old_value)`） |
| 落とした要素を release | `ArrayTruncateBoundsUnchecked`（`release_or_mark_array_slice`）、`ArrayAppendCapacityBoundsUnchecked`（copy 経路の `clone_array_buf` と `src` の release も） |
...
読み出しなので、ここを覆わないと上限表（`plan.md` の、ディスパッチを全部外した測定）と
比べられない。
...
- boxed union の `Match`: variant アームは payload を retain するので、取り出し規則により
  **被検査値の leaf が `DeepLocal` のときに限り** `MatchArm` に印を付ける。印は
  ...
  catch-all アームは被検査値そのものを束縛するだけで retain しない。
...
**状態ディスパッチの置かれ方は 2 通りあり、注釈の届け方もそれぞれ違う。** どちらも
`build_branch_by_refcnt_state` に行き着くが、そこまでの道が違う。
...
**インライン側を落とすと段 1 の取り分が半分になる。** `Retain` は補助関数を通らず
`build_retain` が展開する（`Generator::retain` を通るのは unboxed 値の子孫への降下だけ）ので、
補助関数のキーだけを増やしても boxed 値の retain には何も届かない。`plan.md` の実行回数で
retain は release と同規模の site 種別である。
...
`DeepLocal` 版の traverser を足すのは「対象外」なので、この段では `DeepLocal` も `Local` 版を
指す（「補助関数側の名前」）。足すときに一つ効くのは、**根の下に boxed な子を持たない型では
`Local` 版と `DeepLocal` 版が一致する**ことである — 子孫のディスパッチが存在しないので違いの
出る場所が無い。`Array I64`、スカラーだけの boxed struct がこれに当たり、1 本だけ生成して
両方の注釈から指せる。判定は「この型の traverser が子の release を出すか」で静的にできる。
boxed leaf を持たない型にはそもそも補助関数も traverser も生成されない（`create_traverser` が
`is_fully_unboxed` で `None` を返す）。
...
- boxed union の `Match`: `eval_rc_match` が variant アームで出す payload の retain を、上の
  `Retain` と同じインライン形に差し替える。
- unique-check op（対象が `LOCAL` 版）: `unique_check_operand` が宣言したチェックについて
  `build_branch_by_is_unique` の状態ディスパッチを外し、参照カウントを 1 と比べる分岐だけを出す
  （今日の `local_bb` の本体）。同じ `generate` が出す宣言外のチェックはそのまま。

`RcState` に `DeepLocal` を足す（値の割り当ては「注釈のしかた」）。`implement_rc_program` の
`Retain`/`Release` アームは今 `Unknown` を assert しているので、`Local` と `DeepLocal` も
受けるようにする。
...
設計は割らない。一方で実装は次の順で入れ（以下、段 1-4）、各段で `benchmark/speedtest` を測る:
...
- **`develop_mode` の実行時 assert**: `RcState::Local` または `RcState::DeepLocal` と注釈された全 site で状態バイトを
  ...
  その場の abort に変えるもので、手書き宣言 (P1) の誤りに対する唯一の実効的な防御なので、実装と
...
- **ダンプ水準の回帰テスト**（`--emit-rc-ir`）: 実行時 assert は「誤注釈した site を非 LOCAL な
  ...
  borrow 版の呼び出しに global を渡した後に `borrow_ify` が積む `Release`、
...
- **カバレッジ測定**（一時プローブ、読んだら revert）: speedtest corpus で実行された
  ...
  `plan.md` の上限測定（オペランドの leaf が確保・引数に解決される割合）と
...
| `src/rc_ir/leaf_map.rs`（新規） | 葉ごとの事実を持つコンテナ `LeafMap<T>` / `LeafKey<T>` と `boxed_leaf_paths`。`Provenance` / `Uniqueness` もこれに載せ替える |
| `src/rc_ir/specialization.rs`（新規） | `unique_check_elim` から括り出した複製 skeleton |
| `src/rc_ir/ast.rs` | `RcState` に `DeepLocal`、`Destructure` と `MatchArm` に `RcState`、`RcTarget` |
