...
**記号的サマリ**（相 1、まだ呼び出し元が決まっていない段階）では、leaf ごとに origin の集合
`Origin ∈ { Ext, Arg(input, σ) }` を持つ。集合が挙げるのは「この leaf が `MayExt` になりうる
理由」なので、**その場で確保した値は空集合**になる — `create_obj` は `LOCAL` で初期化するので、
理由が 1 つも無い。確保したコンテナに `MayExt` な値を入れた場合は、その値の origin が転送規則
（`Merge`）で入ってくるので空集合にはならない。

- 空集合 = 入力に関わらず `Local`。束の底。
