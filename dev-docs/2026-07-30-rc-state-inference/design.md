...
底から始めるのは、単調性と有限性から**停止する**というだけでは正当化されない。停止先が
過大に `Local` を主張しない、を別に言う必要がある。

**言えること。** 記号 `⊑` を `ExtCond` の順序（`IfAny(∅)` が底 = 最も多く `Local` を主張、
`Always` が頂 = 何も主張しない）とし、`true(f)` を「`f` の結果の真の条件」とする。示すのは

> ループを抜けた時点の `summary` について、すべての `f` で `true(f) ⊑ summary[f]`。

**前提は 3 つある。** どれも不動点とは別のところで担保する。

- **(P1) 各 op の `locality_flow` が健全**: 宣言した条件が `Local` に解決するなら、実行時にも
  結果の leaf のグラフ全体が `LOCAL` である。扉の数え上げ（発生源）と 77 個の列挙
  （`locality_flow`）がこれを担う。
- **(P2) 束縛の生存中にオブジェクトの `LOCAL` 性が変わらない**。非 threaded ビルドでのみ成立
  （健全性は `threaded = false` に依存する）。
- **(P3) 転送が単調**。join と代入で組み立てているので構造的に成立する。

**まずループの停止条件が意味するもの。** 実装は `summary[f] = summary[f] ⊔ 候補` で更新し、
1 つも動かなくなったら抜ける。抜けた時点では全 `f` で `F(summary)[f] ⊑ summary[f]` が成り立つ
（`F` は「全関数の本体を 1 回走査する」写像）。つまり得られるのは **post-fixpoint** である。

**post-fixpoint なら健全である。** 帰納法は**実行 1 本のステップ番号**の上で回す。静的な
呼び出しグラフは再帰で循環しているが、実行のステップ列は有限で前向きなので、そこなら induction
が通る。呼び出しの入れ子で見れば「戻ってきた呼び出しの木を葉から根へ」だが、木だけでは足りない
— 本体の中の束縛と、呼び出し元が仮定したキーの成立も同時に要るためである。

実行のステップ `n` で確立される事実は 3 種類ある。ステップ `< n` の事実を仮定して、それぞれが
解析の値に収まることを示す。

- **(i) 束縛が値を得る** -> 本体の走査がその束縛に与えた `ExtCond` に収まる。走査は前向き
  1 パスなので、根拠は直前までの束縛だけ。global アトム・間接呼び出し・単位外呼び出しは
  `Always` を出すので何も主張しない。`Llvm` op は (P1)。直接呼び出しは (ii) をステップ `< n`
  で使う。`Match` の合流は join なので両アームの主張より弱い。
- **(ii) 呼び出しが値を返す** -> `summary[f]` に収まる。返る直前の `ret` の値は (i) により
  本体の走査の結果に収まり、その結果は post-fixpoint 性から `summary[f]` に収まる。
- **(iii) 呼び出しが始まる** -> 実引数が、その活性化に仮定された locality キーを満たす。
  call site は呼び出し元のクローンの中にあり、キーは呼び出し元の入力と `summary` から組んだ
  ものなので、(i) と (P3) から従う。

(iii) の連鎖の基底はエントリポイントと global 初期化子で、どちらも canonical 版（全 leaf
`MayExt`）を走る。何も主張しないキーなので無条件に満たされる。

(P2) により、これらの事実は値を保持する束縛が生きている間変わらない。

**底から始めた楽観が悪さをしない理由がここにある。** 初期値 `IfAny(∅)` の「この再帰呼び出しは
`Local` な値を返す」という無根拠な主張は、**その呼び出しが実際に値を返したときにしか使われ
ない**。返さなければ値は存在せず、それに触る RC site も無いので、主張は空回りする。返したなら
その戻りは真に早いステップなので (ii) の帰納法の仮定が使える。輪を閉じて根拠のない `Local` を
得る道が無い。静的な呼び出しグラフの循環は、実行のステップ列には現れない。

これは最小不動点で正しくなる性質（発生源から生成的に定義される may 性質）に固有の議論で、
無限の振る舞いについての must 性質には通用しない — そちらは頂から始めて最大不動点を採る。
`Always` が「まだ見ていない」ではなく明示的な発生源からしか入らないように転送規則を書いて
あることが、この性質を生成的に保っている。

**実行が止まらない場合。** 上の帰納法は自然数（ステップ番号）の上のものなので、実行が無限で
あっても各ステップは有限の番号を持ち、すべて覆われる。「無限番目のステップ」も「極限で存在
し始める値」も無い。示しているのは*実行された*操作についての言明で、メモリ安全に必要なのは
それだけである。

問題になりうるのは「戻らない呼び出しについての楽観的な `summary[f]` を、実際に実行される
RC site が使ってしまう」形だが、これは起きない。`Let(x, App(f, args), k)` の継続 `k` は呼び
出しが戻ってから走る（`codegen` は値を束縛してから `k` を生成する。発散する op の継続は
`unreachable` の後ろ、つまり死んだコードとして生成される）。`x` の値に依存する site はすべて
`k` の中にあるので、戻らない呼び出しの結果は観測されない。

末尾再帰が TCO でループになった場合のように、活性化が無限に続く形も同じ。各活性化の RC site
は有限のステップ番号を持ち、(iii) はそれより早いステップの (i) に寄りかかるだけである。

なお解析自身の停止は、対象プログラムが停止するかどうかと無関係に (P3) と束の有限性から出る。

**この論証が寄りかかっているのは (P1) である。** 77 個の手書き宣言のどれか 1 つが誤っていれば
結論は崩れる。だから `develop_mode` の実行時 assert（`Local` と注釈した site で状態バイトを
読んで検査する）を段階 1 と同時に入れ、結論そのものを全テストプログラムで直接検査する。
