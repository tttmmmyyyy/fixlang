...
**post-fixpoint なら健全である。** 具体実行のうち*戻ってきた*呼び出しについて、その呼び出しが
直接呼び出しでぶら下げた木の高さ `h` に関する強い帰納法で示す。`f` の呼び出しが高さ `h` で
値 `v` を返したとする。本体の走査の各ステップは次のどれかである。

- 扉・間接呼び出し・単位外呼び出し: 転送は `Always` を出すので `Local` を主張しない。前提なし。
- `Llvm` op: (P1) により、オペランドについて成り立っていれば結果についても成り立つ。
- 直接呼び出し `g`: `g` のこの呼び出しは高さ `< h` で戻っているので、帰納法の仮定から
  `true` は `summary[g]` に収まる。代入は単調 (P3) なので、その結果も収まる。

よって `v` は `F(summary)[f]` に収まり、post-fixpoint 性から `summary[f]` にも収まる。(P2) に
より、`v` を保持する束縛が生きている間この事実は変わらない。
...
その実行は木の中で真に低いので帰納法の仮定が使える。輪を閉じて根拠のない `Local` を得る道が
無い。
