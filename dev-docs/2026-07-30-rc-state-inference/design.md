...
- `threaded = false` のビルドでは、マークする遷移は `mark_global` だけで、その対象は初期化子の
  ...
  **ただしこれには契約が要る。** global の初期化子は**遅延実行**で、その global を最初に読んだ
  時点、つまり呼び出し元のフレームが生きたまま中断している状態で走る。そして `mark_global` は
  結果の値から**到達可能なグラフ全体**を marking する。初期化子は引数を取らず global しか
  読めないので、普通は他人が保持しているオブジェクトに手が届かない — 唯一の抜け道が扉 3
  である。

  ```fix
  g : Array I64;
  g = (
      let p = fetch_stashed_ptr.unsafe_perform;   -- C に預けてあったポインタ
      p.boxed_from_retained_ptr.unsafe_perform    -- main が持っている配列のハンドルを復元
      -- 初期化子を抜けたところで mark_global がこのオブジェクトを GLOBAL にする
  );

  main : IO ();
  main = (
      let x = Array::fill(3, 0);      -- 解析: 新規確保なので x は Local
      stash_ptr(x.boxed_to_retained_ptr).unsafe_perform;;
      let arr = [x, x];               -- x が Local なので arr は DeepLocal
      eval g;                         -- ここで初期化子が走り、x のオブジェクトが GLOBAL になる
      loop_reading(arr)               -- arr の注釈は Local のまま
  );
  ```

  壊れ方は**混ざった対**である。`array_get` が要素を取り出すときの op 内部の retain は実行時に
  ディスパッチするので GLOBAL になった今は no-op、一方 RC IR の `Release` ノードは `Local` と
  注釈済みなので実カウントを減らす。1 回読むごとにカウントが 1 減り、まだ参照されているのに
  解放される。

  **「扉は読み出し側に付く」の論証はここを覆わない。** あちらは *初期化子の中で* 実行される
  操作の話（マークより前なのでまだ `LOCAL` を見る）で、ここで壊れるのは *初期化子の外で*、
  中断している呼び出し元の、既に注釈済みの操作である。

  よって「**global の初期化子は、外部が保持している Fix オブジェクトを結果グラフに取り込んでは
  ならない**」を契約として課し、`boxed_from_retained_ptr` のドキュメントにも書く。P2 を
  担保しているのはこの契約である。初期化を遅延でなく `main` の前に一括で行う形にすれば、
  生きている束縛が存在しえないので契約なしに閉じる（#156 で定数初期化子を静的データにする
  作業は、その global について `mark_global` 自体を消すので、この穴を狭める方向に働く）。
