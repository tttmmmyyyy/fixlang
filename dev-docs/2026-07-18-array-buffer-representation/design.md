...
- Array の uniqueness は専用 `Array::_unsafe_is_buffer_unique`(`_buf` を retain せず覗く)。generic
...
**やらないこと**: zero-copy slice(先送り、§3.2)、`Buffer` の public 化(internal と決定、§11.3)。
...
- そこで **増加専用の `_unsafe_grow_size`(「未初期化スロットへ size を伸ばす」、`n >= _size`)に置き換え、
  ...
  は増加しかしない)。すると「呼び出し側が事前に unique を保証する」footgun が消え(op 自身が unique な `_buf`
  にしか size を書かない)、その内部 check は unique-check-elim が provably-unique で畳んで同性能にする
  (§11.4 の方向)。呼び出し側の `_unsafe_force_unique` は畳み込める。残る unsafe 契約は「新スロット
...
`Array::_unsafe_is_buffer_unique : Array a -> (Bool, Array a)` を追加**し、value の `_buf` の refcount を
...
- さらに **generic `unsafe_is_unique` に `[a : Boxed]` 制約を付ける**(現状は無制約で unboxed に `const true`
  ...
  誤 const-true を型システムが弾いて `Array::_unsafe_is_buffer_unique` へ誘導できる。`else { const true }` 枝は
  ...
  は型エラー(本来 arr には誤答なので望ましい破壊)。Array 用の uniqueness assert(`_unsafe_is_buffer_unique`
  ベース)を別途用意する。
...
現行の `Array` InlineLLVM primitive はそれぞれ、`{ _buf, _size, _cap }` + `Buffer` primitive の上に再構成する
(InlineLLVM のまま Buffer storage へ向けるものと、Fix-src(`Buffer` primitive の合成や value field の参照)に
...
| `_unsafe_get_bounds_unchecked` | `Buffer::_unsafe_get(i, arr.@_buf)` | Fix-src(`Buffer::_unsafe_get` = InlineLLVM を呼ぶ。read なので borrow 化で `_buf` の retain を除く) |
| `set`(make_unique, check, write, 旧要素 release) | `_buf` を unique 化(COW)+ `_check_range(i, _size)` + write(旧要素 release)を1 body で | **InlineLLVM**(現行 `InlineLLVMArraySetBody` を Buffer storage へ re-target。in-place mutator ルール) |
| `_unsafe_set_bounds_uniqueness_unchecked_unreleased` | `Buffer::_unsafe_initialize`(この Array 版は §11.4 で削除) | **InlineLLVM**(`Buffer` primitive) |
...
| `_unsafe_empty_capacity_unchecked(cap)` | `Array { _buf: Buffer::_unsafe_allocate(cap), _size: 0, _cap: cap }` | Fix-src(struct 構築 + `Buffer::_unsafe_allocate` = InlineLLVM) |
| `_unsafe_fill_size_unchecked(n, x)` | `_buf(n)` 確保, `Buffer::_unsafe_initialize` の loop で埋め, `_size = n, _cap = n`(最適化器が InlineLLVM 同等にする、実証済み) | Fix-src |
| `_pop_back_nonempty` | **`_unsafe_pop_back_nonempty` へ改名**(empty で呼ぶと `last_idx = -1` の範囲外読み出し + ゴミを boxed 要素として release する UB = memory-unsafe なので、規約どおり `_unsafe_` を付ける)。`_buf` を unique 化, 末尾要素を noretain read して release, value `_size -= 1`(empty check は caller の `pop_back` が担う) | **InlineLLVM**(in-place、COW) |
| array literal `[..]` | `_buf` 確保, 埋め, `_size = len, _cap = len` | compiler lowering(既存の array-literal codegen を `Buffer` allocate + initialize に向ける) |
...
書く mutate primitive は新設しない。既存のこのペアも削除する(§11.4)。** 理由: unchecked な in-place mutate は
...
`Buffer::_unsafe_set` / `_unsafe_get` と punch/plug は、囲む safe op の unique 枝(既に uniqueness を確定済み)の
中でだけ動く生の部品なので可。「bulk op は
Fix-source を基本」(§4)は fill/reserve のコピーの話で、per-element の in-place mutator は InlineLLVM に残す。
...
1. **先送り(決定)** — Zero-copy slice(§3.2)。中核には入れない。近い将来に不要と確認するか、そうでなければ
   `Buffer` の count モデルが変わる(いずれ入れるかは未決)。
2. **未決** — step 2 の可否。`Array` を boxed のまま `Buffer` を内包する中間段が作れるか、それとも反転は atomic で
   なければならない(step 2 を step 3 に畳む)か。実装時に判断する。
3. **決定 — `Buffer` は internal に留める(public `@buf`/`@_buf` accessor は公開しない)。** 裸の `Buffer` 値が
...
4. **決定(採用した方向) — 事前手動 unique-check を要する unsafe 関数を減らす。** `_uniqueness_unchecked` 系の
   ...
   方向で、redesign を機に unsafe API の表面積を縮める(削除対象は §3.4)。前提は §3.4(is_unique)と §3.1
