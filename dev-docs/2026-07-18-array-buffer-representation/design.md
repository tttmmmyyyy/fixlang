...
| `_unsafe_set_bounds_uniqueness_unchecked_unreleased` | `Buffer::_unsafe_initialize` |
| `_unsafe_set_size` | `_unsafe_grow_size`(増加専用)へ改名: 内部 unique check(COW、optimizer 除去)+ value `_size` を伸ばす(新スロット未初期化)。減少は `_pop_back_nonempty` が release+shrink |
...
| `_pop_back_nonempty` | `_buf` を unique 化, 末尾要素を noretain read して release, value `_size -= 1`(empty check 無し = caller の `pop_back` が担う) |
...
**in-place write は COW を InlineLLVM に内包する(Fix-source に分解しない)。** `set` / `mod` / `act` /
`swap` / `push_back` の要素書き込みは、unique check + 必要なら clone(COW)を Array レベルの InlineLLVM body に
内包し(現行 `set` の `force_unique`)、unique-check-elim が provably-unique のとき check を畳んで in-place にする。
**uniqueness check を持たず in-place mutate する InlineLLVM(caller が `_unsafe_force_unique` で unique にした前提で
`_unsafe_set_bounds_uniqueness_unchecked_unreleased` が無条件に書く形)には分解しない。** これは「この配列は
unique」という **verify されない表明**で、正しさが op の外(caller の事前 force-unique)に依存する。borrow /
provenance / unique-check-elim はどこで値が unique か shared かを追って RC を置き check を畳む解析なので、この外部
不変条件を op から検証できず、解析の uniqueness モデルが実体とズレると shared 配列への書き込み(miscompile)や
unsound な fold を招く — unique-check-elim の設計中に判明し、この種の uniqueness-check-less な mutate primitive を
減らしてきた理由。check を op 内に持てば入力が何であれ健全(shared なら clone)で、optimizer の fold は純粋に
安全な簡約になる。`Buffer::_unsafe_set` / `_unsafe_get` はその InlineLLVM 内部(または borrow 化された read)で
使う生の部品で、`_buf` を retain せずその場のストレージに対して動く。「bulk op は Fix-source を基本」(§4)は
fill/reserve のコピーの話で、per-element の in-place mutator はこの理由で InlineLLVM に残す。
