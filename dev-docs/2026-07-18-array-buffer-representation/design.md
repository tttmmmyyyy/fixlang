...
- **(a) count を `Buffer` に持つ。** すると `get_size` が再び heap から読む — 本改修の意味が消える —
  ただし count を value に *複製* する場合を除く。複製すると size 変更のたび 2 箇所に書いて同期する必要が
  出る。却下。
- **(b) `Array` value が要素 release を駆動する。** `Array` の release/mark/clone を custom traverser に
  し、その各点で手元にある value の `_size` を使って `_buf` の生 storage を歩く(`Buffer` は untyped
  storage として扱う)。**採用。**
- **(c) generic な custom-traversal ヘルパに `len` を渡す**(既存の hole path
  `build_release_mark_nonnull_boxed_with`)。(b) の実装手段であって別モデルではない。

**採用: (b)。** `_size` を value だけに保てて(再設計の眼目)、しかも *現行* `Array` destructor が既に回して
いるロジックそのもの — 変わるのは `size` の出所が heap load から value field になる点だけ。具体的には:
...
- `Buffer::_unsafe_set : I64 -> a -> Buffer a -> Buffer a` — 要素書き込み、旧占有者を **release** する
  (初期化済み slot 向け)。
- `Buffer::_unsafe_set_uninitialized : I64 -> a -> Buffer a -> Buffer a` — release **せず** に書き込む
  (live value を持たない fresh capacity を埋める用)。依頼のあった `unsafe_set_uninitialized`。
...
要素ループ(`Buffer::_unsafe_set_uninitialized`)で書けて、`from_map` ベンチで現行 InlineLLVM `fill` と同値と
...
| `_unsafe_set_..._unreleased` | `Buffer::_unsafe_set_uninitialized` |
| `_unsafe_set_size` | value field `_size` を設定(heap 書き込みなし) |
| `_unsafe_empty_capacity_unchecked(cap)` | `Array { _buf: Buffer::_unsafe_allocate(cap), _size: 0, _cap: cap }` |
| `_unsafe_fill_size_unchecked(n, x)` | Fix-source: `_buf(n)` 確保, `Buffer::_unsafe_set_uninitialized` の loop で埋め, `_size = n, _cap = n`(最適化器が InlineLLVM 同等にする、実証済み) |
...
`Array` が boxed でなくなるので、boxed 前提の箇所は `Buffer` へ移る:

- ハードコードされた `Array a : Boxed` instance(`stdlib.rs`)は `Array` が unbox になると嘘になる —
  **削除し、`Boxed` を `Buffer a`(実際に boxed な storage)に与える**。
- `get_data_pointer_from_boxed_value` の `if is_array` 分岐は `_buf` を通して `Buffer` の storage pointer
  へ経路変更する。
- `String = unbox struct { _data : Array U8 }` とその C-interop chain(`_get_c_str`、`_unsafe_from_c_str`、
  `borrow_c_str`)は `Array U8 : Boxed` に依存するので、`_data.@_buf` を通じて `Buffer U8` に届くよう書き
  換える。`Array U8` に `mutate_boxed`/`borrow_boxed` を使う数値の `to_bytes`/`from_bytes` 経路も追随する。
- FFI body 全体(`boxed_to_retained_ptr`、`mutate_boxed`、`with_retained`)で `Array` を boxed と仮定して
  いた `is_box`/`is_unbox` の assert は `Buffer` に適用される。
...
1. **`Buffer a` を導入**: boxed primitive として allocate/get/set/set_uninitialized/data-pointer の op と、
