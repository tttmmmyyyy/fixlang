...
**やらないこと**: zero-copy slice(先送り、§3.2)、`Storage` の public 化(internal と決定、§11.3)。
...
`_size` を value に置くと、共有 `_storage` + より小さい `_size` の slice が「作れてしまう」。これは §3.1 を壊す:
小さい `_size` の最後の保持者が、自分の view の外の要素を leak する。これを支えるには、`Storage` に destruct
用の真の「構築済み要素数」を view の `_size` とは別に持たせる必要があり — 本件のスコープ外の実拡張になる。
コア再設計は `_size` == `Storage` の構築済み要素数 を保ち、slice は将来課題とする。
...
- `Storage::_unsafe_get : I64 -> Storage a -> a` — 要素読み出し(bounds unchecked)。read-fold 用に
  `_unretained` 版。
- `Storage::_unsafe_set : I64 -> a -> Storage a -> Storage a` — 要素書き込み、旧占有者を **release** する
  (初期化済み slot 向け)。**内部で unique 化(COW)はしない** — COW clone は `_size` 個の live 要素の複製
  であり、その `_size` は `Array` value 側にあって `Storage` は持たないので、Storage 単体では正しく clone
...
| `_unsafe_set_bounds_uniqueness_unchecked_unreleased` | `Storage::_unsafe_initialize`(この Array 版は §11.4 で削除) | **InlineLLVM**(`Storage` primitive) |
...
`Storage::_unsafe_set` / `_unsafe_get` と punch/plug は、囲む safe op の unique 枝(既に uniqueness を確定済み)の
...
1. **先送り(決定)** — Zero-copy slice(§3.2)。中核には入れない。近い将来に不要と確認するか、そうでなければ
   `Storage` の count モデルが変わる(いずれ入れるかは未決)。
2. **未決** — step 2 の可否。`Array` を boxed のまま `Storage` を内包する中間段が作れるか、それとも反転は atomic で
   なければならない(step 2 を step 3 に畳む)か。実装時に判断する。
3. **決定 — `Storage` は internal に留める(public `@buf`/`@_storage` accessor は公開しない)。** 裸の `Storage` 値が
