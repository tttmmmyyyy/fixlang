...
- `Array::_unsafe_append_capacity_unchecked : Array a -> Array a -> Array a` — **src の全要素を dst の末尾へ**:
  dst を COW -> src が unique なら memcpy して src の `_size` を 0 に(move、RC 操作ゼロ)、shared なら
  `load ptr -> retain -> store` の融合 1 パスでコピー -> src を release -> `dst._size += src._size`。
  契約は `dst._size + src._size <= dst._cap`(呼び出し側)。`append` がこれで書ける。詳細は下の表の後。
...
| `_unsafe_append_capacity_unchecked(src, dst)` — src の全要素を dst の末尾へ | `dst._size + src._size <= dst._cap` | `append` |
...
- **`_unsafe_append_capacity_unchecked` は src の uniqueness で move と copy を選ぶ**:
  - src が unique: `memcpy` してから **src の `_size` を 0 にして** release。`_size` は value 側なのでこの書き換えは
    ローカルで、release は要素を辿らず storage を free するだけになる。**RC 操作ゼロ**。
  - src が shared: 他の holder も同じ要素を参照しているので、dst 用の参照が要る。`load ptr -> retain -> store` の
...
- **早期脱出は primitive でなく Fix-source 側に置く。** `reserve` の `new_cap <= _cap` や `append` の
  ...
  provenance が「素通りした値のもの ∨ `Fresh`」の合流になって正しい。
...
| `append`(Fix-src: force_unique + 要素ごとに retain して書き込むループ) | `_unsafe_append_capacity_unchecked`(dst を COW、src が unique なら move) | **InlineLLVM**(内部 COW、§4) |
...
1. **表現と独立な unsafe primitive の整理**(§3.3/§5)。現行の boxed レイアウトのまま実装・検証でき、
   ...
   - §4 の builder primitive(`_unsafe_set_capacity_bounds_unchecked` / `_unsafe_append_value_capacity_unchecked` /
     `_unsafe_append_capacity_unchecked`)と `Array::unsafe_set_bounds_unchecked` を追加。早期脱出は Fix-source 側に
     置く(§4)。
...
- **NEW `_unsafe_append_capacity_unchecked`**(InlineLLVM)— `(src, dst) -> Array a`。dst を COW;
  `n = src._size`; src の storage が unique なら `memcpy(dst.storage + dst._size, src.storage, n * elem_size)` +
  `src._size = 0`(move、**要素の RC に触らない**)、shared なら `load ptr -> retain -> store` の融合 1 パス;
  src を release; `dst._size += n`。契約: `dst._size + n <= dst._cap`(呼び出し側)。**COW するのは dst だけ**なので
  force-unique: `Some{1,[0]}` の 1 つで足りる(src 側は move/copy を選ぶ読み取りテスト)。borrows: なし。
  prov: `Fresh`。memcpy でよい理由(オーバーラップし得ない)と、`src._size == 0` の早期脱出を Fix-source 側に
  置く理由は §4。
- **NEW `_unsafe_set_capacity_bounds_unchecked`**(InlineLLVM)— `(new_cap, arr) -> Array a`。unique なら
  ...
  足した形)+ 旧配列を release。契約: `new_cap >= _size`(呼び出し側)。force-unique: `Some{1,[0]}`。
...
- builder: `push_back` / `resize` の伸長 -> `_unsafe_append_value_capacity_unchecked`、`append` -> `_unsafe_append_capacity_unchecked`、`reserve` -> `_unsafe_set_capacity_bounds_unchecked`、`from_map` -> `empty` + push_back ループ。早期脱出(`reserve` の `new_cap <= _cap`、`append` の空 src)は Fix-source 側に置く(§4)
...
**新規 Rust 登録プリミティブ:** `_unsafe_truncate_bounds_unchecked`(`pop_back`/`truncate`)、`_unsafe_append_value_capacity_unchecked`(`push_back`/`fill`/`resize`)、`_unsafe_append_capacity_unchecked`(`append`)、`_unsafe_set_capacity_bounds_unchecked`(`reserve`、unique なら realloc)、`unsafe_set_bounds_unchecked`(bounds check だけ省いた `set`)。いずれも §4。
