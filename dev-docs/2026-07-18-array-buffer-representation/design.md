...
- **payload/data ポインタ**(`get_data_pointer_from_boxed_value` の `is_array` 分岐、`_get_boxed_ptr`、
  `mutate_boxed`、`borrow_boxed`): `Buffer` の storage 先頭ポインタを返す(= 現状の要素 buffer ポインタと
  同じ番地)。`get_data_pointer_from_boxed_value` は `is_array` 分岐を `Buffer`(storage は data index)へ
  差し替え。Array への FFI は `array.@_buf` を渡す。
