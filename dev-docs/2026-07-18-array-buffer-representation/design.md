...
- `Array::_unsafe_append_value_capacity_unchecked : a -> I64 -> Array a -> Array a` — **末尾へ `x` を n 個**:
  COW(内部 unique check)-> `build_retain_n(x, n)` -> `[_size, _size+n)` へ write(未初期化スロットなので旧値
...
- **append-n が n を取る利点**: boxed 要素の `fill(n, x)` で、`x` の retain を **増分 n の 1 回**にできる
  ...
  **`build_retain_n` は `build_retain` の並行実装にしない** — `build_retain` を増分でパラメータ化し
  (`build_retain(obj) = build_retain_n(obj, 1)`)、増分を末端まで通す。retain は単なる加算ではないので、
  並行実装を作ると 3 つの分岐を写し間違える: dynamic object(クロージャのキャプチャ領域)の **null 分岐**
...
- **NEW `_unsafe_append_value_capacity_unchecked`**(InlineLLVM)— `(x, n, arr)`。`if !unique { clone }`;
  `build_retain_n(x, n)`(増分 n の 1 回。n 回のインクリメントではない); `[_size, _size+n)` へ `x` を
  write(未初期化スロットなので旧値 release なし); `_size += n`; **`release(x)` を 1 回**(正味 n-1。n=0 でも正しい)。
  契約は `n >= 0` と `_size + n <= _cap`。`build_retain_n` は `build_retain` を増分でパラメータ化して作る(§4)。
