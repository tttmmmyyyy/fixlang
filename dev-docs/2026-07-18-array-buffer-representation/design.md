...
`_unsafe_empty_capacity_unchecked` + `_unsafe_push_back_capacity_unchecked` ループの Fix-source で書け、最適化器がループを
...
移行の副作用として、**入口の配列が provably unique でないループでは check 回数が 1 回から N 回に増える**
(`_unsafe_force_unique` はループの外で 1 回チェックしていた)。ループ搬送値の provenance は入口(`Arg` = 呼び出し側
依存)とループ末尾(`Fresh`)の合流で決まり、入口が `Dyn` だと合流も `Dyn` に落ちて畳めないため。対処は §11.5。
...
4. **未決(測定) — boxed 要素の zero-RC read-fold。** 削除する unretained getter が担っていた「boxed 要素の合計
   ...
   再導入では埋めない。
5. **未決(optimizer 側の追加課題) — ループ搬送値の uniqueness を畳めるようにする。** §5 のとおり
   `_unsafe_force_unique` の撤去で、**入口が provably unique でないループでは COW check が 1 回から N 回に増える**。
   provenance はループ搬送値を「入口(`Arg` = 呼び出し側依存)とループ末尾(`Fresh`)の合流」で見るので、入口が `Dyn`
   だと合流も `Dyn` になり、`resolve_leaf` が `Dynamic` を返して畳めない。実行時には 1 周目の COW で unique に
   なるのに、2 周目以降もチェックと clone path が残る(clone path がループ本体に入るとベクトル化も阻害する)。
   **対処案**: ループを 1 周 peel して、2 周目以降の operand を `Fresh` にする。これは Array 表現とは独立した
   unique-check-elim 側の拡張。**判断材料**: cp-library の `fft`(入口が Fresh なので specialize で畳める見込み)と
   `next_permutation`(入口が `Arg`)、および `append`/`resize` を測って、peel が要るかを決める。
...
- builder(`_unsafe_force_unique` + `_unsafe_set_size` + unreleased write -> `_unsafe_push_back_capacity_unchecked` の loop): `append`, `from_map`, `reserve`, `push_back`, `resize`
