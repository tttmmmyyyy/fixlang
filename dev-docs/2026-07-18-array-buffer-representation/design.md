...
内包し、unique-check-elim が provably-unique のとき check を畳んで in-place にする(現行 `set` が
`force_unique` の InlineLLVM なのはこのため)。「caller が `_unsafe_force_unique` してから
`_unsafe_set_bounds_uniqueness_unchecked_unreleased` で書く」形や、COW の `if unique { write } else { clone; write }`
分岐を Fix の IR に出す形には分解しない — 前者は COW 判定と write が call 境界で離れて optimizer が繋げにくく、
後者は配列を if/else で通す branch/union plumbing(simplifier が戦うループ内 union と同型)を生み、どちらも
vectorize/最適化を阻むから。`Buffer::_unsafe_set` / `_unsafe_get` はその InlineLLVM 内部(または borrow 化された
read)で使う生の部品で、`_buf` を retain せずその場のストレージに対して動く。「bulk op は Fix-source を基本」
(§4)は fill/reserve のコピーの話で、per-element の in-place mutator はこの理由で InlineLLVM に残す。
