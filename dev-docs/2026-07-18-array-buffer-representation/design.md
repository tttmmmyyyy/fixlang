...
**確定した設計判断**(詳細は各節):
- **`Storage` は internal 型。ユーザーに見えるのは `Array` だけ** — `Storage` の型も `Storage::` の Fix API も
  公開せず、Fix インターフェースは `Array` のもの(public/private・safe/unsafe)だけ(§4/§11.2)
...
`Storage a` は **`Array` の内部実装型**であり、ユーザーコードには現れない — ユーザーは常に `Array a` を持ち、
触れる型と Fix インターフェースは `Array` のものだけ(`Storage` の型も `Storage::` の Fix API も非公開、
§4/§11.2)。
...
- **FFI のデータポインタ**は `Array` の FFI ヘルパ経由(§7 `Array::borrow_elements` / `mutate_elements`)。
  `borrow_elements` は内部で `arr.@_storage` を generic な `borrow_boxed`(safe・clone しない)へ渡し、
  `mutate_elements` は専用 InlineLLVM(COW 内蔵)で storage buffer を触る。`Storage a : Boxed` instance は
  **この内部再利用のためだけ**に与える(裸の `Storage` 値はユーザに渡らない、§11.2)。codegen 側は
  `get_data_pointer_from_boxed_value` の array 分岐を storage の buf レイアウトへ合わせる(§7)。capacity は value の
  `_cap` が持つ。
...
2. **決定 — `Storage` は internal に留める。`Storage::` の Fix 関数 API を作らず、Fix 露出は `Array` インターフェース
   だけにする(§4)。** 生ストレージ op(allocate / get / 未初期化 write / data-ptr)は `Array` の InlineLLVM body の
   codegen 内に閉じ、`Storage` の tycon・`_storage` field・`Boxed` instance は std 内部専用(user-visible な型面で
   ない)。裸の `Storage` 値がユーザーへ漏れると、要素寿命 (b) の「ユーザーは `Array` しか持たず裸の `Storage` を
   持たない」不変条件(§3)が壊れる(`Storage` の destructor は生メモリを free するだけで要素 release は Array の
   `_size` が駆動するので、Array より長生きした裸 `Storage` は use-after-free)。FFI の公開面は §7 の scoped な Array
   borrow ヘルパ(`Array::borrow_elements` 系、コールバック中だけ有効な `Ptr` を渡す)だけにする。これにより
