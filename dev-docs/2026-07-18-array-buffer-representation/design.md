...
- `Array::_unsafe_append_value_capacity_unchecked : a -> I64 -> Array a -> Array a` — **末尾へ `x` を n 個**:
  COW(内部 unique check)-> boxed 要素なら `x` の refcount に `+= n`(1 命令)-> `[_size, _size+n)` へ write
  (未初期化スロットなので旧値 release 無し)-> `_size += n`、を **1 op で**行う。契約は **`_size + n <= _cap`**
  だけ(uniqueness も初期化状態も op 内で閉じる)。旧 `_unsafe_set_bounds_uniqueness_unchecked_unreleased` と
...
- `Array::_unsafe_move_tail_bounds_unchecked : I64 -> Array a -> Array a -> (Array a, Array a)` — **src の末尾 n 個を
  dst の末尾へ move**: src と dst をそれぞれ COW -> memcpy -> `src._size -= n`、`dst._size += n`。**要素の RC に
  触らない**。契約は「n <= src の `_size`」「dst の `_size + n <= _cap`」(どちらも呼び出し側)。`reserve` と
  `append` がこれで書ける。詳細と、memcpy でよい理由・複数 force-unique の前提は下の表の後を参照。
...
**builder が要る storage 操作は 3 種類しかないので、alloc と合わせて primitive は 4 つ**。いずれも COW を op 内に
持つので §5 の規則に適合し、値としても純粋:

| primitive | 契約 | 束ねる関数 |
| --- | --- | --- |
| `_unsafe_empty_capacity_unchecked(cap)` | — | `empty` |
| `_unsafe_append_value_capacity_unchecked(x, n, arr)` — 末尾へ `x` を n 個 | `_size + n <= _cap` | `push_back`(n=1)、`fill`、`resize` の伸長 |
| `_unsafe_truncate_bounds_unchecked(n, arr)` — `_size` を n に | `0 <= n <= _size` | `pop_back`(n=`_size-1`)、`truncate` |
| `_unsafe_move_tail_bounds_unchecked(n, src, dst)` — src の末尾 n 個を dst の末尾へ **move** | src の size チェックなし、dst の cap チェックなし | `reserve`、`append` |

- **append-n が n を取る利点**: boxed 要素の `fill(n, x)` で、`x` の retain を **`rc += n` の 1 命令**にできる
  (n 回インクリメントしない)。unboxed 要素ならループ本体は store だけ(`_size` の +1 は誘導変数、COW チェックは
  畳まれる)。
- **truncate-n はループを生まない**: `_size` の更新は 1 回の減算。要素 release は boxed 要素のときだけ n 回必要で、
  それは本質的な仕事。`pop_back` は release 1 回 + 減算 1 回に落ちる。
- **`_unsafe_move_tail_bounds_unchecked`(tailmove)が `reserve` と `append` を統合する。** どちらも
  「別の storage へ要素を移す」操作で、**move なので要素の RC に触らない**。src が unique な `append`
  (`a.append(b)` の b が一時値、という普通の形)では、現行の「retain n 回 -> 後で release n 回」が**丸ごと消える**。
  `reserve` は `_unsafe_empty_capacity_unchecked(new_cap)` を dst にして全要素を move するだけになる。
  src が shared のときは src を COW してから move するので alloc + memcpy + free が 1 回ずつ余分に付くが、
  retain の回数は現行と同じで、しかも cold path。
- **memcpy でよく、オーバーラップ判定は要らない**: 両方を COW した後、src と dst が同じ storage を指すことは
  あり得ない(共有していれば双方の refcount が 2 以上になり両方 clone される。unique = refcount 1 なので、
  2 つの unique な値が同じ storage を指すことは定義上ない)。n=0 のときに何もしない、だけあればよい。
- **前提条件**: tailmove は src と dst を**独立に COW** するので、uniqueness チェックの機構を複数形にする必要が
  ある — 現状 `unique_check_operand` は `Option` を 1 つ返すだけ、`assuming_unique` はどの分岐を落とすか指定できず、
  `op_containers` も op あたり 1 つ。`Vec` 化と「どの分岐か」の指定を足す(§9 step 1 に含める)。
- `from_map` は `reserve` + `push_back` ループの素朴実装でよい。コールバック呼び出しが支配的で、利用頻度も低い。
...
チェック無しの write を復活させることではなく、**COW 内蔵の bulk op を足すこと**。`array_unsafe_fill` は削除する
...
| `reserve` / `append`(Fix-src: 新 storage 確保 + 要素ごとに retain してコピー) | `_unsafe_move_tail_bounds_unchecked`(両方 COW してから末尾 n 個を **move**、要素の RC に触らない) | **InlineLLVM**(内部 COW ×2、§4) |
...
1. **表現と独立な unsafe primitive の整理**(§3.3/§5)。現行の boxed レイアウトのまま実装・検証でき、
   ...
   - §4 の builder primitive(`_unsafe_append_value_capacity_unchecked` / `_unsafe_move_tail_bounds_unchecked`)と
     `Array::unsafe_set_bounds_unchecked` を追加。tailmove の前に **uniqueness チェック機構の複数形化**
     (`unique_check_operand` の `Vec` 化、`assuming_unique` へのインデックス、`op_containers` の `Vec` 化)を入れる。
...
- **NEW `_unsafe_move_tail_bounds_unchecked`**(tailmove、InlineLLVM)— `(n, src, dst) -> (Array a, Array a)`。
  `if n == 0 { ret }`; src と dst を**それぞれ** COW; `memcpy(dst.storage + dst._size, src.storage + src._size - n,
  n * elem_size)`; `src._size -= n`; `dst._size += n`。**要素の RC には触らない**(move)。契約: `n <= src._size`、
  `dst._size + n <= dst._cap`(どちらも呼び出し側)。force-unique: **2 つ**(operand 1 と 2 の storage leaf)—
  現状の単数機構では表せないので `Vec` 化が前提(§4)。borrows: なし。prov: 両 leaf とも `Fresh`。
  memcpy でよい理由(オーバーラップし得ない)は §4。
...
- **NEW `_unsafe_append_value_capacity_unchecked`**(InlineLLVM)— `(x, n, arr)`。`if !unique { clone }`;
  boxed 要素なら `x` の refcount に **`+= n` を 1 回**(n 回のインクリメントではない); `[_size, _size+n)` へ `x` を
  write(未初期化スロットなので旧値 release なし); `_size += n`。契約は `_size + n <= _cap` のみ。
  borrows: なし(x[0]・arr[2] consume)。force-unique: `Some{2,[0]}`。prov: `Fresh`(storage leaf)。
...
- builder: `push_back` / `resize` の伸長 -> `_unsafe_append_value_capacity_unchecked`、`append` / `reserve` -> `_unsafe_move_tail_bounds_unchecked`、`from_map` -> `reserve` + push_back ループ
...
- `_unsafe_set_size` -> `_unsafe_grow_size`。残る呼び出しは **FFI 出力バッファの経路だけ**: `read_n_bytes`/`unsafe_from_c_str_ptr`(`_io`)/数値 `to_bytes` 一式(builder 群は §4 の primitive へ移る)。**真の shrink を行う `String::from_bytes` と `_unsafe_from_c_str` の切り詰め経路は効率化した `truncate`(safe shrink、§13.3-1)を使う** — これで「共有配列の `_size` をその場で書き換える」経路が無くなる

**新規 Rust 登録プリミティブ:** `_unsafe_truncate_bounds_unchecked`(`pop_back`/`truncate`)、`_unsafe_append_value_capacity_unchecked`(`push_back`/`fill`/`resize`)、`_unsafe_move_tail_bounds_unchecked`(`reserve`/`append`)、`unsafe_set_bounds_unchecked`(bounds check だけ省いた `set`)。いずれも §4。
