...
- uniqueness-check-less な mutate は「直前の op が uniqueness を確立 + operand を op 後に参照しない」契約に載せ、
  確立と使用が離れる `_unsafe_force_unique` は削除(§5/§3.3/§11.3)
...
**削除するのは Fix レベルの「呼び出し側が事前に手動 unique を保証する」primitive のうち 2 つ**:
`_unsafe_set_bounds_uniqueness_unchecked_unreleased`(from_map/push_back/reserve/append/resize)と、**その前に置く
`_unsafe_force_unique`**(上記 + sort_by/reverse — いずれも in-place mutation の前処理)。これらは (1) の COW check を
**skip する**ために存在する。**特に `_unsafe_force_unique` は redundant なだけでなく危険**: doc
...
**punch/plug の uniqueness-unchecked 版は存置する。** 呼び出しは std 内 4 箇所だけ(`mod` /
`_unsafe_act_bounds_unchecked_identity` / `_unsafe_act_bounds_unchecked_tuple2` の plug、
`_unsafe_act_bounds_unchecked` の punch)で、いずれも §5 の契約を満たす。COW 版へ寄せると uniqueness check が残る:
`mod` の plug は operand が punch の結果で、`InlineLLVMArrayPunchBody` の `result_prov` が既定の `Dyn` なので
unique-check-elim が畳めない。`act` の punch は operand が `unsafe_is_unique` の結果で、これも `Dyn` 固定(上述)なので
畳めず、背中合わせの 2 チェックを LLVM の GVN が統合する Max でしか消えない。COW 版へ寄せるなら、先に punch へ
`result_prov`(punched-array leaf = `Fresh`、moved-out 要素 leaf = `Dyn`)を実装し、残存チェックが消えることを
測るのが前提になる。
...
- `Array::_unsafe_initialize : I64 -> a -> Array a -> Array a` — **未初期化スロットへの write**(release せず、
  ...
  安全性は §5 の契約に載る: uniqueness は直前の `_unsafe_grow_size`(内部 COW)または
  `_unsafe_empty_capacity_unchecked` の fresh alloc が確立し、builder は配列を線形に流して op の後で参照しない。
  **窓の不変条件**: builder は size を先に伸ばしてから `[旧size, 新size)` を埋めるので、`_unsafe_grow_size` から
  最後の `_unsafe_initialize` までの区間、配列 value は「`_size` に含まれるのに未初期化」のスロットを持つ。
  **この区間で release / clone / retain を起こしてはならない** — 複製された value が最後に release されると未初期化
  スロットを release する。したがって builder のループ本体では配列を線形に流し、他所へ渡さない。残る `_unsafe_`
  契約は「スロットは未初期化(caller が埋める)」「in-bounds(`< _cap`)」(uniqueness とは直交、§3.3 注)。旧
  `_unsafe_set_bounds_uniqueness_unchecked_unreleased` の後継。この窓を無くす代案は §11.5。
...
**uniqueness-check-less な mutate primitive は、次の契約を満たす呼び出し形でだけ使う。**

1. **uniqueness を直前の op が確立する** — COW を内包した op(punch の force-unique 版、`_unsafe_grow_size`)か、
   fresh alloc(`_unsafe_empty_capacity_unchecked`)の直後に置く。
2. **その operand を op より後で参照しない** — rc 挿入は「own な operand が、その文より後でも live なら op の直前に
   retain を置く」規則で動く(全 opt レベル)。2 が破れると、直前に uniqueness を確認していても op の実行時点で
   refcount が 2 になり、unchecked op が共有配列を破壊する。
3. **将来 Fix レベルの CSE / PRE を入れるときは、`unique_check_operand` を持たない mutate op を対象外にする** —
   これらは値として純粋でないので、同一 operand への 2 つの呼びを 1 つに畳むと、別々の配列を返すはずのところが
   同じ配列を返す。COW を内包した op はこの制約を受けない。

この契約は型でも解析でも検査されないので、呼び出し形をレビューで保つ。該当するのは `_unsafe_initialize`(§4)と
punch/plug の uniqueness-unchecked 版(§3.3)で、いずれも std 内の限られた呼び出し形に閉じている。

**契約 1 の「直前」を任意距離に伸ばす `_unsafe_force_unique` は削除する**(§11.3)。uniqueness の確立と使用が別 op に
分かれるため、間に何が入るかを呼び出し側の形だけでは押さえられない。doc(`std_array_force_unique.md`)のとおり
`f(x); f(x)` の inline 後に生じる 2 つの `force_unique(x)` を CSE が 1 つに畳むと、2 つ目の consumer が non-unique な
配列を掴む。`sort_by`/`reverse` は COW 内蔵 `swap` に、builder は `_unsafe_grow_size` + `_unsafe_initialize` に
寄せれば、契約 1 の「直前」が 1 op に縮む。

`_unsafe_initialize` を **COW 内蔵にはしない**: fill 中は size が低く、COW clone すると `[0,size)` だけコピーして
fresh write を取りこぼす — COW は live-data 上書き(`set`)にのみ意味があり fresh-capacity init には効かない。
...
3. **決定(採用した方向) — uniqueness の確立と使用が離れる unsafe 関数を減らす。** `_unsafe_force_unique` と
   `_unsafe_set_bounds_uniqueness_unchecked_unreleased` を、COW を内包した op(`_unsafe_grow_size`)と、確立の
   直後に置く `_unsafe_initialize` に寄せる(§5 の契約 1 が 1 op に縮む)。size 書き込みは §3.1 の
   `_unsafe_grow_size` が内部 check 化して既にこの一例。fill を Fix-source 化するのと同じ方向で、redesign を機に
   unsafe API の表面積を縮める(削除対象は §3.3)。punch/plug の uniqueness-unchecked 版は §3.3 のとおり存置する。
   前提は §3.3(is_unique)と §3.1(unique-only な size 書き込み)が正しく効くこと。surviving
...
4. **未決(測定) — boxed 要素の zero-RC read-fold。** 削除する unretained getter が担っていた「boxed 要素の合計
   ...
   再導入では埋めない。
5. **未決 — builder の窓(§4 `_unsafe_initialize`)を append 形 primitive で無くす案。** `_unsafe_grow_size` +
   `_unsafe_initialize` の 2 op を、`Array::_unsafe_push_back_capacity_unchecked : a -> Array a -> Array a`
   (COW 内包 + slot `_size` へ write + `_size` を 1 増やす、契約は `_size < _cap` のみ)の 1 op に畳む案。**利点**:
   op 境界で配列 value が常に valid になり §4 の窓が消える。uniqueness-check-less な mutate が builder から
   無くなり、§5 の契約対象が punch/plug だけになる。値として純粋なので将来の CSE 制約(§5 契約 3)も外れる。
   **適用範囲**: std の unreleased write 5 箇所(`append`/`from_map`/`push_back`/`reserve`/`resize`)は
   すべて「末尾から昇順に埋める」形なので、この 1 op で書ける。**残る用途**: FFI 出力バッファ
   (`Array::empty(k)` の後に C 側が埋める `to_bytes`/`read_n_bytes`/`unsafe_from_c_str_ptr`)は「未初期化スロットを
   size に含める」ことが必要なので `_unsafe_grow_size` を残す。要素型が unboxed なので未初期化スロットの traverse は
   無害。**判断材料**: fill/from_map ループの per-element COW check が Max で畳まれるか(畳まれればベクトル化も
   維持される)を測る。
...
`Array::_unsafe_initialize`)、`_unsafe_get_linear_bounds_unchecked_unretained`(両変種)、
`array_unsafe_fill`(fill -> Fix-src)、`_get_ptr`。(`@size`/`@capacity` は削除で
...
- `mod`/`act`(`unsafe_is_unique` -> `_unsafe_is_storage_unique`。punch/plug の版選択と act の分岐構造は維持、§3.3): `mod`, `_unsafe_act_bounds_unchecked_identity`, `_unsafe_act_bounds_unchecked_tuple2`, `_unsafe_act_bounds_unchecked`
...
- Rust 登録プリミティブ: `_unsafe_force_unique`, `_unsafe_set_bounds_uniqueness_unchecked_unreleased`, `_unsafe_fill_size_unchecked`, `_pop_back_nonempty`(-> `_unsafe_truncate_bounds_unchecked` に統合)
