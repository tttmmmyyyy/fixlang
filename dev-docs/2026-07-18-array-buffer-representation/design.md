...
(InlineLLVM のまま Buffer storage へ向けるものと、Fix-src で `Buffer` primitive を組むものがある)。「実装」列が
その別を示す。完全な棚卸しは `investigation-notes.md` §5:

| 現行 `Array` primitive | 移行後 | 実装 |
| --- | --- | --- |
| `@size`(`extract_field(ARRAY_LEN_IDX)`) | `arr.@_size` — register 読み出し(目標) | struct field accessor(compiler 生成の extractvalue) |
| `@capacity`(`extract_field(ARRAY_CAP_IDX)`) | `arr.@_cap` — register 読み出し | struct field accessor(compiler 生成) |
...
| `_pop_back_nonempty` | `_buf` を unique 化, 末尾要素を noretain read して release, value `_size -= 1`(empty check 無し = caller の `pop_back` が担う) | **InlineLLVM**(in-place、COW) |
...
4. **事前 unique-check が要る unsafe 関数を減らす(方向)** — unique-check-elim があるので、
   `_uniqueness_unchecked` 系のような「呼び出し側が事前に unique を保証する」primitive を、自前で unique-check
   する safe 版に寄せる(unique-check-elim が provably-unique で畳んで同性能にする)。size 書き込み
   (`_unsafe_force_unique` + 旧 `_unsafe_set_size`)は §3.1 の `_unsafe_grow_size` が内部 check 化して既にこの
   パターンの一例。fill が Fix-source で InlineLLVM と同値だった実証と同じ方向で、この redesign を機に unsafe
   API の表面積を縮める。
