...
- `_unsafe_set_size` は要素側 Array ではほぼ **増加専用**(from_map/fill/reserve/push_back/append は
  ...
  直接書く**(`_unsafe_set_size` を使わない)。**任意の safe な shrink は `shrink_size`(COW + 切り詰め要素の
  release を内包、§13.3-1)が担う** — `String::from_bytes` 等の `Array U8` shrink 経路や `truncate` はこれを使う
...
| `Array::shrink_size : I64 -> Array a -> Array a` | **public safe** shrink。`n >= _size` なら no-op、そうでなければ切り詰め要素 `[n, _size)` を release し shared なら COW clone して value `_size = n`。unique-check-elim + unboxed release で `Array U8`/unique では「`_size` 下げ」に畳む。`truncate`/`String::from_bytes` が使う。public だが in-place で `_storage` を触るので InlineLLVM |
...
- shrink 経路(新 `shrink_size` を使う、§13.3-1): `truncate`(現状の pop_back ループを 1 op に置換)、`String::from_bytes` / `_unsafe_from_c_str` の切り詰め
...
- `_unsafe_set_size` -> `_unsafe_grow_size`(grow 経路)。呼び出し: `append`/`from_map`/`push_back`/`reserve`/`resize`/`read_n_bytes`/`unsafe_from_c_str_ptr`(`_io`)/数値 `to_bytes` 一式。**真の shrink を行う `String::from_bytes` と `_unsafe_from_c_str` の切り詰め経路は `shrink_size`(safe shrink、§13.3-1)を使う**
...
1. **shrink 経路 — 安全な `shrink_size` を追加(決定)。** `_unsafe_grow_size`(前提 `n >= _size`)は
   `String::from_bytes`(null terminator の後にバイトが続くと真の shrink)や `_unsafe_from_c_str` の一部・数値
   `to_string` の over-allocate 経路を表せない。そこで grow の対として **安全な shrink プリミティブ
   `Array::shrink_size : I64 -> Array a -> Array a`** を追加する(**public**)。契約: `n >= _size` なら no-op、
   そうでなければ size を n に縮める。**内部で unique check し、shared なら COW clone**(他の共有者の要素を巻き添えに
   release しないため)、**切り詰めた要素 `[n, _size)` を release**、value `_size = n`。**どちらも最適化で消える**:
   unique-check-elim が provably-unique で COW を畳み、unboxed 要素(`Array U8` 等)では要素 release が no-op になる —
   よって `String::from_bytes`(unique な `Array U8`)では実質「`_size` を下げるだけ」に落ち、旧 `_unsafe_set_size` の
   shrink と**同性能かつ安全**。grow が `_unsafe_`(未初期化スロットを残す契約)なのに対し shrink は要素 release + COW
   を内包するので **safe**(prefix の非対称は意図的)。`String::from_bytes` は `shrink_size(null_idx+1)` を使う。boxed
   要素の配列でも安全に使える汎用 op。**注: 既存の public `truncate`(先頭 n 要素を残す、n>=size で no-op)と機能が
   重複する** — `truncate` はこの `shrink_size` へ委譲する薄い wrapper に置き換える(pop_back ループを廃止)。両者の
   公開名を残すか `truncate` を deprecate するかは要判断。
