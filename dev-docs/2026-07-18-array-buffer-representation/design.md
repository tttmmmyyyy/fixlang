...
**(1) tycon**: `Array` は現状の `TyConVariant::Array` を維持(新 tycon を足さない)。変わるのは `ty_to_object_ty`
の `Array` arm が返す ObjectType だけ。
...
- FFI `borrow_elements` / `mutate_elements`: storage Ptr を retain / borrow して buffer 先頭ポインタを渡す(§7。
  generic `borrow_boxed` へは委譲しない = storage は Boxed 値でないため、`with_retained` 相当を codegen で再現する)。
...
- `Array::_unsafe_get_bounds_unchecked : I64 -> Array a -> a` — 要素 read(bounds unchecked)。返り値を
  ...
  **非 retain(unretained)版は作らない**: read-fold の per-element RC は、この retaining getter に optimizer
  (borrow 化 §2.1 + retain/release 相殺 §2.2)を効かせて落とす方針(§11.4 の read-fold note)。現状 std も既に
  unretained 版を使っておらず(`_unsafe_get_linear_bounds_unchecked_unretained` は利用ゼロ)、`ArrayIterator::advance`
  はこの retaining getter でループしている。
...
`_unsafe_empty_capacity_unchecked` + `_unsafe_initialize` ループの Fix-source で書け、`from_map` ベンチで現行
InlineLLVM `fill` と同値と実証済み(Ir 7,640,507 vs 7,640,521、差 0.0002%)— 最適化器がループをベクトル化して
手書き LLVM に並ぶ。よって `fill` は Fix-source にし、InlineLLVM の `array_unsafe_fill` は削除する。COW clone /
...
| `_unsafe_fill_size_unchecked(n, x)` | `_unsafe_empty_capacity_unchecked(n)` 確保, `Array::_unsafe_initialize` の loop で埋め(最適化器が InlineLLVM 同等にする、実証済み) | Fix-src |
...
- **`Array a : Boxed` instance を削除する**(`stdlib.rs` のハードコード instance)。移設先は無い(storage は型でない
  ので `Boxed` instance を持てない)。**ユーザー可視の破壊的変更**: Array へ直接 `array.borrow_boxed(...)` /
...
- **データポインタ**(生要素先頭): Array の FFI ヘルパ **`Array::borrow_elements`(+ `_io` / 可変版
  ...
  callback へ渡す。`with_retained`/`borrow_boxed` の意味論(callback 中だけ生存・clone しない)を **codegen で再現**
  する(generic へは委譲しない = storage が Boxed 値でないため)。返る番地は現状の要素領域と同じ。ポインタは
...
3. **`Array` の値レイアウトを unbox `{ Ptr, size, cap }` に反転**(§2.2)。`ty_to_object_ty` の `Array` arm、
   ...
   不可分 unit 述語(§3.2)、layout-constant 箇所すべて(`investigation-notes.md` §8)を一斉に更新。`Array` の
   `Boxed` instance を削除(移設先なし)、`@size`/`@capacity` を extractvalue 版へ、`String`/FFI chain(§7)と
   PunchedArray(§6)を書き換える。
4. **Debug info**(§8)。
...
- redesign 共通: size/cap は value field 読み。COW helper `make_array_unique` は要素数を value `size` から読み、
  storage を unique 化する(内部名は実装時に確定)。以前 flat boxed array で `path:[]`/`Fresh` だったものは、
  この `Array` custom unit を指すよう再表現する。
...
**(6) 型・登録の変更**(op ではないが必要): `Storage` tycon は **新設しない**(storage は codegen 専用 ObjectType、
§2.2)。`Array` は `TyConVariant::Array` のまま、`ty_to_object_ty` の `Array` arm を **unbox `{ Ptr, size, cap }`** へ
変更(index 定数 `ARRAY_STORAGE_IDX` / `ARRAY_SIZE_IDX` / `ARRAY_CAP_IDX`)、storage 用の ad-hoc ObjectType
コンストラクタ `array_storage_object_ty` と `STORAGE_CTRL_IDX` / `STORAGE_BUF_IDX` を追加、hardcoded
`Array a : Boxed` instance を **削除**(移設先なし)、`Array` を不可分 unit 述語へ追加(§3.2)。詳細は §2.2/§7。

### 13.2 std.fix Fix 関数・trait instance(public シグネチャは特記以外すべて不変)

**追加(いずれも Array の InlineLLVM。storage は Boxed 値でないので generic `borrow_boxed`/`mutate_boxed` へは委譲しない):**
...
- trait instance: **`impl Array a : Boxed` を削除**(移設先なし = storage は型でない)。**user-visible break**: `array.borrow_boxed` / `array.boxed_to_retained_ptr` が型エラー -> `borrow_elements` か自作 boxed 型でラップ
...
2. **`mutate_elements` / `borrow_elements` はいずれも Array の専用 InlineLLVM(決定)。** storage を型にしない
   (§2.2)ため storage は Boxed 値でなく、generic `borrow_boxed` / `mutate_boxed` への委譲(Fix-source 化)は
   できない。両者とも codegen が storage を直接扱う:
