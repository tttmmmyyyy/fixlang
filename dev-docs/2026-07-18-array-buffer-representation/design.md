...
**両者は `Array` operand を borrow と宣言する**(`borrows_operand(0, ..) = true`)。scalar な size/cap を読むだけで
storage の参照を取らないので、これで write loop 中の `arr.@size` が storage を retain せず、storage leaf の
provenance が保たれて loop の uniqueness check が畳める。これは「fully unbox な field の読み出しは container の
borrow」という一般機構(`InlineLLVMStructGetBody` が `move_out_struct_field` で実装済み)を、Array は primitive
なので専用 builtin として再現するもの。borrow 宣言に伴い operand は `get_scoped_obj_noretain` で読む(上記不変条件)。
...
`truncate_to_unit`)。ただし `is_box` / `is_union` / `is_punched_array` は「1つの不可分 unit」として扱い、中へ
...
追う `_storage` leaf を `truncate_to_unit` が `Array` unit に丸めて突き合わせる(現行の union/is_box と同じ経路)。よって
...
- **`TypeNode::is_rc_unit_root` に `is_array()` を足す。** この述語は既に導入済みで、`rc_units_go` / `truncate_to_unit` /
...
- **`TypeNode::is_fully_unboxed` を `rc_units` の上に定義し直す**(「RC unit を 1 つも持たない」)。現在の
  ...
  `is_rc_unit_root` の判定より前に early return する**。`boxed_leaf_paths` 経由でもう 1 つ効く:
  各 op の `unique_check_operand` は `unique_check_on_boxed_leaf` を通り、宣言した path が
  `boxed_leaf_paths(container_ty)` に無ければ `None` に潰れるので、この述語が誤ると **Array の COW チェックが
  宣言ごと消えて一切畳めなくなる**(§13.1)。放置すると storage が解放されず(全 `String` が leak)、
...
**この再設計が依存する 2 つの RC 機構は、unique-check-elim から取り込み済み(`f4d9d30f` / `26dca513`)。**

- **fully unbox な field の読み出しは container の borrow**(`f4d9d30f`)。`InlineLLVMStructGetBody` は
  fully-unbox な field を `move_out_struct_field` で取り出し、container を consume せず borrow と宣言するように
  なった。**`@size`/`@capacity` が register 読みになる(=size/cap を value に出した狙い)のはこの機構がある前提**:
  これが無いと write loop 中の `arr.@size` が毎回 storage を retain して leaf を `Unknown` に落とし、後続 `array_set`
  の uniqueness check が残る。Array は primitive で `StructGetBody` を通らないので、`@size`/`@capacity` builtin 自身が
  同じ属性(`borrows_operand(0)=true` + `get_scoped_obj_noretain`)を宣言する(§2.2(5))。これで plan の
  「unbox getter が sibling を op 内で release する」`[#F4]` blocker は解消された。
- **match binding が運ぶ object を追跡する**(`26dca513`)。RC cancellation は `root` を `origin`(`Origin::Join`)に
  置き換え、`match` の枝が返す値がスクルーティニのエイリアス(unbox union の variant slot、catch-all 枝の
  スクルーティニ全体)のとき、その参照が binding に流れることを追えるようになった。**redesign の Fix-source builder
  (`push_back` / LoopState の plumbing)は storage を `match` の外へ返す**ので、これが無いと cancellation が storage を
  早期解放し得た。取り込み済みなので、この形は安全。
...
**generic 版を unbox な Array に呼ぶことは、下記の `[a : Boxed]` 制約で型エラーにする。** 「返るのは
`const true` だが COW 判定は Array 専用版を使うので無害」では済まない — 経路依存の精密化(下記)が入った今、
generic 版が `unique_check_operand` を宣言できてしまう型では **true 枝で検査対象の leaf が `Fresh` と解釈される**。
`unique_check_on_boxed_leaf` は「宣言した path が `boxed_leaf_paths` にあるか」で守っており、素の unbox 構造体は
`[]` に boxed leaf を持たないので `None` になって守られる。ところが反転後の `Array` は §3.2 の修正により
`boxed_leaf_paths` が `[]` を返すので**この守りをすり抜ける** — 実行時は常に `true`、解析は `Fresh`、結果として
共有配列を in-place 破壊する。したがって制約は「望ましい破壊」ではなく **健全性の要件**である。
...
- **属性は generic `unsafe_is_unique` と同型にする**: `unique_check_operand = Some{0, []}`、
  ...
  `result_prov` は **`Unknown` 固定**。最後の 1 つは generic 版と同じ理由による — 第 2 成分は引数そのものだが、
  passthrough(`Arg`)と宣言すると borrow パスがそれをエイリアス兼「非消費」と解釈し、後続 use が引数を
  shared に読ませる retain を抑止するので、共有 storage を unique と誤判定する。
- **`provenance.rs` の 3 箇所へ、この op を認識対象として加える。** generic 版の扱いは既に実装されていて、
  いずれも `llvm_gen.as_any().is::<InlineLLVMIsUniqueFunctionBody>()` という**具象型の判定**で入口を決めている:
  - `interpret_rhs`: 結果の provenance を `result_prov` ではなく局所ルール `is_unique_result` で作る
    (第 2 成分の leaf を operand の leaf からそのまま写す)。`result_prov` を `Unknown` に保ったまま「共有状態は
    同じ」だけを伝えるための仕組みで、`Debug::assert_unique` を挟んだだけで下流の畳み込みが消えるのを防いでいる。
    `_unsafe_is_storage_unique` の結果も `(Bool, Array a)` と同じ形なので、そのまま同じルールに乗る。
  - `interpret_rhs`: `is_unique_tested_paths` に「どの leaf を検査したか」を記録する。
  - `note_unique_flag` / `refine_by_unique_flag`: その記録を使って、**`if unique { .. }` の true 枝で検査対象の
    leaf を `Fresh` と解釈する**(経路依存の精密化)。`act` の unique 枝の COW punch はこれで畳める。
  加える先は具象型の判定なので、Array 版の body 型を足すだけで済む。
- さらに **generic `unsafe_is_unique` に `[a : Boxed]` 制約を付ける**(現状は無制約で unboxed に `const true`
  ...
  dead になり除去可。上のとおりこれは健全性の要件であって、単に望ましい破壊なのではない。現状 unboxed 型に
  呼んでいる箇所は無い(`assert_unique` も Array=boxed にしか使われていない)。波及: `assert_unique :
...
**punch/plug を COW 版へ寄せるのに要る `InlineLLVMArrayPunchBody` の `result_prov` は実装済み** —
`Provenance::fresh_under(result_ty, type_env, &[PUNCHED_ARRAY_FIELD])`、つまり punched-array leaf が `Fresh` で
moved-out 要素 leaf が `Unknown`。**`force_unique` では分岐しない**(§5): チェック無しの版も、optimizer が一意性を
証明した場所か、呼び手が契約で約束した場所にしか置かれない。これで `mod` の plug の operand が `Fresh` になり畳める。
`act` の punch は operand が `unsafe_is_unique` の結果なので、上の経路依存の精密化が入った今は true 枝で `Fresh` と
解釈されて畳める。
...
- `Array::_unsafe_grow_size : I64 -> Array a -> Array a` — COW + value の `_size` を伸ばす(新スロットは未初期化)。
  **用途は FFI 出力バッファのみ**(`Array::empty(k)` を確保し `mutate_elements` で C 側に埋めさせる
  ...
  未初期化スロットが release されない。つまりこの条件は作法ではなく、**窓(`_size` が未初期化スロットを覆う
  区間)を無害にしている当のもの**である。**この用途と条件を doc コメントに明記し、さらに InlineLLVM body の
  `generate` で機械的に強制する** — 結果型 `Array a` から要素型を取り(§3.2 の分類でいう「要素型の取得」)、
  §3.2 で一本化する述語で boxed leaf の有無を見て、あれば op 名と要素型名を出して落とす。判定はその型で op が
  実体化されたときだけ走るので、検出のタイミングとして過不足がない。`_unsafe_` op はユーザーからも呼べる
  (cp-library が現に `_unsafe_` 系を直接使っている)ので、doc コメントだけの契約では破られたときに leak か
  UB になる。(現 `_unsafe_set_size` の doc は「長さを検証せずに更新する」としか書いておらず、これが builder 用の
  汎用 op に見えることが `String::from_bytes` が共有配列を縮めていた不具合の下地になった。)
...
- **早期脱出は primitive でなく Fix-source 側に置く。** `reserve` の `new_cap <= _cap` や `append` の
  ...
  `Unknown` に落とすしかなくなり、下流の COW チェックが畳めなくなる。Fix-source の `if` に置けば、`if` の結果の
...
| --- | --- | --- |
| `@size`(`extract_field(ARRAY_LEN_IDX)` = heap load) | `extractvalue(value, ARRAY_SIZE_IDX)` — register 読み出し | **InlineLLVM**(手登録 builtin のまま。body を heap load から value への `extractvalue` に。Array を borrow と宣言、§2.2(5)) |
...
- 2 周目以降は body も Unique keyed クローンになり、`array_set[unique]` = check 畳み済み・clone path 無し。
  よってステディ状態のループ本体はベクトル化も阻害されない。

入口の配列が shared でも unique でも、この形に落ちる(1 周目が入口の `Unknown` を吸収する)。
...
現行の `Array` の debug 型は**ポインタ**である(`ty_to_debug_embedded_ty` が `is_box` のときだけ
`create_pointer_type` を被せる)。指し先の struct は `<control block>` / `<array size>` /
`<array buffer of T>` の 3 メンバで、最後の入れ子 struct がさらに `<capacity>` と
`<array elements>[DEBUG_ARRAY_ASSUMED_LEN]` を持つ。反転後はこれを次の 2 つに組み替える
(`to_debug_type` / `ty_to_debug_struct_ty`)。

**`Array a` の debug 型 = 3 メンバの value struct**(ポインタではない。unbox になるので
`ty_to_debug_embedded_ty` が自然に struct を返す):

| メンバ | 型 | 備考 |
| --- | --- | --- |
| `_storage` | `#ArrayStorage a` へのポインタ | `#ArrayStorage` は boxed なので `is_box` 経由で自動的にポインタ型になる |
| `<array size>` | I64 | |
| `<array capacity>` | I64 | 現行は `<array buffer of T>` の中にある |

**`#ArrayStorage a` の debug 型 = 2 メンバ**: `<control block>` と
`<array elements>`(要素型の配列、`DEBUG_ARRAY_ASSUMED_LEN` 個を主張)。現行の `<array buffer of T>` が
持っていた 2 メンバのうち、capacity は value 側へ、elements はここへ分かれる。

**サイズ水増しの適用先が変わる。** 現行は `Array` の debug struct のサイズを「最後のメンバのオフセット +
サイズ」に引き伸ばして 100 要素分を覆わせている(デバッガは宣言サイズの内側しか値を読まないため)。反転後の
`Array` は**本物の値レイアウト(24 バイト)を持つので引き伸ばしてはならない**。水増しは `#ArrayStorage` 側だけに
残す。値型から嘘が消える。

**メンバ名**: `<array size>` / `<array capacity>` / `<control block>` / `<array elements>` は角括弧付きのまま
(コンパイラ内部のメンバであることを示す現行の規約)。**storage ポインタだけは角括弧なしの `_storage` にする** —
gdb の式パーサは `arr.<array storage>` を解釈できず、角括弧付きにすると**要素に到達する手段が無くなる**ため。
`SubObject` メンバの既定名は `<subelement of type ...>` なので、`<array size>` と同じ `is_array()` の分岐で名付ける。

gdb から見た形:

```
print arr           -> {_storage = 0x5555..., <array size> = 3, <array capacity> = 3}
print *arr._storage -> {<control block> = ..., <array elements> = {10, 20, 30, ...}}
x/s (char*)s._data._storage + 8      # String のバイト列(要素オフセット)
```

size が逆参照なしで見えるようになる代わりに、size と要素は 1 回の print では出せなくなる。

**`src/tests/test_debug_info.rs` の 3 本を更新する**(テスト更新も実装の一部):

1. `print *arr` -> `print arr`(size を確認)と `print *arr._storage`(要素を確認)の 2 つに分ける。
2. `x/s (char*)s._data + 24` -> `x/s (char*)s._data._storage + 8`。24 は「ControlBlock 8 + size 8 + cap 8」だった。
3. `<array size> = 3` と `<array elements> = {10, 20, 30` の同時 assert を、2 つの print に分ける。

**fixlang#78(要素数を実行時の値にする)との関係**: 固定 100 要素のワークアラウンドは維持する。反転後は要素が
`#ArrayStorage` に、長さが `Array` value にあり**別の struct に分かれる**。DWARF の `DW_AT_count` は同じ struct の
他メンバなら参照できるが、ポインタを保持している側の値は参照できない。したがって DWARF レベルでの解決は
反転後さらに遠のく。代わりの道は §12.4(gdb pretty-printer)。
...
1. **表現と独立な unsafe primitive の整理**(§3.3/§5)。現行の boxed レイアウトのまま実装・検証でき、
   ...
   - `_unsafe_grow_size`(改名前は `_unsafe_set_size`)の要素型 assert を入れる(§4)。
   ...
   - 新設した op ごとに、宣言(`unique_check_operand` / `result_prov` / `borrows_operand`)を固定するケースを
     `src/tests/test_provenance/cases/` に足す。既存ケースと同じ作法で、**まず宣言を誤らせてケースが壊れることを
     確かめてから**入れる — 宣言の誤りは畳み込みが静かに止まるか、共有データを壊すかのどちらかで、どちらも
     通常のテストでは出ない。
...
2. **決定 — Fix レベルの uniqueness-check-less な mutate primitive を全廃する(§5)。** 対象は
   ...
   正しく効くこと(punch の `result_prov` は実装済み、§3.3)。surviving unsafe RMW primitive の削除計画に接続する。
...
### 12.2 `_unsafe_grow_size` を消して未初期化の窓を無くす

`_unsafe_grow_size` は「`_size` を伸ばすが新スロットは未初期化」という窓を設計に残す唯一の op で、要素型 assert
(§4)はその窓を無害に保つ条件を強制しているだけである。窓そのものを消すなら、FFI 出力バッファを
`Array::fill(n, 0_U8)` + `mutate_elements` + `truncate(実際の長さ)` に置き換えて op を削除する。

いま採らない理由は `read_n_bytes` のコスト。`read_string` は 1024 バイトずつこれを呼ぶので、**ファイル全体に対して
memset が 1 パス増える**。現状の経路は fread(stdio バッファからの memcpy)+ 後段の `append` の 2 パスなので、
読み込みのメモリトラフィックが 2 パスから 3 パスになる。窓を消すためだけに払うには高い。

FFI 出力バッファの経路が「長さを先に知れる」形へ移ったときに再検討する。

### 12.3 要素バッファのアラインメント

`#ArrayStorage` の要素オフセットは **8**(ControlBlock のサイズが 8 で、どの要素型でもそこに収まる)。
ブロックの先頭は malloc が置いた位置なので、要素バッファの番地は揃わない。ベクタ化されたループは
32 バイト単位でアクセスするため、64 バイトのキャッシュラインをまたぐアクセスが一定割合で発生する。
またぐアクセスは約 1.75 倍のコストになる。

**実装**: storage のバイト数が `ARRAY_ALIGNED_ALLOC_THRESHOLD` 以上のときだけ、要素バッファが
`ARRAY_BUF_ALIGNMENT` の倍数に載る位置へオブジェクトをずらして確保する。`malloc(sizeof + 31)` を取り、
オブジェクトをブロック先頭から最大 31 バイト後ろへ置く。しきい値未満の storage は今までどおり
`malloc(sizeof)` で、確保サイズもアドレスも変わらない。

しきい値の判定は**分岐ではなくマスク**で行う(`slack = sext(sizeof >= threshold) & 31`、
`shift = ((-(base + buf_offset)) & 31) & mask`)。配列確保は数命令で呼び出し側に展開されるため、
ここに基本ブロックを増やすと確保を含む関数がインラインコストモデル上で軒並み大きくなり、下流の
インライン判断が変わる。分岐版で `cp_lib_lsegtree` の命令数が +27% になった(`_propagate_at` が
展開されなくなった)のに対し、マスク版は +0.15% に収まる。

**ずらした距離は control block に記録する**(`CTRL_BLK_ALLOC_OFFSET_IDX`)。`{i32 refcnt, i8 state}` の
末尾パディングにバイトを 1 つ置くだけなので、control block のサイズは 8 のまま、全 boxed オブジェクトの
レイアウトも不変。解放は `free(ptr - alloc_offset)`。

アドレスの剰余から「ずらしたかどうか」を判定する案は採らない。malloc のアラインメントは実装依存で、
C 規格が要求するのは「確保したサイズに収まる型に適した境界」だけである。mimalloc は 64bit で
**8 バイト以下の確保を 8 バイト境界にしか揃えない**(microsoft/mimalloc#199 の作者回答)。8 バイトは
空配列の storage のサイズそのものなので、判定を誤ってヒープを壊す。

**しきい値を設ける理由**: 全配列を揃える(全 storage にパディングを入れる)実装を測ったところ、
`arrayrw` が 1.65 倍速くなる一方で `cp_lib_scc` が 1.24 倍遅くなった。原因は `aligned_alloc` ではなく
パディングそのもので、8 バイトのパディングでも 1.17 倍遅い。`cp_lib_scc` の確保 22.6 万回の内訳は
空配列 10 万個(8 B)と 2 要素配列 12.6 万個(24 B)で、パディングはこれらを別のサイズクラスへ押し上げる。
確保が支配的なプログラムでは、短いループが取り返せる以上のコストになる。

**`realloc` との関係**: `realloc` にはブロック先頭を渡す(`ptr - alloc_offset`)。返ってきたブロックの
先頭は別の位置になりうるので、そこから置き場所を計算し直し、**ずれが変わったときだけ** control block と
生きている要素を `memmove` する。ページを持つ大きさのブロックでは `realloc` が `mremap` で済ませるため、
確保し直して memcpy する実装にすると要素数に比例したコストが乗る(1M 要素の boxed 配列の `set_capacity`
で 3.9 倍の退行を実測)。`realloc` を残すのはそのため。

### 12.4 gdb pretty-printer で配列を実要素数だけ表示する

`Array` の debug 情報は `DEBUG_ARRAY_ASSUMED_LEN`(100)個の要素を主張し、そのうち先頭 `<array size>` 個だけが
有効という状態が続く(fixlang#78)。反転後は長さと要素が別の struct に分かれるので、DWARF の `DW_AT_count` で
解く道はさらに遠のく(§8)。

代わりに **gdb pretty-printer(Python)** が本命になる。printer は `_storage` と `<array size>` の両方を読めるので、
実要素数ぶんだけ表示でき、100 要素の水増しも隠せる。`String` を文字列として表示することもできる。
...
`unique_check_elim.rs`):
- **force-unique 分岐**: op が `unique_check_operand() -> Some { container_index, path }` で「どの operand(slot)の
  どの boxed leaf の uniqueness で COW するか」を報告し(引数型と `TypeEnv` を受け取り、実際の宣言は
  `unique_check_on_boxed_leaf` を通す)、`assuming_unique()` が **その分岐を落とした(COW 無しの)版**
  ...
  - **誤った path は宣言そのものが落ちる(対処済み)。** `Provenance::leaf_origins_at` は非 leaf path に `None` を返し、
    `leaf_is_unique` はそこで `unreachable!` する。かつて危惧した「未登録 path が束の底として `Unique` に解決され、
    COW が全経路から警告なく消える」経路は、この 2 つで塞がれている。
  - **宣言した path は `boxed_leaf_paths(container_ty)` に含まれなければ、チェックの宣言ごと消える。** 各 op は
    `unique_check_on_boxed_leaf(container_index, path, arg_tys, type_env)` を通して宣言し、この関数は path が
    `boxed_leaf_paths` に無ければ `None` を返す(型によって分岐の有無が変わる op のための仕組み)。落ちはしないが
    **COW が畳めなくなるだけの静かな失敗**なので、`Array` については §3.2 の 3 つの述語修正がここでも load-bearing に
    なる: `is_fully_unboxed` が `Array U8` を「RC 不要」と答えると `boxed_leaf_paths` は即 return して leaf を 1 つも
    返さず、`Some{0, []}` は `None` に潰れて Array の COW は一切畳めない。
  - **`boxed_leaf_paths` が `Array` に対して返すのは `[]`(値そのもの)であり、クロージャ流の `[0]` ではない。**
    クロージャは `[CLOSURE_CAPTURE_IDX]` を返す(値は unbox で、RC unit は capture 側にある)。`Array` は
    `is_rc_unit_root` = 不可分 unit なので `is_box` の枝と同形に「その時点の path を 1 leaf として push」する。
    `build_shape` / `uniform` / `arg_passthrough` はすべて `boxed_leaf_paths` を回すので、これで provenance の
    shape・`unique_check_operand` の path・`truncate_to_unit` の丸め先が 1 つの空間に揃う。
- **borrows_operand(i, &arg_tys, &type_env)**: operand i を borrow(consume しない)か。**借用するかは operand の型に
  よる**(`unique_check_operand` と同じく引数型と `TypeEnv` を受け取る)。default は全 operand consume。`borrow.rs` は
  `borrows_operand(i, ..)` か result_prov に `Arg(i, ·)` として現れる operand のみ非 consume とする。
  **borrow を宣言した op は、その operand を `get_scoped_obj_noretain` で読まなければならない** — 素の
  `get_scoped_obj` は unbox global の boxed subobject を retain するが、borrow には対になる release が無いので leak する。
  **refcount を分岐に使う op では宣言しない**(§5)。
- **result_prov**: 結果の各 boxed leaf の provenance — `Fresh`(新規 unique)/ `Arg(k, path)`(operand k の passthrough
  alias。`root()` が alias とみなし retain を省く)/ `Unknown`(保守的)。**`force_unique` で分岐させない**(§5)—
...
- **`punch`**(`InlineLLVMArrayPunchBody`、InlineLLVM)— `(PunchedArray a, a)` を返す。`if force_unique {
  ...
  elem)`。force-unique: `Some{0,[]}`。borrows: なし。prov: `Provenance::fresh_under(result_ty, type_env,
  &[PUNCHED_ARRAY_FIELD])` = punched-array leaf(path `[0]`)が `Fresh`、moved-out 要素 leaf(path `[1]`)が
  `Unknown`(**実装済み**)。要素は retain せずに取り出しており他所から参照され得るので、そこを `Fresh` にすると
  後続の in-place 更新が共有要素を壊す。**登録シンボルは COW 版だけにする**
...
- **`unsafe_is_unique`**(`InlineLLVMIsUniqueFunctionBody`、InlineLLVM)— `(Bool, a)`。`if !assume_unique &&
  ...
  prov: **`Unknown` 固定(TRAP)** — 第2成分は引数そのものだが passthrough にすると「後続 use が arg を shared に読ませる
  retain」を抑止し fold が誤って on になる。**`Unknown` を保つ**。redesign: `[a:Boxed]` 追加。Array には下記
...
- **NEW `_unsafe_is_storage_unique`**(`Array::_unsafe_is_storage_unique : Array a -> (Bool, Array a)`、InlineLLVM)
  ...
  (operand 0 を意図的に consume)**。prov: **`Unknown` 固定(同じ TRAP)**。generic 版と同型にする理由は §3.3 —
...
- **NEW `mutate_elements` / `_mutate_elements_ios`**(専用 InlineLLVM)— `if force_unique { make_storage_unique }`;
  `ptr = data_ptr(arr.@_storage)`; `r = act(ptr)`; `ret (arr, r)`。**属性は
  `InlineLLVMUnsafeMutateBoxedInternalFunctionBody` をそのまま写す**: `force_unique: bool` フィールド +
  `unique_check_operand` = `unique_check_on_boxed_leaf(arr の operand index, vec![], ..)` + `assuming_unique` +
  prov = `Provenance::fresh_under(result_ty, type_env, &[返る配列の field])`(結果は `(Array a, b)` なので配列側の
  leaf だけ `Fresh`)。§13.3-2。
...
- **`_unsafe_get_bounds_unchecked`**(InlineLLVM)— `arr = noretain(arr)`(borrow); `elem = retaining_read(
  arr.@_storage, idx)`。**borrows: operand 0 = borrow**。prov: `Unknown`(共有 container から retain 済み要素)。存続。
...
- **`@size` / `@capacity`**: 現行の InlineLLVM body(heap load)を value field 1 / 2 への `extractvalue` に変える
  (手登録 builtin のまま、§2.2(5))。scalar なので prov 空・register 読み。**borrows: operand 0 = borrow**
  (`get_scoped_obj_noretain` で読む)。storage を retain しないので write loop で storage leaf の provenance が保たれる
  (§2.2(5))。storage Ptr(field 0)は codegen 内部でのみ読む。
...
| `Array::_unsafe_is_storage_unique : Array a -> (Bool, Array a)` | storage の refcount を **retain せずに**覗く。**専用 InlineLLVM**(属性は generic `unsafe_is_unique` と同型 = `unique_check_operand` あり・`result_prov` は `Unknown` 固定、§3.3) |
...
- `mod`/`act`(punch/plug を COW 版へ、`unsafe_is_unique` -> `_unsafe_is_storage_unique`、act の分岐構造は維持、§3.3): `mod`, `_unsafe_act_bounds_unchecked_identity`, `_unsafe_act_bounds_unchecked_tuple2`。`_unsafe_act_bounds_unchecked` は**既に両方とも checking 版**になっている。その unique 枝の plug が checking でなければならないのは畳み込みのためではなく健全性のためで、**`map` は関数を 2 回以上走らせ得るので、各回が自分専用の配列へ plug する必要がある**(punch 側は unique 枝なので実際には clone しないが、checking 版を通すことで「punched array は unique」が optimizer に伝わり、続く plug の check が落ちる)
