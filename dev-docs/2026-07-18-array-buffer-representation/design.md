...
- `set` / `swap` / `mod`(punch/plug)/ `act` / `_unsafe_truncate` / `mutate_elements`: COW 内蔵の in-place mutator。
...
- `_unsafe_set_size` は要素側 Array ではほぼ **増加専用**(from_map/fill/reserve/push_back/append は
  alloc/reserve 後に size を伸ばす)。**減少は `_unsafe_truncate`(危険トランケート = COW + `release_range([n,size))`
  + `size=n`、契約 `0<=n<=size`)が担う**。**安全な shrink は `truncate`/`pop_back`(公開)が size チェック後に
  `_unsafe_truncate` を呼ぶ**(§13.2、§13.3-1)— `String::from_bytes` 等の `Array U8` shrink 経路は `truncate` を使う
...
| `_unsafe_set_size` | `_unsafe_grow_size`(増加専用)へ改名: 内部 unique check(COW、optimizer 除去)+ value `_size` を伸ばす(新スロット未初期化)。減少は `_unsafe_truncate` が release_range+shrink | **InlineLLVM**(in-place、内部 COW) |
...
| `_pop_back_nonempty` | **削除** — `pop_back` は `_unsafe_truncate(size-1)` を呼ぶ Fix-source(empty guard 付き)に。末尾1要素の release は `release_range([size-1, size))` に一般化される | 削除(`_unsafe_truncate` に統合、§13.2) |
| (新規)`_unsafe_truncate(n, arr)` | `pop_back`/`truncate` 共通コア: COW + `release_range([n,size))` + `size=n`(契約 `0<=n<=size`、size チェックなし) | **InlineLLVM**(in-place、内部 COW、畳める) |
...
- **NEW `_unsafe_truncate`**(InlineLLVM。`pop_back` と `truncate` の共通コア)— 契約 `0 <= n <= _size`(呼び出し側が
  ...
  Fix-source ラッパ**(§13.2): `truncate(n)` = `if n<0 { undefined }; if n>=_size { arr }; arr._unsafe_truncate(n)`、
  `pop_back` = `if _size==0 { arr }; arr._unsafe_truncate(_size-1)`。§13.3-1。
...
- 該当なし。旧 `_pop_back_nonempty`(無条件 COW・非 fold)は削除し、`pop_back` は上記 `_unsafe_truncate(_size-1)` を
...
- `pop_back`: `if size==0 { arr }; arr._unsafe_truncate(size-1)` の Fix-source に(`_pop_back_nonempty` は削除)
- `truncate`: pop_back ループを廃止し、`if n<0 { undefined }; if n>=size { arr }; arr._unsafe_truncate(n)` の Fix-source に(§13.3-1)。`String::from_bytes` / `_unsafe_from_c_str` の切り詰めは `truncate` を呼ぶ
...
- Rust 登録プリミティブ: `_unsafe_force_unique`, `_unsafe_set_bounds_uniqueness_unchecked_unreleased`, punch/plug の uniqueness-unchecked 版, `_unsafe_fill_size_unchecked`, `_pop_back_nonempty`(-> `_unsafe_truncate` に統合)
...
**新規 Rust 登録プリミティブ:** `_unsafe_truncate`(危険トランケート、§13.1(1))— `pop_back`/`truncate` の共通コア。
...
1. **shrink 経路 — 危険トランケート `_unsafe_truncate` を1本作り、`pop_back`/`truncate` がそれを共有する(決定)。**
   ...
   があるので **新しい `shrink_size` は追加しない**。コアとして **`_unsafe_truncate(n, arr)`**(size チェックなし、契約
   ...
   { undefined }; if n>=_size { arr }; arr._unsafe_truncate(n)`、`pop_back` = `if _size==0 { arr };
   arr._unsafe_truncate(_size-1)`。これで現状の pop_back ループ(O(size-n))が **1 回の range-release** になり、
