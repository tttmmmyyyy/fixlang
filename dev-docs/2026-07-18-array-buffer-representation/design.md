...
書く mutate primitive は新設しない。既存のこのペアも削除する(§11.4)。** `Buffer::_unsafe_set` / `_unsafe_get` と
punch/plug は、囲む safe op の unique 枝(既に uniqueness を確定済み)の中でだけ動く生の部品なので可。「bulk op は
