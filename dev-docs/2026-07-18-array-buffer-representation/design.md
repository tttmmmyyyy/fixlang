...
- `_unsafe_set_size` は要素側 Array ではほぼ **増加専用**(from_map/fill/reserve/push_back/append は
  ...
  直接書く**(`_unsafe_set_size` を使わない)。**任意の safe な shrink は `_shrink_size`(COW + 切り詰め要素の
...
| `Array::_shrink_size : I64 -> Array a -> Array a` | **safe** shrink。`n <= _size` に縮小: shared なら COW clone、切り詰め要素 `[n, _size)` を release、value `_size = n`。unique-check-elim + unboxed release で `Array U8`/unique では「`_size` 下げ」に畳む。`truncate`/`String::from_bytes` が使う |
...
- shrink 経路(新 `_shrink_size` を使う、§13.3-1): `truncate`(現状の pop_back ループを 1 op に置換)、`String::from_bytes` / `_unsafe_from_c_str` の切り詰め
...
- `_unsafe_set_size` -> `_unsafe_grow_size`(grow 経路)。呼び出し: `append`/`from_map`/`push_back`/`reserve`/`resize`/`read_n_bytes`/`unsafe_from_c_str_ptr`(`_io`)/数値 `to_bytes` 一式。**真の shrink を行う `String::from_bytes` と `_unsafe_from_c_str` の切り詰め経路は `_shrink_size`(safe shrink、§13.3-1)を使う**
...
1. **shrink 経路 — 安全な `_shrink_size` を追加(決定)。** `_unsafe_grow_size`(前提 `n >= _size`)は
   ...
   `Array::_shrink_size : I64 -> Array a -> Array a`** を追加する。契約: `n <= _size` に size を縮める。**内部で
   unique check し、shared なら COW clone**(他の共有者の要素を巻き添えに release しないため)、**切り詰めた要素
   `[n, _size)` を release**、value `_size = n`。**どちらも最適化で消える**: unique-check-elim が provably-unique で
   COW を畳み、unboxed 要素(`Array U8` 等)では要素 release が no-op になる — よって `String::from_bytes`(unique な
   `Array U8`)では実質「`_size` を下げるだけ」に落ち、旧 `_unsafe_set_size` の shrink と**同性能かつ安全**。grow が
   `_unsafe_`(未初期化スロットを残す契約)なのに対し shrink は要素 release + COW を内包するので **safe**(prefix の
   非対称は意図的)。`String::from_bytes` は `_shrink_size(null_idx+1)` を、`truncate` はこの1 op(現状の pop_back
   ループを置換)を使う。boxed 要素の配列でも安全に使える汎用 op。
