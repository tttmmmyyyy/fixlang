...
- `Array::_unsafe_get_bounds_unchecked : I64 -> Array a -> a` — 要素 read(bounds unchecked)。返り値を
  ...
  **非 retain(unretained)版は作らない** — plan §8(2)(a) の「型に現れない穴」(runtime index には static な
  ...
  この retaining getter を通す(`ArrayIterator::advance` もこれを使う)。boxed 要素の read-fold で per-element の
  retain/release を 0 にするのは、borrow 化(§2.1)と retain/release 相殺(§2.2)の仕事 — `Array` 表現とは直交する
  optimizer 側の課題で、この設計は待たない(削除する unretained getter の実利用は unboxed 要素の 1 箇所だけなので、
  既存コードの性能はこの削除で変わらない)。
...
- **`_unsafe_get_bounds_unchecked`**(InlineLLVM)— `arr = noretain(arr)`(borrow); `elem = retaining_read(
  ...
  read-fold の per-element RC は optimizer が落とす(§4)。**unretained 版は作らない**(§4・plan §8(2)(a) の再導入回避)。
