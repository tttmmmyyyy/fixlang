...
構造体の宣言は、フィールドごとに**そのフィールドを穴にした型構築子**を型環境に持つ。
`mod_<field>` と `act_<field>` は、フィールドを取り出している間の残りをその型で持つ。
穴は場所だけ取って値を持たないので、置き換える構造体の穴の開いた形も、絞ったフィールドの型で
作り直さなければならない。

鋳造するのは `#N` だけでなく、フィールドの数だけの `#N#PunchedAt<n>` である。中身は `#N` と
同じフィールド列の n 番目に穴の印を付けたもので、`punched_from` は `#N` を指す。複製の本体への
型の置換も `MapIterator#PunchedAt<n>` を `#N#PunchedAt<n>` へ送る。

キャプチャリストにこれが要らなかったのは、キャプチャリストに `mod_` を呼ぶものが無いからで、
ユーザが宣言した構造体では成り立たない。
