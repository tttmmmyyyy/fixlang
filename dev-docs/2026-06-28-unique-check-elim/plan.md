...
**末尾呼び出しを壊さない（制約）**: 「呼び出し**後**の `Release`」が**末尾位置（tail position。§1.2 の構造的定義——match arm 内の呼び出しは、その match が tail position のときだけ tail）**の呼び出しに**残る**（相殺で消えない）と tail call でなくなる（`let r = App(f, args); Ret(r)` が `…; Release(x); Ret(r)`）。無限に深くなるのは**閉路の末尾呼び出し**だけ——非末尾化で伸びるスタックは、閉路（intra-SCC の tail back-edge）なら再帰深さぶん（オーバーフロー）だが、cross-SCC なら condensation（DAG）の深さ＝静的定数ぶん（O(1)、無害）。よって**守るのは閉路の末尾呼び出しだけ**で、cross-SCC の末尾は非末尾化してよい（O(1) と引き換えに `Borrow` の利得を取る）。閉路の末尾呼び出しの扱い:
...
- **(B) その関数が所有する fresh 値を borrow 引数として閉路末尾に渡す**: fresh は借用できず後続 `Release` が残るので、その引数を `Own` に留める（callee が consume＝tail 保持。fresh を受ける param は uniqueness を運ばないので `Borrow` 化の利得は元々無い）。(B) で `Own` 固定した param は外部呼び出し元にも `Own` になり、そこが再使用すると `Dynamic` になる（両取りは特殊化。P2）。同じ値が閉路 tail の**複数位置に出る**（`g(x,x)`／`let y=(x,x); g(y)`）ときは、一部だけ `Own` にすると残る Borrow 位置に後続 `Release` が出て非末尾化するので、**f 所有末端は全出現を `Own`** にする（mixed Own/Borrow は不可。複数使用ぶんの `Retain` は lowering が供給し全コピーが consume＝tail 保持、その値は `Dynamic`。単発 `g(fresh())` は `Own` 単発・`Retain` 無し・`Unique` のまま）。
  ...
  for scc in bottom_up(condensation(call_graph(prog))):          # callee 先
    repeat 変化が無くなるまで:                                    # SCC 内不動点（自己/相互再帰）
      for f in scc:
        ...
        for f の各「閉路 tail 呼び出し」（§1.2 の構造的 tail 伝播で body を辿り tail-position の App を拾う: `Ret(App(g,args))`・tail-position の match arm 内 App。flat な `App;Ret(r)` に限らない）:  # intra-SCC の tail 辺だけ
          for (q, x) in enumerate(args), x の各 boxed 末端 x@π:    # q=位置
            # 閉路 tail では f 所有末端は【全出現】を Own にする。一部を Borrow に残すと（同一値が
            # 複数位置に出る g(x,x) 等で）残った Borrow 位置に後続 Release が出て非末尾化する（mixed
            # Own/Borrow は不可）。複数使用ぶんの Retain は lowering が供給し全コピーが consume される
            # （その値は Dynamic 化するが、同一 fresh を二重に渡すのはレアケース。単発 g(fresh()) は
            #  Own 単発・Retain 無し・Unique のまま末尾保持）。
            if owns(f, x@π): own[g.q@π] = Own                       # (ii) case B -> 降格（f 所有末端は全出現 Own）
    ...
    for f.body 中の呼び出し App/Closure(g, args):
      for (q, x) in enumerate(args), x の各 boxed 末端 x@π:        # q=位置
        if own[g.q@π]==Borrow and owns(f, x@π):
          ...
          # ただし対象が tail-position の App（Ret(App)）なら「直後」が無いので
          # Ret(App) を `let r=App; Release; Ret(Var(r))` に非末尾化して挿入する。
          # 閉路 tail は phase(ii) の case B で Own 固定済み（ここへ来ない）＝非末尾化は
          # cross-SCC tail のみ（§2.1 の O(1) スタックで無害）。
...
- **phase(ii) は閉路 tail の f 所有末端を"全て"Own にする（`last_use`／一部だけ降格は却下）**（§2.1）: 同一値が複数位置に出る（`g(x,x)`）とき、mixed（一部 Own・一部 Borrow）は残る Borrow 位置に後続 `Release` を生み**非末尾化**する。その `Release` は §2.2 で消せない（間に Own consume が挟まる＝下記）。「最終位置だけ Own（last_use）」も「非最終を Own」も mixed ゆえ同じく破綻。全 Own なら callee が全 consume・lowering の `Retain` が rc を供給・tail 保持（値は `Dynamic` だが同一 fresh の二重渡しはレア。単発 `g(fresh())` は `Own` 単発・`Unique` のまま）。
