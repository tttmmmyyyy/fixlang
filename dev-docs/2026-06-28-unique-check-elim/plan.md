...
enum RcExpr {
    ...
    Ret(RcRhs),                           // 唯一の終端子。「この式（関数本体 or match arm）の値 = この cexp」。Ret(Var(x)) が旧 Ret(x)・Ret(App(..)) が末尾位置の呼び出し・Ret(Match(..)) が末尾 match。**cexp は Var/App/Match に限る**（[#R10-3]・lowering と全変換の不変条件。LLVM/Closure は必ず let 束縛——`Ret(LLVM(素通し op, [t]))` を許すと t の escape が consume_sites の LLVM 規則〔素通し末端を除外〕でも Ret 規則〔Var 用〕でも計上されず、借用誤分類→double-free）
...
borrow_ify(prog):
  ...
  for v ∈ {f_own, f_borrow}, for v.body 中の直接 App(callee, args):    # callee = step 2 が選んだ版（g_own or g_borrow）
    for (q, x) in enumerate(args), x の各 boxed 末端 x@π:            # q=位置
      ...
      # [#3round9] Release の挿入位置: 非 tail の App でも `Let(r,App,k)` なら「呼び出し直後＝k の先頭」に置けるが、
      #   【let 束縛 match の arm 末尾 `Ret(App)`】（§1.2: この match が非 tail なら arm 内 App も非 tail）は「直後」が
      #   構文上無いので、その arm を `Let(t, App, Release(x@π); Ret(t))` に書き換えて arm 内に挿す。合流後の k に置くと、
...
consume_sites(f):  # 所有権が f から出て行く末端の集合（別名辺で結果へ抜けない Own 位置 = sink）
  App(g /*callee*/, [..x@位置 i..])   -> g の全 boxed 末端（callee closure の cap）           # [#2round9]: クロージャを呼ぶ＝それを consume（callee が cap を release・§1.7 L167・generator.rs:1980-1984）。funptr/空捕捉は boxed 末端無し＝no-op。「呼ぶだけ」の closure param が Borrow 誤分類→cap 二重解放を防ぐ
...
- **[#R10-1] no-retain getter で取り出した要素の `Retain` は相殺対象外（間接デクリメントの穴）**: lowering は「boxed コンテナから **no-retain 取り出し**（結果は unowned alias・root=getter producer・§3.3 で `Dyn`）＋明示 `Retain`(要素)＋`Release`(コンテナ)」を出す（cap 取り出し〔§1.7〕・boxed union match arm〔§1.2 L96〕・boxed struct destructure）。この明示 `Retain(e)` は取り出した要素を**独立所有にする** load-bearing な +1 で、要素の last use が borrow-read だと後続 `Release(e)` と root 一致・間に consume 無しで**消せる形に見える**が、消すと窓内の **`Release(container)` が要素 object を間接デクリメント**して早期解放 → borrow-read が UAF（例: `|i| c.@(i)` の `Retain(c2); Release(cap); read; Release(c2)` で両者消すと `Release(cap)` が `c` を解放）。so **`root(x@π)` が no-retain boxed-container getter に解決される `Retain(x@π)` は cancellation candidate から除外**する（`consume_sites` は**コンテナ release 経由の間接デクリメント**を拾わないので、これを別途 barrier する）。上の「健全性とは無関係」はこの除外を前提に成立。失うのは「取り出し要素を retain して borrow 呼びに渡し drop」の一時相殺（cap/boxed-union 抽出パス・低頻度）のみ。**より精密な代替**（v1 は確実な除外を採り将来差し替え可）: (a) getter を retain-getter に融合し move-out を §6 の op 差し替えで実現（この明示 `Retain` 形自体を無くす）、(c) `Release(y)` 走査時に pending `Retain` の getter の container 引数の root が `root(y)` と一致したら barrier（精密。入れ子コンテナは container 鎖を再帰照合）。§9.5 に反例テスト（read-only capture closure／owned boxed-union payload の read-drop を valgrind/ASan）。
```
# x は rc=1 で入る
Retain(x)      # rc 1->2
...
- `Ret(c)`: この式の値の `Provenance` ＝ c の provenance（`Var(x)`→`env[x]`、`App`→合成、`Match`→上の arm 末尾 join）。**c は Var/App/Match のみ**（[#R10-3]・§1.2 の正準形。LLVM/Closure は let 束縛されるので Ret に直接来ない）。関数本体の `Ret` はその関数の結果 `Provenance`、match arm の `Ret` はその arm の値。
...
- **union の borrow param の別名 Release 除去（[#6round9]・phase(i) の root ベース削除）**: **union 型の borrow param** を match して payload を read して捨てる関数を borrow 化し、caller が同じ値を後段 mutate する入力。(a) unboxed union（`Option (Array I64)` 等）——arm 内で payload を取り出す `Release`（root=param@(k::π)）を phase(i) が **root ベースで削除**すること、(b) boxed union——arm 内の容器 `Release(o)`（root=param）が削除され caller の (ii) Release と釣り合うこと。観測: **UAF/二重解放なし**（valgrind/ASan）。**変数名マッチで実装すると、別名の `Release(payload)`/`Release(o)` が残って過剰解放 → caller UAF**（プラン L307 が明記する誤実装）。§9.5 の他 borrow テストは union の borrow param を含まずこれを踏まない。
