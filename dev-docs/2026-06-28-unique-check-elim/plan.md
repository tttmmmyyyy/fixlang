...
- **（実装済み・設計判断）borrow 化・相殺を codegen ユニット単位で走らせる**（§1.5/§2.1）: `borrow_ify`+`cancel` はコンパイルユニットごとに `lower→insert_rc→split_rc_units→borrow_ify→cancel→implement` の一部として走る（`build_object_files.rs`）。`enable_separated_compilation()` は `opt <= Basic` のときのみ真なので、**max（既定）は分離コンパイル OFF ＝ 全プログラムが 1 ユニット ＝ 全プログラム borrow 化（全精度）**。none は uncurry OFF で funptr 関数が無く `borrow_ify` は no-op。basic のみモジュール単位で cross-module（ユーザ→Std 等）の借用を取りこぼす（健全・不精密）。健全性の根拠: `f_own` は常に all-`Own` ＝ 間接呼び・cross-unit 呼びの ABI 不変、borrow 版は同一ユニット内 routing のみ、cross-unit へ借用値を all-`Own` 位置で渡すときは `call_rc` が呼び出し前 `Retain` を出す（[#F1round8]）。ユニット自己完結でインクリメンタルコンパイルの cache に親和。全プログラム borrow 化（basic の cross-module 精度を取る）は cache を壊しビルド構造の作り直しが要るので採らない。max の性能が最優先で basic は中間レベルのため、この取りこぼしは許容。
