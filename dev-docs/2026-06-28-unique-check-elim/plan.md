...
- `Retain(x@π, k)`: `env[x]` の**末端 π のみ**を `Dyn` に倒す（末端の `LeafSource` を **`Fresh`/`Arg`/混在を問わず `{Dyn}` に置換**——複製したので共有＝Dynamic。既 `Dyn` はそのまま。§2.2 が per-末端に分解済み〔L328〕なので x の他末端は不変＝多末端値で sibling の一意性を保つ。`Fresh`/`Arg -> Dyn` 降格を起こすのは `Retain`〔別名・per-末端〕**のみ**）。
