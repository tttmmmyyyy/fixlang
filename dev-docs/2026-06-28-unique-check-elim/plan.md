...
- `Match(s, arms)`（cexp。`Let(x, Match, k)` か `Ret(Match)` に現れる）: 各 arm を**分岐前 env のコピー**から解析し、各 arm の末尾（`Ret`）の値を join（末端 `LeafSource` を union）＝この Match の provenance。payload 束縛は unboxed union payload → move 取り出しで scrutinee の子末端（`Arg` 系。不在 variant は空 Set）、boxed union payload → getter＝`Dyn`。Bool もここ（2 variant）。`Let(x, Match, k)` は x にこの join を束縛して k へ。
...
- **`unsafe_is_unique` の value 部は passthrough `Arg(0)`**（refcount を**読む**だけで state も count も変えない＝`Fresh` を保つ）。**`mark_threaded`/`mark_global` は InlineLLVM として残さず `MarkThreaded`/`MarkGlobal` ノードへ lower**する（L97）——同一 object を返すが threaded/global 化＝以降 in-place 不可なので、§3.2 の遷移が `env[x]` を全 boxed 末端 `Dyn` に降格する（`Fresh` を保つと threaded/global を elide してレース／不正。§4 の「`is_unique ⟹ LOCAL`」の要）。ノードは RC 中立で x を consume/retain せず継続へ流す（結果は同一 object なので入力 x に別名化。release は x の真の last-use に残る）＝健全性は §3.2 の Dyn 降格だけで担う。
...
- **tail 制御（tail-call elim 維持）の検証**（新 IR で tail position の判定・phi 回避が壊れていないこと。§1.2 tail-position 規則）:
  - **深い末尾再帰が stack overflow しない**: match arm が tail position の再帰（`sum`〔§2.1〕型や `loop`/`fold`）を大 N（例 1e8）で走らせ完走する＝各 tail 呼び出しが実際に tail-call elim されている。arm 内呼び出しでも、その match が tail position なら tail になること（`Let(x, Match{…App…}, Ret(x))` 型）を含める。
