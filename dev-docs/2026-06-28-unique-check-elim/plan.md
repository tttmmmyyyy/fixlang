...
borrow_ify(prog):
  ...
  for f_borrow in prog:
    ...
    # 全使用位置は Borrow/read/drop、【または routing が g_own に落として下記 (ii-c) で呼び出し前 Retain を挿す Own 位置】
  ...
  # (ii) 呼び出し地点の retain/release（全 body v・step 2 で callee は版に materialize 済み）
  for v ∈ {f_own, f_borrow}, for v.body 中の直接 App(g_ver, args):
    for (q, x) in enumerate(args), x の各 boxed 末端 x@π:          # q=位置
      if g_ver==g_borrow and owns(v, x@π):                          # (ii-b) 所有値を借用版へ → 呼び出し後 Release
        呼び出し直後に Release(x@π) を挿入                          # safe 条件より必ず非 tail（tail+所有→g_own）＝非末尾化しない
      if g_ver==g_own and ¬owns(v, x@π):                            # (ii-c) 借用値を全 Own 版へ → 呼び出し前 Retain（[#F1round8]）
        呼び出し前に Retain(x@π) を挿入                             # g_own に本物の所有 +1 を渡す。g_own はそれを
                                                                    # release でも return(escape) でも複製でも一様に扱う
                                                                    # （all-Own baseline ゆえ）。借用値を持つのは f_borrow だけ
                                                                    # （f_own は param 全 Own＝該当なし）。Retain は呼び出し【前】ゆえ tail 保持
