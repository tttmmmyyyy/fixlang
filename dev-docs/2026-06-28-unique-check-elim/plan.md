...
ステータス: P0.5-P3 実装済み（ブランチ `unique-check-elim`）。P3.5・P4（reuse / 順序スケジューリング / bounds-check-elim）は未着手。このプランは設計時の記述で、実装と一部乖離がある（`RcExpr::Destructure` の専用ノード化、`Provenance` の leaf-map 化、§4.1 α-merge の LLVM backend 委譲、§3.2 root-demotion の実装メモ 等）。plan と実装の全差分は `audit-2026-07-20-plan-vs-implementation.md` を参照。
...
- **[実装メモ 2026-07-20]** 実装の `RcExpr`（`src/rc_ir/ast.rs`）は上の 4 種に加えて **`Destructure(RcVar, Vec<(usize, RcVar)>, RcExprNode)`** を持つ。struct/タプルの destructure は、下の「`Let` は単一 Var のみ（Pattern を持たない）」項が書く getter プリミティブ列でなく、**全フィールドを一度に取り出す専用ノード**である。意味は: 容器を消費し、unbox 容器なら末端をフィールド変数へ move（フィールドごとの retain 無し）・名指されないフィールドは drop、boxed 容器なら名指した各フィールドを retain し容器を `Release`。RC 挿入は「destructure 後も容器が生きていれば直前に `Retain`」を**一度だけ**決める。専用ノードにした理由がこれで、per-field getter 列だと同じ判断がフィールドごとに分散する。この形により §8 の「unbox getter を統一 retain-getter にした非パリティ項目」も解消済み（同項の実装メモ）。
...
- **`Let` は単一 Var のみ（Pattern を持たない）**: struct（**タプルは特別 syntax が付いただけの普通の struct**）の destructure `let (x,y)=s` 等は **getter プリミティブ列（`LLVM`）** に lower（役割分担: 構造分解は getter、union 分岐は `Match`）。特別扱いは pair/tuple でなく **boxed / unbox の別**（タプルは unbox struct）: **unbox struct** は getter を**純射影**（取り出し末端＝`Arg` 素通し・**op 内で retain/release せず**・sibling は触らない〔§8〕）にし、**落とす末端は §1.4(c) の明示 `Release(s@π)`**（unbox に container object は無いので `Release(container)` は出さない）。**boxed struct** は getter を **retain-getter**（要素 retain を op 内・[#R10-1]）にし、**箱を `Release(container)`**。lowering は現 destructure codegen（`get_struct_fields`/`get_union_value`）の RC 最小化（move-out）を踏襲し必要最小の RC だけ挿す。
  - **[伸びしろ・未実装] destructure の dead 末端の retain/release 消去（RC IR が旧 codegen を上回れる箇所）**: `let (x,y)=s` で `y` が dead のとき、現 lowering は旧 codegen 同様「**生きている末端も含めコンテナ全体を retain → dead を release**」する（unbox: `Retain(s,[])`〔全 boxed 末端〕＋ `y` を落とす `Release(s@y)`／boxed: retain-getter が全抽出末端を retain ＋ `Release(y)`）。dead 末端の retain と release は対消滅する**無駄**（旧 codegen と同一挙動なので P1 では回帰でない。`process_destructure` の whole-container `Retain` が根拠）。`process_destructure` は field liveness を持つので、**生きている末端だけ retain すれば消せる**。ただし unbox と boxed で消せる pass が違う: **unbox はこのまま §2.2 が拾う**（`Retain(s,[])` を per-末端に分解し、dead 末端を `Release(s@y)` と相殺——line 58「§2.2 分解が per-末端 `Retain(x,π)` を生む」）。**boxed は §2.2 では消せない**——[#R10-1] で要素 retain は retain-getter の **op 内**にあり明示 `Retain` ノードが無いため §2.2 の照合対象にならない。よって boxed は **lowering で「生きている末端だけ retain する selective retain-getter ＋ dead 末端は抽出しない」**にするしかない（＝「ここで消す」）。**要検討**: unbox も §2.2 任せにせず lowering に寄せれば、両者を `process_destructure` の liveness で一貫して消せる（boxed が lowering 必須なので寄せる方が素直）。
...
consume_sites(f):  # 所有権が sink へ出る boxed 末端の集合（別名辺で結果へ抜けない Own 位置 = sink）。sink＝escape（`Ret`／`App` の Own 位置／`Closure` 捕捉）または op が Own 引数を内部処理（結果へ素通ししない Own 末端＝op 内 release 等）。**明示 `Release` ノード（read して drop＝own-then-release）は sink でない＝borrow 可**（[#R12-4]）。本体の全ノードを歩く（値 match の arm 内も）
...
```
- **[実装メモ 2026-07-20]** 実装（`src/rc_ir/provenance.rs`）の `Provenance` と resolve 結果は、上の再帰 enum でなく**末端 path をキーにした map** である: `struct Provenance(Map<Path, LeafSource>)` / `struct Uniqueness(BTreeMap<Path, CTRefCnt>)`。木構造（どの path が boxed 末端か）は**値の型が唯一の権威**で、`boxed_leaf_paths(ty)` が与える。so provenance 側は型と食い違い得る集約構造を持たず、boxed 末端を持たない値（スカラー・フィールド無し集約）は空 map ＝表現が一意になる。`BaseSource` / `CTRefCnt` は上のとおり。`OwnershipShape`（§1.2）は再帰 enum のまま（§2.1 の出力型・印字に使う）。挙動は enum 版と等価で、`provenance.rs` の単体テストが確認している。
...
- **[実装メモ 2026-07-20]** 実装（`provenance.rs::interp_inner` の `Retain` arm）は retained 変数自身の leaf のみを**変数キー**で降格し、上記「root を共有する他別名まで降格」は provenance パスには入れていない。それでも健全: unboxed aggregate の同一 leaf を複数回射影すると RC 挿入が容器（＝root）への明示 `Retain(container, π)` を出し、その変数キー降格が `env[container]@π` を倒すため、以降の射影 `@π(container)` は `Dyn` と読む（複製の `Retain` は alias 側でなく常に root に載るので、この場合の変数キー降格＝root ベース降格）。`test_unique_check_elim_reprojected_alias_shared` / `..._live_across_mutation`（memcheck 付き）が確認。詳細は `audit-2026-07-20-plan-vs-implementation.md`。
...
**前提（既存の decapturing）**: RC IR lowering の前に **decapturing の closure specialization**（`src/optimization/closure_specialization.rs`。`inline` 後・`uncurry` 前）が、body closure を引数に取る `loop` を「その body を焼き込んだ特殊版 `loop#lam`」へ書き換え、内部の `body(s0)` を lift 済み body への**直接呼び出し**にする（`loop` は再帰・自己呼び出しで body を同 index に渡す＝specializable。std `fold` と同型）。§3/§4 はこの直接化後の形（`loop#lam`）に働く——generic な `loop` 単体では `body(s0)` が間接呼び出しで結果 Dyn になり精度が出ない。以下その `loop#lam` の body を単に body と書く。
...
- **[実装メモ 2026-07-20]** 実装（`src/rc_ir/unique_elim.rs::specialize`）は特殊化・到達 clone 閉包・dead-function 除去まで行うが、上記の **α 同値 clone の併合（skeleton による初期分割 + partition refinement）は RC IR 上に持たない**。同値な clone の重複は LLVM backend の function merging（MergeFunctions / ICF）に委譲する。健全性には影響せず、バイナリサイズだけの問題であるため。key は上の v1 どおり**各引数の `Uniqueness` 全体**で、後段最適化として挙げた「force-unique 関連末端への射影」も入れていない。clone 数は代わりに 2 つで抑える: **`beneficial_funcs`**（uniqueness チェックに直接／直接呼び出し越しに到達する関数だけを特殊化する。他は key によらず同一 body）と、**canonical key（全 `Dynamic`）の clone は original 名を再利用**する正規化。
...
- **sort 無退行ゲート（本 plan 完了時の必須条件・§6 は対象外）**: `benchmark/speedtest/cases/sort`（固定 PRNG 配列を `Array::sort`＝introsort/heapsort。`swap_bounds_unchecked` が hot）を speedtest スイートに追加。**実装前ベースライン＝ P0.5 での cachegrind 命令数（harness フラグ `-O experimental --emit-symbols --disable-cpu-feature avx512.*` で `sort-inst` ≈ 1.189e8）**を基準に、**本 plan 完了時（P4）に `sort-inst` がベースライン以下**であること。担保: swap の境界チェックは §6（初版対象外）に頼らず `swap_bounds_unchecked` で最初から無し・uniqueness は §4 が elide。途中フェーズ（P0.7〜P3）は §7 の注意どおり swap に force-unique チェックが 1 個乗る一時劣化を許容し、§4 適用後の回復を log.csv で確認（測定は §9.8）。
...
- **P0.5（P1 前提）**: **Bool を union 化**（std.fix: `type Bool = unbox union {_false,_true};`。`true`/`false` はパーサが絶対パス構築子 `::Std::Bool::_true()`/`_false()` に脱糖）。これが `If` を IR から落とす前提（`If`→`Match` desugar は P1 lowering 内）。性能中立（Bool-union＝i8）。de-risk するなら現 `eval_if` を union Bool 対応にして先行検証、または P1 で `eval_if` 撤去と同時。**Bool は union 化後も debug/is_boolean で特別扱いを維持**（決定・§10）: `is_boolean()`〔object.rs:1715〕が Bool-union の tycon を認識し debug は `DW_ATE_BOOLEAN`〔1722〕のまま出す → P0 debug ベースライン〔§9.1〕不変・デバッガは true/false 表示（Bool-union は boolean と bit 同一なので debug 型も正確）。**`not`・比較演算子は i8 tag を直接触る専用 InlineLLVM**（`not`=`BoolNegBody`＝`icmp eq x,0`・branchless）で残し **Match に脱糖しない**（Fix ソースは union tag を触れず source 実装〔match〕だと分岐＝遅くなるため）。要確認は §10。
- **P0.7（早期・P1 と独立に入れられる）: 配列 read-modify-write を atomic builtin へ集約**（builtin `PunchedArray`＋builtin `swap`、`_unsafe_get_linear_bounds_unchecked_unretained` 系・`_unsafe_swap` を廃止）。狙い: **隠れ穴を作る composable primitive を無くして §8 (a) の DOESNT-FIT を完全消滅**させ、各 atomic op に force_unique フラグを付けて **§4 の除去対象に揃える**（swap もソートで全 unchecked 化）。std の swap 版 `PunchedArray`（std.fix:2648）の要素 move 2 回も無くす。作業:
  ...
  - **release/clone 生成の機構（`PunchedArray` 専用 traverser を 1 つ追加）**: release/retain/mark は `create_traverser(ty, work)` が**型のフィールド構成から生成**し、unboxed 型でも呼ばれる（`build_release_mark` の unboxed 分岐、`generator.rs:1259`）。フィールド**単位**のスキップは `SubObject(ty, is_punched)` の `is_punched` で表せる（`object.rs:1646`。struct 側 punch が使用）。**ただし配列要素 1 個のスキップは既存 array 分岐への差し込みでは実現できない**: `PunchedArray = unbox { Array a, I64 }` の内側 `Array a` は**独立 refcount の boxed 値**で、要素解放は「PunchedArray を release → 内側 array の rc をデクリメント → rc==0 で初めて要素解放」の順に走り、その要素解放は**型 `Array a` で共有される名前付き traverser**（`build_traverse` の `Array` 分岐、`object.rs:1670`）を通る。これは (a) 全 `Array a` 共有ゆえ skip-idx を足すと通常配列まで壊れ、(b) enclosing struct の `idx` にアクセスできない。→ 解決は **`PunchedArray` 専用 traverser の追加**: field1(idx) を読み、field0 の内側 array を**通常の `SubObject` 再帰に委ねず**、その rc をデクリメントし、rc==0 なら **skip-idx 要素解放（idx を scope に持ち `[0,idx)` と `(idx,size)` の 2 レンジを解放）＋buffer 解放**を traverser 内で直接行う（既存 array 解放ロジックの skip-idx 複製）。`loop_over_array_buf`〔`object.rs:321`〕は `size` と `buffer` を取り loop_body が `buffer[idx]` を GEP するので、**`(idx,size)` は buffer を (idx+1) 要素オフセットして渡せば既存ヘルパをそのまま再利用できる**（`[0,idx)` は size=idx）。あるいは `loop_over_array_buf` に start／skip 引数を足してもよい——**最速のものを選ぶ（性能が同じならどれでも可）**。**mark（threaded/global）も同 traverser（work 違い）で idx スキップ**（[#R10-8a] 訂正: `TraverserWorkType` は release/mark のみ。**retain は root refcount+1 で要素を辿らない**〔generator.rs:1034-1064〕ので PunchedArray の retain に idx スキップは不要）。clone（`make_unique`）は traverser でなく `clone_array_buf` 経由なので skip-idx clone を別途用意。**この skip-idx clone は load-bearing**（堅牢性目的でなく実走する）——汎用 `act` の `f(e).map(|e'| plug(e', p))` は p を捕捉し `map` が **plug を複数回呼ぶ**ので p が共有になり、fu plug が p を force-unique して skip-idx clone を走らせる（各 plug が独立配列を返す。§3.3 plug・[1]）。線形の `mod`/`act` 特殊版だけ p が単一使用＝unique で clone は走らない。∴ 「既存 array 分岐への 1 行追加」ではなく **`PunchedArray` 専用 traverser の追加**（既存 `loop_over_array_buf` を再利用する限定的な実装）。boxing は不要（`unbox { Array a, I64 }` のまま、std 版の swap-to-end をこの専用 traverser に置換）。型名は `PunchedArray`。**【実装時の決定・当初案から逸脱】(a) std.fix の `type PunchedArray a = unbox struct {_arr,_idx}` を残し builtin 型には置換しない**——traverser を tycon 名（`is_punched_array`）で特別扱いすれば skip-idx は達成でき、新 `TyConVariant`・網羅 match 追加を避けられる。opaque 型化（`_arr` を取り出して通常 Array として release→穴の二重解放、という誤用を型で封じる hardening）は**見送り**: `_arr`/`_idx`/`_punch`/`_plug` は全て `_` private（公開 docs 非掲載）で、コードベース内の実使用は正しく valgrind 検証済み——`_` private を尊重する限り安全（他 `_unsafe_*` と同契約）。最適化への堅牢性は opaque 非依存（skip-idx は型に紐づき、punch/plug は不透明 op なので最適化器が `_arr` を勝手に取り出さない）。**(b) 実装は「array 解放ロジックの複製」でなく、`release_or_mark_array_buf`/`clone_array_buf` に `hole: Option<idx>` 引数を足し、`build_release_boxed`/`build_mark_boxed` を「rc 簿記」と「所有参照の走査」に分離（`build_release_mark_nonnull_boxed_with(traverse_refs)`）して rc 簿記を再利用**（複製ゼロ）。`build_traverse` の `is_punched_array` 特別扱いから、内側 array を `_with` ＋ `hole=Some(idx)` で release/mark する。
  - **punch/plug（新 builtin・swap 無し）**: `punch : I64 -> Array a -> (PunchedArray a, a)` ＝ idx の要素を no-retain で move-out、穴は idx に据え置き、idx を tag。**force-unique 版も用意**（`mod`/`act` の §4 除去がこの上に乗る）。`plug : a -> PunchedArray a -> Array a`（elem 先・punched が dot subject＝`punched.plug(elem)`。現 std `_plug_in : a -> PunchedArray a -> Array a` と同順・§3.3 の `plug(elem, punched)`〔Arg(0)=elem・Arg(1)=punched〕と一致。[#3]）＝ idx に書き戻し（unreleased）て Array へ戻す。**plug も fu／非 fu の 2 版**（fu＝共有 `punched` を force-unique〔skip-idx clone〕・汎用 `act` 用／非 fu＝`punched` unique 前提・線形 `mod`/`act` 用。§3.3・§8）。所有権は **PunchedArray＝idx 以外の全要素／取り出した要素＝idx** に分割される（∴ 取り出した要素は正真正銘の所有末端＝§8 (a) が CLEAN 化）。**【命名の決定・実装時に確定】4 つとも境界チェックしない（呼び出し側が範囲を保証）ので `_unsafe_` プレフィックス＋`_bounds_unchecked` を付ける（既存 `_unsafe_get_bounds_unchecked`/`_unsafe_swap_bounds_uniqueness_unchecked` と同型）: fu 版 `_unsafe_punch_bounds_unchecked`/`_unsafe_plug_bounds_unchecked`（force-unique・§4 が provably-unique 箇所のチェックを elide する対象）、非 fu 版 `_unsafe_punch_bounds_uniqueness_unchecked`/`_unsafe_plug_bounds_uniqueness_unchecked`（uniqueness も無検査＝危険な最適化。長い明示名）。**bounds 側は §6 まで除去されないので、名前上は 4 つとも恒久的に `_bounds_unchecked` を保持する。**非 fu punch は generic act の `unsafe_is_unique` unique 枝（§8・§4 対象外で恒久使用）、非 fu plug は線形 mod/act が使う。4 つとも `_` private。
  - **抽象に保つ**: 内側 `Array` をフィールド取り出しさせない（取り出して通常 `Array` として release すると idx 二重解放）。punch/plug/release/clone 以外の操作を生やさない。
  - **builtin `swap`（2 公開版・force_unique フラグ付き）**: `Array::swap : I64 -> I64 -> Array a -> Array a`（境界チェックあり・安全）と `Array::swap_bounds_unchecked : I64 -> I64 -> Array a -> Array a`（境界チェックなし）を、単一 InlineLLVM op（`ArraySwapBody { force_unique, bounds_checked }`）で codegen 直書き（`set` 同様＝超高速）。`i,j` の要素を op 内部で move 交換（`read_from_array_buf_noretain`〔`len=None` で境界省略〕＋`write_to_array_buf(release_old=false)` を op 内に閉じ込め、穴を外に出さない）。両版とも `set` と同じ **`force_unique` フラグ**を持ち（uniqueness は常に安全＝現 `_unsafe_swap` の uniqueness 無チェックより堅牢）、§4 が配列を静的 unique と証明できれば force-unique を elide。**ソート（introsort/heapsort、std.fix:439/461/466/473/527/555）は `swap_bounds_unchecked` を使う**——境界チェックは §6 の境界除去（初版対象外）に頼らず最初から無し・uniqueness は §4 が elide、で **§4 完了時に現状（全 unchecked）と同速＝sort 無退行**（§5 の gate）。arg: `i,j` unboxed・`array`=`Own`、result=`Fresh`（force-unique 版・elide 版とも結果は unique）＝**CLEAN**（穴を露出しないので §8 (a) の窓が消える）。**uniqueness 無チェックの公開版は作らない**（§8/P3.5 が排除する穴）。
  - **廃止と置換**: `_unsafe_get_linear_bounds_unchecked_unretained`(+`_forceunique`) と `_unsafe_swap_bounds_uniqueness_unchecked` を**廃止**し、利用者を atomic builtin へ移す—— `mod`/`act`（単一穴、std.fix:166/174/190）→ PunchedArray punch/plug、`swap`（二重穴、570/571）→ builtin swap、`pop_back`（`set_size(len-1)` 後に**境界外**の末端を取り出す、366）→ 専用 builtin（shrink＋末端 move-out を op 内に閉じ込め）、generic `act`・std swap 版 `PunchedArray`（2668/2692/2698）→ builtin PunchedArray。**`_unsafe_set_bounds_uniqueness_unchecked_unreleased` は残す**（穴埋め用途は消えるが、append/push_back/map/reserve の**未初期化スロット書き込み**という別用途で正当に使う。265/307/383/401/689）。生 codegen helper（`read_from_array_buf_noretain` 等）は各 atomic builtin の**内部**に閉じ、Fix レベルの composable op としては露出させない → **隠れ穴 primitive はどこにも残らない**。
    - **【実装時の決定・当初案から逸脱】(A) `_unsafe_get_linear_bounds_unchecked_unretained`(+`_forceunique`) の物理削除は P0.7 では行わず P3/P3.5 に延期する。** std 側の利用者（`mod`/`_unsafe_act_bounds_unchecked_identity`/`_tuple2`/`pop_back`）は atomic op へ移行済み（前者3つは PunchedArray punch/plug、`pop_back` は新 builtin `_pop_back_nonempty`＝force-unique＋末端 release＋shrink を op 内に閉じ込め）だが、**外部ライブラリ cp-library の NTT バタフライ（zp.fix）が 2 要素の linear read-modify-write に直接使用**しており、これは unboxed 要素（ZP＝unbox struct）なので §8(a) の boxed 隠れ穴 footgun ではなく、かつ PunchedArray（単穴）に置換できず、安全 op へ替えると P3 前は配列 clone で激遅になる。∴ line 668 が許す「abolish を P3 に寄せる」を採用。get_linear は登録のまま残す（今や cp-library のみが使用）。∴ この時点では「隠れ穴 primitive がどこにも残らない」は未達（get_linear が残る。P3/P3.5 で完遂）。**(B) 命名**: punch/plug は内部専用なので `_` private ＋ `_unsafe_` ＋ `_bounds_unchecked`（4 版は §7 P0.7 の punch/plug 項参照）。swap の境界無検査版は**公開する**ので先頭 `_` を付けず `Array::unsafe_swap_bounds_unchecked`（`unsafe` 語で危険明示・docs 掲載）とする（当初案の「公開版を作らない」から変更——公開の境界無検査 API を許容し、名前に `unsafe`＋`bounds_unchecked` を付けて契約を明示）。
...
- **P4**: reuse / 順序スケジューリング / 境界チェック除去 等。**完了ゲート（本 plan 全体の締め）**: `benchmark/speedtest/cases/sort` の `sort-inst` が P0.5 ベースライン以下＝**sort 無退行**（§5 の gate・測定は §9.8）。§6 の最適化には頼らない。
...
- **container の boxed/unbox で結果が変わる**（一律に扱わない）: struct `@field`／union `as_` は boxed 容器 → `Dyn`、unbox 容器 → `Arg(i,[field])`（move-out・no-retain の素通し。retain されないので「retain 済みコピー」と誤ると二重計上）。**unbox getter は純射影 op（op 内で retain/release しない・[#F4]・[#R12-3]）**: 取り出す field の末端は結果へ素通し（`result_prov=Arg(i,[field])`・`root` が除外＝op では consume でなく結果の行き先へ先送り）、**取り出さない sibling 末端は `Borrow`（op は触らない＝retain も release もしない）**。so `arg_ownership` は取り出し末端 Own（forward・passthrough で除外）・sibling `Borrow`・result は `Arg(i,[field])`。**落とす sibling は §1.4(c) の明示 `Release(s@π)`** が引き受ける（これは own-then-release の drop＝consume_sites の sink でない）。純射影なら root/consume/provenance が均一に扱え drop が明示ノードで解析可能。**「sibling を op 内で release する版」は別 op（sibling=Own・consume_sites 入り）** で、現 codegen `get_struct_fields` の in-op release がこれに相当——§6 で「明示 Release を getter に融合」する最適化として後入れ可能で、base ではない。2 版は `arg_ownership` の sibling（`Borrow` vs `Own`）で一意に区別され、宣言と実挙動の一致は §9.0 validator が検査。
...
- **（実装済み・2026-07-11）構築・射影・FFI・捕捉取り出しの `LLVMGenerator` 化**: 上の「構築も `LLVM`」を実現するため、現 codegen が `generator.rs` 内で *inline* に生成していて再利用可能な generator を持たなかった操作に、新たな `InlineLLVM` variant を 5 つ追加した（`MakeStructBody`／`ArrayLitBody`／`FFICallBody`／`CaptureProjectBody`〔lift したクロージャの捕捉取り出し retain-getter〕／`StructProjectBody`〔destructure 用の struct フィールド取り出し。boxed 容器なら retain-getter・unbox なら純射影・容器 release は出さない〕）。union payload の取り出しは `Match` codegen 内在なので op 不要。旧 inline パス（`eval_make_struct`/`eval_array_lit`/`eval_ffi_call`）は**そのまま残す**（並行 A/B ロールアウト。alloc+fill の殻の一時的重複は flip で削除）。FFI は `eval_ffi_call` から `build_ffi_call_core` を抽出して共有（旧パスはバイト等価: return object を先に作り、C 関数宣言は module レベルで命令列を並べ替えない）。5 op とも operand を `get_scoped_obj_noretain` で読む（RC IR 契約＝素の get。複製は明示 `Retain`/`Release`）。検証: `cargo build` clean・`test_ffi_call{,_ios,_io}` pass（旧パス変更は挙動保存）。射影の RC モデルは [#R10-1]（boxed=retain-getter〔retain を op 内に埋める〕・unbox=純射影・容器/落とし末端の `Release` は Phase B）。
- **（実装済み・2026-07-12）Phase B（RC 挿入）**: `src/rc_ir/rc_insert.rs` の `insert_rc(prog, type_env)`＝関数/global-init ごとの後ろ向き last-use パス。whole-value `Retain`/`Release`（`RcState=Unknown`）、fully-unboxed 値は RC 省略。operand の Own/Borrow は新メソッド `LLVMGenerator::borrows_operand(i)`（read-getter 10 個の operand 0 のみ Borrow・他は Own。§8 監査を全 `LLVMGenerator` variant 精読で確定）。`Match` は `process_match` が boxed union の容器 `Release` を各 arm 先頭に出し（payload retain-getter は unit-3 Match codegen 内在）、scrutinee を後で使うときのみ手前で `Retain`。`DUMP_RC_IR` で検証（二重使用 borrow getter・borrow last-use release・boxed/unbox match・タプル/struct destructure・捕捉投影＋cap release・`with_retained`・未使用 fully-unboxed）。
- **（実装済み・要レビュー・plan 逸脱）unbox getter を純射影でなく統一 retain-getter にした**: `build_struct_project` が boxed/unbox とも取り出し末端を retain し、容器は `Borrow`（whole-value `Release` を last-use に配置）。利点: 部分パターン（フィールド省略）が容器 `Release` の traverse で自動処理され特別扱い不要。**逸脱**: 「unbox getter=純射影・op 内で retain/release せず」（[#F4]/[#R12-3]・§1.2/114 行）に反し、unbox destructure で boxed 末端ごとに retain/release ペアが入る＝**Phase B が持ち込む唯一の非パリティ項目・P1 パリティゲート前に解消必須**（挙動・リーク無しは不変）。**§2.2 相殺では消えない**（retain は op 内に焼き込みで明示ノードでない・かつ容器 `Release(s)` の traverse が別変数の末端を release＝クロス変数で §2.2 の同一変数相殺の対象外）。除去には per-leaf 純射影（焼き込み retain を止め容器の落とし/後使用末端を per-leaf 処理）または §6 move-out が要る。第一版で end-to-end を通す判断。per-leaf 化を unit 3 前にやるか後にやるかは要相談。
  - **[実装メモ 2026-07-20]** 解消済み。struct/タプルの destructure は getter 列でなく専用ノード `RcExpr::Destructure`（§1.2 の実装メモ）になり、unbox 容器では末端をフィールド変数へ move する（フィールドごとの retain/release ペアを出さない）。容器の retain は「destructure 後も容器が生きているか」で RC 挿入が一度だけ決める。
- **（決定・要 unit-1 反映）catch-all match arm を `MatchArm.variant: Option<usize>` で表す**: 現 `eval_match` は非 variant（`Var`）の arm を末尾 default として許す（generator.rs の「non-variant pattern is at the end」）。現 `MatchArm.variant: usize` はこれを表せないので `Option<usize>` に変える（`Some(tag)`＝variant arm〔payload はその variant のもの〕・`None`＝catch-all〔payload は scrutinee 全体〕）。codegen は「末尾 arm＝default」を維持（`eval_match` の switch と同型）。catch-all を variant ごとの arm へ脱糖する案は arm body を複製するので不可（ANF が避ける）。`lower.rs` 実装時に `ast.rs` を変更し、その slice と一緒にレビュー。
...
- **（確認済み）高階イテレータの直接化は既存 decapturing に依存**: `loop`/`fold` 等は body を closure 引数で受け、その内部呼び出しは間接。uniqueness 特殊化（§4）が内部の force-unique op に届き、Provenance 解析（§3）が精度を出すには、この間接呼び出しが直接化されている必要がある。既存の **decapturing の closure specialization**（`src/optimization/closure_specialization.rs`、`enable_closure_specialization`。`inline` 後・`uncurry` 前＝RC IR lowering の前）が、body を焼き込んだ特殊版（`loop#lam` 等）を生成し内部呼び出しを直接化する（`pull_let` が適用範囲を広げる）。ベンチの `loop((0,arr), |…|)` は適用対象（lambda を直接渡す・自己呼び出しで同 index）。適用外の形（doc の制限: lambda をタプル等に入れて渡す等）は間接のまま残り、§3.2 の規則で結果 Dyn＝除去は効かない（健全・許容）。unique-check-elim を回すときは decapturing を有効にしておくのが前提。
...
- **（実装済み・plan 逸脱・許容）借用可能性の不動点は SCC bottom-up でなく naive な大域反復**（§2.1）: plan §2.1 の擬似コードは「コールグラフを SCC 分解し condensation を bottom-up、SCC 内は不動点」だが、実装 `infer_ownership`（`src/rc_ir/borrow.rs`）は全関数を一巡する単調反復（`Borrow`→`Own` の降格が起きなくなるまで）で決める。結果は同一（単調・有限束・間接呼びは全 `Own` 保守）。SCC は反復回数の最適化にすぎない。実 codegen で稼働させても全テスト（3 opt）・minilib 全 16 でコンパイル遅延は実測されない。コンパイル時間が問題化したら SCC 化する。`root` 別名追跡・消費列挙・routing は plan どおり。
- **（実装済み・設計判断）borrow 化・相殺は `Max` 以上のみ・codegen ユニット単位で走らせる**（§1.5/§2.1）: `borrow_ify`+`cancel` は `Configuration::enable_borrow_optimization()`（＝ `force_all_opt || opt >= Max`。inline/decapturing と同じ）で gate し、実 codegen（`build_object_files.rs`）でも `--emit-rc-ir` dump でも `Max` 未満はスキップ（`lower→insert_rc→split_rc_units→implement` のみ）。理由: `basic` はコンパイル速度重視の層で、borrow のフル効果は decapturing（`Max` 限定）に依存し、`basic` の borrow はモジュール単位で部分的なため、コンパイル時コストに見合わない。`Max` では `enable_separated_compilation()` が偽 ＝ 分離コンパイル OFF ＝ **全プログラムが 1 ユニット ＝ 全プログラム borrow 化（全精度）**。よって borrow が走るのは常に全プログラム単位で、ユニット跨ぎの取りこぼしは起きない。（ユニット単位で走らせる設計自体は、将来 `basic` 等でも borrow を有効化した場合に備えて健全: `f_own` は常に all-`Own` ＝ 間接・cross-unit 呼びの ABI 不変、borrow 版は同一ユニット内 routing のみ、cross-unit へ借用値を all-`Own` 位置で渡すときは `call_rc` が呼び出し前 `Retain` を出す〔[#F1round8]〕。）
- **（実装済み・plan 逸脱・許容・健全）§2.2 相殺は Destructure を一律 consume 扱い（unboxed は過剰保守）**（§2.2）: `cancel` の walk は Destructure で `boxed_leaves(container.ty)` の全末端を consume する。boxed コンテナ（末端 ＝ コンテナ全体）は destructure が実際に release するので正しい。unboxed コンテナは boxed 末端がフィールド変数へ **move（別名）** するだけで消費されないが、`cancel` はこれも consume 扱いにする ＝ その末端の pending `Retain` を `needed`（相殺不可）に固定する。損失は「相殺できたはずの `Retain` が残る」精度損のみ（参照カウントは balanced ＝ リーク/UAF にはならない・過剰 consume は `Retain` を残す安全側）。推論側（`collect_consumes_go` ＋ `root` の `Def::Field`）は unboxed の move を別名として透過し精密。精密化するなら cancel の Destructure を「boxed コンテナ ＋ 落とす無名 unboxed 末端のみ consume」にしフィールド変数へ pending を透過（§9.5 相当のテスト付き）。
...
- **RC IR validator（[#14]・debug ビルド限定の整形性検査）**: RC IR を書き換える全パス（§2.1 phase-2 patch・§2.2 相殺・§4.2 差し替え・将来 §6）の**直後**に走る `validate(RcProgram)`（debug ビルドのみ・release では無効）。検査: (i) 束縛名のグローバル一意（§1.1-3）、(ii) use-after-consume が無い（`Own` 位置で消費した変数がその後現れない）、(iii) 関数ごと・root ごとの**参照収支**（param の Own/Borrow・`Retain`・`Release`・consume の経路和が 0）、(iv) `Closure` 捕捉順と lifted 関数の射影順の一致（§1.2 不変条件）。(iii) は [#F1] のような patch の収支割れ（孤児 `Retain`＝over-retain リーク）を実装直後の任意入力で捕まえる——uniqueness assert ビルドは「unique 誤判定」しか見ず over-retain は見ない・valgrind は global 到達漏れを見逃す、という穴を埋める。P1 で作り以降の全パス後に常時走らせる。
  - **[実装メモ 2026-07-20]** `src/rc_ir/validate.rs` として実装。検査済みは **(i) 束縛名の関数内一意 + use-in-scope** で、**(ii) use-after-consume・(iii) 参照収支・(iv) closure 捕捉順は未実装**。gate・走る位置・未実装検査の定式化・検証手順は `rc-ir-validator.md`、その前提となる消費モデルは `rc-ownership-model.md` にある。
...
- 分岐: `if b {..} else {..}`・`match b { _true() => .., _false() => .. }`（union の variant パターン）。
...
- **sort 無退行（本 plan 完了ゲート・§5）**: `benchmark/speedtest/cases/sort` の `sort-inst`（cachegrind）が **P0.5 ベースライン以下**であること（`swap_bounds_unchecked`＋§4 elide で現状〔全 unchecked〕と同速に戻る）。§6 の境界除去 OFF で測る（§6 は初版対象外）。log.csv に P0.5〜P4 の推移を残し、P0.7〜P3 の一時劣化が §4 で回復したことも確認。
