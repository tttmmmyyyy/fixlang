...
- **plug** `plug(elem, punched)` → `Array a`: `punched`＝`Own`・`elem`＝`Own`（穴へ move-back）・`result=Boxed({Fresh})`（`set` 同様に unique を継続）。
