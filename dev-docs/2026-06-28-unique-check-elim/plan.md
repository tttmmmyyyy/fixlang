...
- **P0.7（早期・P1 と独立に入れられる）: 配列 read-modify-write を atomic builtin へ集約**（builtin `PunchedArray`＋builtin `swap`、`_unsafe_get_linear_bounds_unchecked_unretained` 系・`_unsafe_swap` を廃止）。狙い: **隠れ穴を作る composable primitive を無くして §8 (a) の DOESNT-FIT を完全消滅**させ、各 atomic op に force_unique フラグを付けて **§4 の除去対象に揃える**（swap もソートで全 unchecked 化）。std の swap 版 `PunchedArray`（std.fix:2648）の要素 move 2 回も無くす。作業:
  ...
  - **調査結果（codegen 機構・大幅変更なし。「unboxed 型に専用 release は作れない」は誤解だった）**: release/retain/mark は `create_traverser(ty, work)` が**型のフィールド構成から生成**し、**unboxed 型でもこれが呼ばれる**（`build_release_mark` の unboxed 分岐、`generator.rs:1259`）＝「unboxed 型の専用 traverser（release 含む）」機構は既存。フィールド単位のスキップも `ObjectFieldType::SubObject(ty, is_punched)` の `is_punched` で既にあり（`build_traverse` の `if *is_punched { continue }`、`object.rs:1646`。struct 側 punch がこれを使用）。**新規なのは「配列要素 1 個のスキップ」だけ**——`build_traverse` の `Array` 分岐（`object.rs:1670-1674` の `release_or_mark_array_buf`）に、兄弟 `I64`(idx) を読んで idx を飛ばす skip-idx 版（既存 `loop_over_array_buf`〔`object.rs:321`〕を `[0,idx)`+`(idx,size)` で 2 回呼ぶ）を足し、`PunchedArray` 型の traverser でそれを使う。retain/mark も同じ traverser（work 違い）で一括スキップされる。clone（`make_unique`）だけは traverser でなく `clone_array_buf` 経由なので、共有され得るなら skip-idx clone も要る——ただし**線形フローでは PunchedArray は常に unique ＝ clone は走らない**ので、堅牢性目的の対応（要ノート）。∴ **既存機構の局所拡張のみで、codegen の大幅変更・boxing は不要**（`unbox { Array a, I64 }` のまま skip-idx traverser を持たせる。std 版との差は swap-to-end を skip-idx traverser に置き換える点だけ）。型名は `PunchedArray`（std 同名 struct を builtin で置換）。
