...
**継続入れ子・単一 enum（終端子つき）**形式を採る。`Let`/`Retain`/`Release` は継続を持つ「文」的ノード、`Ret`（式の値＝末尾 cexp）が唯一の終端子。`mark_threaded`/`mark_global` は専用ノードでなく**値を産む InlineLLVM op**（下記）。`Match` は**値を産む cexp（`RcRhs`）**で分岐を表し、`Let(x, Match, k)`（非 tail）／`Ret(Match)`（tail）に現れる（教科書 ANF の `let x = <複合式> in …`）。この形は reset/reuse・borrow 等の RC 最適化を載せやすい。Fix の既存 AST（再帰的）とも同形で流用しやすい。各ノード・各変数は span と具体型を持つ（§1.1-5,6。下では省略）。
```rust
...
enum RcExpr {
    ...
    Retain(Var, Path, RcState, Box<RcExpr>),  // x の Path が指す部分木の【全 boxed 末端】を dup（root +1） → 継続。Path=[]＝値全体（全 boxed 末端）、Path=π＝末端/部分木 π。RcState で state ディスパッチ。lowering は値全体 `Retain(x, [])` を出し、§2.2 分解・§3.2 降格・§2.1 除去が per-末端 `Retain(x, π)` を生む
    Release(Var, Path, RcState, Box<RcExpr>), // x の Path が指す部分木の全 boxed 末端を drop（-1; 0 で解放、構造辿り） → 継続。Path の意味は Retain と同じ（[]＝全体）
    Ret(RcRhs),                           // 唯一の終端子。「この式（関数本体 or match arm）の値 = この cexp」。Ret(Var(x)) が旧 Ret(x)・Ret(App(..)) が末尾位置の呼び出し・Ret(Match(..)) が末尾 match
...
type Path = Vec<usize>;  // 値の unboxed 構造への index 列（struct/タプル＝field 番号、unboxed union＝variant 番号を前置＝§2.1 root の k::π）。boxed 末端または部分木を名指す。[]＝値全体。UniquenessShape/OwnershipShape/Provenance（§3.1）の木もこの Path で末端を引く（Retain/Release・root・per-末端解析が共有する唯一の位置表現）
struct MatchArm { variant: usize, payload: Var, body: RcExpr }  // payload を単一 Var に束縛（さらなる分解は getter で）。body はその arm の値を末尾（Ret）に持つ式
enum RcRhs {                              // cexp（複合式）: Let の RHS か Ret にのみ現れる。App/LLVM の引数は Var（atom）
    ...
    Match(Var, Vec<MatchArm>),            // 唯一の分岐構造（Bool もここ）。値を産む cexp: `let x = match…` は Let(x, Match, k)、末尾 match は Ret(Match)
}
...
- **tail position・末尾呼び出し・phi 回避**: `Ret(c)` は「今いる式（関数本体 or match arm）の値は c」。**tail position（＝関数の返り値になる位置）は構造的に決まる**: 関数本体の `Ret` は tail position、`Ret(Match(…))`（末尾 match）の各 arm の `Ret` も tail position（伝播）、しかし `Let(x, Match(…), k)` で束縛された match の arm の `Ret` は tail position **でない**（値は x に束縛され k が続く）。**呼び出し `App` が tail call ⇔ その `Ret` が tail position**——「直後に `Ret` があるか」でなくこの伝播で判定する（例: `Let(x, Match{…Ret(App f)…}, Let(y=g(x), Ret(y)))` の `App f` は**非 tail**〔後で g が使う〕・`Let(x, Match{…Ret(App f)…}, Ret(x))` の `App f` は **tail**）。これは現 codegen の `tail` フラグ伝播（`eval_match(…,tail)`→各 arm `eval_expr(val,tail)`、generator.rs:2287,2307）と同一。**codegen 規則**: tail position の match は **merge/phi を作らず各 arm が直接 return**する（`call; ret` の隣接を保ち LLVM の tail-call elim を効かせる。複数 arm でも各 arm が独立に `tail call; ret`）。非 tail の match だけ merge ブロック（phi/slot）で結果を x に束縛し k へ。§2.1 の borrow/tail 判定もこの構造的 tail position を使う。加えて、この compiler の tail-call elim は LLVM の `tail` マーカー（`apply_lambda` の `set_tail_call`。generator.rs:1010。**`-g`＝debug info 時は付けない**）にも依存するので、RC IR codegen 付け替え時もこのマーカー付与を維持する（`call; ret` 隣接**と** `tail` マーカーの両方が要る）。
...
ポイント:
- **cap の release は callee（`g#lifted`）**。boxed 捕捉は「`Retain`(取り出し)＋`Release(cap)`」で表す（現 codegen 忠実）。arr2＝取り出した要素と cap＝コンテナは**別 object** なので §2.2 の相殺（同一 root 照合）ではこの形のままでは消えない（両方消すと cap の殻が解放されずリーク＝下の RC 収支が破れる）。**ただし cap が unique なら move-out できる**——ただし **§2.2 の root 照合では不可**（cap は boxed で中身を追わない・boxed getter は毎回別 root なので `Release(cap.@0)` が `Retain(arr2)` と照合しない・arr2 は Dyn の producer で cap のスロットと結びつかない）。代わりに **§6 reuse フェーズの構造的 rewrite**: lowering の形（`let arr2 = proj.0(cap)`）から「arr2＝cap のスロット」と分かるので、cap が unique なら `let arr2 = proj.0(cap); Retain(arr2); …; Release(cap)` を「**`Retain` 省略＋`Release(cap)` を殻解放のみ（中身は release せず arr2 へ移譲）**」に直接書き換える → 残るのは殻解放だけ。cap の uniqueness は §4 の特殊化（unique-cap clone）が与えるので §6（§2 前処理は §3 前で uniqueness 未確定ゆえ不可）。現 codegen の `get_struct_fields`/`get_union_value` の move-out 最小化の一般化。空捕捉なら cap は null で `Release(cap)` は no-op。
...
**escape-reachable な関数は複製して borrow 化する（間接呼び出し規約を壊さない）**: borrow 化は関数の呼び出し規約（`Own`＝callee が release／`Borrow`＝呼び出し側が release）を変えるが、**間接呼び出し**（`Closure` 値/funptr への `App`）は callee 不明ゆえ**固定 all-Own 規約**で呼ぶ（上記「間接呼び出しは全 Own」の呼び出し側）。so ある関数が `Closure` ノード／funptr atom に現れて間接呼び出しされ得る（**escape-reachable**）のに直接呼び出しの都合で `Borrow` に降格すると、間接呼び出し側（all-Own で所有権を渡し release を期待）と衝突して**リーク／under-release** する（同一 funptr が両規約を同時に満たせない）。対処: **escape-reachable かつ直接呼び出しもある関数は、all-Own の original を残し**（`Closure`/funptr がこれを参照＝間接呼び出しは original を all-Own で呼ぶ）、**borrow 化した clone を作って直接呼び出し地点だけそれに張り替える**（直接呼び出しの精度を保つ）。間接のみなら original を残すだけ（clone 不要）、**直接のみ**（escape しない。decapturing 後の `loop`/`fold` body 等）はその場で borrow 化。これは **§4.1 の clone-for-direct-keep-original-for-indirect（[6]/[8]）の OwnershipShape 版**——escape-reachability の根は §4.1 dead-func 到達根（`Closure` 参照・funptr atom）と同じで、dead-func が両版（original／clone）を生かす。順序上は §2 が所有権版を作り、§4 が直接到達版を uniqueness 特殊化する（間接用 original は [6] で非特殊化）。
...
**末尾呼び出しを壊さない（制約）**: 「呼び出し**後**の `Release`」が**末尾位置（tail position。§1.2 の構造的定義——match arm 内の呼び出しは、その match が tail position のときだけ tail）**の呼び出しに**残る**（相殺で消えない）と tail call でなくなる（`let r = App(f, args); Ret(r)` が `…; Release(x); Ret(r)`）。非末尾化された呼び出しが**閉路**に乗っていると再帰深さぶんスタックが伸びて overflow する。**閉路判定は保守的にしかできない**——間接呼び出し（`Closure`/funptr 値への `App`）は静的コールグラフに辺を張れないので、間接辺で閉じる動的サイクルは静的には非閉路に見える。so「cross-SCC だから非末尾化して O(1) 安全」と決め打つと、この間接サイクル上の直接 tail 辺を誤って非末尾化して overflow する（round-6 [#2]。実測: 現コンパイラは間接 tail 呼びのサイクルを release ビルドで TCO し O(1) で回すので、非末尾化は退行になる。generator.rs:1008-1010 が全 `App` を tail マーク）。**方針（[#2] の修正＝B 案）: 末尾呼び出しは一切非末尾化しない**（閉路判定に依存しない・自明に baseline と同等）。tail 呼びでは `Borrow` 化の caller 側利得が元々ゼロ（呼び出しの「後」が無い＝借用値を再利用する先が無い。しかも tail では所有値を `Own` で渡しても手放すだけで `Retain` 不要）なので、tail を `Own` に倒しても**損失は無い**。末尾呼び出しの扱い:
- **(A) read-only 値を param から param へ素通し（caller が借用）**: 不動点がその値の通る全 param を `Borrow` にするので、末尾呼びは borrow -> borrow で release が出ず、tail は**自動で保たれる**（所有者は呼び出し元）。普通の read-only 再帰・素通しラッパ（`sum`／`sum2 = |a,i,c| sum(a,i,c)`）はこれ——ラッパは転送 param を所有せず**借用するだけ**なので、何段挟んでも borrow が伝播する（B でも Own 化しない）。
- **(B) caller が所有する値（fresh・または他で consume される値）を borrow 引数として末尾呼びに渡す**: 所有値は借用位置へ渡すと後続 `Release` が残る（＝非末尾化）ので、その引数を `Own` に留める（callee が consume＝tail 保持。fresh を受ける param は uniqueness を運ばないので `Borrow` 化の利得は元々無い）。判定は `owns(f, x@π)`（root が f 所有の producer/param に解決）のみで、**閉路か否かを問わず全 tail 呼びに適用**する（cross-SCC の tail もここで `Own` 固定＝[#2] の穴を閉じる）。(B) で `Own` 固定した param は外部呼び出し元にも `Own` になり、そこが**非 tail** で再使用すると `Dynamic` になる（両取りは特殊化。P2。この周辺損失は稀——ある callee を「所有値で tail 呼び」しつつ別所で「非 tail 借用」する形でだけ生じ、ベンチのホットパスに無い〔read-only ヘルパは非 tail 借用・自己再帰ループは Own/unique 志向〕）。同じ値が tail の**複数位置に出る**（`g(x,x)`／`let y=(x,x); g(y)`）ときは、一部だけ `Own` にすると残る Borrow 位置に後続 `Release` が出て非末尾化するので、**f 所有末端は全出現を `Own`** にする（mixed Own/Borrow は不可。複数使用ぶんの `Retain` は lowering が供給し全コピーが consume＝tail 保持、その値は `Dynamic`。単発 `g(fresh())` は `Own` 単発・`Retain` 無し・`Unique` のまま）。
...
- phase(ii) tail `App(loop_fresh,[n1,fr]);Ret(r)`: `owns(loop_fresh, fr@[])`＝`root` が fill producer ⇒ **True** ⇒ **`own[loop_fresh.1@[]]=Own`**（case B 降格。判定は `owns` のみ——`last_use` は使わない。tail の f 所有末端は【全て】Own＝[#2] で閉路以外の tail にも拡張・§10）。
  ...
  repeat 全体で変化が無くなるまで:
    for scc in bottom_up(condensation(call_graph(prog))):        # callee 先（ラウンド内スケジュール）
      for f in scc:
        ...
        for f の各 **tail 呼び出し（全て）**（§1.2 の構造的 tail 伝播で body を辿り tail-position の App を拾う: `Ret(App(g,args))`・tail-position の match arm 内 App。flat な `App;Ret(r)` に限らない）:  # 閉路/非閉路を問わず全 tail（[#2]＝B 案）
          for (q, x) in enumerate(args), x の各 boxed 末端 x@π:    # q=位置
            # tail 呼びでは f 所有末端は【全出現】を Own にする（閉路か否かを問わない＝[#2] B 案: tail を
            # 非末尾化しない）。一部を Borrow に残すと（同一値が複数位置に出る g(x,x) 等で）残った Borrow
            # 位置に後続 Release が出て非末尾化する（mixed Own/Borrow は不可）。複数使用ぶんの Retain は
            # lowering が供給し全コピーが consume（その値は Dynamic 化するが、同一 fresh の二重渡しはレア。
            # 単発 g(fresh()) は Own 単発・Retain 無し・Unique のまま末尾保持）。owns が偽（caller が借用）なら
            # borrow->borrow で tail は自動保持＝Own 化しない（素通し＝ケース(A)。sum/sum2 ラッパはこれ）。
            if owns(f, x@π): own[g.q@π] = Own                       # (ii) case B -> 降格（全 tail・f 所有末端は全出現 Own）

  # 1.5 escape-reachable の複製（間接呼び出し規約を壊さない・散文 L181・[#5]）: 不動点確定後、【escape-reachable
  #     ＝FuncRef が Closure ノード/funptr atom に現れる、かつ直接呼び出しもある】関数 g を 2 版に分ける——
  #     original（全 param を Own 固定。Closure/funptr の間接呼び出しはこれを呼ぶ）と borrow 化 clone（step 1 の
  #     own を持つ。直接呼び出し地点だけこれへ張り替え）。間接のみ＝original だけ残す／直接のみ（escape しない・
  #     decapturing 後の loop/fold body 等）＝その場で borrow 化（clone 不要）。§4.1 の clone-for-direct／
  #     keep-original-for-indirect（[6]/[8]）の OwnershipShape 版で、dead-func が両版を生かす。
  #     以降の step 2 は【張り替え後のプログラム】に一様適用する: original は Own 固定ゆえ remove/insert の対象
  #     末端を持たず実質触られず、borrow clone と非 escape 関数だけが書き換わる。

  # 2. RC 書き換え（own 確定後、all-Own の RC を patch。step 1.5 で張り替え済みのプログラムに対して）
  for f in prog:
    remove: root(f, y@π) が own==Borrow の param 末端に解決される全 `Release(y@π)`・`Retain(y@π)` を削除（どちらも per-末端・root ベース）   # callee は所有しない。root ベース([6])：p 直接名指しだけでなく別名の Release も拾う——unboxed union payload drop の Release(payload)(root=p@(k::π))・passthrough drop（let (b,_)=p.unsafe_is_unique の @1 の Release, root=p）。変数名マッチだと残って過剰解放→caller UAF
    # Retain も消す理由（消さないと孤児 Retain がリーク）: lowering は非 last-use の【Own 消費】用に Retain を入れる。
    # param p の末端が Borrow に確定した ⟺ phase(i) が p@π0 に Own 消費 sink を 1 つも見なかった ⟺ その末端の
    # 全使用位置は Borrow/read/drop ＝ p 向けに入れた Retain(p@π0) は宛先（消費側）を失った stale な +1。
    # §2.2 は Retain-Release の【対】しか消せず、対応 Release を持たない孤児 Retain は残り呼び出しごとに +1 漏れる
    # （例 `|arr| sum(arr)+sum(arr)`：sum.arr=Borrow ⇒ 両 App は非消費 ⇒ 1回目の非 last-use 用 Retain(arr) が
    #  孤児化。remove がここで消して net-0。テストは §9.5）。per-末端なので多末端値〔@0 が Borrow・@1 が Own〕でも
    # §2.2 分解後に Borrow 末端の Retain だけ消える。escape-reachable の all-Own original は Own 固定ゆえ対象外。
    for f.body 中の呼び出し **App(g, args)**:                       # Closure は対象外（[#6]）: 捕捉は常に move-in＝
                                                                    # Own consume（consume_sites・下記）で、借用化も
                                                                    # 後続 Release 挿入もされない。own[g.q@π] は g の
                                                                    # param 表を引くので、param でない捕捉スロットには型が合わない
      for (q, x) in enumerate(args), x の各 boxed 末端 x@π:        # q=位置
        if own[g.q@π]==Borrow and owns(f, x@π):
          呼び出し直後に x@π の Release を挿入                      # 借用末端なら何もしない
          # この App は必ず【非 tail】: tail 呼びで owns(f,x@π) が真なら phase(ii)(全 tail・[#2] B 案)が
          # 既に own[g.q@π]=Own に固定しているので、条件（Borrow ∧ owns）は tail では成立しない。ゆえに
          # 「直後」が常に存在し、非末尾化は起きない（tail 呼びは一切壊さない）。assert: この g(args) は非 tail。
...
**tail 認識の一貫性（不変条件）**: phase(ii)（**全 tail** の case B 降格）と 2. の RC 書き換えは **§1.2 の同じ構造的 tail 認識**を使う（`Ret(App)`・tail-position の match arm 内 App を含む）。phase(ii) が tail を漏らすと、書き換えが借用末端に後続 `Release` を挿入して**非末尾化 → overflow**する（例: `if` 脱糖で arm 内 tail になる `loop_fresh` の再帰。§2.1 の worked example L260-264 は arm 内 tail に phase(ii) を適用済み）。「呼び出しの直後に `Ret` があるか」という flat 判定は不可。

**抜けない末尾再帰ループ（`main`/event/server ループ）も同じ tail-call 保存で扱える**: 静的解析は有限束の不動点で必ず停止する（実行時に停止するかに非依存）。borrow 化の case B（[#2]＝B 案で**全** tail の f 所有末端を全 `Own`）が tail に後続 `Release` を出さないので、実行時にスタックが伸びず overflow しない——抜けないループはむしろ tail-call 保存が「有限スタックで回り続ける」ための核心ケース。非末尾の無限再帰は RC に関係なく実行時 overflow するので対象外（末尾ループのみ）。テストは §9.6。
...
- `Retain(y, k)`: **retain した各 boxed 末端 π について、`root(y@π)`〔§2.1 の root。§2.2/uniqueness で共有・L372〕を共有する全生存変数の当該末端**の `LeafSource` を **`Fresh`/`Arg`/混在を問わず `{Dyn}` に置換**する（whole `Retain(y)` は y の**全** boxed 末端、§2.2 分解後の per-末端 `Retain(y@π)` は末端 π のみ——どちらも root ベース。`let y=(x,z); Retain(y)` なら `root(y@[0])=x` と `root(y@[1])=z` の両方を各別名ごと Dyn。複製で別名を作った＝共有＝Dynamic。**L600「別名を作る操作で対象を Dyn」**）。**root ベースが要る理由**: unboxed aggregate は getter が中身を追う（`env[y]=env[t]@[0]` の別名辺）。`let y=t.@0; Retain(y)` を**変数キーで `env[y]` だけ**降格すると `env[t]@[0]` を漏らし、後の再取り出し `a=t.@0` が `Fresh` に化けて §4 が誤 elide → 共有末端を破壊。root ベースなら y も `t.@0`〔root 共有〕も再取り出しも一斉に `Dyn`。boxed 容器は getter が元々 `Dyn`（中身不追跡・L424）なのでこの漏れは起きない。既 `Dyn` はそのまま。`root(x@π)` を**共有しない**他末端は不変＝多末端値で sibling の一意性を保つ（§2.2 が per-末端に分解済み〔L328〕）。`Fresh`/`Arg -> Dyn` 降格を起こすのは `Retain`〔別名・per-末端・root ベース〕**のみ**。
...
- `Match(s, arms)`（cexp。`Let(x, Match, k)` か `Ret(Match)` に現れる）: 各 arm を**分岐前 env のコピー**から解析し、各 arm の末尾（`Ret`）の値を join（末端 `LeafSource` を union）＝この Match の provenance。payload 束縛は unboxed union payload → move 取り出しで scrutinee の子末端（`Arg` 系。不在 variant は空 Set）、boxed union payload → getter＝`Dyn`。Bool もここ（2 variant）。**非 tail `Let(x, Match, k)`** は k を全 arm で共有するので、x に結果値 join を束縛するだけでなく、**k の env は全生存変数末端について各 arm 出口 env を pointwise join（§3.1 の union）**する（片枝でのみ生じた `Dyn`——`Retain` 降格や mark op の結果——を k へ持ち越す。怠ると k で誤って `Unique` 扱い → elide → 破壊）。**tail `Ret(Match)`** は継続が無く各 arm が leaf（関数結果＝arm 結果値の join）。
- `Ret(c)`: この式の値の `Provenance` ＝ c の provenance（`Var(x)`→`env[x]`、`App`→合成、`Match`→上の arm 末尾 join、`LLVM`/`Closure`→宣言）。関数本体の `Ret` はその関数の結果 `Provenance`、match arm の `Ret` はその arm の値。
...
**解析の停止性・抜けない関数**: 基底集合は有限（RC IR は monomorphic〔§1.2〕＋boxed 中身を追わない〔§3.1〕ので provenance 木の末端は有限、各末端 `LeafSource` も BaseSource〔`Fresh`/`Dyn`/`Arg(i,π)`・π 有限〕の有限集合）で反復は単調（union）ゆえ、**解析は実行の停止性に依らず有限時間で停止**する。**抜けない関数**（`serve = |s| Ret(App(serve, [step(s)]))` 等、到達可能な非再帰 `Ret` が無い）の結果 `Provenance` はこの不動点で **⊥（空 Set）に収束**する（値を産まない）。この ⊥ は戻らない関数の**呼び出し後＝到達不能コード**にしか現れず（`resolve(⊥)` は `Unique` だが dead ゆえ無害）、状態（ループを流れる値）の由来は §3.4 と同じ不動点で収束するので **in-loop の elision は効く**（抜けるかに依らず body の解析は同じ）。
...
- **plug** `plug(elem, punched)` → `Array a`: `punched`＝`Own`・`elem`＝`Own`（穴へ move-back）。**2 版**（struct の `#plug_in`(fu)/(fu=false) と同型・§8 L605/606）: **fu 版**——`punched` が共有なら force-unique（＝skip-idx clone）し `result=Boxed({Fresh})`。**汎用 `act` が `map` で plug を複数回呼び punched が共有される**ので必須（[1]）。**非 fu 版**——`punched` unique 前提で穴へ書き戻し（旧スロット unreleased）、`result=Boxed({Arg(1)})`（move-back。呼び出し側 move-out 前提でのみ健全）。線形の `mod`/`act` 特殊版が使う。
...
- 別名健全性は「別名を作る操作で対象を **`root` ごと** `Dynamic` にする」で担保（§3.2 L407 の root ベース降格・§2.1 root は L372 で共有）。`Retain(y)` は `root(y)` を共有する全末端を `Dyn` にするので、unboxed aggregate の `let y=t.@0; Retain(y)` でも `env[t]@[0]` を漏らさず再取り出し `t.@0` が `Fresh` に化けない（[4]）。変数キー降格では unboxed で source 末端を漏らし破壊するので不可。boxed 容器は getter が元々 `Dyn` を返す（中身不追跡）ので漏れない。
...
- **（決定）borrow 化の release 遅延はメモリ・トレードオフとして許容**（§2.1・round-6 [#4]）: borrow 化は callee 内部の release を caller の呼び出し後へ移すので、**非再利用 caller × read-only callee** では対象 object の解放が遅れる: (a) **ピークメモリが増え得る**（大きい借用 object ＋ callee が last-use 後に大量 alloc の例で最大 2 倍）、(b) **release 直後の alloc による領域再利用も失われる**（早期解放なら次の malloc が同領域を再利用できたのに、遅延で不可に）。baseline は正しい Fix コードで通常 clone しない（値は mutate 時点で unique・retain/release は一時的で set 前に unique に戻る）ので、この遅延を補償する「clone 回避のメモリ勝ち」は**無い**——borrow 化は CPU（retain/release 削減＋is_unique チェック除去）の勝ちで、メモリは中立〜微負。Own 規約は早期解放・領域再利用の利点があり欠点は retain が増えることのみ。**現状は許容トレードオフとし、問題化したら将来対応**（§10 の clone フォールバック＝非再利用 caller に Own 版を振り分け／「callee が last-use 後に大 alloc なら Own のまま」ヒューリスティック）。**`Destructor` の dtor 遅延も同根で契約違反ではない**: 文書化された契約は「必ず解放される」（release は消えず遅れるだけ＝保存）と「`Destructor::borrow` の work 中は生存＝早すぎる解放をしない」（borrow 化は遅らせるのみで早めない＝保存）で、遅延はどちらも破らない。精密な dtor タイミングが要るコードは opaque な `Destructor::borrow`/`borrow_io`（最適化対象外）を使う。
...
- **`pop_back` builtin（[#12]・no-retain 末端 move-out ＋ shrink 安全）**: P0.7 で `pop_back` を専用 builtin 化（境界外末端を retain せず move-out ＋ shrink）するので、**boxed 要素**で突く: `[[1],[2],[3]].pop_back` を unique 文脈（in-place shrink＋末端 move-out）と shared 文脈（clone パスで取り出し要素の retain が要る）で。観測: 取り出した要素と残り配列の正しさ＋**リーク/二重解放なし**（§9.0 leak）。clone パスの取り出し要素の retain 漏れ・shrink 後の旧末端二重解放は valgrind でのみ出るので必須。
...
- **tail 制御（tail-call elim 維持）の検証**（新 IR で tail position の判定・phi 回避が壊れていないこと。§1.2 tail-position 規則）:
  ...
  - **非 tail の match は phi があってよい**: 結果を束縛して後続で使う形（`Let(x, Match, k)`、k で x を使用）は merge/phi でよく、arm 内呼び出しは tail でない——これも意図どおりであることを確認（tail 判定が「直後に Ret」でなく構造的伝播であることの裏取り）。
...
- **unboxed aggregate の別名降格（[4]・root ベース）**: `let t=([1,2,3],[4,5,6]); let y=t.@0; let t2=t.mod0(|a| a.set(0,99)); eval (y.@(0), t2.@0.@(0))` == `(1, 99)`。`y` が `t.@0` を共有するので `mod0` 内の `a.set` は clone すべき（in-place だと y が [99,2,3] に化ける）。**変数キー降格の実装だと破壊**（`Retain(y)` が `env[t]@[0]` を漏らし `a=t.@0` が Fresh→elide）＝root ベース（§3.2 L407）の回帰検出。boxed 版（`Pair` box struct）は getter が Dyn を返すので元々壊れない（対照）。
...
- **間接呼び出しの規約（[1]・リーク検証）**: escape-reachable な関数を**直接と間接の両方**から呼ぶ入力——例: 読むだけの lambda `f = |x| x.@size` を specializable な `loop`/`fold` に渡し（decapturing が直接化）、同じ `f` を値としても間接呼び出しする（`let g = [f]; g.@(0)(arr)`）。観測: (a) **リーク／二重解放なし**（§9.0 の leak チェック。漏れる `arr` は **global に載せず**ローカルに閉じて definitely-lost に出す・大 N 反復で RSS 一定）、(b) ownership ダンプで **all-Own original（間接用）と borrow 化 clone（直接用）の両版が残り**、`Closure`/funptr は original を参照・直接呼び出し地点は clone を呼ぶこと、(c) borrow 化した関数を間接呼び出ししても under-release／UAF しないこと（sanitizer/assert ビルド）。escape しない直接専用関数だけが in-place borrow 化されることも確認。
...
- **phase(ii) は【全】tail の f 所有末端を"全て"Own にする（`last_use`／一部だけ降格は却下・[#2] で「閉路 tail のみ」から拡張）**（§2.1）: 同一値が複数位置に出る（`g(x,x)`）とき、mixed（一部 Own・一部 Borrow）は残る Borrow 位置に後続 `Release` を生み**非末尾化**する。その `Release` は §2.2 で消せない（間に Own consume が挟まる＝下記）。「最終位置だけ Own（last_use）」も「非最終を Own」も mixed ゆえ同じく破綻。全 Own なら callee が全 consume・lowering の `Retain` が rc を供給・tail 保持（値は `Dynamic` だが同一 fresh の二重渡しはレア。単発 `g(fresh())` は `Own` 単発・`Unique` のまま）。
- **tail 呼びは一切非末尾化しない＝全 tail を保護（cross-SCC tail の非末尾化・閉路判定への依存は却下＝[#2]）**（§2.1）: 初案「閉路の tail だけ守り cross-SCC tail は非末尾化して O(1) 無害」は、**間接呼び出し（`Closure`/funptr への `App`）が静的コールグラフに辺を張れず、間接辺で閉じる動的サイクルが静的には非閉路に見える**ため破綻する（cross-SCC と誤判定した tail を非末尾化 → 間接サイクルで overflow。実測: 現コンパイラは間接 tail サイクルを release で TCO し O(1) で回すので非末尾化は退行。generator.rs:1008-1010 が全 `App` を tail マーク）。対処は**全 tail を `Own` 固定（非末尾化しない）**——閉路判定に依存せず自明に baseline と同等。tail 呼びでは `Borrow` 化の caller 側利得が元々ゼロ（呼び出しの「後」が無い・所有値を `Own` で渡しても `Retain` 不要）なので損失は無い。**周辺損失**（callee を「所有値で tail 呼び」しつつ別所で「非 tail 借用」する形でのみ、callee param が Own に全域化して非 tail 借用側が retain を負う。read-only ヘルパ〔非 tail 借用〕・自己再帰ループ〔Own/unique 志向〕には無いのでベンチのホットパスに出ない）が将来ホットパスで問題化した場合の**フォールバック案**: その callee を **Own 版と Borrow 版に clone**し、tail-owned な呼び出し地点は Own 版・非 tail 借用地点は Borrow 版へ振り分ける（escape-reachable の original/clone 機構〔§2.1 L181〕を tail-ness 軸へ再利用＝実務 2 版。引数ごとに owned/borrowed が混在する tail 地点だけ all-Own 版に丸める〔最悪 2^引数 の per-site-最適版は作らない〕）。**保守的な間接辺グラフで閉路を精密判定する案（cross-SCC tail の非末尾化を残す）も健全**だが、B（全 tail 保護）が単純かつ対象ワークロードで無損失なので v1 は B を採る。
...
- **`Match` は値を産む cexp（`RcRhs`）・式の値は末尾 `Ret`・tail position は構造的（Match に束縛機能を持たせる案は却下）**（§1.2）: `let x = match…; k` を継続複製の指数爆発なく表すには Match が値を産む必要がある。Match に `Option<(result, 継続)>` を持たせる案は Let の束縛機能と重複するので却下。**Match を `RcRhs`（cexp）に置き、束縛は Let 一本・式の値は末尾 `Ret(cexp)`**（教科書 ANF の `let x = <複合式> in …`）。`RcExpr`/`RcRhs` 分離の価値（全値に名前・順序固定・オペランド atom → RC/uniqueness の全パスが線形/木の走査）は保つ（App/LLVM の引数は Var のまま）。**tail position**（tail-call elim・phi 回避・borrow/tail を駆動）は「呼び出しの直後に `Ret` があるか」でなく**構造的伝播**で決める（`Ret(Match)` の arm は tail・`Let(x,Match,k)` の arm は非 tail）——現 codegen の `tail` フラグと同一。tail の match は **phi を作らず各 arm 直接 return**（LLVM tail-call elim 維持）。
- **escape-reachable 関数は borrow 化を諦めず複製する（all-Own へのピン留めは却下）**（§2.1）: `Closure` ノード／funptr atom に現れて間接呼び出しされ得る関数は、間接呼び出し（callee 不明ゆえ固定 all-Own 規約）と直接呼び出しで呼び出し規約が衝突する（borrow 化＝callee が release しない、を間接呼び出し側が観測できずリーク）。so **all-Own original（間接用）＋ borrow 化 clone（直接用）**に分け、直接呼び出し地点を clone へ張り替える。all-Own にピン留め（borrow 化を全面的に諦める）は直接呼び出しの精度を捨てるので却下。これは **§4.1 [6]/[8] の clone-for-direct-keep-original-for-indirect の OwnershipShape 版**——「間接呼び出し側が観測できない変換（呼び出し規約の変更／uniqueness 特殊化）を escape-reachable な関数の共有 funptr に適用しない」という同じ原則。
