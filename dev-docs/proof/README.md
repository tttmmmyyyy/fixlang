...
| `proof_refs.py` | 枠の塊のタグと、証明がそれを引く `QUOTE {タグ}` の埋め込み (`--write` で生成、`--new` でタグを作る) |
...
**証明が枠を指すときは、タグで指す。番号でも節名でも数でも指さない。**

- **引きたい塊にタグが無ければ、タグを入れに行く** (`proof_refs.py --new` が未使用のタグを作る)。
  **タグを足すのは追記なので誰がやってもよい。枠の本文を変えるのは枠の持ち主だけである。**
- **タグは 7 桁の 16 進のランダムである。** 内容を表す名前を付けると、内容を直したときに名前を
  直したくなり、直せば引く側を全部直すことになる。**タグは識別子であって説明ではない。**
- **本文を引くときは `QUOTE {タグ}` と書き、中身は `--write` が埋める。**
  手で写した本文は枠が動いた瞬間に古くなる。**再生成が一致を保証する。**
- **引きたい塊が段落の途中に埋まっていたら、塊に割ってからタグを振る。**
