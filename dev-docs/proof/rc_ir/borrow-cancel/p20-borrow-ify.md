...
対象コミット `7a9826ed6d3e1a4ea2ba0d2f0d7a5e13a4c47a13` (ブランチ `proof-critic-round1` の先端)。定義・仮定・
命題の番号は同ディレクトリの `README.md` による。読んだコードは作業ツリーの版であり、`src/` の内容は
対象コミットと一致する。
...
ことを言う。(b) は `insert_rc` が「使うたびに 1 つ用意する」ことで作るものであり、`A19` (ii) の
`held_ρ(τ, C) ≥ 1` と同じ内容である。
...
<1>3. 一斉の名前替えは D11 を保つ。
  ...
  P9 の前半よりこの 5 つは `ρ_f` の下で保たれる (`ρ_f` は単射なので、A6 と A11 が要求する束縛と使用の
  対応も保たれる)。`func` のパラメータ・capture と `f_borrow` のそれは `ρ_f` で対応し、型は等しい。
  BY A6, A11, D3, D6, D9, D10, D11, P9
...
<1>5. CASE `τ` の節点が `Let(x, App(callee, args), k)` である。`B'_V` の対応する塊は L16 の (A-前) の列、
  ...
  <2>4. `B_V` の `App` は、`u` の下の inhabited なすべての leaf を消費する。
    第 10.2 節の言明より `B_V` を読む割り当てでは呼び出し先が全 unit を所有する。D9 の `App` の行と
...
<1>5. (i) と (ii) のオブジェクトは解放されていない。
  ...
  <2>2. `obj(x, λ)` がグローバル状態のとき、`<1>2` による。
    BY <1>2
  <2>3. `obj(x, λ)` が計数下で `T_ρ(x, λ)` が所有されるとき。H1 (b) より
        ...
        `<1>3` より解放されていない。
    L9 より `obj(T_ρ(x, λ)) = obj(x, λ)` である。H1 (b) が当たるのは、(i) と (ii) の位置が `B_V` でも
