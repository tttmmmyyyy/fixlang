...
**D29 (対応する活性化)**
`B` を 1 つの本体、`B'` を `borrow_ify` または `cancel` がそれを写した本体とする。`B'` の活性化 `α'` に
**対応する** `B` の活性化とは、D21 の 2 つのデータ -- パラメータ・capture の値と、オペランドから結果が
決まらない 3 種の構文の各位置での結果 -- を `α'` と共有する `B` の活性化である。構文どうしの対応は、
観測点の対応 (D19) を値を計算するすべての構文へ広げたものによる。`borrow_ify` と `cancel` は値を計算する
構文を作らず、消さず、並べ替えないので、この対応は全単射である。

D21 より、この 2 つを与えれば本体の活性化は 1 つに決まる。よって **`α'` に対応する `B` の活性化は
ちょうど 1 つ存在する。対応は前提ではなく、D21 と P22 から作られる。**

**`App` の結果を共有することが実際の実行で成り立つかは、この定義の主張ではない。**「2 つのプログラムの
対応する呼び出しが同じ値を返し、各オブジェクトの参照カウントを同じだけ変える」は P25 が活性化の木に
ついての帰納で示す。
...
- **P21** (削除は解放後の読みを作らない)。入力が D12 を満たすとき、`cancel` の出力の各活性化 `α'` と、
  それに対応する入力の活性化 `α` (D29) について、各読む構文の各位置で解放されているオブジェクトの集合は
  等しい。
...
- **P25** (活性化の木の対応)。`P` を D12 を満たすプログラム、`P'` を `borrow_ify(P)` または `cancel(P)` と
  する。`P'` の 1 つの実行を取り、その活性化の木の各節点 `α'` に、D29 が与える入力の対応する活性化 `α` を
  当てる。このとき **2 つの木は同型であり、対応する各 `App` の位置において、`α` と `α'` は同じ値を返し、
  各オブジェクトの参照カウントに同じ変化を与える。**

  D29 は `App` の結果を本体の外から与えられる量として扱う。P25 はそれが 2 つの実行で一致することを、
  活性化の木についての帰納で示す。帰納の段が P21 と P23 (`borrow_ify` の側は P14) であり、帰納の仮定が
  呼び出し先の活性化についての同じ主張である。無限の木では、各有限の前置について示す -- (S-a) と (S-c) は
  前置について閉じており、(S-b) は活性化が完了する場合にだけ課される。
