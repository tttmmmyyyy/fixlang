...
落とす場合に `call_rc` が補うこと) の 3 人。**(i) については誰も。** 検査: 無し。
...
それを写した各本体 (すなわち `cancel` の入力) の両方**について、各実行路 `ρ`、`ρ` を辿る各活性化に
ついて、次の 2 つを仮定する。
...
- **(i)** 各時点と各計数下オブジェクト `O` について、`H(O)` は、**その時点で生きているすべての活性化**の
  ...
  **この節を果たす者は居ない。** `p60-insert-rc.md` が `insert_rc` の出力について示したのは (ii-a) であり、
  `split_rc_units` の段について示したのは (i) の**保存**である。入力の側で (i) が成り立つことを述べる証明は
  無い。呼び出しの入れ子を跨ぐ主張なので、1 つの本体についての議論では届かない。
...
| (P-insert) | `p60-insert-rc.md` | 有 (A19 の (ii-a) と (ii-b)) | 証明済み。(O1) は無条件、(O2) は前提 (N) の下、`split_rc_units` の段は第 13 節。**(i) は誰も果たさない** | 未着手 |
| T | `p70-main-theorem.md` | -- | 証明済み。(T1) から (T4) まで、引用する命題の言明の上で閉じる | 未着手 |

**T が閉じることは、T が引く命題が閉じることを意味しない。**各命題の状態はこの表が述べる。誰も果たさない
仮定は A3、A4、A12、A18、および **A19 の (i)** の 5 つである。A19 の (ii-a) と (ii-b) は
`insert_rc`・`split_rc_units`・`borrow_ify` の 3 人が果たす。
