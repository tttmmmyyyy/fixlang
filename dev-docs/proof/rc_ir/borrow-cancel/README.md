...
証明しないものを名前で挙げておく。**評価順の保存**と、**FFI の副作用の列の保存**である。これらは D11 の
軸の上に無い。**返り値の一致**は `cancel` については P25 が与え、`borrow_ify` については与えない。
...
**`App` の結果を共有することが実際の実行で成り立つかは、この定義の主張ではない。**「2 つのプログラムの
対応する呼び出しが同じ値を返し、各計数下オブジェクトの参照カウントを同じだけ変える」は P25 が活性化の林
についての帰納で示す。
...
- **P21** (削除がカウントと解放に与えるもの)。入力が D12 を満たすとき、`cancel` の出力の各活性化 `α'` と、
  ...
  **2 つの活性化が同じアームを選ぶことは、前提ではなく D29 から出る。** 倒れうる経路は 1 つあった --
  ...
  持ち、同じアームを選ぶ。同じ理由で `App` の結果も共有され、その共有が実際の実行で成り立つことは P25 が
  与える。
...
- **P25** (活性化の林の対応)。`P` を D12 を満たすプログラム、`P'` を `cancel(P)` とする。`P'` の 1 つの
  実行を取り、その活性化の林 (D24) の各節点 `α'` に、D29 が与える入力の対応する活性化 `α` を当てる。
  このとき **2 つの林は同型であり、対応する各 `App` の位置において、`α` と `α'` は同じ値を返し
  (D29 の全単射のもとで)、各計数下オブジェクトの参照カウントに同じ変化を与える。**

  D29 は `App` の結果を本体の外から与えられる量として扱う。P25 はそれが 2 つの実行で一致することを、
  活性化の林についての帰納で示す。帰納の段が P21 と P23 であり、帰納の仮定が呼び出し先の活性化に
  ついての同じ主張である。無限の林では各有限の前置について示す -- (S-a) と (S-c) は前置について閉じて
  おり、(S-b) は活性化が完了する場合にだけ課される。

  **`borrow_ify` にはこの命題を置かない。**借用版が呼び出し元の参照を処分しないので、参照カウントの
  変化は設計上ずれる (D29)。
...
| P21, P23 | `p40-cancel-soundness.md` | 有 | **書き直し待ち** -- 言明が D29 の上に載る形に変わり、`L44` の `H3` は P25 になった | 検証済み (指摘 18 件を反映) |
| P25 | `p55-activation-tree.md` | 有 | **走行中** | 未着手 |
...
データで決まり、D29 の「対応する活性化」は前提でなく**構成**になった。`App` の結果が実際に両側で一致する
ことだけが残り、それが **P25** (活性化の木についての帰納) である。
