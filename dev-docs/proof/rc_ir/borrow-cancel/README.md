...
挟まらない。**この節が無いと、`H = 0` でありながらまだ解放されていない点ができ、「解放されていない」から
「`H ≥ 1`」を出す議論が段内の点の粒度で通らなくなる** -- (S-c) からその向きを出す段が
`p50-observation.md` の `L7` と `L7a` に在る。記憶域への書き込みについて上に置いたのと同じ形の節で
ある。
