...
- **P28** (参照の持ち手はちょうど 1 つ、活性化は許容される)。**D12 の意味で RC 規律を満たすプログラム**の
  実行 (D24) の各時点について、次の 2 つが成り立つ。
...
| P28 | `p05-holders.md` | 有 | (a) は証明済み (D12 の下で)。段を 6 種の素動作へ分解し、各素動作が持ち手ごとの勘定を保つことを示す。**(b) は 1 つの帰納へ組み直す作業が残る** | 未着手 |
...
P28 (b) が示す。**(ii-b) は前提 (N) -- 名前は別名類を決める -- の下でしか閉じていない。**
