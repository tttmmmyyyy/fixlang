...
(i-d) 実行の最初の時点に環境が持ち込むオブジェクトから到達できる (D25) オブジェクトのクロージャの欄が
名指す関数は、このプログラムの関数である。環境が持ち込む boxed な値は、このプログラムが作ってその番地を
環境へ渡したものだからである (`boxed_to_retained_ptr` と `boxed_from_retained_ptr` の対)。この実行のどの
段もその欄を書かないので、この節が無いと `p50-observation.md` の `L9b` と `L10` が閉じない。
