...
したがって**第 1 段が (E1) であるとき、その初期 `Obl` は空である**。C のエントリ点はこれを満たす --
`main` は (E5) の後グローバル状態であり、`IOState` は boxed leaf を持たない。`FFI_EXPORT` のエントリ点が
boxed な引数を取るとき、その参照は環境が持ってきたものであり、A17 (i) がそれを渡すことを言う。
...
Fix の側へ返すまで処分しない。`InlineLLVMBoxedFromRetainedPtrIOS` が retain を出さずに参照を得るのは
