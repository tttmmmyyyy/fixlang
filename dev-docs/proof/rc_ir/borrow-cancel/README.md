...
`Retain` と `O2` の `Release` を対にして両方を削除し、`O1` の二重解放と `O2` の漏れになる (第 7 節の (N))。
...
**(N) の否定は「1 つの名前が 2 つの別名類にまたがる」ことである。** 1 つのオブジェクトが道ごとに 2 つ
以上の名前を持つことではない -- そちらは (N) と両立し、そのとき `un_bump` は `shares_an_object` が偽に
なって `NoBracket` を返し、対を見つけずに諦める。すなわち保守的な側に倒れる。

**(N) は `p13-disposals-and-pending.md` の `L14` の後半が与える** -- 「`obj(x, λ)` が計数下であるとき、
`o = origin(x, λ).identity()` は `ρ` の上のスロットであって `C_ρ(o) = C_ρ(x, λ)` である」。`p60` はその
補題を輸入しながら前半 (`obj(C)` が定まること、`L12a` の内容) しか読んでいない。**(N) を前提として
残す理由は無い。**

**(N) が偽になりうるのは A3 の 1 節を落としたときだけである。** 「複数の元を宣言する op は存在しない」を
落とすと 1 つの名前が 2 つの類にまたがりうる。そのとき `un_bump` は `VarPath` の鍵で対を選ぶので、
`O1` の `Retain` と `O2` の `Release` を対にして両方を削除する -- `O1` の二重解放と `O2` の漏れである。
**A19 (ii-b) の下でこの節が load-bearing なのはこの経路による。**
