...
オブジェクトが同じであり、**その段の素動作の列が対応する** (下記) とき、対応は次の段へ伸びる。
...
読める点はそこに限る。**節点の実行の途中で非負性を要る段は、`p60-insert-rc.md` の `L17` (c) を読む** --
その段は、遷移の中の各節点 `n` とスロット `s` について、`n` が `μ(s)` を下げる回数を `k_n(s)` とすると
`n` の入口で `μ(s) ≥ k_n(s)` であることを与える。**入れ子の呼び出しで中断中の時点も、その活性化の
...
- **P3** (`origin` の健全性 -- `Exactly`)。**解析がその鍵で `origin` を呼び**、
...
- **P4** (`origin` の健全性 -- `Join`)。**解析がその鍵で `origin` を呼び**、
...
分からないからである。下の欄は直近の検証が返した判定の数を記録する -- **完了の条件は、`FALSE` と
`INCOMPLETE` が連続する 2 周でゼロになることである。**

| 命題 | ファイル | T への寄与 | 証明 | 検証 |
|---|---|---|---|---|
| P1, P2 | `p10-leaves-and-units.md` | 有 | 証明済み (P1 は A10 を満たす型について) | 直近の検証の指摘 2 件、FALSE 1)。**修理が入ったので検証が要る** |
| P2a | `p30-cancel-walk.md` | 有 | 証明済み (A3 の決定性の節の下で)。第 2 節が局所補題 `L0` から結論する。命題は 1 つも引かない | 直近の検証の指摘 8 件、FALSE 0。**A3 の内部可変性の節が偽であることを差し戻し、README を直した**)。**修理が入ったので検証が要る** |
| P3, P4 | `p11-origin-soundness.md` | 有 | 証明済み (スロットが D8 の意味の参照を持つときについて) | 直近の検証の指摘 8 件、FALSE 0)。**修理が入ったので検証が要る** |
| P5 (a), (b) | `p12-identity-and-consumes.md` | 有 | 証明済み (A16 の下で) | 直近の検証の指摘 10 件、FALSE 0)。**修理が入ったので検証が要る** |
| P5 (c) | `p12-identity-and-consumes.md` | 有 | 証明済み | 直近の検証の指摘 10 件、FALSE 0)。**修理が入ったので検証が要る** |
| P6, P7 | `p12-identity-and-consumes.md` | 有 | 証明済み (P6 は A16 の下で) | 直近の検証の指摘 10 件、FALSE 0)。**修理が入ったので検証が要る** |
| P7e, P7d | `p15-ownership-uniformity.md` | 有 | 証明済み (P7e の言明は (a)/(b) に書き直した) | 直近の検証の指摘 10 件、FALSE 0)。**修理が入ったので検証が要る** |
| P7a | `p15-ownership-uniformity.md` | 有 | 証明済み (1 ⟹ 3 と 2 ⟹ 1) | 直近の検証の指摘 10 件、FALSE 0)。**修理が入ったので検証が要る** |
| P7c, P7f | `p13-disposals-and-pending.md` | 有 | 証明済み (P7c の言明は 2 度書き直した) | 直近の検証の指摘 13 件、FALSE 0)。**修理が入ったので検証が要る** |

| P8 | `p20-borrow-ify.md` | 有 | 証明済み (`App` の引数の位置を除く形に狭めた) | 直近の検証の指摘 15 件、FALSE 2)。**修理が入ったので検証が要る** |
| P9 - P13 | `p20-borrow-ify.md` | 有 | 証明済み | 直近の検証の指摘 15 件、FALSE 2)。**修理が入ったので検証が要る** |
| P14 | `p20-borrow-ify.md` | 有 | 証明済み (A13、A19 (ii-a)、A20、A21、A23、A24、A26 の下で) | 直近の検証の指摘 15 件、FALSE 2)。**修理が入ったので検証が要る** |
| P15 - P18 | `p30-cancel-walk.md` | 有 | 証明済み | 直近の検証の指摘 8 件、FALSE 0)。**修理が入ったので検証が要る** |
| P18b | `p13-disposals-and-pending.md` | 有 | 証明済み | 直近の検証の指摘 13 件、FALSE 0)。**修理が入ったので検証が要る** |
| P18c | `p40-cancel-soundness.md` | 有 | 証明済み (A19、P14a、`p13` の `L17` の下で) | 直近の検証の指摘 15 件、FALSE 0)。**修理が入ったので検証が要る** |
| P18a | `p13-disposals-and-pending.md` | 有 | 証明済み (A19 の下で)。A19 (ii-b) は、この証明が要る条件付きの形に書き直した | 直近の検証の指摘 13 件、FALSE 0)。**修理が入ったので検証が要る** |

| P19, P20, P22, P24 | `p40-cancel-soundness.md` | 有 | 証明済み | 直近の検証の指摘 15 件、FALSE 0)。**修理が入ったので検証が要る** |
| P14a | `p20-borrow-ify.md` | 有 | 証明済み (A13、A20、A21、A23、A24 の下で) | 直近の検証の指摘 15 件、FALSE 2)。**修理が入ったので検証が要る** |
| P14b | `p20-borrow-ify.md` | 有 | 証明済み。局所補題 `L18a` が同じ内容を持っていた | 直近の検証の指摘 15 件、FALSE 2)。**修理が入ったので検証が要る** |
| P21, P23 | `p40-cancel-soundness.md` | 有 | 証明済み (D29 と D21 の上で。局所の仮説は無い) | 直近の検証の指摘 15 件、FALSE 0)。**修理が入ったので検証が要る** |
| P26 (`cancel` の半分) | `p50-observation.md` | 有 | 証明済み (D29 の上で) | 直近の検証の指摘 21 件、FALSE 0)。**修理が入ったので検証が要る** |
| P26 (`borrow_ify` の半分) | `p50-observation.md` | 有 | 証明済み。第 1 文は**等号**で立つので (X1) の出口が起きない。第 2 文は `cancel` の側だけの主張になった -- `borrow_ify` は (X3) を開くため。**量化は観測の事象へ広げた** (段の中の観測を含む) | 直近の検証の指摘 21 件、FALSE 0)。**修理が入ったので検証が要る** |
| P27 | `p51-runs.md` | 有 | 証明済み。(R1)(R2)(R3) が立ち、`L0` の数え上げは A21 が片付けた | 直近の検証の指摘 20 件、FALSE 2)。**修理が入ったので検証が要る** |
| (P-insert) | `p60-insert-rc.md` | 有 (A19 の (ii-a) と (ii-b)) | 証明済み。(O1) と (O2) はどちらも無条件、`split_rc_units` の段は同名の節 | 直近の検証の指摘 13 件、FALSE 0)。**修理が入ったので検証が要る** |
| P28 | `p05-holders.md` | 有 | 証明済み (D12、P14b の内容、A20 の下で)。(a) と (b) を点の上の 1 つの帰納で示す | 直近の検証の指摘 13 件、FALSE 3)。**修理が入ったので検証が要る** |
| P29 | `p51-runs.md` | 有 | 証明済み。局所補題 `L0b` を仮説つきで立て、入力に当てる | 直近の検証の指摘 20 件、FALSE 2)。**修理が入ったので検証が要る** |
| P30 | `p51-runs.md` | 有 | 証明済み。局所補題 `L0` がその内容を持つ | 直近の検証の指摘 20 件、FALSE 2)。**修理が入ったので検証が要る** |
| T | `p70-main-theorem.md` | -- | 証明済み。(T1) から (T4) まで、引用する命題の言明の上で閉じる | 直近の検証の指摘 8 件、FALSE 0)。**修理が入ったので検証が要る** |
