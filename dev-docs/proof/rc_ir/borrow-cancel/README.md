...
**#552 (`cancel` の対の判定の非対称、未修正)。** P18a の証明が閉じない原因はコードにあった。`Release` の
...
**残る道は、P18a に前提を足してそれを果たすことである。** 前提は「`Match` の後で消費される値を返すアームは、
その中に `Retain` を持つ」で、果たす者の候補は `insert_rc` の `retain_if_live` である。`borrow_ify` の
`rewrite_rc` が借用版でその `Retain` を落としうるので、そこを通った後も成り立つかが問題として残っている。
