...
`held` は段内の点でも定まっていなければならない。**この定義も 1 か所にしかない** -- 証明ファイルが自分で
...
**環境は実行の途中で新しいオブジェクトを持ち込まない。** (i-c) の「その時点に生きている各計数下
...
- **P28** (参照の持ち手はちょうど 1 つ、活性化は許容される)。**D12 の意味で RC 規律を満たすプログラム**で
  あって、**借用する unit を持つ本体の活性化を作る段が (E3) に限られる**ものを取る。その実行 (D24) の
