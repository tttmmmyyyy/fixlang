...
- **(S-c) 解放後の読みが無い**: **その活性化がその時点まで解放について閉じている (D11a) とき**、D7 の
  ...
  **段の中の点で言うのが要る。** 解放は段ではなく段の一部であり ((F))、1 つの段が解放と読みの両方を
  行いうる。節点の入口だけで言うと、段の中で解放したオブジェクトを同じ段の中で読む本体をこの節が縛らない。
  **動作の直前の点に置くのは、`Release` 自身がその段の後の点で解放を起こすからである** -- カウントが 1 の
  オブジェクトを `Release` するとき、触れる動作の直前の点でそのオブジェクトは解放されておらず、解放するのは
  その `Release` である。禁じているのは、すでに解放されたオブジェクトのカウントに触れることである。
...
- **P27** (実行の合成)。プログラムのすべての本体が D11 を満たし、**借用する unit を持つ本体の活性化を
  ...
  **活性化を作る段についての前提と A20 が要るのは、この命題が P28 を引くからである。** P28 はその 2 つを
  ...
  P14b (`borrow_ify` の出力について) と P24 の `cancel` の節 (`borrowed_units` が `cancel` を通って
  変わらないこと) の組である。
