...
返す腕)、**スロットではない**。そこが指すのは funptr かグローバル状態のオブジェクト (A8、D26) であり、
...
(`p13-disposals-and-pending.md` の第 7.5 節が定める)。**別名類と `held` の定義がこの文書でなく
`p13-disposals-and-pending.md` に在るのは、どちらも走査の帳簿と噛み合う形でしか書けず、その帳簿を定めるのが
その文書だからである。** `bumps` を D27 に委ねているのと同じ形である。**`borrow_ify` の入力の各本体と、`borrow_ify` が
...
**A21 (関数の値を作る演算)** -- 果たす者: `builtin.rs` の op の集合。検査: `apply_lambda` の develop mode の
表明 (`CODE src/generator.rs: Generator::apply_lambda`)。
...
- **T** (パイプラインが保存するもの)。`split_rc_units` の出力が A1 と A2 を満たすとき、
  ...
  3. D30 が対応させる 2 つの実際の実行の共通接頭の上では、入力で一意だった観測点は出力でも一意である
     (P26 の第 1 文を `borrow_ify` と `cancel` に順に適用し、D19 の 2 つの対応を合成する)。

     **共通接頭が終わる出口の向きは、合成については言えない。** `cancel` の側は P26 の第 2 文が (X1) と
     (X2) のどちらも観測値が改善する向きだと言うが、`borrow_ify` の側は (X3) を開くので出口の向きを言う者が
     居ない (D30、P26)。よって合成の接頭が終わった後については何も主張しない。第 9 節の「測って外した設計」
     が、その半分を閉じる判定とその代償を記録している。
  4. D12 が見ない部分は P24 のとおりに保たれる。

  2 が依頼者の言葉での結論である -- 「この 2 つのパスが原因の segv と二重解放は起きない」。1 だけでは
  そこまで届かない。

  **3 が接頭の上に限られることの意味。** 接頭が終わるのは、2 つの実行が一意性の観測、カウントを読む op の
  分岐、または解放の段で分かれた点である。そこから先の 2 つの実行について、この定理は何も言わない。
...
仮定は A3、A4、A12、A18 の 4 つである。A19 は 3 節とも果たす者を持つ -- (ii-a) と (ii-b) は
