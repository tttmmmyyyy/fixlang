...
**D21 (実行と、それが辿る実行路)**
本体 `B` の 1 回の**実行**とは、`B` の根から始まる 1 つの計算である。実行は各節点で D3 と同じ規則で進むが、
...
D6 のスロット、D7 の参照カウント `H`、D10 の義務集合はいずれも実行についての量であり、実行路だけでは
決まらない。「実行路 `ρ` の上で `H(o) ≥ 2`」のような言い方は、`ρ` を辿るすべての実行についての主張として
読む。

実行はアームの外の情報を使ってアームを選ぶので、1 つの実行路を辿る実行が存在しないこともある。D11 が
