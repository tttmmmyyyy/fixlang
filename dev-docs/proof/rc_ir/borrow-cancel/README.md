...
その本体について成り立つことから出る性質であって、モデルの取り決めではない。** 実行 (D24) の各時点で
閉じていることは `p51-runs.md` の `L3` が示す。
...
**P22・P24・P30 が層 2 の頭に在るのは、依存の順がそうだからである。** どれも書き換えが何を変えるかに
ついての構造の言明で、RC 規律を読まない。これらを読むのは P7c・P14・P14b・P18a・P19 であり、番号が
...
- **P24** (D12 が見ない部分の保存)。`borrow_ify` と `cancel` は、D12 が見ない部分について次を満たす。
  ...
  - **`cancel` は `RcFunc` の `body` 以外の欄を 1 つも変えない。** とくに `borrowed_units` と `capture` は
    ...
    `borrow_ify` は `borrowed_units` を書くので、この節は `cancel` についてだけである。P14b の結論が
    `cancel` の出力へ渡るのはこの節による。
...
- **P30** (`borrow_ify` の出力についても、静的に決めた呼び出し先は実行時の呼び出し先である)。
  ...
  **P29 と別に置くのは、証明が P24・P9・P1 を読むからである。** P29 は `borrow_ify` の入力についての
  命題なのでそれらを読まず、層 0 に置ける。
...
- **T** (パイプラインが保存するもの)。**第 4 節の仮定がすべて成り立ち**、`split_rc_units` の出力が
  ...
  2. よって出力のどの実行でも、解放されたオブジェクトの読みは起きず、どのオブジェクトも高々 1 回しか
     ...
     (E3) に限る」は、`borrow_ify` の出力について P14b が、`cancel` を通ることについて P24 の
     `cancel` の節が与える。
...
| (P-insert) | `p60-insert-rc.md` | 有 (A19 の (ii-a) と (ii-b)) | 証明済み。(O1) は無条件、(O2) は前提 (N) の下、`split_rc_units` の段は第 13 節 | 検証済み (指摘 20 件を反映)。**3 周目が要る** |
...
なく D21 が活性化に課す制限であり (第 4 節)、実行が作る活性化がそれを満たすことを P28 (b) が示す。**(ii-b) は前提 (N) -- 名前は別名類を決める -- の下でしか閉じていない。**
