...
| P2a | `p30-cancel-walk.md` | 有 | 証明済み (A3 の決定性の節の下で)。第 2 節が局所補題 `L0` から結論する。命題は 1 つも引かない | 検証は 2 周 (直近の周の指摘 13 件、FALSE 0)。**3 周目が要る** |
...
| P7e, P7d | `p15-ownership-uniformity.md` | 有 | 証明済み (P7e の言明は (a)/(b) に書き直した) | 検証は 3 周 (直近の周の指摘 10 件、FALSE 0)。**4 周目が要る** |
| P7a | `p15-ownership-uniformity.md` | 有 | 証明済み (1 ⟹ 3 と 2 ⟹ 1) | 検証は 3 周 (直近の周の指摘 10 件、FALSE 0)。**4 周目が要る** |
| P7c, P7f | `p13-disposals-and-pending.md` | 有 | 証明済み (P7c の言明は 2 度書き直した) | 検証は 4 周 (直近の周の指摘 13 件、FALSE 0)。**5 周目が要る** |

| P8 | `p20-borrow-ify.md` | 有 | 証明済み (`App` の引数の位置を除く形に狭めた) | 検証は 4 周 (直近の周の指摘 15 件、FALSE 2)。**5 周目が要る** |
| P9 - P13 | `p20-borrow-ify.md` | 有 | 証明済み | 検証は 4 周 (直近の周の指摘 15 件、FALSE 2)。**5 周目が要る** |
| P14 | `p20-borrow-ify.md` | 有 | 証明済み (A13、A19 (ii-a)、A20、A21、A23、A24、A26 の下で) | 検証は 4 周 (直近の周の指摘 15 件、FALSE 2)。**5 周目が要る** |
| P15 - P18 | `p30-cancel-walk.md` | 有 | 証明済み | 検証は 2 周 (直近の周の指摘 13 件、FALSE 0)。**3 周目が要る** |
| P18b | `p13-disposals-and-pending.md` | 有 | 証明済み | 検証は 4 周 (直近の周の指摘 13 件、FALSE 0)。**5 周目が要る** |
| P18c | `p40-cancel-soundness.md` | 有 | 証明済み (A19、P14a、`p13` の `L17` の下で) | 検証は 4 周 (直近の周の指摘 15 件、FALSE 0)。**5 周目が要る** |
| P18a | `p13-disposals-and-pending.md` | 有 | 証明済み (A19 の下で)。A19 (ii-b) は、この証明が要る条件付きの形に書き直した | 検証は 4 周 (直近の周の指摘 13 件、FALSE 0)。**5 周目が要る** |

| P19, P20, P22, P24 | `p40-cancel-soundness.md` | 有 | 証明済み | 検証は 4 周 (直近の周の指摘 15 件、FALSE 0)。**5 周目が要る** |
| P14a | `p20-borrow-ify.md` | 有 | 証明済み (A13、A20、A21、A23、A24 の下で) | 検証は 4 周 (直近の周の指摘 15 件、FALSE 2)。**5 周目が要る** |
| P14b | `p20-borrow-ify.md` | 有 | 証明済み。局所補題 `L18a` が同じ内容を持っていた | 検証は 4 周 (直近の周の指摘 15 件、FALSE 2)。**5 周目が要る** |
| P21, P23 | `p40-cancel-soundness.md` | 有 | 証明済み (D29 と D21 の上で。局所の仮説は無い) | 検証は 4 周 (直近の周の指摘 15 件、FALSE 0)。**5 周目が要る** |
...
| P27 | `p51-runs.md` | 有 | 証明済み。(R1)(R2)(R3) が立ち、`L0` の数え上げは A21 が片付けた | 検証は 3 周 (直近の周の指摘 20 件、FALSE 2)。**4 周目が要る** |
| (P-insert) | `p60-insert-rc.md` | 有 (A19 の (ii-a) と (ii-b)) | 証明済み。(O1) と (O2) はどちらも無条件、`split_rc_units` の段は同名の節 | 検証は 2 周 (直近の周の指摘 13 件、FALSE 0)。**3 周目が要る** |
| P28 | `p05-holders.md` | 有 | 証明済み (D12、P14b の内容、A20 の下で)。(a) と (b) を点の上の 1 つの帰納で示す | 検証は 2 周 (直近の周の指摘 13 件、FALSE 3)。**3 周目が要る** |
| P29 | `p51-runs.md` | 有 | 証明済み。局所補題 `L0b` を仮説つきで立て、入力に当てる | 検証は 3 周 (直近の周の指摘 20 件、FALSE 2)。**4 周目が要る** |
| P30 | `p51-runs.md` | 有 | 証明済み。局所補題 `L0` がその内容を持つ | 検証は 3 周 (直近の周の指摘 20 件、FALSE 2)。**4 周目が要る** |
| T | `p70-main-theorem.md` | -- | 証明済み。(T1) から (T4) まで、引用する命題の言明の上で閉じる | 検証は 2 周 (直近の周の指摘 8 件、FALSE 0)。**3 周目が要る** |
