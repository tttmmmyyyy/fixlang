...
**(i) を果たす道は D12 を通る。** P28 は D12 の意味で RC 規律を満たすプログラムについての命題なので、
(i) を果たす議論はその前提を要る。**量化範囲の 2 つの本体はどちらもそれを満たす** -- `borrow_ify` の入力は
A1 が、`borrow_ify` の出力は P14 が与える。よって (i) の言明にはこの前提を書かない。
