...
- **P23** (`cancel` は RC 規律を保存する)。D12 の意味で RC 規律を満たすプログラムを入力とすると、`cancel` の
  出力も D12 の意味で RC 規律を満たす。前提は P21 と同じである -- 出力の各活性化に、同じ実行路を辿る入力の
  活性化が対応すること。**その対応が存在することは、この命題の前提であって結論ではない。**
...
| P14a | -- | 有 | **未着手** (P10, P11, A20 から出る見込み) | 未着手 |
| P21, P23 | `p40-cancel-soundness.md` | 有 | **証明されていない** -- P18a と P14a に載る。`L44` は本体を跨ぐ同時帰納が要る (第 9 節) | 検証済み (指摘 18 件を反映) |
...
### 残っている構造的な問題 -- 本体を跨ぐ同時帰納

`p40` の `L44` は、`App` の結果が両側の実行で等しいことを要求する。その結果を作るのは**呼び出し先の
活性化**であり、その本体は入力と出力で別である。すなわち `L44` は呼び出し先について P21/P23 の結論を
先に要求しており、1 つの本体についての帰納では閉じない。

**P21 と P23 を、活性化の木についての同時帰納に組み替える必要がある。** 鍵は「呼び出し先の終端で欠損が
0 である」ことである。これは言明の組み直しであり、この 2 つが証明されない残る理由の 1 つである。
