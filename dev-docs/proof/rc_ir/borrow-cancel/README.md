...
- **(i)** 各時点と各計数下オブジェクト `O` について、`H(O)` は `O` を指す別名類が持つ参照の総数以上である。
  すなわち相異なる類の参照は相異なる。
- **(ii)** 各時点と各別名類について、その類が持つ参照は、**走査がその類について `pending` に数えている
  bump より 1 つ多い**。
...
**果たす者の 2 人のうち、`borrow_ify` の側は示されている** (同ファイルの `L16`)。借用版が `Retain` を落と
すのは `owns_unit` が偽のときだけで、`App` の引数以外の消費では P8 と P7a がそれを真にし、`App` の引数では
`call_rc` が同じ `Retain` を呼び出しの直前に置き直す。**`insert_rc` の側は示されていない。** それは
`insert_rc` についての命題であり、この証明の対象の外にある。同ファイルの第 7.5.6 節がその言明を書き出す。
