...
- **P14a** (借用する終端の類は活性化の間 参照を持つ)。`borrow_ify` の出力の各本体、各実行路、各活性化に
  ついて、ρ-終端が借用する (D14) パラメータ・capture の leaf である別名類は、活性化の間ずっと参照を
  少なくとも 1 つ持つ。

  この類の参照を減らしうる事象は 2 つしかない。`Release` は、借用版では `rewrite_rc` が落とす (P10)。
  消費は、`call_rc` が呼び出しの直前に `Retain` を置く (P11)。そして呼び出し元がその参照を呼び出しの間
  処分しないことは A20 が言う。**仮定ではなく命題として置けるのは、この 3 つが揃っているからである。**
...
| P14a | `p20-borrow-ify.md` | 有 | **走行中** | 未着手 |
