...
- **P14a** (借用する終端の類は活性化の間 参照を持つ)。`borrow_ify` の出力の各本体、各実行路、各活性化に
  ...
  P21 と P23 の (S-a) が、この性質を要る。`p40-cancel-soundness.md` の `L41a` は、義務集合が非負である
...
**4 度目までは「読める点を限っていることを見落とす」形だったが、5 度目は「量化する類を限っている
ことを落とす」形である。** 限定の付いた項を書いたら、**限定が本文の別の文に在るか、太字の中に在るか**が
