...
**`split_rc_units` の段も閉じた** (`p60-insert-rc.md` の第 13 節)。(ii-a) は保存の形で通る。
...
割る前より大きい時点がありうる。そこで第 11 節を本体について読む 4 つの前提に括り出し、割った後の出力が
その 4 つを満たすことを示す形にした。
...
読む者は `p20-borrow-ify.md` の第 10.7 節である。
...
| P14b | `p20-borrow-ify.md` | 有 | 証明済み (第 12 節)。局所補題 `L18a` が同じ内容を持っていた | 検証は 4 周 (直近の周の指摘 15 件、FALSE 2)。**5 周目が要る** |
...
| (P-insert) | `p60-insert-rc.md` | 有 (A19 の (ii-a) と (ii-b)) | 証明済み。(O1) と (O2) はどちらも無条件、`split_rc_units` の段は第 13 節 | 検証は 2 周 (直近の周の指摘 13 件、FALSE 0)。**3 周目が要る** |
...
`p60-insert-rc.md` の第 14 節が持っていた活性化ごとの勘定は、そこへ移る。
