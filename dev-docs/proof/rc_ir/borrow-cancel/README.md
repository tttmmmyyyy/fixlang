...
証明の対象は、コミット `6af3eb3b4d9b38c9fe75890cce0b499ff6753498` の
...
すなわち 1 つの本体の活性化は、**パラメータ・capture の値と、この 3 種の構文の各位置での結果**を与えると
決まり、辿る実行路もそれで決まる。この 3 種を与えない限り活性化は決まらないので、これらは活性化の側の
...
活性化とは、D21 の 2 つのデータ -- パラメータ・capture の値と、オペランドから結果が決まらない 3 種の
構文の各位置での結果 -- を `α'` と共有する `B` の活性化である。構文どうしの対応は、観測点の対応 (D19) を
値を計算するすべての構文へ広げたものによる。`cancel` は値を計算する構文を作らず、消さず、並べ替えない
(P22) ので、この対応は全単射である。
...
- D10 の生成の表が割り当てを行う位置 -- `Closure(f, caps)` の capture object と、`result_prov` が単一の
  ...
  対応する。
- `App` の結果は、この定義が外から与えるデータである。

D21 より、この 2 つのデータを与えれば本体の活性化は 1 つに決まる。よって **`α'` に対応する `B` の活性化は
ちょうど 1 つ存在する。対応は前提ではなく、D21 と P22 から作られる。**

**`App` の結果を共有することが実際の実行で成り立つかは、この定義の主張ではないし、どの命題の主張でも
ない。** D11 と D12 は D21 の意味のすべての活性化について条件を課すので、`α` が実現するかどうかを問う
...
- **グローバルのアクセサ**。初期化済みの旗を見て、まだならグローバル初期化子の本体を持つ関数
  ...
  記憶域を読むだけである (`CODE src/rc_ir/codegen.rs: Generator::implement_rc_global`)。**アクセサは
...
**この修正には穴が 4 度あり、いずれも証明が見つけた。** 4 度とも症状は同じ「`-O max` でだけ
...
広げた分の代償はゼロだった (クロージャを多用する 10 ケースで命令数が一致)。回帰テストは 4 本あり、いずれも
対応する辺を外すと落ちる。

**この 4 件は「列挙を手で並べると落とす」という族である。** 同じ族が、キャッシュの鍵が手書きの列挙で
4 つ漏れていた件にある。落とした列挙が安全側に倒れるなら気付かないが、この門は落とすと**借用版を通して
しまう**向きなので、落とした分だけ穴が開く。3 件目までは「在りかを 1 か所に集める」で対処したが、4 件目は
在りかが `LLVMGen` の実装 78 個に散っているので集められない。**集められないときは、落としたことが検出
できる形にする**のがこの門で採った答えである。
...
**D21 に、活性化が運ぶ割り当てはオペランドから結果が決まらない 3 種の構文の結果を含む、と書いた。**
一意性の観測点、外部の状態を読む `Llvm` の演算、そして `App` である。これで 1 つの本体の活性化はその 2 つの
