...
**(i) も閉じた** (`p60-insert-rc.md` の第 14 節)。`L36` の前提は **D12** であり、A19 (i) の量化範囲は
それをちょうど満たす -- `borrow_ify` の入力の D12 は A1 が、出力の D12 は P14 が与える。どちらにも `L36` を
当てて (i) が出る。**(i) はプログラムが D12 を満たすことに立っている**。P28 が D12 を前提に取るからで、
D12 を落とすと P28 の 2 つの反例がそのまま (i) の反例になる。
...
| (P-insert) | `p60-insert-rc.md` | 有 (A19 の (ii-a) と (ii-b)) | 証明済み。(O1) は無条件、(O2) は前提 (N) の下、`split_rc_units` の段は第 13 節、(i) は第 14 節 (P28 と D12 の上で) | 検証済み (指摘 20 件を反映)。**2 周目が要る** |
