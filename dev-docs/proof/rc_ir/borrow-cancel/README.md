...
- **(i)** 各時点と各計数下オブジェクト `O` について、`H(O)` は `O` を指す別名類が持つ参照の総数以上である。
  すなわち相異なる類の参照は相異なる。
- **(ii)** 各時点と各**計数下の**別名類について、その類が持つ参照の個数は、**走査がその類について
  `pending` に数えている bump の個数以上**である。

  (ii) を「bump より 1 つ多い」と書くと、字義どおりには偽である -- その類の最後の参照が処分された後の
...
ついて示すこと。**(O2)** 台帳の不等式 `U + X ≥ D`。(O2) で開いているのは `Join` の束縛変数の場合だけで、
埋め合わせの機構は特定されている -- アーム内の `retain_if_live` が置く `Retain` の要素を `merge` が
`entered_with` のゲートで必ず落とすので `X` が増える -- が、常に足りることは示されていない。反例も
見つかっていない。
...
| P8 | `p20-borrow-ify.md` | 有 | 証明済み (`App` の引数の位置を除く形に狭めた) | 未着手 |
| P9 - P13 | `p20-borrow-ify.md` | 有 | 証明済み | 未着手 |
| P14 | `p20-borrow-ify.md` | 有 | **証明済み** (A19 と A20 の下で) | 未着手 |
| P15 - P18 | `p30-cancel-walk.md` | 有 | 証明済み | 検証済み (指摘 9 件を反映) |
| P18b, P18c | `p13-disposals-and-pending.md` | 有 | 証明済み (P18c は `p40` の `L42`) | 未着手 |
| P18a | `p13-disposals-and-pending.md` | 有 | 証明済み (A19 の下で。A19 の半分は未証明) | 未着手 |

| P19 - P24 | `p40-cancel-soundness.md` | 有 | 証明済み (P21/P23 は A19 と路の対応の下で) | 未着手 |
...
**#551 (一意性が倒れる、未修正)。** P26 の証明が閉じない原因はコードにあった。借用化は、本体のどこでも
