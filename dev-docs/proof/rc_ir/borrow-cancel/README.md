...
**実行 (D24) が作る活性化がこの制限を満たすことは `p60-insert-rc.md` の第 14 節が P28 と A20 の上で示す**
ので、制限は空虚でない。
...
示した性質は、実際の実行についても成り立つ。**その包含を与えるのは `p60-insert-rc.md` の第 14 節である。**
...
D21 の不等式 (A19 (i)) を満たすことは、`p05-holders.md` の P28 と `p60-insert-rc.md` の第 14 節が実行の
点の上の 1 つの帰納で示す。よって D11 と D12 が課す条件は、実行が作る各活性化に当たる。
...
4 種である -- `Binding::Param` (第 2 行と第 3 行)、`Binding::Producer` と `Binding::Llvm` の `Fresh`/`Unknown`
の位置 (どちらも D10 の生成の表が行を持つ位置なので第 1 行)、そして記号の位置 (その類は計数下でない、
D33)。
...
- **(i)** **これは仮定ではなく、D21 が活性化に課す制限である。** 活性化 `a` の各時点、各計数下
  ...
  **制限であって仮定でないのは、`H` が活性化の側のデータだからである** (D21)。仮定として D21 の全活性化へ
  ...
  満たすことは `p60-insert-rc.md` の第 14 節が P28 と A20 の上で示す。**
...
- **P28** (参照の持ち手はちょうど 1 つ)。**D12 の意味で RC 規律を満たすプログラム**の実行 (D24) の各時点に
  ...
  **この命題の証明は、実行の各点で D11 を引く。** D11 が課す条件は、その点までの接頭について読む --
  3 つの節はいずれもその位置までの情報で決まる。D11 を当てるには活性化が D21 の制限を満たすことが要り、
  その制限を示すのがこの命題を読む `p60-insert-rc.md` の第 14 節なので、2 つは 1 つの帰納として組む。
...
なく D21 が活性化に課す制限であり (第 4 節)、実行が作る活性化がそれを満たすことを `p60-insert-rc.md` の
第 14 節が P28 と A20 の上で示す。**(ii-b) は前提 (N) -- 名前は別名類を決める -- の下でしか閉じていない。**
...
**A19 (i) が制限であることの重み。** P28 は `p05-holders.md` が段を 6 種の素動作へ分解して示し、
`p60-insert-rc.md` の第 14 節が実行の作る活性化について制限を示す。**この 2 つは循環するので 1 つの
帰納として組む** -- 実行の点 `p` までの接頭で D11 が当たることから `p` の許容性が出て、それが `p+1` の
D11 を当てる。
