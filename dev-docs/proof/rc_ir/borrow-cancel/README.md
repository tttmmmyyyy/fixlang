...
**残るのは `split_rc_units` の段である。** A19 の範囲は `borrow_ify` の入力、すなわち `split_rc_units` の
出力であり、第 10 節と第 11 節が示すのは `insert_rc` の出力についてである。`Retain(v, [])` を unit ごとの
鎖へ割る段が A19 を保つことは示されていない。果たす者の一覧にも `split_rc_units` は挙がっていない。(O2) で開いているのは `Join` の束縛変数の場合だけで、埋め合わせの機構は特定されている --
...
| (P-insert) | `p60-insert-rc.md` | 有 (A19 の残る半分) | 証明済み。(O1) は無条件、(O2) は前提 (N) の下。残るのは `split_rc_units` の段 (第 4 節の A19) | 未着手 |
| T | `p70-main-theorem.md` | -- | 証明済み。(T1) から (T4) まで、引用する命題の言明の上で閉じる | 未着手 |

**T が閉じることは、T が引く命題が閉じることを意味しない。**各命題の状態はこの表が述べる。誰も果たさない
仮定は A3、A4、A12、A18 の 4 つであり、A19 は `split_rc_units` の段が残る。
