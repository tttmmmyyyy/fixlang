...
節点の位置にあるとき、`g` のアクセサが `g` の初期化子の `init` の活性化 `b` を作る。`Obl(b)` は空で始まる
(D1 より `init` はパラメータも capture も持たない)。`a` は `b` が終わるまで**中断中**であり、その間 `a` は
段を持たない。`b` が終わると、(E4) と同じく `b` の終端の `Ret` が消費する参照が `Obl(b)` を離れるが、
行き先は呼び出し元ではなく `E` である -- アクセサはその値を `g` の記憶域へ格納する。続けて (E5) の段が
走り、`a` が再開する。
...
- **P28** (参照の持ち手はちょうど 1 つ)。実行 (D24) の各時点において、処分されていない各参照は、D25 が挙げる
  3 種の持ち手のちょうど 1 つに属する。すなわち 2 つの持ち手が同じ参照を持つことはなく、どの持ち手にも
  属さない参照も無い。
