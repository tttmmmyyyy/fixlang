...
- **(i)** 各時点と各計数下オブジェクト `O` について、`H(O)` は、**その時点で生きているすべての活性化**の
  別名類のうち `O` を指し、かつ **ρ-終端が借用する (D14) パラメータ・capture の leaf ではない**ものが持つ
  参照の総数以上である。すなわち相異なる類の参照は相異なり、それは呼び出しの入れ子を跨いでも相異なる。
  「`O` を指す類」と言えるのは、1 つの別名類のすべてのスロットが同じオブジェクトを指すからである
  (`p13-disposals-and-pending.md` の `L12a` と `L14`)。

  **借用する終端の類を除かないと (i) は偽である。** `p13-disposals-and-pending.md` の
  `DEF 類ごとの参照` は、借用する終端の類に開始値 1 を与える -- 呼び出し元が持つ参照がそれである。
  反例は 2 行で書ける。`f_borrow` はパラメータ `b` を借用する借用版、呼び出し元 `g` は所有パラメータ `a` を
  持ち、本体が `Let(u, App(f_borrow, [a]), Release(a, [], s, Ret(u)))` であるとする。`call_rc` はこの向きに
  呼び出しの**後**へ `Release` を置くだけなので、呼び出しの間 `H(O) = 1` である。ところが `g` の類 `C_a` は
  所有パラメータとして 1 から始まり、`f_borrow` の類 `C_b` は借用パラメータとして 1 から始まる。総和は 2 で
  `H(O)` を超える。

  **`held` の読みを 1 つにして (i) と (ii) を同時に満たすことはできない。** (ii-a)・(ii-b) と P14a は、
  借用する終端の類が `held ≥ 1` を持つ読みを要求する -- 借用位置へ渡した参照を呼び出し先が読んでよいことが
  そこに載っているからである。(i) はその参照を呼び出し元の類が既に数えているので、二重に数える。除外は
  この食い違いを (i) の側で解く。除外した後の (i) は義務集合の水準の主張になり、`p60-insert-rc.md` の
  第 14 節が示すのはまさにその形である (`L34` が `Σ μ = Obl` を与える)。

  **この節が出るのは D25 の持ち手の分割からである** (`p60-insert-rc.md` の第 14 節)。呼び出しの入れ子を
  跨ぐ主張なので 1 つの本体についての議論では届かず、届かせるのは「処分されていない各参照はちょうど 1 つの
  持ち手を持つ」(P28) の方である。2 つの生きている活性化の義務集合はそれゆえ交わらない。残りは、活性化 1 つ
  について `p60-insert-rc.md` の第 10 節の帰属 `μ` が D10 の義務集合と同じ帳簿であること (同文書の `L34`)
  だけである。`H(O)` が和より大きくなりうるのは、環境が持つ参照と生きているオブジェクトの leaf が持つ参照が
  D25 の残る 2 種の持ち手だからである。
...
**`split_rc_units` の段も閉じた** (`p60-insert-rc.md` の第 13 節)。(i) と (ii-a) は保存の形で通り、
...
**(i) も閉じた** (`p60-insert-rc.md` の第 14 節)。`insert_rc` の出力について `L36` を当てて (i) が出て、
`split_rc_units` の出力へは `L30` が、`borrow_ify` の出力へは P14 が D12 を与えて運ぶ。**(i) はプログラムの
側の性質ではない** -- `L36` が読むのは D11 の (S-a) だけで、`insert_rc` が `Retain` をどこに置くかも liveness
も読まない。(S-a) を満たすどの本体でも成り立つ。
...
| (P-insert) | `p60-insert-rc.md` | 有 (A19 の (ii-a) と (ii-b)) | (O1) は無条件、(O2) は前提 (N) の下、`split_rc_units` の段は第 13 節。**(i) の第 14 節は修復が要る** -- 持ち手の一意性を D25 に帰しているが、それを述べるのは P28 であり、P28 は D12 を前提に取る | 検証済み (指摘 20 件)。**修復が要る** |
...
**(i) を示す `p60` の第 14 節は、この移動に追いついていない。** 持ち手の一意性を D25 に帰しているが、
D25 はそれを述べない。また第 14 節の補題の前提は D11 の (S-a) だけであり、P28 が要求する D12 に足りない。
書き換えは前提を D12 へ強めるだけで済む -- 読み手の量化範囲がそれを満たすからである。
