...
落とす場合に `call_rc` が補うこと) の 3 人。(i) については `insert_rc` と、その出力を写す
`split_rc_units` と `borrow_ify` (`p60-insert-rc.md` の第 14 節)。検査: 無し。
...
**(i) は形が変わったので、示し直しが要る。** `p60-insert-rc.md` の第 14 節が示したのは、活性化ごとの
`Σ_a Obl_a(O) ≤ H(O)` -- すなわち `d` の総和の側である。角括弧の 1 (その活性化が借りている参照の分) を
足す段が新しい。材料は同じ節に在る -- P14a がその類に参照が在ることを、P28 がその持ち手がこの活性化で
ないことを与える。前提の D12 も変わらない (入力は A1、出力は P14)。
...
- **T** (パイプラインが保存するもの)。`split_rc_units` の出力が A1 と A2 を満たすとき、
  ...
  3. D30 が対応させる 2 つの実際の実行の共通接頭の上では、入力で一意だった観測点は出力でも一意であり、
     共通接頭が終わるのは観測値が改善する向きだけである (P26 を `borrow_ify` と `cancel` に順に適用し、
     D19 の 2 つの対応を合成する)。
...
| (P-insert) | `p60-insert-rc.md` | 有 (A19 の (ii-a) と (ii-b)) | (O1) は無条件、(O2) は前提 (N) の下、`split_rc_units` の段は第 13 節。**(i) は形が変わったので示し直しが要る** -- 第 14 節が示したのは `d` の総和の側で、角括弧の段が要る | 検証済み (指摘 20 件を反映) |
