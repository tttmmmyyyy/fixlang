...
- **対応するオブジェクトが保持する値は対応する。** 全単射の定義域は、対応する 2 つの活性化がそれぞれ到達
  ...
  **スカラの成分については、対応は等号である。** boxed leaf でない成分 -- unbox union のタグ、整数、
  浮動小数、funptr の番地 -- は、対応する 2 つの値で等しい。D21 は `Match` のアームを `v` の値の実行時の
...
`X` の段と `X'` の段を、`π` が消した節点と入れた節点の段を除いて、**先頭から順に** 節点の対応 (D19 と
P22) で 1 対 1 に並べる。並んだ 2 つの段が同じ位置の節点を実行し、その段が作る値が等しく、その段が名指す
オブジェクトが同じであるとき、対応は次の段へ伸びる。**伸びる限りの部分を共通接頭と呼び、対応はそこまでで
定まる** -- 共通接頭の外の段は、どの段とも対にならない。
...
- **(i)** 各時点、各計数下オブジェクト `O`、その時点で生きている各活性化 `a` について、次が成り立つ。
  ...
  **`held` は中断中の時点でも読む。** D34 は各時点について `held_ρ(τ, C)` を定めるが、
...
1 つの持ち手を持つ** -- 誰も指さないまま解放されずに在るオブジェクトを、環境は持ち込まない。`FFI_EXPORT` のエントリ点が boxed な引数を取る実行で、
...
4. **宣言と生成コードの一致。** D9 の消費、D10 の生成、D13 の `origin` はすべて宣言から定義されている。
   ...
   列挙の中に在った。**
5. **`cancel` の鎖が載っている未証明の前提。** A19 (ii-b) を読むのは P18a・P18c・P19・P21 であり、
   それは前提 (N) の下でしか閉じていない (第 7 節)。
6. **プログラムが参照の閉路を作らないこと。** 「ちょうど 1 回解放される」はそこにぶら下がっており、
   支えは valgrind の下のテストだけである (A18 (a))。
7. **終わらない活性化。** (S-b) は終端の `Ret` の 1 点にしか掛からない。(E6) で終わる実行と発散する実行
...
**A19 の果たす者。** (ii-a) は `insert_rc`・`split_rc_units`・`borrow_ify` の 3 人が果たす。(i) は同じ
3 人が P28 の上で果たす。**(ii-b) は前提 (N) -- 名前は別名類を決める -- の下でしか閉じていない。**
`p60-insert-rc.md` の第 6.2 節がそれを証明しないと述べ、`split_rc_units` の段も (N) の下の主張である。
**(ii-b) を読むのは P18a、P18c、P19、P21 -- `cancel` の健全性の鎖の全体である。** そして (N) の否定は、
第 8 節が数え上げた #519・#529・#545 の形そのもの -- 1 つのオブジェクトが道ごとに 2 つ以上の名前を持つ
こと -- である。**この作業が見つけたバグ族の否定が、鎖の下に未証明の前提として敷かれている。**
