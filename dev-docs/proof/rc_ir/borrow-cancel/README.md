...
各命題の証明は、`.claude/skills/code-proof/SKILL.md` が定める構造化証明の記法で書く。
記法の規則はそちらが持つ。この節が述べるのは、この証明に固有の読み方だけである。
...
**以後の時点については、これは保存の命題ではなく制限である。** `H` の増減のうちこの活性化の外から来る分を
...
`p15-ownership-uniformity.md` の `R1` と `R2` は、これを「A1 から A26」と読んで枝番の A26a を落とした。
...
自分の局所命題として示すからであり、依存の順がそれを強いる (P2a は層 1 で、層 0 の `p10` は引けない)。
...
- **P31** (`insert_rc` の出力は A19 (ii) を満たす)。`insert_rc` の出力に `split_rc_units` を適用した <!--#470e86a-->
  プログラムの各本体は、A19 の (ii-a)・(ii-b)・(ii-c) を満たす。証明は `p60-insert-rc.md`。

  **この言明を「その別名類の残りの消費と `Release` の個数」で書いた形は偽である。** まだ実行されて
  いない `Retain` が作る参照を数え落とすので、`insert_rc` が実際に出力する 5 節点の本体が反例になる。
  **数え落としを直した形でも足りない** -- 直した形は `p13-disposals-and-pending.md` の `C1` が満たし、
  `C1` は A19 (ii) を破る。`p60-insert-rc.md` は、`insert_rc` が置く `Retain` が
  その変数を名指す構文の直前にしか現れないことで `C1` を弾いて渡る。
