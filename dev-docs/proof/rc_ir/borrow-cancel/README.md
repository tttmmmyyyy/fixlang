...
行うならば対応するオブジェクトを解放することをいう。**段の中で走る活性化にも `π` の違いは現れる** --
...
| P26 (`borrow_ify` の半分) | `p50-observation.md` | 有 | 証明済み。第 1 文は**等号**で立つので (X1) の出口が起きない。第 2 文は `cancel` の側だけの主張になった -- `borrow_ify` は (X3) を開くため。**量化は観測の事象へ広げた** (段の中の観測を含む) | 検証は 1 周 (指摘 37 件を反映)。**広げた後で 2 周目が要る** |
