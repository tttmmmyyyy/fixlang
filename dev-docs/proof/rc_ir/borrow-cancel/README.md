...
- **P6** (`acted_references` は静的な上位近似である)。**1 つの本体 (D23)** の 1 回の活性化について、`acted_references(v, π)` が返す `Map` は、`π` の下の
  ...
  **主語が関数でなく本体なのは、グローバル初期化子の `init` の活性化についてもこれを読む段が在るから
  である** -- `p60-insert-rc.md` の `L5a` がそれである。`p12-identity-and-consumes.md` は最初から
  1 つの本体 (関数の `body` かグローバル初期化子の `init`) を固定して示しており、`VarTable::body_only`
  で作った表 -- `param_tys` が空である表 -- の場合を場合分けに持つ。同じ理由で P5 も本体を主語にする。
