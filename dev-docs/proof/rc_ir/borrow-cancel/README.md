...
**この修正には最初の版に穴があり、それも証明が見つけた。** 到達を直接呼び出しのグラフだけで取っていたので、
局所変数が持つクロージャを経た間接呼び出しの先にある観測点に届かなかった。`relay = |n, x, w, cl| (... ; cl())`
の形で `-O max` だけが止まるプログラムを作って確かめた。直したのは、間接呼び出しを持つ関数にクロージャが
運びうる全関数への辺を張ることである (A7 が呼び出し先の解決を放棄している以上、これが健全な近似)。広げた
分の代償もゼロだった (クロージャを多用する 10 ケースで命令数が一致)。
