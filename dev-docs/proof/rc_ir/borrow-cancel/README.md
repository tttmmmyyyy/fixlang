...
- **P12** (振り分けの安全性)。`route` が借用版へ回すのは、末尾位置でない呼び出しか、所有する unit を持つ引数を
  ...
  (元の版そのものか、その `borrow_versions` の像) であり、`prog.funcs` に存在する。
...
- **P19** (削除される retain の性質)。`cancelled()` が返す集合に含まれる `Retain` `t` について、`t` を含む
  ...
  削除される `Release` 群が `t` の `outstanding` を空にする。さらに、`t` とともに削除される各 `Release` は、
  実行路の上で `t` より後ろにある。
...
| P8 - P14 | `p20-borrow-ify.md` | 有 | **閉じない** -- P14 がコードで偽 | 未着手 |
| P15 - P18 | `p30-cancel-walk.md` | 有 | 証明済み | 未着手 |
| P19 - P24 | `p40-cancel-soundness.md` | 有 | 未着手 | 未着手 |
