...
- **(i)** 各時点と各計数下オブジェクト `O` について、`H(O)` は、**その時点で生きているすべての活性化**の
  別名類のうち `O` を指すものが持つ参照の総数以上である。すなわち相異なる類の参照は相異なり、
  それは呼び出しの入れ子を跨いでも相異なる。「`O` を指す類」と言えるのは、1 つの別名類のすべてのスロットが
  同じオブジェクトを指すからである (`p13-disposals-and-pending.md` の `L12a` と `L14`)。
