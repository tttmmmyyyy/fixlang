...
2. `ρ` の上の位置 (D6) と `ρ'` の上の位置は、変数と leaf を同じくする対応で 1 対 1 に対応し、その対応は
   ...
   開始しているか (D34) が一致し、計数下であるか (D26) が一致し、`obj` が D29 の全単射で対応する。
   とくに各計数下オブジェクト `O` について `S(τ, O)` と `S'(τ, O)` はこの対応で移り合う。
3. 各計数下オブジェクト `O` について
...
<1>2. 2 が成り立つ。
  <2>1. 位置 (D6) が対応する。D6 より位置は対 `(x, λ)` であり、`x` は値を得た変数か束縛を持たない名前、
        `λ` は `ty(x)` の inhabited (D16) な boxed leaf である。<1>1 の木の変形は変数の型を変えないので
        `boxed_leaf_paths(ty(x))` は 2 つで同じであり (D4)、D16 の inhabited は値が通る unbox union の
  ...
  <2>3. 別名類が対応し、`obj` が対応する。D33 は 1 つの実行路の上の位置を ρ-終端が等しいという関係で
        分けた同値類を別名類と定め、`obj(C)` を類の各位置が指すオブジェクトとする。<2>1 と <2>2 より
        位置と ρ-終端が対応するので、類も対応し、`obj` は D29 の全単射で対応する。D29 の第 5 行より
        計数下かグローバル状態か (D26) の区別も対応する 2 つのオブジェクトで一致する。
...
成り立ち、かつ各計数下オブジェクト `O` について `H(τ, O) = H'(τ, O) + d(τ, O)` であるとする。このとき
`α` は `τ` で D21 の制限 -- A19 (i) の不等式 -- を満たす。
...
<1>1. `α'` は `τ` に対応する時点で A19 (i) の不等式を満たす。すなわち各計数下オブジェクト `O` に
  ...
  活性化であり (D29)、`τ` に対応する時点で生きている (D23)。DEF 類ごとの義務 より、A19 (i) の `S` は
  `S'(τ, O)`、`d(C)` は `obl_{ρ'}(τ, C')`、角括弧は `β(C') = 1` の類が在るかである。
...
<1>2. (a)、(b)、(c)、(d)、(e)、(f) を `ρ` の上の位置についての帰納法で示す。D3 より `ρ` は有限の列なので
  ...
  <2>2a. `α` は位置 `q` で D21 の制限を満たす。
    BY L43c, <2>2
    L43b の仮定 -- `q` までに 2 つの活性化が `Del` の節点を除いて同じ節点を実行し、その時点までに値を
    得ている各変数の値が対応すること -- は <2>2 の (a) が与え、`H(q, O) = H'(q, O) + d(q, O)` は
    <2>2 の (b) が与える。
  <2>2b. (d) が `q` で成り立つ。
  ...
  <2>3. CASE `q` の節点が `Del` の `Retain` 節点である。`α'` はこの節点を実行しない。`α` では D10 の
    ...
    <3>3. (f) がこの節点の実行の各時点で成り立つ。
      BY L43c, <3>1, <3>2, <2>2a
      <3>1 よりこの節点の実行の時点は `q` と `q'` の 2 つである。`q` については <2>2a が与える。`q'` に
      ついては、L43b の仮定を <3>2 の (a) が、`H(q', O) = H'(q', O) + d(q', O)` を <3>2 の (b) が与える。
    <3>4. (e) が成り立つ。
    ...
    <3>2. `q` の節点の実行の各時点で 2 つの活性化の時点が対応し、(f) が成り立ち、
      ...
      <4>3. この節点の実行の事象の個数についての帰納法で、各時点について、2 つの活性化の時点が対応し、
        ...
        <5>3. `τ` の次の時点で、2 つの活性化の時点が対応し、(f) が成り立ち、`H = 0` と `H' = 0` が
              同値である。
          BY <5>2, L43c, L41c, D29, <2>2, <3>1, <4>2, 本場合の仮定
          ...
          与えるので、L43c より (f) が成り立つ。<4>2 より `d(τ, O) = d(q, O)` である。`d(q, O) = 0` の
  ...
  <2>6. QED
    BY <2>1, <2>2, <2>2a, <2>2b, <2>3, <2>4, <2>5, L43, L32
