...
作用が活性な名前で一致する)、`L11` (活性な名前では一致し、非活性な名前では `B` は空)、`L17`
...
<1>1. PICK `C0 ∈ S(τ, O)` SUCH THAT `bumps_ρ(τ, C0) ≥ 1`。
  BY 本補題の仮定, DEF 類ごとの義務, D27, p13 の L11
  `b(τ, O)` は `S(τ, O)` を渡る `bumps_ρ` の和であり、`p13` の `L11` より各項は 0 以上なので、和が
  1 以上ならば 1 以上の項が在る。
...
<1>3. `q` が `ρ` の終端の `Ret` であるとき `d(q, O) = 0` であり、`b(τ, O) ≥ 0` である。
  BY L38, L43, D27, p13 の L11
  L38 の 2 より `q` で pending な `CT` の要素は無いので、L43 の等式より `d(q, O) = 0` である。
  `b(τ, O)` は `B_ρ` の個数の総和 (DEF 類ごとの義務、D27) であり、`p13` の `L11` より各個数は 0 以上で
  ある。
<1>4. `B_ρ(τ, e)[o] ≥ 1` であって `o` が活性かつ `obj_ρ(o) = O` である各名前 `o` は、`ρ` の上の
  ...
  開始している。
<1>5. QED
  BY <1>1, <1>2, <1>3, <1>4, L43, D27, DEF 類ごとの義務, p13 の L11
  ...
  現れる。残る項は `p13` の `L11` より 0 以上である。位置 `q` 自身については <1>1 が同じことを述べる。
...
<1>4. (a)、(b)、(c)、(e) を `ρ` の上の位置についての帰納法で示す。D3 より `ρ` は有限の列なので整礎で
  ...
  <2>5. CASE `q` の節点が `Del` に入らない。`ρ'` の対応する位置にも同じ節点がある (L30、L31)。
    <3>1. 2 つの実行はこの節点について D10 の同じ行を同じ値に対して適用し、この節点が束縛する変数に
      ...
      <4>4. QED
        BY <4>1, <4>1a, <4>2, <4>3, D10, D16, A4, DEF 欠損,
           CODE src/rc_ir/ast.rs: RcExpr, CODE src/rc_ir/ast.rs: RcRhs
        言明の前半は <4>2 と <4>1a であり、後半は <4>3 である。D10 の行が名指す leaf は節点の形と値で
        決まる (D16、A4) ので、2 つの実行で同じである。`q` は `Del` に入らないので DEF 欠損 の 2 つの
        数え上げは変わらない。
    <3>2. `q` の節点の実行の間の各時点 `τ` について `H'(τ, O) = H(τ, O) - d(q, O)` であり、
      ...
      <4>4. 第 2 群・第 3 群・第 4 群の各時点 `τ` について、`H'(τ, O) = H(τ, O) - d(q, O)` である。
        ...
        <5>4. 第 4 群の事象は 2 つの実行で対応し、`H` を同じだけ変える。
          BY <5>3, <3>1, D24, D21, D29, A5
          D24 の (F) より、解放されるのはカウントが 0 になった計数下オブジェクトであり、<5>3 より
          その集合は 2 つの実行で等しい。解放が処分するのは、そのオブジェクトが保持する値の inhabited な
          各 boxed leaf の参照であり (D24 の (F)、A5)、D29 の第 5 行より対応するオブジェクトが保持する
          値は対応するので、処分される参照は対応する。`Destructor` のオブジェクトの解放が作る retain は
