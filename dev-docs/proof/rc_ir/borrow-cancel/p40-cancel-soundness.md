...
<1>1. 段の境界について、`q` の実行が `Obl` と `H` に与える動きは D24 の (E2)・(E3)・(E4)・(E7) が挙げる
  ...
  並べる。**」と数え上げる。そのうち (E7) が `q` の中で走る場合は、その活性化の素動作が `q` の実行に
  属する (`DEF 節点の実行の素動作` の第 3 の箇条)。**(E9) は環境が段そのものとして参照を操作する段で
...
<1>5. 名前ごとの寄与をオブジェクトごとの寄与に直す。`Retain`/`Release` が実際に作る (処分する) 参照は、
  ...
  BY p13 の DEF 名前の活性, p13 の DEF 実行時の作用, P5, P6, DEF 節点の量
...
<1>4. `B_ρ(n, e)[o] ≥ 1` であって `o` が活性かつ `obj_ρ(o) = O` である各名前 `o` は、`ρ` の上の
  ...
  BY p13 の L9, P5, P6, p13 の DEF 名前の活性, D27, D33, L40a, L45, DEF 節点の量, DEF 類ごとの義務
...
`H`・`Obl`・`held`・`bumps` と、そこに至る `ρ` の接頭が決める走査の状態 -- である。** よってこの補題は
(f) を点ごとに
...
同じ形である** -- A19 は (ii-c) を「**節点の実行の途中の各点 (D24 の段内の点) と、その点で `held_ρ` が
定まる各計数下の別名類について、`held ≥ 0` である。**」と、点ごとの言明として置き、その点の `held` の
ほかに何も結ばない。この文書がその点で (ii-c) を読むのは `L41a` の `<1>2` である。
