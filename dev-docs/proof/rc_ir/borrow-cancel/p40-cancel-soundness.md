...
(活性な名前では一致し、非活性な名前では `B` は空)。これらは `p30 の L10`、`p13 の L11` のようにファイル名を
添えて引用する。
...
| P19 | 証明済み |
| P20 | 証明済み |
| P21 | **H3 の下でだけ証明した。** 呼び出し先の活性化の対応を前提に置いている (差し戻し 3)。前提の側に (C3) も足した (差し戻し 4) |
| P22 | 証明済み |
| P23 | **証明されていない。** (S-b) と (S-c) は P21 と同じ H3 の下で閉じるが、(S-a) は `L42` に載り、`L42` は H1 と H2 を要する。言明の側にも前提が足りない (差し戻し 5) |
| P24 | 証明済み |
| P18c | **証明されていない。** `L42` が H1 と H2 の下でだけ閉じる (差し戻し 1 と 2) |

**開いている 3 点はどれもこの文書の外に在る。** H1 は `p13` の補題に、H2 は `borrow_ify` の出力の性質に、
H3 は活性化の木を跨ぐ同時帰納に属する。加えて、P21 と P23 の言明が持つ前提が 2 つ足りない (差し戻し 4 と
5)。第 10 節が、それぞれを誰がどこで果たせるかを述べる。
...
1 回の活性化 `α'` が**対応する**とは、次の 3 つが成り立つことをいう。

- **(C1)** `α` と `α'` は同じ入力から始まる。すなわち、パラメータと capture に同じ値を受け取り、開始時の
  ヒープが同じである。
- **(C2)** `α` が辿る実行路 (D21) は `ρ` であり、`α'` が辿る実行路は `ρ'` である。
- **(C3)** オペランドから結果が決まらない演算 -- 一意性の観測点 (D18) と、外部の状態を読む `Llvm` の
  演算 -- について、`α` と `α'` は対応する位置で同じ結果を得る。

**(C2) と (C3) は前提であって結論ではない。** (C2) について README の P21 がそう述べる -- 一意性の観測
(D18) は参照カウントを読むので、削除がカウントを下げると観測値が変わり、その先で選ばれるアームが
変わりうる。(C3) が別に要るのは、観測値が `Match` を通らずに値へ届く道があるからである。第 10 節の
差し戻し 4 と 差し戻し 5 が、この 2 つを述べる。

**オブジェクトの同一視**。(C1) より開始時のヒープは同じであり、`L44` の (a) が、2 つの活性化が `Del` の
節点を除いて同じ順に同じ割り当てを行うことを示す。オブジェクト `O` について 2 つの実行の量を並べて書く
とき、開始時から在るオブジェクトはそれ自身と、活性化が作るオブジェクトは対応する割り当ての事象が作る
ものと同一視する。

### DEF 実行時の量

活性化の**時点**とは、D10 の行が定める参照の作成・処分の事象の間の区切りをいう。`B` の 1 回の活性化
`α` の各時点 `τ` とオブジェクト `O` について、`H(τ, O)` をその時点の `O` の参照カウント (D7)、
`Obl(τ, O)` をその時点の義務集合 (D10) が持つ `O` への参照の個数とする。`B'` の活性化 `α'` については
`H'`、`Obl'` と書く。
...
<1>4. (a)、(b)、(c) を `ρ` の上の位置についての帰納法で示す。D3 より `ρ` は有限の列なので整礎である。
  ...
  <2>5. CASE `q` の節点が `Del` に入らない。`ρ'` の対応する位置にも同じ節点がある (L30、L31)。
    <3>1. 2 つの実行はこの節点について D10 の同じ行を同じ値に対して適用し、この節点が束縛する変数に同じ
          値を置く。`d` は変わらない。
      <4>1. `q` は `Del` に入らないので、L30 と L31 より `ρ'` の対応する位置の節点は `q` と同じ式の
            変位・変数・path・`RcState` を持つ。<2>2 の (a) より、その時点までに値を得ている各変数の値は
            2 つの実行で等しい。
        BY L30, L31, <2>2
      <4>2. CASE `q` の式が `RcExpr::Let(x, RcRhs::App(callee, args), k)` である。H3 より、2 つの実行が
            作る呼び出し先の活性化は同じ値を返すので `x` に置かれる値は等しく、その活性化が完了した
            時点で 2 つの実行は各計数下オブジェクトの参照カウントを同じだけ変え、同じオブジェクトの
            集合を解放する。
        BY H3, <4>1
      <4>3. CASE `q` の式がそれ以外である。この節点が変数に置く値は、D2 の節点の表が定めるとおり、節点の
            形とそれが名指す変数の値だけで決まるか、オペランドから結果が決まらない演算の結果である。
            前者は <4>1 より 2 つの実行で等しく、後者は DEF 対応する活性化 の (C3) より等しい。
        BY D2, A3, A4, <4>1, DEF 対応する活性化
      <4>4. QED
        BY <4>1, <4>2, <4>3, D10, DEF 欠損, CODE src/rc_ir/ast.rs: RcExpr, CODE src/rc_ir/ast.rs: RcRhs
        `RcExpr` の変位のうち `Let(x, App(..), k)` を <4>2 が、残りを <4>3 が尽くす。D10 の行が名指す
        leaf は節点の形と値で決まる (D16、A4) ので、2 つの実行で同じである。割り当ては D10 の生成の行が
        起こす事象なので、2 つの実行はこの節点で同じ割り当てを行う。`q` は `Del` に入らないので
        DEF 欠損 の 2 つの数え上げは変わらない。
    <3>2. D10 の行を適用した後、カウントが 0 になるオブジェクトの集合は 2 つの実行で等しく、その各要素
      ...
      <4>4. QED
        ...
        される参照も等しく、<4>1 の帰納法が進む。A18 (a) より生きているオブジェクトのグラフは非巡回で
        あり、段数は有限である。
    <3>3. QED
...
<1>4a. (e) が成り立つ。
  BY <1>1, <1>4
  `d(q, O) = 0` のとき <1>4 の (b) より `H'(q, O) = H(q, O)` である。`d(q, O) ≥ 1` のとき <1>1 の (d) より
  `H(q, O) ≥ d(q, O) + 1 ≥ 2 > 0` であり、<1>4 の (b) より `H'(q, O) = H(q, O) - d(q, O) ≥ 1 > 0` である。
<1>5. QED
  BY <1>1, <1>4, <1>4a
...
**言明 (README)** --- 入力が D12 を満たすとき、削除の前後で、対応する 2 つの活性化の各読む構文の各位置に
おいて解放されているオブジェクトの集合は変わらない。

**証明する形**。`cancel` の入力プログラム `P` が D12 を満たすとする。`ρ` を `B` の実行路、`ρ'` を対応する
`B'` の実行路、`α` と `α'` を対応する活性化 (DEF 対応する活性化) とする。示すのは、`ρ` の上の**すべての**
位置について、解放されている (DEF 解放されている) オブジェクトの集合が `α` と `α'` で等しいことである。
読む構文の位置はその特別な場合であり、`Retain`/`Release` 節点の位置も含まれる -- D11 の (S-c) は
その 2 つが触れる先も条件に入れるので、後者が要る。H3 の下で示す。
...
<1>2. グローバル状態のオブジェクト (D26) は 2 つの実行のどちらでも解放されない。
  BY D26, A8
<1>3. QED
  BY <1>1, <1>2, DEF 解放されている, L30, L31, L44
  DEF 解放されている より、`α` の位置 `q` で `O` が解放されているとは、`H(q0, O) = 0` を満たす `q` 以前の
  位置 `q0` が在ることであり、`α'` については `H'` で同じである。<1>1 をその各 `q0` に当てると、この 2 つは
  同値である。計数下でないオブジェクトについては <1>2 が両方とも解放されないことを与える。L30 と L31 より
  `ρ'` の各位置の節点は `ρ` の対応する位置の節点と同じ式の変位・変数・path を持ち、L44 の (a) よりその
  位置の変数の値も等しいので、読む構文とその読む値は 2 つの実行で同じである。
...
**証明する形**。入力プログラムを `P`、出力を `P'` と書く。`P` は D12 を満たし、A19 を満たすとする。D11 は `B'` のすべての実行路についての述語なので、示すのは、`B'` の各実行路 `ρ'` と、それを辿る
各活性化 `α'` について (S-a)、(S-b)、(S-c) が成り立つことである。**この文書が示すのは、対応する `B` の
活性化 `α` が在る `α'` についてである** (DEF 対応する活性化)。`α'` ごとに `α` が在ることは、README の
P21 の注記が言うとおり前提であって結論ではない。第 10 節の 差し戻し 5 が、この前提が P23 の言明にも要る
ことを述べる。H1、H2、H3 の下で示す。
...
<1>3. `ρ'` に対応する `B` の実行路 `ρ` を取る。`α` を、`α'` に対応する `B` の活性化とする (証明する形の
      前提)。`ρ` について D11 の (S-a)、(S-b)、(S-c) が成り立つ。
  BY D12, L31, L32, DEF 路の対応, DEF 対応する活性化
  L32 の 5 より `Del` は `Retain`/`Release` 節点だけなので L31 が使え、`ρ'` に対応する `B` の実行路 `ρ` が
  ちょうど 1 つ定まる。`P` が D12 を満たすので、`B` のすべての実行路について D11 の 3 つが成り立つ。
...
<1>5. (S-a) が `ρ'` で成り立つ。
  ...
  <2>3. `Σ_{C : obj(C) = O} bumps_ρ(τ, C) ≥ d(q, O)` である。
    ...
    H1 の対応で名前ごとの和を類ごとの和に並べ替えると、`bumps_ρ(τ, ・)` の和はこれ以上である。
  <2>4. QED
    BY <2>2, <2>3, <1>4
...
## 10. README とほかのファイルへ差し戻す点

この文書が閉じられなかった 3 点と、言明に足りない前提 2 点を挙げる。番号は差し戻しの番号であり、
第 2 節の H1 - H3 と対応する。

### 差し戻し 1 (H1 -- `p13` の 7.5.4 の <1>2 を名前付きの補題へ)

`L42` は `N(q, O) = Σ_{C : obj(C) = O} bumps_ρ(q, C)` を使う。同じ言明を
`p13-disposals-and-pending.md` の第 7.5.4 節の <1>2 が述べて示しているが、それは別の命題 (P18a) の証明の
内部のステップであり、記法の規約は「兄弟の証明の内部は参照できない」と定める。**この文書はそれを引用せず、
H1 として置いた。**

求めるのは、`p13` の第 7.5 節に名前付きの補題 (たとえば `L17`) としてこの言明を置き、7.5.4 の <1>2 が
それを引用する形にすることである。そうすればこの文書は `p13 の L17` を引用でき、H1 は消える。

README の第 9 節が、この言明が立つ `obj(C)` -- 別名類の全スロットが同じオブジェクトを指すこと -- が
まだ定まっていないと記録している。H1 を補題に上げるときはそこも同時に閉じることになる。

### 差し戻し 2 (H2 -- 借りた終端の類は参照を持ち続ける)

`L42` (= P18c) の残る穴は 1 か所である。ρ-終端が借用する (D14) パラメータ・capture の leaf である類 `C`
について、`bumps_ρ(τ, C) = 0` の時点で `held_ρ(τ, C) = 0` になりうると、`obl_ρ(τ, C) = -1` となり、
`Obl(τ, O) ≥ Σ_C bumps_ρ(τ, C)` が破れる。第 3 節の「H2 が要る所」が、破れる形を具体的に書いた。

**A19 の弱い形のままでは、場合分けでは埋まらない。** `p13` の第 7.5.4 節の <1>3 がとる形 -- `bumps ≥ 1` の
類を 1 つ選び、他の類には `held ≥ bumps` だけを使う -- で得られる余りは 1 つだけであり、`Obl` の側では
`β(C) = 1` の類の**個数**だけ引かれるからである。P18a (`H` の側) にこの問題が無いのは、`H` の側では
`β` を引かないためである。

**H2 は仮定でなく命題として置けると見込む。** 借用する終端を持つ類の `held_ρ` を減らす事象は 2 つしか
なく、どちらも `borrow_ify` が扱っている。その類のスロットを名指す `Release` は借用版では `rewrite_rc` が
丸ごと落とし (P10、P7a)、その類のスロットの消費には `call_rc` が直前に `Retain` を置く (P11、`p13` の
`L16`)。すなわち H2 は「`borrow_ify` の出力の各版において、借用する終端を持つ類は活性化の間ずっと参照を
1 つ持つ」という層 2 の命題であり、A20 (呼び出し元は呼び出しの間 処分しない) の本体側の対である。
置き場所は 2 つある -- 層 2 の命題として `p20-borrow-ify.md` に置くか、A19 に (ii-c) として足すか。
**どちらにするかは判断であって、この文書は決めない。**

### 差し戻し 3 (H3 -- P21 と P23 は活性化の木を跨ぐ同時帰納を要る)

`L44` の (a) は「2 つの実行は同じ節点を同じ値に対して実行する」と言う。`Let(x, App(callee, args), k)` の
位置で `x` に置かれる値を作るのは**呼び出し先の活性化**であり、その本体は `P` と `P'` で異なる (どちらも
`cancel` を通っている)。よって (a) は、呼び出し先について「同じ値を返し、同じだけカウントを変え、同じ
オブジェクトを解放する」ことを先に要求する。**この文書はそれを H3 として置いた。**

H3 を落とすには、P21 と P23 を 1 つの本体についての主張としてではなく、**活性化の木についての同時帰納**
として証明する形に組み替えることになる。すなわち、プログラム全体の 2 つの実行を歩数で帰納し、呼び出し
先の活性化については帰納法の仮定として (a)(b)(c)(e) を使う。呼び出し先の活性化が完了した時点で欠損が 0 で
あること (`L44` の <1>1 の <2>2 が、その活性化の終端の `Ret` について与えるもの) が、この帰納を回す鍵で
ある。

**これは組み替えであって、この文書の中では閉じない。** 層 5 の P27 (実行の合成) が活性化の木を扱う唯一の
命題だが、P27 は 1 つのプログラムの実行についての主張であり、2 つのプログラムの実行を突き合わせる形を
持たない。

### 差し戻し 4 (対応する活性化の (C3) -- 実行路が同じだけでは足りない)

README の P21 は「対応する活性化」を「同じ実行路を辿る 2 つの活性化」と定め、アームの選択が同じである
ことを前提に置く。**これだけでは `L44` の (a) が立たない。** `L44` の (a) は「各変数の値が 2 つの実行で
等しい」を言うが、一意性の観測点 (D18) が置く値はその 2 つの実行で異なりうる。観測値が `Match` の
scrutinee になる場合は (C2) が押さえるが、RC IR ではその `Bool` は `Llvm` の演算のオペランドにも、
`App` の引数にも、`Ret` の値にもなれる。オペランドが違えば結果の値も違いうるので、(a) は (C2) からは
出ない。同じことが、オペランドから結果が決まらない演算 (外部の状態を読む `Llvm` の演算) についても
言える。

この文書は DEF 対応する活性化 の (C3) として「オペランドから結果が決まらない演算について 2 つの活性化は
同じ結果を得る」を置いた。README の P21 の前提にこの条項を足すのが素直である。P26 は観測値が真から偽へ
倒れないことを述べるが、偽から真への向きは許すので、(C3) を与えない。

### 差し戻し 5 (P23 の言明にも P21 と同じ前提が要る)

P23 は「`cancel` の出力も D12 を満たす」と述べ、前提を持たない。D12 は `B'` のすべての実行路と、それを
辿るすべての活性化についての主張なので、`B'` の活性化 `α'` ごとに、対応する `B` の活性化 `α` が在ることが
要る。**`α'` から `α` を作れるとは限らない。** 同じ入力で `B` を動かした活性化が辿る実行路が、`ρ'` に
対応する `ρ` であるとは限らないからである (差し戻し 4 と同じ理由)。

P21 の言明はこの前提を「対応する 2 つの活性化の」という形で持っている。P23 にも同じ前提を書くのが素直で
ある。

### 気づいたコードの欠陥

新しいコードの欠陥は見つからなかった。差し戻し 2 が述べる `L42` の穴は、A19 の言明の強さの問題であって、
`cancel` の実装の欠陥ではない -- 破れる形を作るには、借用する参照を本体の中で処分する版が要り、
`borrow_ify` はそれを作らないと見込まれる (差し戻し 2)。
