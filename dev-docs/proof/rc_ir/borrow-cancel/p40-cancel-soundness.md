...
対象コミットは `a8dab6e04bdc82916b6a1296e4ab186027347ef3` であり、README が名指すコミットと同じである。
...
| P19 | 証明済み |
| P20 | 証明済み |
| P21 | **`G3` と `G4` の下で証明した** (第 11 節)。(a) が `G4` を、(b) が `G3` を読む |
| P22 | 証明済み |
| P23 | **`G3` の下で証明した。** (S-a) は `L42` に、(S-b) と (S-c) は `L44` に載る |
| P24 | 証明済み |

**`G3` と `G4` は、どちらも D29 の対応が何を結ぶかについての点である。** 第 2 節の
`DEF README へ差し戻す 2 点` が言明を、第 11 節が求める直しを述べる。ほかに開いている点は無い。
P18c、P19、P20、P22、P24 はどちらも読まない -- `G3` を読むのは `L44` の (a) と、それに載る P21 と P23 で
ある。

**対応する 2 つの活性化が開始時に等しい参照カウントを持つことは D29 の第 2 行が、子の活性化を作る段が
参照カウントに与える変化を活性化の側のデータとすることは D21 の第 4 行が与える。** `L44` はこの 2 つを
そこから読む。
...
全単射である。** `O` はその全単射が対応させるオブジェクトを渡る。`G3` (第 2 節) より、その定義域は 2 つの
...
**`d(q, O)` は `G4` (第 2 節) が言う量である。** L43 の <1>8 が、`d(q, O)` が「`q` までに実行された `Del` の
`Retain` が `O` に作った参照のうち、その `Retain` と対になる `Del` の `Release` がまだ処分していないものの
個数」であることを示す。README の P21 (a) が `k(O)` を書く形との違いは第 11 節が述べる。
...
- **G3 (全単射は保持する値まで届く)**。D29 の全単射の定義域は、対応する 2 つの活性化がそれぞれ到達
  できる (D25) オブジェクトの全体である。対応する 2 つのオブジェクトは、`ρ` と `ρ'` の対応する各位置に
  おいて、inhabited (D16) な各 boxed leaf が全単射で対応するオブジェクトを指し、計数下かグローバル状態か
  (D26) の区別も一致する。読む所: `DEF 実行時の量`、`L44` の <1>4 の <2>5 の <3>1 の <4>2 の <5>1、
  第 7 節の <1>2 と <1>4、第 9 節の <1>7 の <2>1。
- **G4 (`k(O)` は作った参照から処分された分を引いた個数である)**。README の P21 (a) の `k(O)` は、その
  位置までに `α` が実行した削除済みの `Retain` が `O` に作った参照のうち、その `Retain` と対になる削除
  済みの `Release` がまだ処分していないものの個数である。読む所: `L43` の <1>8、第 7 節の <1>1。
...
<1>2. 1 が成り立つ。
  ...
  <2>2. `leaf_origins_at(σ)` は `Some` を返す。`decl` は `Provenance::build_shape` が作る `LeafMap` で
        あり、その鍵は `boxed_leaf_paths(result_ty)` の各元である。`leaf_origins_at` はその写像を鍵 `σ`
        で引くだけであり、`σ` は `boxed_leaf_paths(ty(u)) = boxed_leaf_paths(result_ty)` の元である。
    BY <2>1, CODE src/rc_ir/provenance.rs: Provenance::build_shape,
...
さらに `d(q, O)` は `G4` (第 2 節) の量である。すなわち、`q` より前に実行された `Del` の `Retain` 節点が
`O` に作った参照のうち、その `Retain` と対になる `Del` の `Release` 節点がまだ処分していないものの個数で
ある。
...
<1>8. QED
  ...
  等式は <1>6、境界は <1>7、`q_end` の値は <1>7a である。`G4` の量が `d(q, O)` に等しいことは次から出る。
...
<1>4. (a)、(b)、(c)、(e) を `ρ` の上の位置についての帰納法で示す。D3 より `ρ` は有限の列なので整礎で
  ...
  <2>3. CASE `q` の節点が `Del` の `Retain` 節点である。`α'` はこの節点を実行しない。`α` では D10 の
    ...
    <3>2. QED
      ...
      しない。(e) が見る時点は <3>1 より `q` と `q'` の 2 つである。どちらでも (b) が等式を与え、
      `d = 0` のときは両辺が等しく、`d ≥ 1` のときは <1>1 の (d) より `H ≥ d + 1 ≥ 2 > 0` かつ
      `H' ≥ 1 > 0` なので、0 になることは同値である。
  <2>4. CASE `q` の節点が `Del` の `Release` 節点である。`α'` はこの節点を実行しない。`α` では D10 の
    ...
    <3>2. QED
      ...
      しない。<3>1 より解放の走査は走らず、`Release` 節点は子の活性化も作らない -- D24 で子を作る段は (E3) の
    ...
    <3>1. 2 つの実行はこの節点について D10 の同じ行を同じ値に対して適用し、この節点が束縛する変数に
      ...
      <4>2. この節点が束縛する値は 2 つの実行で対応する。
        <5>1. CASE その値が、節点の形と、それが名指す変数の値と、その値から到達できる (D25) オブジェクト
              ...
              <4>1 より名指す変数の値は 2 つの実行で対応し、`G3` (第 2 節) より対応するオブジェクトが
          ...
          BY D2, D9, D10, A3, A4, A5, D25, G3, <4>1
      ...
      <4>3. この節点が子の活性化を作るとき、その子の事象が参照カウントに与える変化は 2 つの実行で同じ
        ...
        段、(E7) のグローバルの初期化である。D21 の第 4 行は「**子の活性化を作る段** (D24 の「活性化の
        林」)。`App` の段 (E3)、オペランドを適用する `Llvm` の段 (E2)、そしてグローバルの初期化の段
        (E7) である。返る値と、参照カウントに与える変化は、子の本体が決める。1 つの本体だけを見ている間、
        これは外から与えられる量である」と述べ、この 3 つを 1 つの行で扱う。D29 の第 4 行はそのデータを
        対応する 2 つの活性化に同じものとして与える。
      <4>4. QED
...
活性化とする。**`k(O)` は `G4` (第 2 節) の読みで取る。** README の文面が 2 通りに読めることと、その
どちらの読みでも (a) が偽になることは、第 11 節が反例つきで述べる。

**証明**

<1>1. (a) が成り立つ。
  BY L44, L43, G4
  L43 の最後の文より `G4` の `k(O)` は DEF 欠損 の `d(q, O)` であり、L44 の (b) がその等式である。
<1>2. 計数下のオブジェクトについて、2 つの活性化は同じ位置で同じオブジェクトを解放する。ここで `O` は
      D29 の全単射が対応させるオブジェクトを渡り、`G3` (第 2 節) よりその定義域は 2 つの活性化がそれぞれ
  ...
  <2>1. `ρ` の上の位置 `q` の節点の実行の間に `α` が `O` を解放するのは、その実行の間のある時点 `τ` で
    ...
    BY D7, DEF 解放されている, DEF 実行時の量, G3, D25
  <2>2. QED
    BY <2>1, L44
...
<1>3. グローバル状態のオブジェクト (D26) は 2 つの活性化のどちらでも解放されない。
  BY D26, A8
<1>4. (b) が成り立つ。
  BY <1>2, <1>3, DEF 解放されている, L44, L30, L31, G3, D26
  ...
  `G3` より計数下かグローバル状態か (D26) の区別は対応する 2 つのオブジェクトの間で一致するので、この
  場合分けは 2 つの活性化で同じに分かれる。
...
`L42` に要る -- `L42` の <1>2 の <2>4 が P14a を、<2>3 が A19 (ii-b) を読み、どちらも範囲が
...
<1>5. (S-a) が `ρ'` で成り立つ。
  ...
  <2>3. `Σ_{C : obj(C) = O} bumps_ρ(τ, C) ≥ d(q, O)` である。
    <3>1. `q` で pending である `CT` の要素は `τ` の時点でも走査の `pending` に在り、その `B_ρ` は
          `q` の入口での値と等しい。
      <4>1. CASE `q` が `ρ` の終端の `Ret` である。L38 の 2 より、各 `t ∈ CT` が pending である区間は
            終端の `Ret` より真に前で終わるので、`q` で pending である `CT` の要素は無く、言明は空虚に
            成り立つ。
        BY L38
      <4>2. CASE `q` が `ρ` の終端の `Ret` でない。D9 の消費の表と `Release` 節点のどちらについても、
            `q` の式は `Retain`、`Release`、`Destructure`、`Eval`、または右辺が `Match` でない `Let` で
            あるから、L34 の 1 より `q` の訪問が `pending` に施す操作は `push`、`consume_objects`、
            `un_bump` の 3 つである。
        <5>1. `push` は末尾に新しい要素を足すだけで、既に在る要素の `node` と `outstanding` を変えない。
          BY CODE src/rc_ir/borrow.rs: CancelAnalysis::walk_inner の `RcExpr::Retain(v, path, _, k)` の腕
        <5>2. `consume_objects` はそれらの要素を取り除かず、その `B_ρ` も変えない。
          BY L38, L36, D27
        ...
        <5>3. `un_bump` はそれらの要素の `outstanding` を引かず、取り除きもしない。
          BY p30 の L5, L32, D27, <2>1,
             CODE src/rc_ir/borrow.rs: CancelAnalysis::walk_inner
          `un_bump` を呼ぶのは `walk_inner` の `RcExpr::Release(v, path, _, k)` の腕だけなので、
          `un_bump` が走るとき `q` は `Release` 節点である。
          `p30` の `L5` より、`un_bump` が `pending` を変えるのは `InBracket` を返すときだけであり、
          そのとき変わるのは選ばれた 1 つの要素の `outstanding` (`subtract` の分) と、それが空になった
          ときのその要素の除去だけである。返る `NodeId` はその要素の `node` である。それが `CT` の要素
          `t` であるとすると、L32 の 4 より `q` は `un_bump_releases[t]` に入り、`t ∈ CT` なので L32 の
          3 より `q ∈ Del` である。`Del` の `Release` は <2>1 が除いてあるので `q` は `Del` に入らず、
          これは矛盾である。よって選ばれる要素は `CT` の要素の由来を持たない。
        <5>4. QED
          BY <5>1, <5>2, <5>3, D27
          D27 は、`un_bump` が `InBracket` でその要素を選ぶ `Release` の訪問でだけ `B_ρ` を引き、ほかの
          どの操作も `B_ρ` を変えないと定める。<5>1 から <5>3 より、`q` の訪問はそれらの要素を取り除かず
          `B_ρ` も変えない。
      <4>3. QED
        BY <4>1, <4>2
        (S-a) が見る操作の位置は、D9 の消費の表の 6 行と `Release` 節点である。終端の `Ret` を <4>1 が、
        残りを <4>2 が扱う。
...
<1>7. (S-c) が `ρ'` で成り立つ。
  <2>1. `ρ'` の読む構文 (D7) と `Retain`/`Release` 節点は、`ρ` のそれらから `Del` の節点を除いたもので
        ...
        `G3` (第 2 節) より対応する値の inhabited な各 boxed leaf は全単射で対応するオブジェクトを指す
        ので、読みうるオブジェクトと触れるオブジェクトは 2 つの実行で対応する。
    BY L30, L31, L32, L44, D7, A5, G3
...
第 2 節の `DEF README へ差し戻す 2 点` が言明を置いた `G3` と `G4` について、README のどこが何を述べて
いて、何を足すことを求めるかを書く。**この 2 つのほかに、この文書が閉じられなかった点は無い。**

### 差し戻し 1 (G3 -- D29 の全単射の定義域と、対応するオブジェクトが保持する値)

**README の D29 が述べていること。** 第 3.1 節の D29 は、全単射を生成する行を 4 つ挙げる。
...
**この 4 行が生成しないオブジェクトが在る。** オブジェクトの中から読み出される参照が指す先である。3 つ
ある -- boxed 容器の `Destructure` の名前付きフィールド、boxed union の変位アームの payload、そして単一の
`Unknown` を宣言する `Llvm` の結果の leaf である。この 3 つは D10 の生成の表に行を持つが、**割り当てを
行う位置ではない** -- D24 の (E2) の `H` の表がそれぞれ「+1」と述べるとおり、参照は既存のオブジェクトに
対して新しく作られる。そのオブジェクトは入力の束縛が受け取った値そのものでも、`Closure` や `Fresh` が
割り当てたものでも、子の活性化の結果でもない。

**それが要る所。** 2 つある。

- **P21 (b) の言明が量化するのは、活性化の各計数下オブジェクトである。**「2 つの活性化は同じ位置で同じ
  オブジェクトを解放する」の「同じオブジェクト」は D29 の全単射で言うほかなく、全単射の定義域の外に
  ある計数下オブジェクトについては、この言明が意味を持たない。定義域が上の 3 つを含まないと、
  `Destructure` で読み出した payload の解放について P21 (b) は何も言っていないことになる。
- **`L44` の (a) が、節点が束縛する値が 2 つの活性化で対応することを言う。**上の 3 つの値は、節点の形と
  それが名指す変数の値だけでは決まらない -- 決めるのは、その変数が指すオブジェクトが**保持している値**で
  ある。D29 は対応するオブジェクトが保持する値について何も述べない。

**求めるのは、D29 に次の 1 行を足すことである。**

> - **対応するオブジェクトが保持する値は対応する。** 全単射の定義域は、対応する 2 つの活性化がそれぞれ
>   到達できる (D25) オブジェクトの全体である。対応する 2 つのオブジェクトは、対応する各位置において、
>   inhabited (D16) な各 boxed leaf が全単射で対応するオブジェクトを指し、計数下かグローバル状態か
>   (D26) の区別も一致する。

**開始時だけの形では足りない。** 「活性化の最初の時点において」と限ると、途中でオブジェクトの中身を
書き換える `Llvm` の演算の後に、対応する 2 つのオブジェクトが別々の値を保持しうる。開始時の形に限る
なら、それを保つ段が別に要る -- そして **README にはその段を書く材料が無い。** A3 が宣言について述べる
のは `result_prov`、`borrows_operand`、`applies_a_function_operand` の 3 つであり、op がオペランドの
指すオブジェクトへ**何を書き込むか**は述べない。A4 が述べるのは `Retain`/`Release`/`Destructure`/
`Match` の実装だけである。「1 つの op が記憶域へ書き込む内容は、その op と、オペランドの値と、D21 が
活性化の側のデータとする 4 種の結果で決まる」という行が、A3 か A4 の側に要る。**どちらの形を採るかは
README の判断である。**

**参照カウントの側は、この行が無くても閉じている。** D29 の第 2 行が開始時のカウントの一致を与え、
以後のカウントの変化は D10 の行と D21 の第 4 行 (子の活性化を作る段) が決めるので、`L44` の (b) と (e) は
この行を読まない。読むのは、値の対応 ((a)) と、定義域が全部を覆っていること (P21 (b) の言明) の 2 つで
ある。

### 差し戻し 2 (G4 -- P21 (a) の `k(O)` の言い直し)

**README の P21 (a) が述べていること。** 第 5 節の P21 (a) は次のとおりである。

> - **(a)** 各位置において、各計数下オブジェクト `O` について `H_{α'}(O) = H_α(O) - k(O)` である。
>   `k(O)` は、その位置までに `α` が実行した削除済みの `Retain` のうち、対になる削除済みの `Release` を
>   **まだ実行していないもの**が `O` に作った参照の個数である。

**この文面は 2 通りに読め、どちらの読みでも偽になる。** 1 つの削除される `Retain` `t` と対になる削除
される `Release` は 1 つとは限らない。`L38` の 3 が `R_ρ(t)` と書く集合であり、`Σ_{r ∈ R_ρ(t)}
ActRefs(r) = ActRefs(t)` を満たす。`|R_ρ(t)| ≥ 2` であって、その一部だけが実行済みである位置で、上の
文面は `t` を数えるとも数えないとも読める。
...
**求めるのは、P21 (a) の `k(O)` を次の形に書き直すことである。**
...
`L44` の (b) が `H'(q, O) = H(q, O) - d(q, O)` を与えるので、書き直した P21 (a) はそのまま出る。
