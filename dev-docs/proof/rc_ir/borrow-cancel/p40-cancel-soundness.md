...
<1>2. (a)、(b)、(c)、(d)、(e)、(f) を `ρ` の上の節点についての帰納法で示す。D3 より `ρ` は有限の列なので
  ...
  <2>5. CASE `q` の節点が `Del` に入らない。`ρ'` の対応する節点も同じ節点である (L30、L31)。
    ...
    <3>3. QED
      ...
      <3>2 の帰納 (`<4>3` の `<5>2`) が 2 つの実行の素動作を 1 対 1 に対応させるので、対応する各点で
      `Obl` と `Obl'` は `q` の入口の点からの増減が等しく、<2>2 の (c) と合わせて (c) が `q` の実行の
      各点で成り立つ。<3>1 より束縛する変数には対応する値が置かれるので
...
<1>5. (S-a) が `ρ'` で成り立つ。
  ...
  <2>1a. CASE その素動作が `ρ` の終端の `Ret` の消費のものである。L38 の 2 より終端の `Ret` で pending
         ...
         `α` について「取り除かれる参照はその点の `Obl` に入っている」を与えるので、`α'` についても
         同じである。
