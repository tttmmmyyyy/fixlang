...
- **割り当て**。D10 の生成の表のうち `Closure(f, caps)` の capture object と、`result_prov` が単一の
  `Fresh` を宣言する `Llvm` の結果の leaf がこれを伴う (D24 の (E2) の `H` の表)。D24 より、割り当てと、
  割り当てたオブジェクトの欄を埋める受け渡しのあいだには点が在る。
...
<1>1. 計数下のオブジェクト `O` と `α` の各点 `τ` について
  ...
  <2>5a. D10 の 1 つの事象は、`Obl(・, O)` と `Σ_{C ∈ S(・, O)} obl_ρ(・, C)` を同じだけ動かす。
    ...
    第 6 行が消費、第 1 行が生成である。D34 はこの 3 行の事象を「**それを運ぶ素動作の直後の段内の点**」で
    `held` に映し、第 1 行と第 2 行の開始値を「**その類の終端の参照が `Obl(a)` に入る素動作の直後の
    段内の点**」に置くので、両辺は同じ点で動く。`β(C)` は時点に依らないので `obl_ρ` の動きは `held_ρ` の
    動きに等しい。初期値の 2 行については、所有する (D14) パラメータ・capture の leaf では D10 の初期値が
  ...
  <2>5b. `α` の素動作の列の上の帰納で `<1>1` を示す。
    ...
    <3>2. 帰納段。ある点で `<1>1` が成り立つとき、次の素動作の後の点でも成り立つ。
      ...
      とき、右辺は同じ理由で動かず、左辺は L39a より連鎖が終わった点の値以上であって、連鎖が終わった点で
    ...
    <3>3. QED
      BY <3>1, <3>2, D24
      D24 の (F) より解放の連鎖は有限で終わり、D3 より `ρ` は有限の列なので、`α` の素動作の列は有限か、
      有限の接頭ごとにこの帰納が届く。
  <2>6. QED
    BY <2>5a, <2>5b
...
<1>2. (a)、(b)、(c)、(d)、(e)、(f) を `ρ` の上の節点についての帰納法で示す。D3 より `ρ` は有限の列なので
  ...
  <2>3. CASE `q` の節点が `Del` の `Retain` 節点である。`α'` はこの節点を実行しない。`α` では D10 の
        ...
        作った個数を `m_τ(O)` と書き、`m(O) := m_{q'}(O)` と置く。`H` と `Obl` はそれぞれ `m_τ(O)` 増え、
        DEF 欠損 より `d(τ, O) = d(q, O) + m_τ(O)` である。
    ...
    <3>4. (e) が成り立つ。
      <4>1. `q` の実行の各点 `τ` について、`H(τ, O) = 0` と `H'(τ, O) = 0` は同値である。
        ...
        `H'(τ, O) = 0` ならば `H'(q, O) = 0` であり、同じ理由で `d(q, O) = 0` かつ `H(q, O) = 0` である。
  ...
  <2>4. CASE `q` の節点が `Del` の `Release` 節点である。`α'` はこの節点を実行しない。`α` では D10 の
        ...
        行う (`DEF 節点の実行の素動作`)。`q` の実行の点 `τ` について、`τ` までにこの節点が `O` への参照を
        処分した個数を `c_τ(O)` と書き、`c(O) := c_{q'}(O)` と置く。`H` と `Obl` はそれぞれ `c_τ(O)` 減り、
        DEF 欠損 より `d(τ, O) = d(q, O) - c_τ(O)` である。
    ...
    <3>1. この節点の実行はどのオブジェクトも解放しない。よってその素動作は D10 の `Release` の行が定める
          処分だけである。
      <4>1. この節点が解放を起こすとすれば、`q` の実行のある点 `τ` と計数下オブジェクト `O0` について
            `c_τ(O0) ≥ 1` かつ `H(τ, O0) = 0` である。
        BY D24, DEF 節点の実行の素動作, D7
        D24 の (F) より解放が起きるのは、参照を処分して計数下オブジェクトのカウントが 0 になったときで
        あり、D24 は「**解放は、それを起こした処分に付随して起きる。**」と述べる。`Release` 節点が行う
        処分は D10 の `Release` の行の分だけなので、最初に解放されるオブジェクト `O0` については、その
        処分の直後の点 `τ` で `H(τ, O0) = 0` であり、その処分がこの節点のものなので `c_τ(O0) ≥ 1` で
        ある。
    ...
    <3>1a. `Release` 節点は子の活性化を作る 3 種の段にも当たらず、グローバル化も伴わない。
    ...
    <3>2. `q` の実行の各点 `τ` で (a) が成り立ち、`τ` の点は 2 つの活性化で対応する。とくに `q'` で
          (a)、(b)、(c) が成り立つ。
      BY <2>2, <3>1, <3>1a, D10, D9, DEF 欠損, DEF 対応する活性化の量, L30, L31, L32, D29
    ...
    <3>4. (e) が成り立つ。
      <4>1. `q` の実行の各点 `τ` について、`H(τ, O) = 0` と `H'(τ, O) = 0` は同値である。
        ...
        `H'(q, O) = 0` であり、同じ理由で `d(q, O) = 0` かつ `H(q, O) = 0` である。`H(τ, O)` は 0 以上で
      ...
      <4>3. この節点の実行の素動作の個数についての帰納法で、各点について、2 つの活性化の点が対応し、
        ...
        <5>2. 対応が定まっている点 `τ` について、`τ` の次の素動作は 2 つの活性化で対応する。
          ...
          <6>2. CASE 次の素動作が D10 の行が定める作成・処分、または割り当てである。<3>1 より 2 つの
                ...
                第 3 行が与える** -- 「D10 の生成の表が割り当てを行う位置 … について、対応する位置が
                割り当てる 2 つのオブジェクトは対応する。」
            BY <3>1, D10, D24, D29
