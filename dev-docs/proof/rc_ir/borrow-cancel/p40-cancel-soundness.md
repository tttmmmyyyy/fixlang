...
この文書が読んだコードのコミットは `409378f6a8132088c432e2d1b3b76476e2a8fe88` である。README が証明の
...
**この文書に開いている点は無い。** 各段が依拠するのは、冒頭が挙げる README の定義・仮定・命題と、そこに
据えた 2 つの外部の結果 (`EXT`) と、引用したコードだけである。**P21 と P23 は入力を `borrow_ify` の
出力に限る** -- `L41a` が読む A19 (ii-b) と P14a の範囲がそこだからであり、第 7 節と第 9 節の
「証明する形」がその限定を書き出す。

**`α` の参照カウントの推移は、D29 の第 2 行が `α'` のそれに欠損 `k` を足したものとして与える。**
`L44` の (b) と (e) の等式はその与件を読んだものであり、`L44` が示すのは 2 つの活性化の時点が対応する
...
`α` がそれを満たすことは `L44` の (f) が位置ごとに示す。移し替えを行うのは `L43b` と `L43c` であり、
`α` についてこの制限を読むのは `L41`・`L41c`・`L42` である。
...
カウント (D7) とする。**`H` は活性化の側のデータである** -- D21 は「活性化はさらに、各時点の各計数下
オブジェクトの参照カウント `H` を持つ。`H` も活性化の側のデータである」と述べ、その増減のうち別の制御の
流れの段から来る分はこの活性化の外から来ると述べる。節点の時点 (下記) については、`Obl(τ, O)` を
その時点の義務集合 (D10) が持つ `O` への参照の個数とする。`B'` の活性化 `α'` については `H'`、`Obl'` と
書く。
...
**2 つの活性化の時点の対応。** 2 つの活性化は同じ時点で始まり、対応する節点の実行が起こす事象を順に
突き合わせて、時点を 1 対 1 に対応させる。`Del` の節点の実行が作る時点は `α` の側にしかないので、
`α'` の側の値としてその節点の実行の直後にあたる時点の値を取る (位置についての上の約束がこれである)。
**この対応が `α` のどの時点についても定まることは `L44` が示す** -- (a) が 2 つの活性化が `Del` の
節点を除いて同じ節点を実行することを、(e) が 1 つの節点の実行の中で 2 つが同じオブジェクトを解放する
ことを与える。

**その対応の上で `H` と `H'` の関係を与えるのは D29 の第 2 行である。** D29 は「`α` の参照カウントの
推移は、`α'` のそれに欠損 `k` (P21 (a)) を足したものとして与える」と述べ、その `k` は `DEF 欠損` の `d`
である (L43)。すなわち `L44` の (b) と (e) の等式は、証明が事象ごとに積み上げる量ではなく、D29 が
`α` に与えるデータである。
...
**言明** --- `ρ` の上の節点 `q` の実行の第 4 群 (DEF 実行時の量) について、その群が終わった時点の
`Obl` は、群が始まった時点の `Obl` に等しい。

**支えは D24 の網羅である。** D24 は各段について、`Obl` を離れる参照とその行き先、作られる参照の持ち手、
そして `H` の動きを書く。第 4 群の事象について D24 が `Obl` に触れると述べるのは、(F) の retain と
(E4) の 2 つの返り、および D9 の消費の行と (E2) の行き先の一覧だけであり、以下はその数え上げである。
**主語は D21 の意味の 1 つの活性化であり、実行 (D24) が作る活性化に限らない。**

**証明**

<1>1. 第 4 群で起きるのは D24 の (F) の解放の連鎖である。各解放は、`o` が持つ参照の処分と、`o` が
...
<1>2. `o` が持つ参照の処分は `Obl` を変えない。
  ...
  D25 の 2 つ目より、その参照の持ち手は生きているオブジェクト `o` である。D24 は各段について `Obl` を
  離れる参照とその行き先を書くが、(F) の記述がこの処分を `Obl` を離れる参照として挙げることはない --
  (F) が `Obl` について述べるのは、`_dtor` の retain が作る参照の持ち手がこの解放を含む段を実行して
  いる活性化であることと、`_value` の参照が `o` を離れて子の `Obl` の初期値に入ることの 2 つである。
...
<1>3. `_value` の欄の参照は `Obl` を通らない。D24 の (F) より、`o` の `_value` の leaf が持つ参照は
  ...
  BY D24
<1>4. この解放が `Obl` に入れる参照は 3 種であり、どれも同じ群の中で `Obl` を離れる。
  <2>1. `_dtor` の欄の関数に retain が与える参照。D24 の (F) より、その持ち手はこの解放を含む段を
        ...
        同じ群の中で `Obl` を離れ、1 つ目の活性化の `Obl` の初期値に入る。
    BY D24, D9
  <2>2. 1 つ目の活性化が返す参照。D24 の (E4) は「(F) が作る 1 つ目の活性化は `_dtor` の欄の関数を
        ...
        より消費するので、同じ群の中で `Obl` を離れて 2 つ目の活性化の `Obl` の初期値に入る。
    BY D24, D9
...
<1>5. QED
  ...
  すべて同じ群の中で `Obl` を離れ、それ以外に `Obl` を動かす事象が無いことを述べる。

### L39b (子の活性化の事象は義務集合を動かさない)

**言明** --- `ρ` の上の節点 `q` の実行の第 1 群 (DEF 実行時の量) の事象は、`Obl` を動かさない。

**証明**

<1>1. 第 1 群は、D24 の (E3)、(E7)、および (E2) のうちオペランドを適用する `Llvm` の段が作る子の活性化
      の事象である。子の活性化自身の事象が動かすのは、その子の `Obl` である (D24 の (E2)、(E3)、(E4))。
  BY DEF 実行時の量, D24
<1>2. (E3) の `App` について、`α` の `Obl` を動かすのは D9 の `App` の行の消費と (E4) の受け取りであり、
      どちらも `α` 自身の節点について D10 の行が定めるものなので、DEF 実行時の量 の第 2 群と第 3 群に
      在る。第 1 群には無い。
  BY D24, D9, D10, DEF 実行時の量
<1>3. (E2) のうちオペランドを適用する `Llvm` の段についても同じである。渡る参照は D9 の `Llvm` の行が、
      ...
      述べる。どちらも第 2 群と第 3 群に在る。
  BY D24, D9, D10, DEF 実行時の量
<1>4. (E7) のグローバルの初期化は `α` の `Obl` を動かさない。D24 の (E7) より、初期化子の活性化の終端の
      `Ret` が消費する参照の行き先は呼び出し元ではなく `E` である。グローバルを読む節点が参照を得ない
      ことは A8 と D26 が言う (D24 の (E7))。
  BY D24, A8, D26
<1>5. QED
  BY <1>1, <1>2, <1>3, <1>4, DEF 実行時の量
  DEF 実行時の量 より第 1 群の事象はこの 3 種の段が作る活性化のものに尽きる。
...
<1>1. 計数下のオブジェクト `O` と各時点 `τ` について `Obl(τ, O) = Σ_{C ∈ S(τ, O)} obl_ρ(τ, C)` である。
  <2>1. D10 が `Obl` を変える事象は、初期値、`Retain`、`Release`、生成、消費の 5 種である。移動は `Obl` を
        変えない。D26 より、数えるのは計数下のオブジェクトへの参照だけである。DEF 実行時の量 の第 1 群
        (子の活性化の事象) は `Obl` を動かさない (L39b)。第 4 群 (解放の走査) は `Obl` を動かすが、群の
        終わりには戻る (L39a)。節点の時点は、その節点の第 1 群の後であって第 4 群より前 (位置 `q` は
        第 1 群より前だが第 1 群がこれを動かさないので同じ値であり、第 2 群の直後、第 3 群の直後は
        第 1 群の後にある) か、前の節点の第 4 群が終わった後なので、この 5 種を数えれば足りる。
    BY D10, D26, L39a, L39b, DEF 実行時の量
...
<1>4. `H(τ, O) ≥ Σ_{C ∈ S(τ, O)} obl_ρ(τ, C) + [S(τ, O) に β(C) = 1 の類が在るならば 1]` である。
  ...
  不等式は仮定ではなく、D21 が活性化に課す制限である。その不等式は「`a` の計数下の別名類のうち
  `obj(C) = O` であり開始の時点がその時点以前であるものの全体を `S` とし、各類について
  `d(C) = held(C) - [C の ρ-終端が借用する (D14) leaf ならば 1]` と置くと、
  `H(O) ≥ Σ_{C ∈ S} d(C) + [S に借用終端の類が在るならば 1]`」である (A19 (i))。A19 は「各時点」が
...
<1>1. `α'` は `τ` に対応する時点で A19 (i) の不等式を満たす。すなわち各計数下オブジェクト `O` に
      ...
      である。
  BY D21, A19, D23, D29, DEF 類ごとの義務, DEF 実行時の量
  D21 は「**活性化は、その各時点で A19 (i) の不等式を満たすものに限る。**」と述べる。`α'` は `B'` の
  活性化であり (D29)、DEF 実行時の量 の時点の対応より `τ` に対応する `α'` の時点が定まり、本補題の仮定
...
活性化、`α` を D29 が `α'` に対応させる `B` の活性化とする。`ρ` の上の各位置 `q` と各計数下オブジェクト
`O` について次の 6 つが成り立つ。`ρ` の終端の `Ret` の消費を行った後の位置についても同じである。

**`B` が `borrow_ify` の出力の本体であることを読む。** `<1>2` が読む `L41`・`L41c` は P18a を、`L41c` が
読む `L41b`・`L41a` は A19 の (ii-a)・(ii-b) と P14a を `α` に当てる。その範囲が `borrow_ify` の出力の
本体だからである (第 1 節)。**入力プログラムが D12 を満たすことは、この補題のどの段も読まない。**
...
- **(e)** `q` の節点の実行の間の各時点 `τ` -- 位置 `q` 自身と、その実行が終わった時点 `q'` を含む
  (DEF 実行時の量) -- について、2 つの活性化の時点が対応し、`H'(τ, O) = H(τ, O) - d(τ, O)` であり、
  `H(τ, O) = 0` と `H'(τ, O) = 0` は同値である。`q ∉ Del` のときは `d(τ, O) = d(q, O)` である
  (DEF 欠損)。
- **(f)** `α` は、`q` までの各時点と `q` の節点の実行の各時点で、D21 の制限 -- A19 (i) の不等式 -- を
  満たす。

**(b) と (e) の等式は D29 の与件である。** D29 の第 2 行は「`α` の参照カウントの推移は、`α'` のそれに
欠損 `k` (P21 (a)) を足したものとして与える」と述べ、L43 の最後の文よりその `k` は `DEF 欠損` の `d` で
ある。`H` は活性化の側のデータなので (D21)、これは証明が事象ごとに積み上げる量ではなく、`α` を決める
データの一部である。**この補題が示すのは、その与件を読む先 -- 2 つの活性化の時点の対応
(DEF 実行時の量) -- が `ρ` の全体にわたって定まることと、(c)(d)(e)(f) である。**

**与件は 2 つの活性化の自分の事象と整合する。** D21 より、活性化自身の事象は D10 の表のとおりに `H` を
動かし、それ以外の増減はこの活性化の外から来る。対応する 2 つの時点の間に 2 つの活性化が実行する自分の
事象は `Del` の節点の分だけ違い ((a))、`Del` の `Retain`/`Release` 節点が D10 の表によって `O` の `H` に
...
ついての言明であり、時点 `τ` についての結論が結ぶのは `τ` までの量 -- その時点の `H`・`Obl`・`held`・
`bumps` と、そこに至る `ρ` の接頭が決める走査の状態 -- である。** よってこの補題は (f) を位置ごとに
示し、位置 `q` までの制限の上で `q` についてこれらを読む。README の第 2 節が「**各時点についての言明は、
その時点までの接頭で読む。**「各実行路と、それを辿る各活性化の各時点について …」の形の言明 -- A19
(ii-a)、A19 (ii-b)、P14a、P18a、P18b、P18c、D11 の 3 つの節 -- は、時点 `τ` について `τ` までの量だけを
結ぶ」と定めるのがこの読みであり、README の P21 の脇が「位置ごとの帰納にするのはこのためであり、各位置で
(a) から `α` の許容性を出してから (b) を出す」と述べるのがこの形である。
...
<1>1a. `Del` の節点はどれも `ρ` の終端の `Ret` より真に前にある。よって、`ρ` の終端の `Ret` の消費を
  ...
  BY L38, L37, L32, L30, L31, D23, D21, DEF 実行時の量
...
<1>2. (a)、(b)、(c)、(d)、(e)、(f) を `ρ` の上の位置についての帰納法で示す。D3 より `ρ` は有限の列なので
      整礎である。各位置では (a) と (b) と (c) から (f) を出し、(f) の上で (d) と (e) を出し、(e) が
      次の位置の (a) を与える。
  <2>1. 基底。`ρ` の最初の位置は `B` の根の入口である。DEF 欠損 より `d = 0` である。どちらの活性化も
        まだ節点を実行しておらず、D29 より 2 つの活性化はパラメータと capture に対応する値を受け取るので
        (a) が成り立つ。D29 の第 2 行より最初の時点では `k = 0` であり、2 つの活性化は対応する各計数下
        オブジェクトについて等しいカウントで始まるので (b) が成り立つ。D10 の初期値は所有する unit の
        下の inhabited な leaf で決まり、<1>1 より割り当ては同じなので `Obl'` は `Obl` に等しく、(c) が
        成り立つ。
    BY DEF 欠損, D29, D10, <1>1
  <2>2. 帰納法の仮定: `ρ` の上の位置 `q` について (a)、(b)、(c) が成り立つ。`q` の節点の実行の後の位置を
        `q'` とする。基底の位置についてこれを与えるのは <2>1 であり、それ以外の位置については、`ρ` の
        上でその直前にある節点についての <2>3・<2>4・<2>5 が与える。
    BY 帰納法の仮定, <2>1
  <2>2a. `α` は位置 `q` で D21 の制限を満たす。
    BY L43c, <2>2, <1>1a
    `q` が最後の位置であるとき、<1>1a より D21 の制限はその時点について何も課さず、言明は空虚に
    成り立つ。それ以外の位置では <1>1a より `α'` は対応する時点で生きている。L43b の仮定 -- `q` までに
    2 つの活性化が `Del` の節点を除いて同じ節点を実行し、その時点までに値を得ている各変数の値が対応
    すること -- は <2>2 の (a) が与え、`H(q, O) = H'(q, O) + d(q, O)` は <2>2 の (b) が与える。
  <2>2b. (d) が `q` で成り立つ。
    <3>1. CASE `q` が節点の入口である。`d(q, O) ≥ 1` とする。L43 より `N(q, O) ≥ d(q, O) ≥ 1` である。
          L41 より `H(q, O) ≥ N(q, O) + 1 ≥ d(q, O) + 1` である。L41 を `α` について読むのに要る D21 の
          制限は <2>2a が与える。
      BY L41, L43, <2>2a
    <3>2. CASE `q` が最後の位置 (終端の `Ret` の消費を行った後) である。L43 の最後の文より
          `d(q, O) = 0` なので (d) は空虚に成り立つ。
      BY L43
    <3>3. QED
      BY <3>1, <3>2, DEF 実行時の量
      DEF 実行時の量 より、`ρ` の上の位置は節点の実行の直前の時点か、`ρ` の終端の `Ret` の消費を行った
      後の時点である。
  <2>3. CASE `q` の節点が `Del` の `Retain` 節点である。`α'` はこの節点を実行しない。`α` では D10 の
        `Retain` の行により `O` への参照を `m(O)` 個作り、`H` と `Obl` をそれぞれ `m(O)` 増やす。
        DEF 欠損 より `d` も `m(O)` 増える。
    ...
    <3>2. `q'` で (a)、(b)、(c) が成り立ち、`q'` の時点は 2 つの活性化で対応する。
      BY <2>2, <3>1, D10, D9, DEF 欠損, DEF 実行時の量, L30, L31, L32, D29
      `q` は `Del` の `Retain` 節点なので、L30 と L31 より `ρ'` にはこの節点が無く、`α'` はこれを実行
      しない。D9 の 2 つの表より `Retain` は値を作らず、移さず、手放さないので、`q'` で値を得ている
      変数とその値は `q` のものと同じであり、(a) が `q'` で成り立つ。<3>1 よりこの節点の実行が作る
      時点は `q'` だけであり、DEF 実行時の量 の時点の対応より `q'` に対応する `α'` の時点は `q` に
      対応するものと同じである。D29 の第 2 行がその時点の (b) を与える。`Obl` は D10 の `Retain` の行の
      分だけ増え、`α'` の側は変わらず、`d` も同じだけ増えるので (c) が `q'` で成り立つ。
    <3>3. (f) がこの節点の実行の各時点で成り立つ。
      BY L43c, <3>1, <3>2, <2>2a, <1>1a
      <3>1 よりこの節点の実行の時点は `q` と `q'` の 2 つである。`q` については <2>2a が与える。`q'` に
      ついては、L43b の仮定を <3>2 の (a) が、`H(q', O) = H'(q', O) + d(q', O)` を <3>2 の (b) が与え、
      `α'` が対応する時点で生きていることを <1>1a が与える -- `q'` は `Del` の節点の直後なので最後の
      位置ではない。
    <3>4. (e) が成り立つ。
      <4>1. `d(q', O) ≥ 1` ならば `H(q', O) ≥ d(q', O) + 1` である。
        BY L41, L43, <3>3, D2, DEF 実行時の量
        `q` は `Retain` 節点なので継続を 1 つ持ち (D2)、`q'` はその継続の節点の実行の直前の時点で
        ある (DEF 実行時の量)。L43 より `N(q', O) ≥ d(q', O) ≥ 1` であり、L41 より
        `H(q', O) ≥ N(q', O) + 1` である。L41 を `α` について読むのに要る D21 の制限は <3>3 が与える。
      <4>2. QED
        BY <3>1, <3>2, <2>2, <2>2b, <4>1, DEF 欠損
        (e) が見る時点は <3>1 より `q` と `q'` の 2 つであり、DEF 欠損 より `d` の値はそれぞれ
        `d(q, O)` と `d(q', O) = d(q, O) + m(O)` である。時点の対応は <2>2 の (a) と <3>2 が与える。
        位置 `q` では <2>2 の (b) が、位置 `q'` では <3>2 の (b) が等式を与える。`d = 0` のときは両辺が
        等しく、`d ≥ 1` のときは <2>2b と <4>1 より `H ≥ d + 1 ≥ 2 > 0` かつ `H' = H - d ≥ 1 > 0` で
  ...
  <2>4. CASE `q` の節点が `Del` の `Release` 節点である。`α'` はこの節点を実行しない。`α` では D10 の
    ...
    <3>1. この節点の実行はどのオブジェクトも解放しない。
      ...
      <4>3. QED
        BY <4>1, <4>2, <2>2b
        <4>1 と <4>2 より `d(q, O0) = c(O0) ≥ 1` なので、<2>2b の (d) より
        `H(q, O0) ≥ d(q, O0) + 1 = c(O0) + 1` である。これは <4>1 の `H(q, O0) = c(O0)` に反する。
    <3>2. `q'` で (a)、(b)、(c) が成り立ち、`q'` の時点は 2 つの活性化で対応する。
      BY <2>2, <3>1, D10, D9, D24, DEF 欠損, DEF 実行時の量, L30, L31, L32, D29, D7
      ...
      `Llvm` の段は `Llvm` の節点であり、(E7) が起きるのは「まだ初期化されていないグローバルを読む
      節点の位置」であって、D7 は `Release` を読む構文に数えない。よって DEF 実行時の量 の 4 群のうち
      第 1 群・第 3 群・第 4 群は空であり、この節点の実行の時点は `q'` だけである。D9 の 2 つの表より
      `Release` は値を作らず、移さず、手放さないので (a) が `q'` で成り立つ。DEF 実行時の量 の時点の
      対応より `q'` に対応する `α'` の時点は `q` に対応するものと同じであり、D29 の第 2 行がその時点の
      (b) を与える。`Obl` は D10 の `Release` の行の分だけ減り、`α'` の側は変わらず、`d` も同じだけ
      減るので (c) が `q'` で成り立つ。
    <3>3. (f) がこの節点の実行の各時点で成り立つ。
      BY L43c, <3>2, <2>2a, <1>1a
      <3>2 よりこの節点の実行の時点は `q` と `q'` の 2 つである。`q` については <2>2a が与える。`q'` に
      ついては、L43b の仮定を <3>2 の (a) が、`H(q', O) = H'(q', O) + d(q', O)` を <3>2 の (b) が与え、
      `α'` が対応する時点で生きていることを <1>1a が与える -- `q'` は `Del` の節点の直後なので最後の
      位置ではない。
    <3>4. (e) が成り立つ。
      <4>1. `d(q', O) ≥ 1` ならば `H(q', O) ≥ d(q', O) + 1` である。
        BY L41, L43, <3>3, D2, DEF 実行時の量
        `q` は `Release` 節点なので継続を 1 つ持ち (D2)、`q'` はその継続の節点の実行の直前の時点で
        ある (DEF 実行時の量)。L43 より `N(q', O) ≥ d(q', O) ≥ 1` であり、L41 より
        `H(q', O) ≥ N(q', O) + 1` である。L41 を `α` について読むのに要る D21 の制限は <3>3 が与える。
      <4>2. QED
        BY <3>2, <2>2, <2>2b, <4>1, DEF 欠損
        (e) が見る時点は <3>2 より `q` と `q'` の 2 つであり、DEF 欠損 より `d` の値はそれぞれ
        `d(q, O)` と `d(q', O) = d(q, O) - c(O)` である。位置 `q` では <2>2 の (b) が、位置 `q'` では
        <3>2 の (b) が等式を与える。`d = 0` のときは両辺が等しく、`d ≥ 1` のときは <2>2b と <4>1 より
        `H ≥ d + 1 ≥ 2 > 0` かつ `H' = H - d ≥ 1 > 0` である。よって 0 になることは同値である。
    <3>5. QED
      BY <3>2, <3>3, <3>4
  <2>5. CASE `q` の節点が `Del` に入らない。`ρ'` の対応する位置にも同じ節点がある (L30、L31)。
    <3>1. 2 つの実行はこの節点について D10 の同じ行を同じ値に対して適用し、この節点が束縛する変数に
      ...
      <4>3a. `App` の節点について、D9 の `App` の行が名指す leaf は 2 つの実行で同じである。
        ...
        である。<1>1 よりその関数の `params` と `borrowed_units` は 2 つのプログラムで等しいので、
        D14 の所有も等しい。
      <4>4. QED
    ...
    <3>2. `q` の節点の実行の各時点で 2 つの活性化の時点が対応し、(f) が成り立ち、
          `H'(τ, O) = H(τ, O) - d(τ, O)` であり、`H(τ, O) = 0` と `H'(τ, O) = 0` は同値である。位置 `q`
          自身と、その実行が終わった時点 `q'` についても同じである。本場合の仮定より `q` は `Del` に
          入らないので、DEF 欠損 より `d(τ, O) = d(q, O)` である。すなわちこれは (e) と、(f) の
          「`q` の節点の実行の各時点で」の半分である。
      <4>1. 位置 `q` 自身について、時点は対応し、`H'(q, O) = H(q, O) - d(q, O)` であり、`H(q, O) = 0` と
            `H'(q, O) = 0` は同値であり、(f) が成り立つ。
        BY <2>2, <2>2a, <2>2b
        <2>2 の (a) が時点の対応を、<2>2 の (b) が等式を、<2>2a が (f) を与える。`d(q, O) = 0` のとき
      ...
      <4>3. この節点の実行の事象の個数についての帰納法で、各時点について、2 つの活性化の時点が対応し、
            (f) が成り立ち、`H(τ, O) = 0` と `H'(τ, O) = 0` が同値であることを示す。
        <5>1. 基底は位置 `q` であり、<4>1 がこの 3 つを与える。
          BY <4>1
        <5>2. 対応が定まっている時点 `τ` について、`τ` の次の事象は 2 つの活性化で対応する。
          <6>1. CASE 次の事象が第 1 群のもの (子の活性化の事象) である。<3>1 の後半より、この節点が
                作る子の活性化の事象が参照カウントに与える変化は 2 つの実行で同じである。D21 の第 4 行が
                それを活性化の側のデータとし、D29 の第 4 行が 2 つの活性化に同じものとして与える。
            BY <3>1, D21, D29
          <6>2. CASE 次の事象が第 2 群または第 3 群のもの (D10 の処分と作成) である。<3>1 より 2 つの
                実行はこの節点について D10 の同じ行を同じ値に対して適用する。
            BY <3>1, D10
          <6>3. CASE 次の事象が第 4 群のもの (解放の走査) である。D24 の (F) より、解放されるのは
                ...
                おいて、inhabited な各 boxed leaf が全単射で対応するオブジェクトを指し」が与える。
          ...
          <6>4. QED
            BY <6>1, <6>2, <6>3, <4>2
            <4>2 より、この節点の実行の事象は 4 群のいずれかに属する。
        <5>3. `τ` の次の時点で、2 つの活性化の時点が対応し、(f) が成り立ち、`H = 0` と `H' = 0` が
              同値である。
          BY <5>2, L43c, L41c, D29, <2>2, <3>1, <4>2, <1>1a, 本場合の仮定
          <5>2 より次の時点でも 2 つの活性化の時点は対応し、D29 の第 2 行がその時点で
          `H'(τ, O) = H(τ, O) - d(τ, O)` を与える。L43b の仮定 -- その時点までに 2 つの活性化が `Del` の
          節点を除いて同じ節点を実行し、値を得ている各変数の値が対応すること -- は <2>2 の (a) と <3>1 が
          与える。その時点が最後の位置 -- `q` が `ρ` の終端の `Ret` であるときの `q'` -- であるときは、
          <1>1a より D21 の制限はその時点について何も課さないので (f) は空虚に成り立つ。それ以外の
          時点では <1>1a より `α'` は対応する時点で生きているので、L43c より (f) が成り立つ。<4>2 より
        ...
        <5>4. QED
          BY <5>1, <5>2, <5>3, D24
          D24 の (F) より解放の連鎖は有限で終わるので、この節点の実行の事象の個数は有限であり、この
          帰納法は整礎である。
      <4>4. QED
        BY <4>1, <4>2, <4>3
        <4>3 が各時点について 3 つを与え、<4>2 が `d(τ, O) = d(q, O)` を与える。
    <3>2a. `q` の節点の実行が `Obl` に与える変化は、D10 の行が定める分だけである。
      BY L39a, L39b, D10, DEF 実行時の量
      DEF 実行時の量 より、この節点の実行の時点は 4 群である。第 2 群と第 3 群は D10 の処分と作成で
      ある。第 1 群は L39b より `Obl` を動かさず、第 4 群は L39a より `Obl` を群の始まりの値に戻す。
    <3>3. QED
      ...
      `q'` の時点についての等式であり、(e) と (f) の後半も <3>2 である。
  <2>6. QED
    BY <2>1, <2>2, <2>2a, <2>2b, <2>3, <2>4, <2>5, L43, L32, DEF 実行時の量
    `q` の節点は `Del` の `Retain`、`Del` の `Release`、`Del` に入らないもののいずれかである
    (L32 の 5)。節点を持たない最後の位置については、L43 の最後の文より `d = 0` なので (b) から
    `H' = H` であり、(d) は <2>2b が、(f) は <2>2a が、(e) の 2 つの言明はその位置について <2>2 の (b) と
    <2>2a が与える。
    **(f) の「`q` までの各時点」の半分は、位置についての帰納がそのまま与える。** DEF 実行時の量 より、
    `q` より前の各時点は `ρ` の上で `q` より前にある節点の実行の時点であり、その節点についての
    <2>3・<2>4・<2>5 が (f) の「その節点の実行の各時点で」の半分をそこで与えている。位置 `q` 自身に
    ついては <2>2a である。
<1>3. QED
  BY <1>1, <1>2, D3, D21, D29
  <1>2 が `ρ` の各位置について (a) から (f) を与える。D3 より `ρ` の位置は有限個なので、(f) を
  すべての位置について集めると `α` はその全時点で D21 の制限を満たす。すなわち `α` は D21 の意味の
...
<1>1a. `α` は D21 の制限 -- A19 (i) の不等式 -- をその全時点で満たす。すなわち `α` は D21 の意味の
       ...
       立つ。
  BY L44, D21, D29
  L44 の (f) が位置ごとにこれを示し、L44 の `<1>3` がそれを `ρ` の全位置について集める。
...
`<1>7` の `<2>2` がそれである。**「直前の点」は節点の入口ではないので、そこで使う対応は位置の粒度の
P21 (b) ではなく、点の粒度の `L44` の (e) である。**
...
<1>5. (S-a) が `ρ'` で成り立つ。
  ...
  <2>2. CASE それ以外。その操作が位置 `q` の節点で `O` への参照を `c` 個取り除くとする。DEF 実行時の量 の
        ...
        `Obl(τ, O) ≥ b(τ, O)` である。L42 を `α` について読むのに要る、`α` が D21 の意味の活性化で
        あること -- L42 が読む L41a が A19 の (ii-a)・(ii-b) と P14a を `α` に当てるので要る -- は
        <1>3 が与える。
    BY L42, DEF 実行時の量, D10, D3, L33a, <1>3, D21
...
<1>7. (S-c) が `ρ'` で成り立つ。すなわち `α'` が `ρ'` の上の点 `p'` まで解放について閉じている
  ...
  <2>1a. `α` の各時点 `τ` と、それに対応する `α'` の時点について、各計数下オブジェクト `O` が `τ` で
         解放されている (DEF 解放されている) ことと、対応する時点で `O` が解放されていることは同値で
         ある。
    BY L44, DEF 解放されている, D29, D26
    L44 の (e) は、`ρ` の上の各節点の実行の間の各時点 -- 位置 `q` 自身とその実行が終わった時点を含む --
    について、2 つの活性化の時点が対応し `H(τ, O) = 0` と `H'(τ, O) = 0` が同値であることを述べる。
    **これは点の粒度の言明である** -- (e) が量化するのは節点の実行の間の各時点であって、節点の入口だけ
    ではない。DEF 解放されている は「`τ` かそれより前の時点で参照カウントが 0 であること」を解放されて
    いることと定めるので、`τ` 以前の各時点にこの同値を当てれば、2 つの活性化で解放されているオブジェクト
    は対応する。`O` はどちらの活性化のものも D29 の全単射の定義域に入る (D29 の第 5 行)。計数下でない
    オブジェクトはどちらの活性化でも解放されない (D26、A8)。**(S-c) が見る「直前の点」もこの粒度で
    足りる** -- DEF 実行時の量 は時点を参照の作成・処分の事象の間の区切りと定めるので、隣り合う 2 つの
    時点の間で `H` は動かず、どの点の `H` もその直前の時点の値である。
  <2>2. `α'` が点 `p'` まで解放について閉じている (D11a) ならば、`α` は `p'` に対応する `α` の点まで
        閉じている。
    BY <2>1a, D11a, D26
    D11a は、時点が解放について閉じているとは、その時点で `H(O) ≥ 1` である各計数下オブジェクト `O` が
    その時点で解放されていないことと定め、`τ` まで閉じているとは `τ` 以前の各時点が閉じていることと
    定める。`α` の時点 `σ` と `H(σ, O) ≥ 1` である計数下 `O` を取る。<2>1a の同値 (`H = 0` と `H' = 0`)
  ...
  <2>3. QED
    ...
    <2>1a よりその点で解放されていることも 2 つで同値なので、`ρ'` の側でも解放されていない。
<1>8. QED
...
本体で示す。**この節は反例だけからなり、ほかの節が読む結論を持たない。この文書に閉じられなかった点は
無い** -- 第 0 節の表の 7 命題はすべて、冒頭が挙げるものの上で閉じている。
