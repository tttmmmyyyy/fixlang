...
<1>5. (S-a) が `ρ'` で成り立つ。
  ...
  <2>1a. CASE その素動作が `ρ` の終端の `Ret` の消費のものである。L38 の 2 より終端の `Ret` で pending
         な `CT` の要素は無いので、L43 の等式より終端の `Ret` の入口の点で `d = 0` である。L44 の
         `<1>1a` より `Del` の節点はどれも終端の `Ret` より真に前にあるので、終端の `Ret` の実行の各点
         でも `d = 0` である (DEF 欠損)。L44 の (c) よりその各点で `Obl' = Obl` であり、<1>3 の (S-a) が
         `α` について「取り除かれる参照はその時点の `Obl` に入っている」を与えるので、`α'` についても
         同じである。
    BY L38, L43, L44, DEF 欠損, <1>3, <1>4
