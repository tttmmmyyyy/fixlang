...
`README.md` の第 3.6 節が定める。解放について閉じていること (D11a) も同じく枠が定める。この文書はそれを
...
**`L3` は D11a を実行の側で果たす補題である。**D11a は「`H(O) ≥ 1` である各計数下オブジェクトが
解放されていない」点を解放について閉じていると呼び、D11 の (S-c) はその接頭を条件に取る。実行 (D24) の
各時点でこれが成り立つことを `L3` が示し、`L2b` がその条件を段内の点ごとに供給する。
...
**残っているものの節が 2 つを挙げる。**README の P27 の言明が P28 の 2 つの前提を持たないこと、
...
<1>1. QED
  ...
  在る。`params` も
  `borrowed_units` も実行時の呼び出し先の欄なので、後半の文が従う。
...
持つ本体の活性化を作る段が (E3) に限られること、A20 -- の下で、実行の各時点についてそれを述べる。
この 2 つと D25 の持ち手の 3 分から、各計数下オブジェクト `o` について
...
P28 の 3 つの前提が揃っている場合にだけ在る** -- それを引く L3、L5、P27 はどの 3 つも言明の仮定に持つ。
...
**D11a を段内の点について読む。** D11a は「`H(O) ≥ 1` である各計数下オブジェクト `O` が解放されて
いない」点を**解放について閉じている**と呼び、「`τ` 以前の各時点が解放について閉じている」ことを
**`τ` まで閉じている**と呼ぶ。D11 の (S-c) は、その活性化がその時点まで閉じていることを条件に取り、
読む構文が読みうるオブジェクトと `Retain`/`Release` が触れるオブジェクトが**その読み・その触れる動作の
直前の点で**解放されていないことを言う。動作の直前の点は段内の点なので、条件の側の「各時点」も段内の点を
渡る。よってこの文書は、D11a の 2 つの語を段内の点についても読む -- 段内の点 `p` が**解放について
閉じている**とは、`p` において `H(O) ≥ 1` である各計数下オブジェクト `O` が `p` において解放されて
いないことをいい、**`p` まで閉じている**とは、`p` 以前の各段内の点が解放について閉じていることをいう。
D24 の時点はその段の最初の段内の点なので、この読みは D11a の読みを狭めない。
...
する。`S` を持つ活性化の本体と、`S` の中で (F) の解放が作る活性化の本体は D11 を満たすとする。このとき
次の 2 つが成り立つ。
...
作る活性化の節点も参照を作ることである -- その節点は `t` の直後ではなく段の途中で走るので、`t` を主語に
した言明では D11 の (S-c) が与えるものと噛み合わない。もう 1 つは、(S-c) の結論そのものが「その読み・
...
<1>1a. 参照を作る素動作の直前の点を `p` とし、`p` まで閉じているとする。その動作を行う節点が D7 の読む
       構文であるとき、その構文が読みうるオブジェクト -- 名指された値の inhabited な各 boxed leaf が指す
       オブジェクト -- は `p` で解放されていない。そこから到達できる (D25) オブジェクトも `p` で解放されて
       いない。
  ...
  時点を含み、D11a の「`τ` まで閉じている」はその条件である。よって (S-c) が当たり、読みうる各オブジェクト
  について、**その読みの直前の点** すなわち `p` で解放されていないことを言う。到達できるオブジェクトへは
  L2 が広げる -- `p` は解放について閉じているからである。グローバル状態のオブジェクトは A8 より解放され
  ない。
  BY A8, D3, D7, D11 (S-c), D11a, D21, D23, D25, L2, DEF 段の素動作と段内の点, <1>1
...
<1>1b. 参照を作る素動作の直前の点を `p` とし、`p` まで閉じているとする。その動作を行う節点が
  ...
  辿った節点の列はその本体の実行路 (D3) である (D21, D23)。その活性化は `p` まで閉じている (D11a) ので
  (S-c) が当たり、(S-c) は、読む構文が読みうるオブジェクトに加えて `Retain(v, π)` と `Release(v, π)` が
  触れる各オブジェクトについても、**その触れる動作の直前の点** すなわち `p` で解放されていないことを言う。
  BY D3, D7, D11 (S-c), D11a, D21, D23, DEF 段の素動作と段内の点, <1>1
...
<1>5. CASE `Llvm` の行、および `InlineLLVMBoxedFromRetainedPtrIOS` の行。
  ...
  <2>1. CASE 宣言が単一の `Fresh`。
    ...
    <3>2. QED
      ...
      leaf の参照になる -- 処分でも新しい割り当てでもない」と述べる。すなわちその leaf については参照は**移る**の
  ...
  <2>2. CASE 宣言が単一の `Unknown`。
    ...
    <3>0a. CASE op が `applies_a_function_operand` を宣言する。
      ...
      D8 より `H(o) ≥ 1` であり、`p` は解放について閉じているので `o` は `p` で解放されていない。そこから
      到達できるオブジェクトへは L2 が広げる。`b` が返した参照をそのまま持つ leaf は `<2>0` が除く。
      BY A3, A8, D7, D8, D11a, D25, L1, L2, <1>1a, <2>0
...
<1>8. `p` まで閉じている段内の点 `p` の直後の素動作が `S` の中で参照を作るとき、その参照が指す
  ...
  が扱う -- `<1>1a` と `<1>1b` はどちらも「参照を作る素動作の直前の点」を主語にしており、`<1>1` が挙げる
  2 種の活性化のどちらの節点についても、その本体が D11 を満たすことと `p` まで閉じていることから (S-c) を
  引く。`<1>1c` が扱う割り当ての場合は、そのオブジェクトが `p` に在らないので `p` で解放されていない。
  BY <1>1, <1>1a, <1>1b, <1>1c, <1>2, <1>3, <1>4, <1>5, <1>6, <1>7
...
**(a) が D11a が名指す性質である。** D11a は「実行 (D24) の各時点で閉じていることは `p51-runs.md` の
`L3` が示す」と書き、D11 の (S-c) の接頭条件はこの性質である。
...
<1>4. (d) が成り立つ。
  ...
  <2>2. 1 つの段が作る活性化は有限個である。
    (E1)、(E3)、(E7) はそれぞれ 1 つ作る。(E5)、(E6)、(E8) は活性化を作らない。残るのは 2 つである。
    **オペランドを適用する `Llvm` の段 (L0a) が作る活性化**は、L0a (a) よりその段について 1 つである。
    **(F) の解放が作る活性化**は、解放されるオブジェクト 1 つにつき 2 つである (D24 の活性化の林)。
    ...
    段内の点)。
    BY D24 (F), D24 (活性化の林), L0a, DEF 段の素動作と段内の点
  <2>4. 林は有限である。
  ...
  <2>6. QED
    ...
    (E4)、(E5)、(E7)、(E8) はどれも割り当てを持たず、(E6) の後に段は無く、(F) の解放は参照の処分と記憶域の
...
**3 番目の仮定と A20 は P28 の前提である。**P28 は D12 とこの 2 つを前提に取り、証明は P28 (a) を
「`H` の分解」を通じて引く。README の P27 の言明はこの 2 つを挙げていないが、`borrow_ify` の出力に
ついて果たすのは P14b と A20 であり、`cancel` は `borrowed_units` も `Closure` の目標も初期化子も
変えない (P24)。
...
<1>1. (R1) が成り立つ。
  <2>1. CASE (読み-1) D7 の読む構文の実行。
    ...
    いる (D11a) ので、(S-c) の接頭条件が揃う。(S-c) は「D7 の読む構文がその位置で読みうる各オブジェクトは、
    **その読みの直前の点で**解放されていない」を言い、D32 の (読み-1) が読むのはちょうどそのオブジェクト
    である。
...
unit を持つ本体の活性化を作る段が (E3) に限られること、A20 -- は `borrow_ify` の出力について P14b と A20 が
果たす。P27 の言明を D12 だけの形に戻すには、P28 (a) を残る 2 つに依らない形で示す道が要る。

## 6. 残っているもの

**(1) P27 の言明が P28 の 2 つの前提を持たない。** P27 の証明は「`H` の分解」を通じて P28 (a) を引き、
P28 は D12 のほかに、借用する unit を持つ本体の活性化を作る段が (E3) に限られることと A20 とを前提に
取る。この文書は 2 つを P27 の仮定に置いた。README の P27 は「プログラムのすべての本体が D11 を満たし、
A17 と A18 が成り立つならば」とだけ書く。足す形は 2 つある -- P27 の言明にこの 2 つを足すか、P28 (a) を
残る 2 つに依らない形で示すかである。前者を取るときは、T が P27 に渡すのが `cancel` の出力なので、
`cancel` の出力がその限定を満たすことを言う者も要る。
