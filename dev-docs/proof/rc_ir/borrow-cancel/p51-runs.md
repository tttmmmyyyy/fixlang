...
<1>1a. 参照を作る素動作の直前の点を `p` とし、`p` まで閉じているとする。その動作を行う節点が D7 の読む
  ...
  **読みの直前の点は `p` と同じ点ではない。** `p` は参照を作る素動作の直前の点であり、その 2 つのあいだに
  ...
  参照を作る瞬間まで解放されない。`p` はその区間の中の点である。到達できるオブジェクトへは L2 が広げる --
  `p` は解放について閉じているからである。グローバル状態のオブジェクトは A8 より解放されない。
  BY A8, A26, D3, D7, D11 (S-c), D11a, D21, D23, D25, L2, DEF 段の素動作と段内の点, <1>1
