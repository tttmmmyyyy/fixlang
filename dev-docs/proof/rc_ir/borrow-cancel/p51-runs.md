...
<1>1b. 参照を作る素動作の直前の点を `p` とし、`p` まで閉じているとする。その動作を行う節点が
  ...
  **`π` の下に inhabited な leaf が 2 つ以上あるとき、触れる動作は leaf ごとに在り、その直前の点は
  ...
  名指す点は `p` ではない。`μ` の点から `p` までのあいだにこの節点が行う動作は、D24 の (E2) の `H` の表の
  `Retain` の行が挙げる生成だけであり、生成は `H` を上げるので、D24 の (F) が解放を起こす条件
  (`H` が 0 になること) をどのオブジェクトについても満たさない。よってその区間で新たに解放される
  オブジェクトは無く、`μ` のオブジェクトは `p` でも解放されていない。`λ` 自身については (S-c) が名指す点が
  `p` である。
