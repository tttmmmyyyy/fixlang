...
- **(a)** その点で `H(o) ≥ 1` である各計数下オブジェクト (D26) `o` は、その点で生きている (D25)。
  **ある値の inhabited な boxed leaf が計数下オブジェクト `o` を指すときも、`o` はその点で生きている。**
- **(b)** オブジェクト `o'` がその点で生きており、`o'` から `o''` へ到達できる (D25) ならば、`o''` も
  解放されていない。`o'` と `o''` は計数下でもグローバル状態でもよい。

**(a) の主語を「記憶域が在るオブジェクト」に取らないのは、読み手がその形の仮定を持たないからである。**
D7 は記憶域が在ることを「`o` を割り当てた素動作 (D24) より後、`o` の記憶域を返す素動作 ((F)) より前」と
定め、D8 は `H(o)` を未処分参照の総数と定めるだけなので、参照の存在や読みの存在から割り当て済みで
あることを出す節はそのどちらにも無い。`<1>0` がその渡りを引き受ける。
...
<1>1. (a) が成り立つ。
  <2>1. `H(o) ≥ 1` である計数下オブジェクト `o` は、この点で生きている。
  ...
  <2>2. ある値の inhabited な boxed leaf が計数下オブジェクト `o` を指すならば、`H(o) ≥ 1` である。
    A5 より、値が保持する参照はその型の `boxed_leaf_paths` が列挙する leaf のうち、inhabited であって
    計数下のオブジェクトを指すものにちょうど 1 つずつある。よってその leaf は `o` への参照を 1 つ持つ。
    D24 の (E2) が「**処分**された参照は以後存在せず、`H` が 1 下がる」と述べるので、値が保持している
    参照は処分されていない。D8 より `H(o) ≥ 1` である。
    BY A5, D8, D24 (E2), D26
  <2>3. QED
    BY <2>1, <2>2
...
<1>1a. `o'` がその点で生きており、`o'` が `o` への参照を持つならば、`o` もその点で生きている。とくに
  ...
  未処分であり、D8 より `H(o) ≥ 1` である。`<1>1` より `o` はその点で生きており、とくに解放されていない。
