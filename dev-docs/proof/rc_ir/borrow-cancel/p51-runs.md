...
`131b1a21a7f865b88c320dd3d6a06bf6f5bbc97d` である。
...
自分で置くのは、局所定義「勘定が合っている時点」と補題 L0-L5 だけである。
...
まだ数え上げていないものが 1 つ残る。L0 が読む「実行時の関数の値の funptr がどの LLVM 関数を指しうるか」
の数え上げであり、「残っているもの」の節が述べる。P27 の証明は L0 を引かないので、この穴は P27 の 3 つの
結論には及ばない。
...
<1>5. `resolve_callee_params` が `None` を返す場合、実行時の呼び出し先の `borrowed_units` は空である。
  ...
  <2>4b. 実行時の関数の値が funptr の欄に持つ LLVM 関数は、次の 3 つのいずれかである。
    ...
    - **(M2)** `prog.funcs` の鍵である名前を読む段が返す funptr。`<1>4` より、それは
      `implement_rc_program` がその関数のために作った LLVM 関数である。
    ...
    値が配列の要素、構造体のフィールド、union の payload、グローバルの記憶域、FFI の境界を通っても、
    funptr の欄の中身はこの 3 か所のどれかが書いたものである。それらは値を写すだけで funptr を作らない。
    **この数え上げは全 `LLVMGen` の実装 78 個の走査に立つ。**「残っているもの」の節を見よ。
    BY <1>4, CODE src/rc_ir/codegen.rs: Generator::build_rc_closure,
  ...
  <2>5. QED
    ...
    ある。(M1) の `fref` は `<2>3` より原本の名前であり、(M2) の名前は `<2>4a` より借用版ではないので
    やはり原本であり、(M3) の関数は `<2>4c` より capture を持つので `<2>1` より借用版を持たない原本で
    ある。`<2>2` より原本の `borrowed_units` は空である。
...
(b) (E4) の段で `Obl(b)` を離れる参照の多重集合は、その段の行き先 -- `b` を (E3) が作ったなら呼び出し元の
`Obl(a)`、`b` を (E1) か (E7) が作ったなら `E` -- が得る参照の多重集合に一致する。
...
<1>6. (b) が成り立つ。
  ...
  <2>4. QED
    D24 の活性化の林より、活性化を作るのは (E1)、(E3)、(E7) の 3 つの段だけである。
    BY D24, <2>2, <2>3
...
<1>2. 勘定が合っている時点の直後の 1 段の後の時点も、勘定が合っている。
  ...
  <2>3. CASE (E3) 呼び出しの段。
    ...
    BY L1, D24 (E3), D25
  <2>4. CASE (E4) 返りの段。
    <3>1. L1 (b) より、`Obl(b)` を離れる参照はそのまま行き先 -- `b` を (E3) が作ったなら呼び出し元の
          `Obl(a)`、`b` を (E1) か (E7) が作ったなら `E` -- に入る。
      BY L1, D24 (E4), D24 (E7)
  ...
  <2>7. QED
    D24 は段を (E1)-(E7) の 7 種に尽くす。(F) の解放は段ではないので独立した場合を持たず、`<2>2` が
...
**言明**。活性化 `a` が (E3) または (E7) の段で活性化 `b` を作ってから、`b` が終わって `a` が再開する
までの間、`Obl(a)` は変わらない。
...
<1>2. `Obl(a)` を変える段は 3 種である。`a` が段を持つ (E2) と (E3) の段、`a` 自身が終わる (E4) の段、
  ...
  参照が入る先 -- 終わった活性化を (E3) が作ったならその呼び出し元の `Obl`、(E1) か (E7) が作ったなら
  `E` -- である。段の中で起きる解放 (F) が処分するのは、オブジェクトが持つ参照だけである。
  BY D24
...
<1>3. その間に (E4) の段で終わる活性化のうち、参照を `Obl(a)` へ入れるものは無い。
  D24 の (E4) より、参照が `Obl(a)` へ入るのは、終わる活性化を `a` の (E3) の段が作った場合だけである。
  `a` が (E3) の段を持つのは `a` が段を持つときであり、`<1>1` よりその間 `a` は段を持たないので、その間に
  `a` が新しい子を作ることは無い。`a` が `b` より前に作った子は、D24 の (E3) と (E7) より、それぞれの中断が
  終わった時点ですでに終わっている。よってその間に終わりうる `a` の子は `b` だけであり、`b` の (E4) は
  この区間の終わりである。
  BY D24, <1>1
...
**帰納法の仮定**。D24 の**活性化の林** -- (E1) が作る活性化を根、(E3) と (E7) が作る活性化をそれを作った
活性化の子とする -- の上の構造帰納であり、活性化 `a` についての帰納法の仮定は「`a` が作ったすべての活性化が
終わっている」である。D31 の後の注より 1 つの活性化が作る活性化は有限個であり、正常終了する実行の段は有限個
なので、この林は有限であって帰納は well-founded である。
...
<1>2. 活性化 `a` が終わるのは、`a` が作ったすべての活性化が終わった後である。
  `a` が (E3) または (E7) の段で `b` を作ると、`a` は `b` が終わるまで中断中であり段を持たない
  (D24 (E3), D24 (E7))。`a` が終わるのは `a` 自身の (E4) の段であり、それは `a` が段を持つときである
  (D23)。
  BY D23, D24 (E3), D24 (E4), D24 (E7)
...
| 主張 | 使う仮定 |
|---|---|
| (R1) | D11 の (S-b) と (S-c)、A3 (D24 の生成の表と L2b を通じて)、A5 (D25 を通じて)、A8、A12、A14 (L1 を通じて)、A16 (L2b を通じて)、A17 |
| (R2) | (R1) が使うもののすべて |
...
**L0 の `<2>4b` が立つ数え上げ。** 「実行時の関数の値の funptr が指す LLVM 関数は (M1)(M2)(M3) の 3 つの
いずれかである」は、全 `LLVMGen` の実装 78 個を走査して、関数の値に funptr を書き込む op が
`InlineLLVMFixBody` のほかに無いことを確かめた結果である。この走査は `impl LLVMGen for` の宣言を数え上げた
もので、README の A3 が「`impl LLVMGen for` は 78 個ある」と述べる同じ集合を対象とする。走査を検査に
掛ける機構は無いので、op が 1 つ足されたときにこの数え上げが破れることは、この文書の外では検出されない。
P27 の証明は L0 を引かないので、この穴は (R1)(R2)(R3) には及ばない。及ぶのは、D9 と D10 の「呼び出し先」を
実行時の関数と読むことが `borrow_ify` と `cancel` の静的な解決と食い違わない、という主張の方である。

**README への提案が 2 つ残る。**どちらも文言であって、証明の段には影響しない。

- **D22 のグローバルのアクセサの行。** 「返った値に `mark_global` で印を付けてから記憶域へ格納する」と
  あるが、`mark_global` を呼ぶのは `InitValue#<symbol>` の側であって、アクセサではない。
  `implement_rc_global` は初期化子の本体を `eval_rc_expr` で評価した直後、`build_return` の前に
  `self.mark_global(obj.clone())` を呼ぶ (`CODE src/rc_ir/codegen.rs: Generator::implement_rc_global`)。
  アクセサ (`acc_fn`) は `store_init_value` でその関数を呼び、返った値を記憶域へ格納するだけである。
  D24 が要る順序 -- 活性化が終わってから (E5) が走る -- はこのコードでも成り立つ。印を付けるのは本体の
  評価が終わった後だからである。
- **(E4) の行き先の 3 つ目。** (E4) の本文は「`b` を作ったのが環境 (E1) であれば、それらの参照は `E` に
  入る」と 2 つの場合を挙げる。(E7) が作った活性化の行き先は (E7) の本文が述べており ((E4) と同じく、
  ただし行き先は `E`)、2 つを合わせれば 3 つの場合を尽くす。(E4) の側にも (E7) を書けば、行き先の表が
  1 か所で読める。

**`proof_links.py` が README を走査しない。**この文書から README へ移った定義が引くコード -- 
`Generator::mark_global`、`Generator::build_release_boxed_with`、`build_free_boxed`、
`Generator::build_traverser_work`、`build_main_function`、`run_ios_runner`、
`ExportStatement::implement`、`Generator::build_rc_closure`、`InlineLLVMUndefinedInternalBody::generate`、
`Lowerer::lower_var`、`Generator::add_global_object` -- のうち、他の `p*.md` が引かないものは、
`citations.tsv` と `// PROOF:` の注釈から落ちる。`citations_of` が読むのは `p*.md` だけで、README の
本文の `CODE` は見ない (`dev-docs/proof/proof_links.py` の `citations_of`)。定義が引くコードにも
リンクを張るなら、走査の対象に README を足すことになる。
