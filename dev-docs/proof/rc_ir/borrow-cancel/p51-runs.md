...
「`H` の分解」を通じて引く。`borrow_ify` の出力について 3 番目の仮定を果たすのは P14b である。
README の P27 の言明はこの 2 つを挙げていない -- 第 6 節の (1) がその形を述べる。
...
ついて前者を果たすのは P14b である。P27 の言明を D12 だけの形に戻すには、P28 (a) を残る 2 つに依らない
形で示す道が要る。
...
**前者を取るときは、`cancel` の出力について限定を言う者も要る。** T が P27 に渡すのは
`cancel(borrow_ify(・))` の出力であり、限定を果たす P14b は `borrow_ify` の出力についての命題である。
P24 が挙げる保存の一覧に `borrowed_units` は無く、`cancel` がそれを保つことも、活性化を作る段の種類が
`cancel` を通って変わらないことも、どの命題も述べていない。
