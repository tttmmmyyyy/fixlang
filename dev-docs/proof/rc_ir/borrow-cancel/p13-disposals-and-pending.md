...
(S-b) は、どれも計数下のオブジェクトへの参照だけを対象とする」より、`b` の leaf は D8 の意味の参照を
持たない。よって D10 の初期値は空であり、`Release(b, [])` も終端の `Ret(z)` も `Obl` から参照を
取り除かないので、(S-a) は取り除く操作が無いことで、(S-b) は `Obl` が空のまま終わることで成り立つ。
(S-c) は A8 が与える -- グローバル状態のオブジェクトが解放されることは無いので、`Release(b, [])` が
触れる先はその触れる動作の直前の点で解放されていない。読む構文の側は計数下の場合と同じである。
D34 は計数下の類にしか `held_ρ` を定めず、A19 (i) が量化するのも計数下オブジェクトなので、この場合に
それらは掛からない。
