...
ある。**第 7.5.4 節がこの下限を読む** -- そこで書く (i') の形は、借用する終端の類について
`held_ρ(n, C) ≥ 1` を要求する。
...
**この文書がかつて差し戻した 6 点は、どれも README にすでに在る。** 直近の 3 点 (計数下の安定性、
A19 (ii-b) の形、`held` の帰属の委ね先) は、この文書が差し戻した後に README が答えたものである。
...
### 新しく差し戻す点 -- A19 (i) は P18a の要る段を与えない

**第 7.5.4 節の `<1>3a` を与える者が README に無い。** その段は
`H(O) ≥ Σ_{C ∈ S(n, O)} held_ρ(n, C)` であり、`S(n, O)` は固定した活性化の `obj(C) = O` である計数下の
別名類のうち開始の時点が `τ(n)` 以前であるものの全体である。A19 (i) の現在の文は次のとおりで、その総和は
`S(n, O)` のうち ρ-終端が借用する leaf である類を渡らない。

> **(i)** 各時点と各計数下オブジェクト `O` について、`H(O)` は、**その時点で生きているすべての活性化**の
> 別名類のうち `O` を指し、かつ **ρ-終端が借用する (D14) パラメータ・capture の leaf ではない**ものが持つ
> 参照の総数以上である。

除いた分は 0 と読めない -- `call_rc` が借用する unit の前に置く `Retain` (P11) が、借用する終端の類の
`held` を 2 にし、その要素は `App` の節点の入口で `pending` に在る。第 7.5.4 節がその位置を書く。

**要る形は第 7.5.4 節が (i') として書いた。** 各類 `C` について
`d(C) = held_ρ(n, C) - [C の ρ-終端が借用する leaf ならば 1]` と置き、生きている各活性化 `a` について

> `H(O) ≥ Σ_{C ∈ a の計数下別名類, obj(C) = O} d(C) + [a が obj(C) = O である借用終端の類を持つならば 1]`

を言う形である。第 7.5.4 節が、この形から `<1>1` の PROVE が出ることと、README が (i) の根拠として
名指す `Σ μ = Obl` の形がまさに `d` の側であることを述べる。**類ごと除く現在の (i) は、それより 1 段
強い除き方であり、借用する終端の類が `Retain` で得た分まで落とす。**
