...
<1>4. QED
  ...
  L14 の `(x, λ)` がその対である -- L14 の仮定より `(x, λ)` は `ρ` の上のスロットなので `λ` は inhabited であり、
...
より、A19 (i)・(ii-a)・(ii-b) の `held` はこの `held_ρ` である。D8 は同じオブジェクトへの参照を区別しないので、どの参照をどの類のものと数えるかは
...
<1>3. `pending(n)` のある要素 `p` について `B_ρ(n, p)[o] ≥ 1` である名前 `o` が `ρ` の上のスロットで
  ...
  であり、D33 より `(v, λ) ∈ C` である。よって `o` は `C` のスロットの `origin` の identity、すなわち `I(C)` の元で
  ある。
...
<1>5. `H(O) ≥ Σ_{C ∈ S(n, O)} d(C) + [S(n, O) に ρ-終端が借用するパラメータ・capture の leaf である類が
  ...
  固定した活性化に取ると L17 の `S(n, O)` がこれである。(i) の `d(C)` は `held(C) - [C の ρ-終端が借用する (D14) leaf ならば 1]` で
...
<1>4. CASE 消費が `App` の所有位置の引数以外 -- `App` の callee、`Closure` の capture、`Llvm` の
  ...
  <2>1a. `ren` を、`F` の本体の束縛変数には `rename` を当て、それ以外の名前 -- 直接呼び出しが名指す
    ...
    <3>3. `F` の本体に現れる名前 `y` と `FieldPath` `ν` について
      ...
      値に等しい。L8a は `vars` を「プログラムのいずれかの本体 (D23) の `VarTable`」と量化するので、`F` の本体についても
      `Pre(V)` についても読める。
  ...
  <2>3. `origin_V(p, μ)` の各候補 `(r, q)` について `owns_object(r, q)` は真である。
    ...
    <3>3a. `origin_V(p, μ)` の各候補 `(r, q)` について `q ∈ boxed_leaf_paths(ty(r), type_env)` である。
      <4>0. L7a、L8 の (i) と (B)、および L8a は、`Pre(V)` の `VarTable` について読める。
        BY L7a, L8, L8a
        L7a は `vars'` を「プログラムのいずれかの本体 (D23) の `VarTable`」と量化する。L8 は本体を `B'` と
