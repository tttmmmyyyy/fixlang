...
**結論を先に書く。P18b は A1・A2・D12 から証明できた。P18a は閉じていない。**
...
**A19 を引いても P18a は閉じない。** (ii-a) と (ii-b) から `Σ_C held ≥ N + 1` までは出る (第 7.5.4 節の
`<1>3`) が、そこから `H(O) ≥ N + 1` へ渡る段が要求するのは、その総和についての上界である。A19 (i) の
総和は ρ-終端が借用する (D14) パラメータ・capture の leaf である類を渡らないので、それを与えない。
第 7.5.4 節がその段で止まり、何が要るかを述べる。

### 7.1 局所の定義

この節から先で使う補題を `L7` から `L17` と番号を付ける (`L15` は使わない)。後から挟んだものには `L10a` のように枝番を
振る。`p60-insert-rc.md` の補題の名前は、README の A19 の文面を引用する箇所にだけ現れ、そこでは
...
- 7.5.4 が引くのは A19 の (i)、(ii-a)、(ii-b) である。(ii-b) は条件付きの形
  ...
  1 段で出す。
- **`<1>3` から先が閉じない。** A19 (i) はその総和から借用する終端の類を除くので、`<1>3` が作った
  `Σ_C held_ρ(n, C) ≥ N_ρ(n, O) + 1` の左辺を `H(O)` で抑えられない。7.5.4 がその位置で止まり、
  除いた分が 0 と読めないことと、どの形の (i) ならこの段を与えるかを述べる。
...
<1>1a. `origin(x, λ)` の計算を、memo を使わずに `origin_inner` の再帰呼び出しをそのまま展開して得られる
  ...
  **memo を使わない展開について言う必要がある。** `origin` は答えを `vars.origins` に記録して次から返す
  ので、実際の再帰は memo が当たった位置で止まるのに対し、ρ-歩み (DEF ρ-歩みと ρ-終端) は `origin_inner`
  が呼ぶ相手を辿るのでその先も辿る。`origin` が記録を行うのは `origin_inner` から**戻った後**なので、
  memo が当たる位置の答えは、それより前に戻り終えた呼び出しが計算したものである。memo を使わない展開は、
  実際の呼び出しの木の memo が当たった各葉を、その記録元の呼び出しの展開で置き換えて得られる。記録の
  順序についての帰納法で各展開が有限であることが出る -- 記録元の呼び出しの内側で当たる memo はそれより
  前に記録されたものだけであり、記録元の呼び出しの木は <1>1 より有限だからである。`origin(x, λ)` 自身の
  展開についても同じ置き換えが当たる。
...
グローバル値を借用位置の引数に渡す活性化では、限定の無い言明は偽になる」と述べる。**この文書の第 7 節は
P14a を使う段を持たない。**ここに引くのは、借用する終端の行が開始値だけを与えることと、その後の下限が
別の命題で与えられることを分けて示すためである。
...
>   別名類のうち `O` を指し、かつ **ρ-終端が借用する (D14) パラメータ・capture の leaf ではない**ものが
>   持つ参照の総数以上である。すなわち相異なる類の参照は相異なり、それは呼び出しの入れ子を跨いでも
>   相異なる。
...
**(i) の除外が第 7.5.4 節に効く。** (i) の総和は、ρ-終端が借用するパラメータ・capture の leaf である類を
**渡らない**。第 7.5.4 節が要るのは、除かない形の総和 -- 固定した活性化の `obj(C) = O` である計数下の類
**すべて**にわたる `held_ρ` の和 -- についての上界なので、この (i) からは出ない。
...
**この文書は `DEF 類ごとの参照` の表をその取り決めとして採り、A19 (ii-a) と (ii-b) の `held` を
`held_ρ` と読む。** A19 を引く段 -- `L14b` と第 7.5.4 節 -- は `DEF 類ごとの参照` をこの読みとして
併せて引く。

**(i) はこの読みの下でも (ii) と同じ量を数えない。** (i) が数えるのは、この読みの `held_ρ` のうち、
ρ-終端が借用するパラメータ・capture の leaf である類の分を**落とした**総和である。README はその理由を
「(ii-a)・(ii-b) と P14a は、借用する終端の類が `held ≥ 1` を持つ読みを要求する … (i) はその参照を
呼び出し元の類が既に数えているので、二重に数える。除外はこの食い違いを (i) の側で解く」と述べる。
すなわち 1 つの帰属の下で (i) と (ii) は別の集合を渡る和であり、この文書はその差を第 7.5.4 節で扱う。

##### L14b (`held` は `bumps` 以上である)

**言明**。計数下の別名類 `C` と `ρ` の上の節点 `n` について、`bumps_ρ(n, C) ≥ 0` であり、
`held_ρ(n, C) ≥ bumps_ρ(n, C)` である。
...
#### 7.5.4 A19 から P18a を出す段 -- `<1>3a` が閉じない
...
<1>1. SUFFICES ASSUME NEW `ρ` の上の節点 `n`、NEW 計数下 (D26) オブジェクト `O`、`N_ρ(n, O) ≥ 1`
  ...
  よる。**以下の `<1>2` と `<1>3` は、この ASSUME が導入した `n`、`O` と仮定 `N_ρ(n, O) ≥ 1` の
  下で書く。**
...
<1>3. `Σ_{C ∈ S(n, O)} held_ρ(n, C) ≥ N_ρ(n, O) + 1` である。
  BY A19, DEF A19 の `held` の読み, DEF 類ごとの参照, DEF bumps, DEF 節点の時点, L14b, <1>1, <1>2
  <1>1 と <1>2 より `bumps_ρ(n, C0) ≥ 1` である類 `C0 ∈ S(n, O)` が少なくとも 1 つ在る -- L14b より
  `bumps_ρ(n, ・)` は非負なので、和が 1 以上ならある項が 1 以上である。その `C0` については
  A19 (ii-b) が `held_ρ(n, C0) ≥ 1 + bumps_ρ(n, C0)` を直接与える -- DEF A19 の `held` の読みより
  A19 の `held` はこの文書の `held_ρ` であり、DEF bumps より A19 の `bumps` は `bumps_ρ` であり、
  DEF 節点の時点より `τ(n)` は A19 の言う時点の 1 つである。他の各類 `C ∈ S(n, O)` については L14b が
  `held_ρ(n, C) ≥ bumps_ρ(n, C)` を与える。`S(n, O)` の各類は開始の時点が `τ(n)` 以前なので
  `held_ρ(n, ・)` を持つ (DEF 類ごとの参照)。足し合わせると
  `Σ_C held_ρ(n, C) ≥ Σ_C bumps_ρ(n, C) + 1 = N_ρ(n, O) + 1` である。**`+1` は 1 回しか立たない** --
  (ii-b) を使うのは `C0` の 1 つだけで、他の類には L14b を使うからである。

**`<1>3a` -- `H(O) ≥ Σ_{C ∈ S(n, O)} held_ρ(n, C)` -- を与える者が無い。** 残る段はこれだけであり、
これが出れば `<1>3` と繋いで `<1>1` の PROVE が出る。**A19 (i) はこれを与えない。** (i) の総和は
`S(n, O)` のうち **ρ-終端が借用する (D14) パラメータ・capture の leaf である類を渡らない**からである。

**除いた分は 0 と読めない。** 借用する終端の類も `Retain` で増える。`call_rc` は、呼び出し元が借用し
呼び出し先が所有する unit の前に `Retain` を置く (P11)。借用版 `V` がパラメータ `b` の unit `u` を借用し、
その本体が `App(h, [b])` (`h` は対応するパラメータの `u` を所有する) を持つとき、`V` の本体にはその
呼び出しの直前に `Retain(b, u)` が立つ。その `App` の節点 `n` の入口では、その要素は `pending` に在るので
`bumps_ρ(n, C_b) = 1`、`held_ρ(n, C_b) = 2` (開始の 1 と `Retain` の 1) であり、`N_ρ(n, O) = 1` である。
A19 (i) の総和は `C_b` を渡らないので、呼び出し元の類が持つ 1 しか与えず、`H(O) ≥ 2` に届かない。
**実行時のカウントは 2 である** (呼び出し元が持つ 1 と `Retain` の 1) ので、P18a 自身がこの位置で偽に
なるわけではない。足りないのは (i) の側である。
...
`d` はちょうどそれを引く。README も「除外した後の (i) は義務集合の水準の主張になり、
`p60-insert-rc.md` の第 14 節が示すのはまさにその形である (`L34` が `Σ μ = Obl` を与える)」と書く。
角括弧の 1 は、その活性化が借りている参照である -- P14a よりその類は参照を少なくとも 1 つ持ち、その 1 つは
`Obl` に入らないので、P28 (処分されていない各参照の持ち手はちょうど 1 つ) よりその持ち手はこの活性化では
ない。`H(O)` は持ち手を問わず `O` への参照を数えるので、両者を足したもの以上である。

**類ごと除く (i) は、これより 1 段強い除き方である。** 借用終端の類が `Retain` で得た分まで落とすので、
上の `V` の例で 1 だけ足りない。README が (i) の根拠として名指す `Σ μ = Obl` の形は `d` の側であり、
(i') はその形に角括弧の 1 を足したものである。

**README の (i) の反例は (i') を破らない。** README が挙げる `f_borrow` を呼ぶ `g` の形では、
`f_borrow` の活性化について `d(C_b) = 1 - 1 = 0` で角括弧が 1 なので `H(O) ≥ 1` を、`g` の活性化に
ついて `d(C_a) = 1` で角括弧が 0 なので `H(O) ≥ 1` を要求し、どちらも `H(O) = 1` に収まる。1 つの参照を
2 つの借用位置へ渡す `App(f, [a, a])` の形でも、借りた側の活性化は `d(C_b1) = d(C_b2) = 0`、角括弧 1 で
`H(O) ≥ 1` しか要求しない。**(i) を活性化ごとの言明にしても、`d` を取らずに `held` をそのまま足すと
この形で偽になる** -- `held(C_b1) + held(C_b2) = 2 > 1 = H(O)` である。
  ...
  <2>3. `origin(p, μ)` の各候補 `(r, q)` について `owns_object(r, q)` は真である。
    ...
    <3>3a. `origin_V(p, μ)` の各候補 `(r, q)` について `q ∈ boxed_leaf_paths(ty(r), type_env)` である。
      <4>1. `λ ∈ boxed_leaf_paths(ty(x))` であるすべての対 `(x, λ)` について、`origin(x, λ).acted_on()`
        ...
        `origin(x, λ)` の計算の再帰についての帰納法で示す。P2 より `origin` は停止するので再帰の木は
        有限であり、この帰納法は整礎である。L8 (A) の (i) と (B) はどちらも `ρ` を読まないので、
        `ρ` の上で値を得ていない変数 -- `ρ` が選ばなかったアームの結果変数と、その先 -- についても
        使える。
...
### 新しく差し戻す点 -- A19 の `held` に帰属の取り決めが無い

A19 (ii-b) の現在の文は次のとおりである。

> **(ii-b) 帳簿の形** -- 読む者: P18a、P19、P21。各時点と各計数下の別名類について、走査がその類に
> ついて `pending` に数えている bump の個数を `bumps`、その類が持つ参照の個数を `held` とすると、
> **`bumps ≥ 1` である時点では `held ≥ 1 + bumps` である。**

`bumps` の側は D27 が帰属を定めるので個数が定まる。**`held` の側にはそれが無い。** D8 は「同じ
オブジェクトへの参照どうしは互いに区別されない」と述べるので、「その類が持つ参照の個数」は、どの参照を
どの類のものと数えるかを決めない限り定まらない。1 つのオブジェクトを 2 つの別名類が指す本体 --
第 7.5.8 節の `C2` の `C_o` と `C_y` がそれである -- では、`H(O) = 1` の時点で「その類が持つ参照の
個数」は取り決め次第で 0 にも 1 にもなる。

この文書は `DEF 類ごとの参照` の 6 行の表をその取り決めとし、`DEF A19 の held の読み` でそれを宣言して
A19 を引いた。**README の A19 は、`held` をこの表として名指す必要がある。**書き直しの案は、(ii-b) と
(i) の前に次の 1 文を置くことである。

> `held` は `p13-disposals-and-pending.md` の `DEF 類ごとの参照` が定める帰属である -- 別名類の
> ρ-終端が生成・パラメータ・capture から得る 1 つを開始値とし、その類のスロットを名指す `Retain` で
> 増え、`Release` と D9 の消費で減る。`bumps` について D27 が走査自身の帰属を採るのと同じ形である。

`別名類` の定義を A19 が `p13` の第 7.5 節に委ねているのと同じ委ね方であり、この文書はその 1 文の下で
A19 を読んでいる。
