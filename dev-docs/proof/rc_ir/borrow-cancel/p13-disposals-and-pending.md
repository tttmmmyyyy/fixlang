...
- **(b)** 計数下の別名類 `C` について、`I(C) = {origin(x, μ).identity() : (x, μ) ∈ C}` と置くと
...
<1>2. `C` が計数下の別名類であるとき、`I(C)` の各元 `o` は `ρ` の上のスロットであって
      `C_ρ(o) = C` である。
  BY L12a, L14, D33
  `I(C)` の元は `C` のスロット `(x, μ)` についての `origin(x, μ).identity()` である。L12a より
  `obj(x, μ) = obj(C)` であり、`C` は計数下なのでこれは計数下である。L14 の後半は「さらに
  `obj(x, λ)` が D26 の意味で計数下であるとき、`o` は `ρ` の上のスロットであって
  `C_ρ(o) = C_ρ(x, λ)` であり、`o` は `ρ` で活性 (DEF 名前の活性) であって `obj_ρ(o) = obj(x, λ)` で
  ある。」と述べるので、`o` は `ρ` の上のスロットであり、D33 より `C_ρ(o) = C_ρ(x, μ) = C` である。
...
<1>3. `pending(n)` のある要素 `p` について `B_ρ(n, p)[o] ≥ 1` である名前 `o` が `ρ` の上のスロットで
  ...
  計数下である。L14 の後半より `C_ρ(o) = C_ρ(v, λ)` なので `C_ρ(v, λ) = C` であり、D33 より
  `(v, λ) ∈ C` である。よって `o` は `C` のスロットの `origin` の identity、すなわち `I(C)` の元で
  ある。
