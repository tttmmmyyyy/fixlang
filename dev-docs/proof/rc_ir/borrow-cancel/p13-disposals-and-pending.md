...
範囲を絞って P18a を示す。第 7.5.1 節がこの制限と、それが下流に渡すものを述べる。
...
- **この節が示すのは、実行 (D24) が作る活性化についての P18a である。** A19 (i) は「その時点で生きている
  ...
  在る。7.5.1 がその制限を置き、それが下流に渡すものを述べる。
...
- `A19` を果たす者は `insert_rc` と `borrow_ify` である。**この文書が示すのは `borrow_ify` の側だけで
  ...
  置く。`insert_rc` の側は `p60-insert-rc.md` が示す (README の A19)。
...
作らない活性化について (i) は何も言わない。

**D21 の「活性化は、開始の時点で A19 (i) の不等式を満たすものに限る」の節は、この制限の代わりに
ならない。** 7.5.4 の `<1>5` が (i) を当てるのは `τ(n)` -- 活性化の位置が `n` に着いた直後の時点 --
であって、開始の時点ではない。D21 自身が「残る義務は、活性化の間それが保たれることである」と書くとおり、
開始の時点の不等式から `τ(n)` の不等式へ渡る段は別に要る。この文書はその段を持たず、A19 (i) を
`τ(n)` で直接引く。

**第 7.4 節の P18b はこの制限を要らない** -- P18b が引くのは L11 と P6 であり、そのどちらも 1 つの
本体と 1 つの実行路の上で閉じる。

**この制限が下流に渡すもの。** P18a を読むのは P19 と P21 であり、P21 を読むのは P23 である。制限された
P18a からは、実行が作る活性化についてしか P21 (b) が出ないので、P23 の結論も同じ範囲に狭まる。
...
**ρ-歩みの各段は D20 の別名の辺である。これが D33 がこの文書に残す 1 つ目である。** D20 が挙げる
6 種の辺は、L8 (A) が挙げる `origin` を呼ぶ 6 つの腕 (`Move`、`Join` の取ったアーム、unbox 容器の
`Field`、unbox union の変位 `Payload`、catch-all の `Payload`、`Llvm` の単一 `Arg`) にちょうど
対応する。
...
<1>1. `o` は `ρ` の上の位置 (D6) である。`o` が記号の位置であるときは `o = T_ρ(x, λ)` であり、`o` が
      指すオブジェクトはグローバル状態である。
  ...
  指すのは funptr かグローバル状態のオブジェクトであり (D6)、funptr の型は boxed leaf を持たないので
  (D4 の第 1 規則) `σ` はグローバル状態のオブジェクトを指す。`u` が束縛を持たないとき `origin_inner` は
  `None` の腕に入って `origin` を呼ばないので、DEF ρ-歩みと ρ-終端 より `(u, σ)` は ρ-終端である。
...
<1>3. `obj(o) = obj(C_ρ(x, λ)) = obj(x, λ)` である。さらに `obj(x, λ)` が D26 の意味で計数下であるとき、
  ...
  `o = T_ρ(x, λ)` であり、L12a の `obj(C_ρ(x, λ)) = obj(T_ρ(C_ρ(x, λ)))` がそのまま
...
<1>4. 束縛を持たない名前 (`None` の腕) を ρ-終端とする別名類は計数下ではない。よって計数下の `C` の
  ...
  オブジェクトである。funptr の型は boxed leaf を持たない (D4 の第 1 規則) ので `σ` が指すのは
...
<1>5. `H(O) ≥ Σ_{C ∈ S(n, O)} d(C) + [S(n, O) に ρ-終端が借用するパラメータ・capture の leaf である類が
      在るならば 1]` である。
  BY A19, D23, D24, D34, DEF 節点の時点, L12a, L17,
     <1>1, <1>2, <1>2a
  ...
  D34 より A19 の `held` はこの文書の `held_ρ` なので、<1>2a の `d` と同じものである。`obj(C)` が
...
(D3) と、`ρ_V` を辿る 1 回の活性化 (D21) を固定する。`(p, μ)` をその活性化のスロット (D6) -- `ρ_V` の
