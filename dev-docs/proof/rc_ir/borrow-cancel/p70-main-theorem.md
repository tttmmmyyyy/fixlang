    ...
    PROVE
      ...
      (T3) `X0`・`X1`・`X2` をそれぞれ `p0`・`p1`・`p2` の**実際の**実行 -- 観測点が返す値が
           ...
           **`C` は `X1` の段の列の接頭とは限らない。** D30 の `π` は `borrow_ify` か `cancel` の
           ...
           対応させた `X0` の観測の値が真ならば、`C12` がその観測に対応させた `X2` の観測の値も
           真である。この 3 つの観測が起きる 3 つの観測点は、D19 の 2 つの対応を合成したもので
           結ばれている。
...
(H4-H6 は `<1>9` の証明の中でだけ立つ。)
  ...
  **段の中で起きる観測については、それを含む段が除かれる段でありうる。** D30 が並びから除くのは
  ...
  入らない。この段が言うのは `C` の段の上の観測についてだけである。

  **その段の節点が観測点であるものについては、除かれる段でありえない。** 一意性の観測点は `Llvm` の
  演算 1 つが現れる位置であり (D18)、D19 は `borrow_ify` と `cancel` について「値を計算する構文を
  作らず、消さず、並べ替えない」と述べ、P24 は「本体について書き換えが変えるのは、`Retain`/`Release`
  の節点と、`App` の callee の名前だけである」と述べる。よってどちらのパスも観測点の節点を消さず、
  入れない。
