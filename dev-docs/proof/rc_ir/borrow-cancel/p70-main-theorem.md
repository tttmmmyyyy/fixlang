...
第 3 節が、T が立つ 24 個の仮定を果たす者とともに 1 か所に並べる。第 4 節が、T が届かないところを
...
**T** (パイプラインが保存するもの)
    ...
    ASSUME NEW p0 : RcProgram, NEW type_env : TypeEnv, NEW develop_mode : bool,
      ...
      H3. 第 4 節の仮定がすべて成り立つ。第 4 節が挙げるのは A1 から A24 までの 24 個であり、
          そのうち A1 と A2 を `p0` について述べたものが H1 と H2 である (第 3 節が 24 個すべてを
          並べる)。
    PROVE
...
### <1>1. `optimize_rc_program` が `borrow_ify` に渡すプログラムは `p0`、`cancel` に渡すプログラムは `p1` であり、`cancel` の返り値は `p2` である
  ...
  **<2>3. QED**
  ...
  次の文 `validate(&prog, "after split_rc_units")` は `prog` を変えない (`<2>2`)。`if` の条件式も
  `prog` を読まない (`<2>1`)。よって `if` の本体の 1 行目に着く時点の `prog` は `p0` である。

  その 1 行目 `prog = borrow_ify(&prog, type_env, config.develop_mode)` は、第 1 引数に `p0` を
  渡す。T の `type_env` と `develop_mode` は自由変数 (`NEW type_env : TypeEnv`,
  `NEW develop_mode : bool`) なので、`type_env` を `optimize_rc_program` の引数 `type_env` に、
  `develop_mode` を `config.develop_mode` に取れば、この行の後の `prog` は第 1 節の `p1` である。
  ...
  同じ `type_env` なので、その返り値は第 1 節の `p2` である。`<2>1` より、この 6 行の間に `prog` に
  書く文はほかに無く、`borrow_ify` と `cancel` を呼ぶ位置もそれぞれこの 1 つである。
...
(H4-H6 は `<1>9` の証明の中でだけ立つ。)
  ...
  **<2>3.** `cancel` の実例 (H6) について、次の 2 つが出る。
...
T は、引用する命題が証明されている仮定の集合の上に立つ。README 第 4 節の仮定は A1 から A24 までの
24 個あり、その全部がここに載る。「果たす者」は README 第 4 節の言葉である。
...
挙げている。A4 と A12 には検査が無い。

**README が「検査: 無し」と書き、かつ果たす者が居るのは A16・A17・A19・A20・A22・A23 の 6 つで
ある。** A18 (b) にも「検査: 無し」と書いてあるが、A18 は果たす者が居ない側に入る。A20 は呼び出し元の
側の契約であり、この文書のどの命題も、出力の呼び出し元がそれを満たすことを示していない。
...
あることは P26 の第 2 文が言う (`<2>3`)。`C01` の側については向きを言う者が居ない。P26 が
...
A13** を挙げる。**A3** の `applies_a_function_operand` の検査と、**A10** の「最適化が作る型の
再検査」も同じである。すなわち出荷ビルドでは、この **5 つ**の仮定は果たす者だけに立つ。
