...
(H4-H6 は `<1>9` の証明の中でだけ立つ。)
  ...
  **`e1` に `e0` と `e2` を対にするのは D30 の観測の節である** -- 「対応する 2 つの活性化の対応する
  節点で起きる一意性の観測 (D18) は、互いの対である」。段の節点が観測点である場合は段の対応がその対を
  与え、(F) の解放が作る活性化の中で起きる観測については、その活性化どうしが、それを作る動作が素動作の
  列の元であることから対になるので、その活性化の節点の対応がその対を与える。段の素動作の並びが観測を
  対にするのではない -- 一意性の観測は素動作の 6 種のどれでもなく、活性化を作る動作でも終わる動作でも
  ない。`<2>2` と `<2>3` が「`C01` が対応させる」「`C12` が対応させる」と書くのはこの対である。
