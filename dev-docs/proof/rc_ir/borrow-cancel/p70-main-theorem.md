...
### <1>7. (T2)
  ...
  **<2>1a.** `p2` の各関数の `borrowed_units` と `capture` は `p1` の対応する関数のものに等しく、
  `p2` の各本体が `p1` の本体と違うのは `Retain`/`Release` の節点と `App` の callee の名前だけで
  ある。

  P24 は `cancel` について「`cancel` は `RcFunc` の `body` 以外の欄を 1 つも変えない。とくに
  `borrowed_units` と `capture` は入力のものに等しい」と述べ、本体については「書き換えが変えるのは、
  `Retain`/`Release` の節点と、`App` の callee の名前だけである」と述べる。`<1>1` より `cancel` が
  受け取るのは `p1` であり、返すのは `p2` である。
  ...
  **<2>1b.** `p2` の実行 (D24) において、借用する unit を持つ本体の活性化を作る段は、(E3) の
  呼び出しの段に限る。
    ...
    **<3>1.** (E7) の段は、借用する unit を持つ本体の活性化を作らない。

    D24 の (E7) が作るのはグローバル初期化子の `init` の活性化であり、D1 より `init` はパラメータも
    capture も持たない。D14 が所有と借用を割り当てるのは関数の各パラメータ・capture の各 unit なので、
    `init` は借用する unit を 1 つも持たない。

      BY D1, D14, D24

    **<3>2.** `p1` の実行において、借用する unit を持つ本体の活性化を作る段は (E3) に限る。

    P14b の言明が `borrow_ify` の出力の実行についてこれを述べ、`<1>1` より `p1` は `borrow_ify` の
    出力である。
    ...
    **<3>3. QED**
    ...
    P24 の `cancel` の節が、`<3>2` の結論を `cancel` の出力へ渡す。その節は `borrowed_units` と
    `capture` の一致を述べたうえで「P14b の結論が `cancel` の出力へ渡るのはこの節による」と述べる。
    `<2>1a` がその一致と、本体が `Retain`/`Release` の節点と `App` の callee の名前でしか違わない
    ことを `p1` と `p2` について与える。`<1>1` より `p2` は `cancel` の出力である。**この段が
    P14b の量化範囲の外に出ていることは、第 4 節が述べる。**(E7) については `<3>1` が、この段に
    依らずに同じことを与える。

      BY <1>1, <2>1a, <3>1, <3>2, P24
...
| A19 | bump の下に余りが在る | (ii-a) と (ii-b) は `insert_rc`・`split_rc_units`・`borrow_ify` の 3 人 (**検査: 無し**)。(i) は仮定ではなく D21 が活性化に課す制限であり、実行が作る活性化がそれを満たすことを P28 (b) が示す | (ii-a) は P14・P18c、(ii-b) は P18a・P18c・P19・P21。(i) の不等式は D21 が活性化への制限として読み、P28 (b) がそれを実行の作る活性化について示す |
...
> 活性化に課す制限であり (第 4 節)、実行が作る活性化がそれを満たすことを P28 (b) が示す。
> **(ii-b) は前提 (N) -- 名前は別名類を決める -- の下でしか閉じていない。**
...
**残っているのは (ii-b) が立つ前提 (N) である。** README 第 7 節は、(N) を
`p13-disposals-and-pending.md` の `L14` の後半が与えると述べ、「**(N) を前提として残す理由は無い**」と
書くが、その後半を `p60-insert-rc.md` が読んでいないことも同じ節が述べる。A19 は T の前提 H3 に
入っているので、**(T1) から (T4) までのすべては、(N) をそこに持つ。**
...
### (T2) が P14b の量化範囲の外に出る 1 段

P27 の前提のうち「借用する unit を持つ本体の活性化を作る段が (E3) に限られる」を `p2` について
与えるのは `<1>7` の `<2>1b` である。**P14b がこれを述べるのは `borrow_ify` の出力の実行に
ついてであり、`p2` は `cancel` の出力である。**2 つを繋ぐのは P24 の `cancel` の節であり、その節が
述べるのは `RcFunc` の欄の一致 (`borrowed_units` と `capture`) と、本体が `Retain`/`Release` の
節点と `App` の callee の名前でしか違わないことである。**欄と本体のこの一致から、実行についての
言明である P14b の結論が `p2` の実行について出ることを、P24 のほかに述べる者は居ない。**その一文を
持つのは P24 の節自身 --「P14b の結論が `cancel` の出力へ渡るのはこの節による」-- である。

**この 1 段の外は P14b に依らない。**(E7) の段については `<1>7` の `<2>1b` の `<3>1` が、D1 と D14 と
D24 だけで同じことを与える。残るのは、関数の値から呼び出し先を取る 3 種 -- (E1)、オペランドを
適用する `Llvm` の段 ((E2))、(F) の `Destructor` -- であり、そこがこの 1 段に載っている。

**範囲の中へ入れる形は、P14b の量化範囲を広げることである。**「`borrow_ify` の出力と、`cancel` が
それを写したプログラム」-- A19 が (ii-a) と (ii-b) を仮定する範囲、および P23 が入力に置く条件と
同じ形 -- にすれば、`<1>7` の `<2>1b` は P14b だけを引き、P24 の `cancel` の節は `borrowed_units` が
`cancel` を通ることを述べる `<1>7` の `<2>1a` にだけ要る。
