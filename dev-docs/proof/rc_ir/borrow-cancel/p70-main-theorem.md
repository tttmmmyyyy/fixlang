...
欄の値の等しさは種類ごとに読む。**本体** (`body` と `init`) は下の木としての等しさで読む。**型**
...
**どの位置とどの位置が同じ `Arc` を指すかは、この等しさの成分ではない。**D2 は本体の木の位置を節点と
呼び、位置が相異なれば節点も相異なるものとするので、値を決めるのは位置とその内容だけであり、共有の
度合いはそのどちらでもない。**内部可変性を持つ欄が保持する memo も成分ではない。**その欄は一度だけ
書かれる memo であって、その値はその型の `PartialEq` が読む成分の関数だからである (A3 の「値の
等しさ」の節)。
