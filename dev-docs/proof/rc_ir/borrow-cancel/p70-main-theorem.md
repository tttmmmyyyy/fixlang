...
対象コミット: `30ef78ce1e9fcd68b272708f7e7bbe95a0d940ae`
    ...
    PROVE
      ...
      (T3) `X0`・`X1`・`X2` をそれぞれ `p0`・`p1`・`p2` の**実際の**実行 -- 観測点が返す値が
           ...
           **`C` は `X1` の段の列の接頭とは限らない。** D30 の `π` は `borrow_ify` か `cancel` の
           ...
           真である。**観測どうしを対にするのは D30 である** -- 共通接頭の段が 1 対 1 に並び、
           並んだ 2 つの段の素動作も 1 対 1 に並ぶ。この 3 つの観測が起きる 3 つの観測点は、
           D19 の 2 つの対応を合成したもので結ばれている。
...
(H4-H6 は `<1>9` の証明の中でだけ立つ。)
  ...
  **<2>4.** `C` の段の上で起きる一意性の観測 (D18) は、`C01` の上の観測でもあり、`C12` の上の
  ...
  D30 は 2 つの実行の段を先頭から順に 1 対 1 に並べ、伸びる限りの部分を共通接頭と呼ぶ。`C` の
  定め方より、`C` の段は `C01` の中で `X0` の段と対になり、`C12` の中で `X2` の段と対になっている。
  すなわちその段は 2 つの共通接頭のどちらにも入っている。
...
| **A3** | 宣言されたモデルの忠実さ | **誰も**。ただし `applies_a_function_operand` については `Generator::apply_lambda` の develop mode の検査が、`result_prov` の元数については `validate` の `check_rhs` の develop mode の検査が果たす | D10、D21、D24、D29、D30、P5 (a)。元数の節を読むのは「名前は別名類を決める」である (第 7 節) |
...
| A17 | 環境の契約 | 環境のコード (**検査: 無し**) | D21 (`H` の外から来る増減)、D24 の「実行の最初の時点」と (E8)、P27 の言明。(i-d) は `p50-observation.md` の `L9b`・`L10` |
...
| A20 | 借りた参照は活性化の間 生きている | 呼び出し元 (**検査: 無し**) | A19 (i)、P14 の (S-c)、P14a、P14b の項、P27 と P28 の言明の前提 |
...
| A26 | 節点は、読んでから手放す | コード生成 (**検査: 無し**) | `p20-borrow-ify.md` の第 10.7 節 (P14 の (S-c)) |
...
**よって A19 の 2 節はどちらも果たす者を持つ。** (ii-a) を果たすのは `insert_rc`・`split_rc_units`・
`borrow_ify` の 3 人であり、(ii-b) は無条件に閉じている。T の前提 H3 が A19 を束ねるとき、束ねる先は
この形の A19 である。
...
節点そのものが観測点である場合はこの穴が無い** -- どちらのパスも観測点の節点を消さず、入れないから
である (`<1>9` の `<2>4`)。
...
閉じることを意味しない。**その表で `検証` の欄が `未着手` なのは P2a・P26 (`cancel` の半分)・T の
3 行であり、そのうち T が引くのは P26 (`cancel` の半分) の 1 つである。**この数え上げは検証が 1 周
進むたびに動く。読むときは README 第 7 節の表を読み直すこと。**
