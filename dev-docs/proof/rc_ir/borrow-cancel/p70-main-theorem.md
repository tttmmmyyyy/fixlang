...
| A19 | bump の下に余りが在る | (ii-a) は `insert_rc`・`split_rc_units`・`borrow_ify` の 3 人 (**検査: 無し**)。(ii-b) は無条件に閉じている (第 7 節)。(i) は仮定ではなく D21 が活性化に課す制限であり、実行が作る活性化がそれを満たすことを P28 (b) が示す | (ii-a) は P14・P18c、(ii-b) は P18a・P18c・P19・P21。(i) の不等式は D21 が活性化への制限として読み、P28 (b) がそれを実行の作る活性化について示す |
...
閉じることを意味しない。**その表で `検証` の欄が `未着手` なのは P2a・P14b・P26 (`cancel` の半分)・
P28・P29・P30・T の 7 行であり、そのうち T が引くのは P14b と P26 (`cancel` の半分) である。
