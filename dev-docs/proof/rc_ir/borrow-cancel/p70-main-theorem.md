...
- **命題**: P9・P14・P14a・P23・P24・P26・P27。
- **定義**: D1・D12・D14・D18・D19・D23・D30。
- **仮定**: A1・A2 (前提 H1・H2 として)、A13 と A19 (前提 H3 が束ねる A3-A21 の中から名指す)。
- **コード**: `optimize_rc_program` が 2 つのパスを呼ぶ順序、`validate`、`borrow_ify` と `cancel` が
  `RcProgram` を組み立てる 3 つの欄、`clone_func`、`borrow_funcref`、`RcProgram` の型。

T の言明はこのほかに、D7・D21・D24・D25・D26・D31 を名前で引く。

第 3 節が、T が立つ 21 個の仮定を果たす者とともに 1 か所に並べる。第 4 節が、T が届かないところを
...
**T** (パイプラインが保存するもの)
    ...
    ASSUME NEW p0 : RcProgram, NEW type_env : TypeEnv, NEW develop_mode : bool,
      ...
      H3. A3 から A21 までの 19 個の仮定が成り立つ (第 3 節が A1 から A21 までの 21 個すべてを
          並べる)。
    PROVE
      ...
      (T3) `X0`・`X1`・`X2` をそれぞれ `p0`・`p1`・`p2` の**実際の**実行 -- 観測点が返す値が
           ...
           段が訪れる 3 つの観測点は、D19 の 2 つの対応を合成したもの (`<1>8`) で結ばれている。
      ...
      (T4) `p0` から `p2` を作る 2 つのパスの合成は、D12 が見ない部分について次を保つ。
           ...
           - **関数**: `p2.funcs` の各エントリには、P24 の「出力の各関数は入力のちょうど 1 つの
             ...
             **`p1` と `p2` は `p0` に無い関数を持つ。**`borrow_ify` は借用版を足すからである
             ...
             異なる (`<1>10` の `<2>1a`)。この言明が借用版について述べるのは、上の 4 つの欄を
             複製元の関数と共有することだけである。
...
### <1>1. `config.enable_borrow_optimization()` が真のとき、`optimize_rc_program` が `cancel` に渡すプログラムは `p1` であり、その返り値は `p2` である
  ...
  **<2>3. QED**
  ...
  ある。`<2>1` より、この 6 行の間に `prog` に書く文はほかに無い。
...
である。`<1>6` が第 1 の前提を `p2` について与え、H3 が A17 と A18 を与える。
...
(H4-H6 は `<1>9` の証明の中でだけ立つ。)
  ...
  **<2>2.** P26 を `borrow_ify` の実例 (H5) に当てると、次の 2 つが出る。

  - `C01` の上の各一意性の観測点 (D18) で、`X0` の観測値と `X1` の観測値は**等しい**。とくに
    `X0` の観測値が真ならば、D19 が対応させる `p1` の観測点の `X1` における観測値も真である。
  - `C01` が終わる出口は (X1) ではない。

  P26 は第 2 文 -- 出口が (X1) か (X2) であり、その向きが観測値の改善する向きであること -- を
  `borrow_ify` については主張しない。P26 の言明がそう書いている。
  ...
  **<2>3.** P26 を `cancel` の実例 (H6) に当てると、次の 2 つが出る。
  ...
  **<2>4.** 一意性の観測点を訪れる `X1` の段は、H5 の対にも H6 の対にも現れる。

  D30 が 1 対 1 に並べる段から除かれるのは、「`π` が消した節点と入れた節点の段」である。一意性の
  観測点は `Llvm` の演算 1 つが現れる位置である (D18)。D19 は `borrow_ify` と `cancel` について
  「値を計算する構文を作らず、消さず、並べ替えない」と述べ、P24 は「本体について書き換えが変えるのは、
  `Retain`/`Release` の節点と、`App` の callee の名前だけである」と述べる。よってどちらのパスも
  観測点の節点を消さず、入れないので、観測点を訪れる段はどちらの対からも除かれない。

  **除かれる段は 2 つの実例で別である。** `C01` の側で除かれるのは `borrow_ify` が消した節点と
  入れた節点 (`call_rc` が置く `Retain`/`Release`) の段であり、`C12` の側で除かれるのは `cancel` が
  消した節点 -- そのうちには `p0` から来て `borrow_ify` が残した節点もある -- の段である。この 2 つは
  同じ集合ではないので、2 つの対の定義域は一方が他方を含むとは限らず、`C01` と `C12` を `X1` の段の
  列の接頭として比べることはできない。この段が言うのは、観測点を訪れる段については両方に現れると
  いうことだけである。
  ...
  **<2>5. QED**
  ...
  `X2` の段 `s2` と対になっている。`<2>4` より、観測点を訪れる段はどちらの対からも除かれない。
  すなわち `C` は、2 つの共通接頭がともに及ぶ範囲にある観測点の段を、対が無いという理由で落とす
  ことがない。
...
### <1>10. (T4)
  ...
  **<2>1a.** 第 2 のループが入れる各借用版の名前は、`p0.funcs` のどの鍵とも、`p0.funcs` のどの
  ...
  `borrow_funcref` は元の名前の `name` フィールドの文字列に `"#borrow"` を継ぎ足す
  ...
  `<元の名前>#borrow` として作ることを挙げる。A13 は「**束縛名に限らない**」と明示し、直接呼び出しが
  名指す関数の名前もその範囲に入れる。`p0.funcs` の鍵も、そこに収まる各関数の `name` も、
  `borrow_ify` の入力に現れる名前であるから、A13 が当たる。

  P24 も同じことを別の言葉で述べている -- 関数の対応を鍵の一致で述べられない理由として
  「`borrow_ify` は入力に無い関数 (借用版) を足すからである」と書く。

    BY H3, A13, P24, `CODE src/rc_ir/borrow.rs: borrow_funcref`
...
T は、引用する命題が証明されている仮定の集合の上に立つ。README 第 4 節の仮定は A1 から A21 までの
21 個あり、その全部がここに載る。「果たす者」は README 第 4 節の言葉である。
...
| **A12** | 束縛の形と型が合っている | **誰も** (検査するコードは無い) | A3。A12 自身の項が P2 と `held_field_type`、`rhs_consumes` の停止性を挙げる |
| A13 | 名前の形 | `Lowerer::fresh_var` と `clone_fresh` (検査は develop mode の `check_clone_names_are_fresh`) | P9 の後半。このファイルの `<1>10` の `<2>1a` |
| A14 | 過適用が無い | 型検査と lowering | A14 自身の項が `call_rc` と `rhs_consumes` を挙げる |
...
| A21 | 関数の値を作る演算 | `builtin.rs` の op の集合 (検査は develop mode の `apply_lambda` の表明) | P27 (第 7 節: 「`L0` の数え上げは A21 が片付けた」)。A21 自身の項が「実行時の呼び出し先を静的な名前から決める議論」を挙げる |
...
**README が「検査: 無し」と書き、かつ果たす者が居るのは A16・A17・A19・A20 の 4 つである。**
A18 (b) にも「検査: 無し」と書いてあるが、A18 は果たす者が居ない側に入る。A20 は呼び出し元の側の
契約であり、この文書のどの命題も、出力の呼び出し元がそれを満たすことを示していない。

**A19 の 3 つの節はどれも果たす者を持つ。** README 第 7 節が「A19 は 3 節とも果たす者を持つ --
(ii-a) と (ii-b) は `insert_rc`・`split_rc_units`・`borrow_ify` の 3 人、(i) は同じ 3 人が P28 の
上で果たす」と述べる。README 第 4 節が、果たす作業の状態として次の 4 つを述べる。
...
`C12` が終わるかであり、`C12` の側については P26 の第 2 文が向きを言う -- 出口は (X1) か (X2) で、
どちらも観測値が改善する向きである (`<2>3`)。`C01` の側については向きを言う者が居ない。P26 が
...
「**共通接頭が終わる出口の向きは、合成については言えない。**」と書き、その半分を閉じる判定と代償を
第 9 節に記録している。

**接頭の中では「一意だったものが共有になる」ことは起きない。** これは `<2>2` と `<2>3` の第 1 項が
与えるもので、出口の向きとは別の主張である。
...
> `borrow_ify` の入力に現れる**すべての名前**について、その `name` フィールドを `#` で区切った
> 最後の断片は、文字 `b` の後に 10 進数字だけが続く形ではなく、`borrow` でもない。後者は
> `borrow_funcref` が借用版の名前を `<元の名前>#borrow` として作るからで ... これが無いと借用版の
> 名前が入力の名前と衝突しうる。
...
`values_mut` も鍵を取り除かないことから出る (`<2>1`、`<2>3`)。

### 開発ビルドでだけ走る検査

README 第 4 節は、`develop_mode` のときだけ走る表明を「3 段目より弱い」とし、そこに **A9・A11・
A13・A21** を挙げる。**A3** の `applies_a_function_operand` の検査と、**A10** の「最適化が作る型の
再検査」も同じである。すなわち出荷ビルドでは、この **6 つ**の仮定は果たす者だけに立つ。

このうち A3 と A21 の検査は `Generator::apply_lambda` の develop mode の表明であり、`borrow_ify` と
`cancel` の外 -- コード生成の側 -- に在る。A10 の再検査は `validate_layouts` の側にある。残る
A9・A11・A13 の 3 つがこの 2 つのパスの側に在る。
...
挙がっているのが `check_clone_names_are_fresh` である。
...
T は P9・P14・P14a・P23・P24・P26・P27 の言明を引用する。それらが対象コミットに対してどこまで
...
その対応を 2 度たどってつなぐ (`<1>10` の `<2>5`)。コードの上の読み (`<2>1`、`<2>1a`、`<2>3`、
`<2>3a`) が足すのは鍵と名前についての事実だけであり、4 つの欄の等式は P24 の対応の側から出る。
