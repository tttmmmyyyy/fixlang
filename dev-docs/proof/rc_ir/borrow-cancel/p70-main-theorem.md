...
**この 1 段の外は P14b に依らない。**D24 が数え上げる活性化を作る 5 種のうち、(E3) は結論の側、
(E7) については `<1>7` の `<2>1b` の `<3>1` が D1 と D14 と D24 だけで同じことを与える。残る
3 種 -- (E1)、オペランドを適用する `Llvm` の段 ((E2))、(F) の `Destructor` -- がこの 1 段に
載っている。

**範囲の中へ入れる形は、P14b の量化範囲を広げることである。**「`borrow_ify` の出力と、`cancel` が
それを写したプログラム」-- A19 が (ii-a) と (ii-b) を仮定する範囲と同じ形 -- にすれば、`<1>7` の
`<2>1b` は P14b だけを引き、P24 の `cancel` の節は `borrowed_units` が `cancel` を通ることを述べる
`<1>7` の `<2>1a` にだけ要る。
