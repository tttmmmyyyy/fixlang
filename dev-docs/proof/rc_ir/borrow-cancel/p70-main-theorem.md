...
命題を持つので、検証の状態はファイルごとに 1 行だけ書く。**」と述べ、`report.md` の第 7.1 節の行に
検証の欄は無い。よって P26 の 2 行がいま述語のどちら側に在るかは、`report.md` の第 7.2 節の
`p50-observation.md` の行が決める。
...
**その述語が掛かるのは命題ではなくファイルである** (`report.md` の第 7.2 節)。T が引く命題は次の
