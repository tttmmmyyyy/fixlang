...
ある。実行の水準の定義 -- 環境 D22、活性化 D23、実行 D24、参照の持ち手 D25、対応する活性化 D29、対応する
2 つの実行と共通接頭 D30、実行の終わり方 D31、実行の読み D32 -- は README の第 3.6 節にある。
...
広がりの中に `InlineLLVMBoxedFromRetainedPtrIOS` の段が無いこと (同 項目 2)、そして実行の最初の時点に
環境が持ち込む値についての 3 つ (同 項目 3) である。
...
<1>3. 活性化が終わるのは、その位置が本体の終端の `Ret` に着き、その `Ret` の消費 (D9) を行う段である。
  BY D23
...
<1>5. 対応する 2 つの段は、同じ個数の活性化を同じ順に作る。`<1>2` の 5 種で場合を分ける。
  ...
  <2>2. (E3)。その段は 1 つの活性化を作る (D24)。呼び出し先は callee の値が指す関数であり、callee の値が
        ...
        意味で対応する。
    BY A13, D23, D24, D29, D30, L8, P12, P22, P24
...
<1>5. `p` までに `X` が実行した削除される `Retain` の実行 (活性化と節点の対) の全体を `T(p)`、削除される
  ...
  <2>1. `<1>1` と `<1>3` より、削除された節点以外の各段は 2 つの実行で同じ節点を同じ値に対して実行し
        ...
        よりその 3 つは 2 つの実行で対応する。よってこれらの段は `H` を同じだけ動かす。
...
<1>5. `H'(q, O) ≥ 1` である。
  ...
  実行 (D24) の各時点について D11a が与える。よって D11 の (S-c) より `O` は `q` の時点で解放されて
  いない。D7 より、解放されるのはカウントが 0 になったオブジェクトである。
...
<1>2. (X2) について。分岐が違う点を `p`、その op が参照カウントを読むオブジェクトを `O` とする。
  ...
  <2>2a. `H'(p, O) ≥ 1` である。`<2>0` の 4 か所はいずれも `Llvm` の op の生成コードの中にあり、`O` は
        ...
        の入力は `borrow_ify` の出力なので、P23 より `P'` は D12 を満たす。(S-c) の接頭条件 -- その活性化が
        その時点まで解放について閉じている (D11a) こと -- は実行 (D24) の各時点について D11a が与えるので、
        D11 の (S-c) より `O` は `p` の時点で解放されていない。D7 より、解放されるのはカウントが 0 に
        なったオブジェクトである。
...
**前提 (環境が持ち込む値)。** 以下 L9b から L13 は、次の 3 つを満たす実行についての主張である。D24 は
「C のエントリ点から始まる実行では、その時点に参照は 1 つも無い」と書き、**`FFI_EXPORT` のエントリ点が
boxed な引数を取る実行では在る**と書く。3 つが内容を持つのは後者の実行についてである。
...
3. **実行の最初の時点に環境が持ち込む値 (L9b、L10)。** D24 は「**`FFI_EXPORT` のエントリ点が boxed な
   引数を取る実行では在る**」と書き、A17 (i-c) がその参照を勘定に入れる。**A17 (i-d) はその半分を閉じた**
   -- そのオブジェクトから到達できるオブジェクトのクロージャの欄が名指す関数は、このプログラムの関数で
   ある。残るのは次の 3 つで、第 5 節の 前提 (環境が持ち込む値) の (P-1)(P-2)(P-3) がそれである。
