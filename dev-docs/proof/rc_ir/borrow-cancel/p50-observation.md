...
**言明**。`cancel` の入力 `P` を `borrow_ify` の出力とする。観測の事象 `q` (`DEF 観測の事象と観測の点`)
...
**言明**。D30 の共通接頭の段の各段内の点 `p` (`p51 の DEF 段の素動作と段内の点`) と計数下オブジェクト
...
`InlineLLVMBoxedFromRetainedPtrIOS` の節点の実行が無いこと) と 前提 (環境について残る 3 つ) の下で、
...
また L11 の仮定と 前提 (環境について残る 3 つ) の下で、観測の事象 `q` (`DEF 観測の事象と観測の点`) で
