...
入れた節点の動作を除いて**、2 つの実行で順序を保って 1 対 1 に対応する。共通接頭が終わる段については、
対応は最初に食い違う素動作の手前まで伸び、その食い違いは D30 の (X1)、(X2)、(X3) のいずれかである。
除外の節が要るのは、段の中で走る活性化にも借用版がありうるからである -- そこでは L8 の (a)(b)(c) の
節点が片方の実行にしか無い。D30 が段の列について同じ除外を置くのと同じ形である。
...
<1>2a. 対を持たない動作 -- `π` が消した節点と入れた節点の動作 -- は、言明が量化する範囲では活性化を作らず、
      ...
      すれば D30 の (X3) が共通接頭の出口に数え、段の中の動作についても 前提 (共通接頭の段の中の対応) より
      そこで対応が終わる。よってどちらもこの帰納を動かさない。`Retain`/`Release` の節点は本体の終端の
      `Ret` ではないので、活性化を終わらせもしない (D23)。
  BY D10, D23, D24, D30, L8, L8c, P22, 前提 (共通接頭の段の中の対応)
...
<1>3. 前提 (共通接頭の段の中の対応) より、2 つの段の素動作は順序を保って 1 対 1 に対応する。対応する
  ...
  <2>3. **解放。** 解放は、処分によって計数下オブジェクトの `H` が
    ...
    BY D24, D26, 前提 (共通接頭の段の中の対応), 帰納法の仮定
...
前提 (共通接頭の段の中の対応) の下で、D30 の共通接頭の段の各段内の点 `p`
...
**言明**。`cancel` の入力 `P` を `borrow_ify` の出力とする。前提 (段が作る活性化の決定性) と
前提 (共通接頭の段の中の対応) の下で、観測の事象 `q` (`DEF 観測の事象と観測の点`)
...
前提 (共通接頭の段の中の対応) の下で、D30 の共通接頭が終わるとき、その出口は (X1) か (X2)
...
<1>0. 出口が (X1) か (X2) であるとき、その分かれ目の直前の段内の点までは対応が伸びており、その点について
      ...
      その段の中に在るときは、前提 (共通接頭の段の中の対応) より、対応はその段の最初に食い違う素動作の
      手前まで伸びる。観測値を返す動作も、参照カウントを読んで分岐する動作も、その食い違いそのものなので、
      その直前の段内の点は対応の上にある。
  BY D30, 前提 (共通接頭の段の中の対応)
...
前提 (段が作る活性化の決定性)、前提 (共通接頭の段の中の対応) の下で、
...
<1>2. `<1>1` の後者ではない。すなわち `q` で `after` の `Release` 節点の位置にいる活性化は無い。
  ...
  <2>3. `after` の `Release` 節点はその形にならない。それは L8 の (c) の節点であって `borrow_ify` が
        ...
        ときは 前提 (共通接頭の段の中の対応) がその動作を除くので、どちらでも対を持たない。L8c の
...
前提 (共通接頭の段の中の対応) の下で、観測の事象 `q` (`DEF 観測の事象と観測の点`) で
...
2. **A3 の `Unknown` の行が `InlineLLVMBoxedFromRetainedPtrIOS` を覆っていないので、L11 が仮定を負う。**
   ...
   L12 と L13 が運ぶ。**参照の持ち手の側は枠が既に持っている** -- `boxed_to_retained_ptr` が渡した参照を環境が持つことは
...
7. **D30 の対応が段の粒度で止まっている (L5a、L6、L9、L12、L13)。** README の P26 は「範囲に入るのは、
   共通接頭の段のあいだの時点で起きる観測だけではない。共通接頭の段の**中で**起きる観測も入る」と述べ、
   `Std::FFI::Destructor` の `_dtor` が `unsafe_is_unique` を含みうることをその理由に挙げる。ところが
   D30 が定める対応は段の粒度である -- 「並んだ 2 つの段が同じ位置の節点を実行し、その段が作る値が
   等しく、その段が名指すオブジェクトが同じであるとき、対応は次の段へ伸びる」。**段が名指すオブジェクト**
   は「その段の中で解放されるオブジェクト」を含むので、対応する 2 つの段が解放するオブジェクトの
   **集合**は同じだが、**段の中のどの点でそれが解放されるか**は言明に無い。

   足りないのは次の 2 つである。

   - **素動作の順序。** 段の中の解放の点が 2 つの実行でずれると、一方が解放の走査とデストラクタの
     活性化を走らせている間に他方は節点の残りの処分を続けるので、素動作の列はそこで分かれる。集合が
     同じであることはこの形を弾かない。第 2 節の 前提 (共通接頭の段の中の対応) がこの一文であり、
     L5a がそれを読む。**`cancel` の半分については P21 (b) が節点の粒度で同じ内容を与える** --
     対応する 2 つの活性化は同じ位置で同じオブジェクトを解放する。`borrow_ify` の半分にはその者が居ない。
     **要る形は D30 の (X3) を「対応する 2 つの段が、対応する素動作で同じオブジェクトを解放する」に
     することである。**
   - **P28 の量化。** P28 の言明は「その実行 (D24) の各時点について」であり、段内の点を量化しない。
     D24 が「`p05-holders.md` がその形で P28 を示す」と書いて証明が素動作の粒度で進むことを述べるので、
     L9 の `<1>2` はそれを読んで段内の点で持ち手の分割を使う。**要る形は P28 の言明の「各時点」を
     「各時点と各段内の点」にすることである。**
