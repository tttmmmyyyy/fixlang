...
**言明**。`cancel` の入力 `P` を `borrow_ify` の出力とする。D30 の共通接頭の段の中の各観測の事象 `q`
(`DEF 観測の事象と観測の点`) について、`X` の `q` での観測値が真ならば、`X'` の `q` での観測値も真で
ある。参照カウントの量 `H(q, ・)` と `H'(q, ・)` は、`q` の観測の点 -- その事象が参照カウントを読む
直前の段内の点 -- で読む。
...
<1>1. `q` のオペランドの leaf `[]` が指すオブジェクトは `X` と `X'` で同じ `O` である。
  <2>1. CASE `q` が (O-a) である。D30 の**段が名指すオブジェクト**は「その段の節点が読む各変数の
        inhabited な各 boxed leaf が指すオブジェクト」を含み、`q` の節点は D7 の読む構文の表の `Llvm` の
        行であってそのオペランドを読む。共通接頭の上では対応する 2 つの段が名指すオブジェクトは同じで
        ある (D30)。
    BY D7, D30
  <2>2. CASE `q` が (O-b) である。D30 の段が名指すオブジェクトはその段の**節点**が読む値についての
        ものなので、段の中の節点が読むオブジェクトをそれから出すことはできない。代わりに L5a を読む --
        (a) より `q` を実行する 2 つの活性化は対応し、対応する位置にあり、(b) より対応する値の
        inhabited (D16) な各 boxed leaf が指すオブジェクトは対応する。`q` のオペランドの leaf `[]` は
        L0 の `<1>3a` よりその op が読むオブジェクトを指す leaf である。
    BY D16, L0, L5a
  <2>3. QED
    `DEF 観測の事象と観測の点` より事象は (O-a) と (O-b) の 2 種である。
    BY <2>1, <2>2
...
その動作が作る活性化の節点が行う (i)(ii)(iii)(v) も同じ段の中の動作である (L5a の `<1>1`)。以下の各段は、
...
D30 の共通接頭の段の中の各観測の事象 `q` (`DEF 観測の事象と観測の点`) について、その観測が読むオブジェクト
`O` が計数下 (D26) であるとき、`q` の観測の点で `Δ(q, O) := H'(q, O) − H(q, O) = 0` である。
...
<1>4. `c` から `a_q` への親子の道の各元は、L9a の (i)(ii)(iii)(v)(vi) のいずれかの活性化を作る動作で
      ...
      動作が作る活性化の節点が行う (i)(ii)(iii)(v) も同じ段の中の動作である (L5a の `<1>1`)。
  BY D24, L5a, L9a
...
また L11 の仮定と 前提 (環境について残る 3 つ) の下で、D30 の共通接頭の段の中の各観測の事象 `q`
(`DEF 観測の事象と観測の点`) において、`X` の観測値と `X'` の観測値は**等しい**。とくに `X` の観測値が
真ならば `X'` の観測値も真であり、D30 の (X1) の出口は起こらない。参照カウントの量は `q` の観測の点で
読む。

<1>1. `q` のオペランドの leaf `[]` が指すオブジェクトは `X` と `X'` で同じ `O` である。
  <2>1. CASE `q` が (O-a) である。D30 の**段が名指すオブジェクト**は、その段の節点が読む各変数の
        inhabited な各 boxed leaf が指すオブジェクトを含み、共通接頭の上では対応する 2 つの段が名指す
        オブジェクトは同じである。
    BY D7, D30
  <2>2. CASE `q` が (O-b) である。D30 の段が名指すオブジェクトはその段の**節点**が読む値についての
        ものなので、段の中の節点が読むオブジェクトをそれから出すことはできない。代わりに L5a を読む --
