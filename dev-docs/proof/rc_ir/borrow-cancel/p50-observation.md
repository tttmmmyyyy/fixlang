...
<1>2. 段の最初の段内の点で (a)(b)(c) が成り立つ。その点は D24 の時点である
      (D24)。共通接頭が終わる段についても、その手前の各段は対応しているので
...
<1>3. DEF 共通接頭の段の中の対応 より、2 つの段の素動作は順序を保って 1 対 1 に対応する。対応する
  ...
  <2>1. **参照の受け渡し・生成・処分。** どの参照がどこへ動くかは、D9 の消費・移動の表と D10 の生成の表が
        ...
        union の各節の実行時のタグと、その leaf が null ポインタであるかである。前者は D29 の
        「**スカラの成分については、対応は等号である。** boxed leaf でない成分 -- unbox union のタグ、
        整数、浮動小数 -- は、対応する 2 つの値で等しい」が、`borrow_ify` については L8c の同じ形の節が
        与える。後者は capture が空のクロージャの capture の leaf であること (D16、A5) であり、値が
        対応するので 2 つの活性化で一致する。よって同じ leaf について同じ向きの動作が起きる。
        持ち手 (D25) も対応する。
    BY A5, D9, D10, D16, D24, D25, D29, L8, L8c, P22, P24, 帰納法の仮定
...
<1>5. `H'(q, O) ≥ 1` である。
  ...
  ある時点ではどの節点も `o` を名指せない」を言うので、`q` の観測の点 -- その事象が参照カウントを読む
  直前の段内の点 -- が段の中の点であっても読める。`<1>1a` より `O` は計数下 (D26) なので D8 の意味の参照を
...
<1>2. (X2) について。分岐が違う段内の点を `p`、その op が参照カウントを読むオブジェクトを `O` とする。
  ...
  <2>2a. `H'(p, O) ≥ 1` である。`<2>0a` より `O` はその分岐を含む op のオペランドの inhabited な boxed
        ...
        「`H(o) = 0` である時点ではどの節点も `o` を名指せない」を言うので、`p` が段の中の点であっても
        読める。`<2>1` より `O` は計数下 (D26) なので D8 の意味の参照を持ちうる対象であり、この節は
