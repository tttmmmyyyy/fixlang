...
<1>1. `q` のオペランドの leaf `[]` が指すオブジェクトは `X` と `X'` で同じ `O` である。
  D30 より、共通接頭の対応する段は同じオブジェクトを名指す。
  BY D30
...
<1>5. 場合は 2 つに分かれる。**(A)** `G` を名指すクロージャの値の funptr 欄を、この実行のある段が書いた。
      **(B)** その値は、実行の最初の時点に環境が持ち込んだオブジェクトから到達できる (D25) オブジェクトの
      クロージャの欄が持っていたものである。場合 (A) では、その欄に書き込む段のうち実行の中で最も早い
      ものを `s` とする。
  `<1>3` よりその段が適用する値はクロージャ型であり、`<1>4` よりその funptr 欄に書き込むのは 2 か所の
  生成コードだけであって、環境の段はこの欄を書かない。よってこの値の funptr 欄の内容は、この実行のある段が
  書いたものか、この実行が始まる前から在った記憶域の内容である。後者を持つオブジェクトは、環境が持ち込んだ
  オブジェクトから到達できる -- A17 (i-c) より、実行の最初の時点に生きている各計数下オブジェクトは
  少なくとも 1 つの持ち手を持ち、その時点に在る参照は環境が持つか、環境が持ち込んだオブジェクトの leaf が
  持つので、持ち手を辿ると環境が持つ参照に着く (D25)。
  BY A17, D25, <1>3, <1>4

<1>5a. 場合 (B) では `g ∈ closure_targets` である。
  BY 前提 (P-1), <1>5

<1>6. 場合 (A) で `s` が `InlineLLVMFixBody` の段であるならば、`g ∈ closure_targets` である。
  ...
  <2>5. その値がクロージャ型ならば、その funptr 欄は `G` を名指す。その欄に `<1>5` の場合分けを当てると、
        場合 (A) ではその欄に書き込む段が `b` が作られるより前にあり、`s` は `b` の段なので `b` が作られた
        後にあって、`s` の最小性に反する。場合 (B) では `<1>5a` より `g ∈ closure_targets` である。
    BY <1>5, <1>5a
  <2>6. QED
...
<1>7. QED
  場合 (B) では `<1>5a` が結論を与える。場合 (A) では、`s` が `InlineLLVMFixBody` の段であれば `<1>6` が
  結論を与え、そうでなければ `<1>4` より `s` は `build_rc_closure` の段であり、それは `Closure(g, caps)`
  節点の評価である。その節点は出力の `funcs` のある本体か `globals` のある初期化子の中にあるので、
  `<1>1a` より `g ∈ closure_targets` である。
...
<1>1. 参照の持ち手 (D25) のうち、生きているオブジェクトが持つ分と環境が持つ分は、2 つの実行で
  ...
  <2>1. オブジェクトが持つ分。A5 より、生きているオブジェクトの inhabited な各 boxed leaf のうち計数下の
        オブジェクトを指すものが、ちょうど 1 つの参照を持つ。`<1>0` より生きているオブジェクトは 1 対 1 で
        あり、L8c より対応するオブジェクトは対応する値を保持し、inhabited であるかと計数下であるかの区別も
        一致する。
    BY A5, D26, L8c, <1>0
...
<1>3. 対応する生きている活性化の対 `(a, a_in)` について、`B(a)` が借用版でないならば次が成り立つ。
  ...
  <2>5. 返りの段 (E4)。`a` が作った子が終端の `Ret` に着き、D9 の終端の `Ret` の行が消費する参照が
        ...
        のときは、D24 の (E4) より `a` はその段の同じ位置で続きを実行する。
  ...
  <2>6b. デストラクタを走らせる動作 (L9a の (vi))。`a` の段が参照を処分して `Std::FFI::Destructor` の
        ...
        この動作は `_dtor` 欄の関数に `build_retain` で参照を 1 つ作り、`_value` 欄と `_dtor` 欄の値を
        取り出して適用する。作られた参照の持ち手はこの段を実行している活性化であり、`_dtor` の適用が
        それを消費する。**`<1>0` より共通接頭の上では 2 つの実行が同じ段で同じオブジェクトを解放するので、
        この動作は 2 つの実行で同じ段で起こり、同じ参照を作り同じ参照を消費する。**よって
        `Obl'(a) − Obl(a_in)` は変わらない。
    BY L9a, D10, D24, <1>0, CODE src/generator.rs: Generator::build_run_destructor
  <2>7. QED
...
<1>3. (I2) は各段で保たれる。オブジェクトに参照を書き込む動作は 2 つある。
  ...
  BY A18, D24, D25, <1>1a, <1>2, CODE src/generator.rs: Generator::build_run_destructor
...
真であり、(X2) では入力が複製を作り出力が作らない」である。**第 2 文が量化するのは `π` が `cancel` の
ときである** (README の P26)。
...
- **(X2) の向きは `cancel` と逆になりうる。** L7a の `<1>2` が `cancel` について使ったのは
  ...
  高い。`unique_check_operand` を宣言する op は `funcs_observing_uniqueness` の門が閉じる対象ではないので
  (門が読むのは `observes_uniqueness` である)、窓の中でカウントの高い側が複製を作る形は排除されていない。
- **(X3) の出口 -- 同じオブジェクトを 2 つの実行が違う段で解放する。** `borrow_ify` は借用した参照の処分を
...
3. **D30 の (X3) が、対を持たない段の解放と、片方の実行だけの解放を覆わない。** 第 2 節の
   前提 (解放の一致) がこれである。D30 は共通接頭が終わる点を 3 種類と数え、(X3) を「同じオブジェクトを
   2 つの実行が**違う段で**解放する」と書く。この文書が要るのは、**対を持たない段 -- `π` が消した節点と
   入れた節点の段 -- がオブジェクトを解放する場合**と、**片方の実行だけが解放する場合**にも、対応が
   その手前で終わっていることである。解放はその段の動作を変える -- 走査が解放されるオブジェクトを読み
   (D32 の (読み-2))、そのオブジェクトが `Std::FFI::Destructor` であればデストラクタの活性化を作り、
   返った値を `_value` の欄へ書き戻す (D24 の (F))。要る形は「2 つの実行が対応する段で同じオブジェクトを
   解放しないとき、対応はそこで終わる」である。読むのは L5 の `<1>5`、L8c の `<1>2a` と `<1>2b`、L9 の
   `<1>0` である。**`cancel` の半分では P21 (b) がこれを果たす** -- 対応する 2 つの活性化は同じ位置で
   同じオブジェクトを解放する。残るのは `borrow_ify` の半分である。果たす者: 誰も。

4. **実行の最初の時点に環境が持ち込むオブジェクト (L9b、L10)。** D24 は「**`FFI_EXPORT` のエントリ点が
   boxed な引数を取る実行では在る**」と書き、A17 (i-c) がその参照を勘定に入れる。そのオブジェクトは
   この実行のどの段も割り当てておらず、それが持つクロージャの funptr 欄をこの実行のどの段も書いていない
   ので、L9b の `<1>5` (適用される値の funptr 欄を書いた段が在る) と L10 の `<1>6a` の `<2>2`
   (`Destructor` のオブジェクトを割り当てた段が在る) が閉じない。門の側にはこの穴は開いていない --
   `builds_a_destructor` は各関数のパラメータ・capture の型と返り値の型、各グローバルの型も見るので、
   `Destructor` を `FFI_EXPORT` の引数として受け取るプログラムでは真になる
   (`CODE src/rc_ir/borrow.rs: builds_a_destructor`)。閉じないのは、そのオブジェクトが持つクロージャが
   名指す関数がこのプログラムの `closure_targets` に載る保証が無いことである。この文書は
   「実行の最初の時点に環境が持つ参照が無い」を前提に置く。果たす者: 誰も。

5. **D30 の (X2) と P26 の第 2 文が書く「複製」が、4 つ目の分岐に当たらない。** (X2) は「カウントが 1 の
   ときこの op は複製を作らず、そうでないとき複製を作る」と書き、P26 の第 2 文は「(X2) では入力が複製を
   作り出力が作らない」と書く。ところが L7a の `<1>2` の `<2>0` が数え上げる 4 つの分岐のうち、
   `InlineLLVMArrayAppendCapacityUnchecked` が `src` について立てる分岐では**どちらの腕も新しい
   オブジェクトを割り当てない** -- 共有の腕は要素を 1 つずつ retain して `dst` の末尾へ写し、一意の腕は
   `memcpy` で移す (`CODE src/fixstd/builtin.rs: InlineLLVMArrayAppendCapacityUnchecked`)。L7a が示すのは
   「入力が共有の腕を、出力が一意の腕を取る」であり、要る形はその向きである。果たす者: 誰も。

6. **(X2) の内部の一意性検査は A3 が扱っている。** `LLVMGen::unique_check_operand` が宣言する一意性検査は、
   ...
   `Fresh` を読むのは L10 の `<1>6a` の `<2>3` と L11 の `<1>2` の `<2>1` である。前者は宣言される leaf の
   型だけを読むのでこの但し書きに当たらず、後者は但し書きの場合を別に扱っている。

7. **引用と `// PROOF:` の対応。** この文書が引く記号はすべて実在し、
   `python3 dev-docs/proof/proof_links.py --write` が `citations.tsv` と各記号の `// PROOF:` の欄を
   この文書の引用から作る。存在しない記号を 1 つ足すと指摘を出すことを確かめた。

## 13. この文書が読んだコードの版

対象コミット `4e7da73927595f2780b8a8d8198a7f4d200aa8ec` の `src/` を読んだ。
