...
L8 はこの補題を引かないので、この補題の言明と `<1>4`・`<1>4a`・`<2>2`・`<2>3` がそれを引くことで循環は
生じない。L8 の言明は
...
L8c はこの補題を引かないので、`<1>2`・`<1>2a`・`<2>1`・`<2>2a`・`<2>3` が L8c を引くことで循環は
生じない。
...
<1>3. DEF 共通接頭の段の中の対応 より、2 つの段の素動作は順序を保って 1 対 1 に対応する。対応する
      ...
      **記憶域への書き込みは場合ではない。** D24 は「**オブジェクトの記憶域への書き込みは、この 6 種の
      ...
      この補題に名指す。`<2>2a` が、付随して書き込まれる内容が対応することを与える。
...
<1>2a. **帰納法の仮定 -- その段より前の各時点でこの補題の言明が成り立つこと -- の下で**、節点の実行以外に
...
オブジェクトは最初の時点より前に割り当てられている (A17 の (i-d))」と続ける。この補題が要るのはその後者、
