...
D30 の段の対応で対応する段で始まり対応する段で終わるものをいう。ただし (F) の解放が作る活性化は自分の段を
持たないので (D23)、それを起こした解放が対応することで対応する。`X'` の活性化 `a` に対応する `X` の
活性化を `a_in` と書く。この全単射が存在してただ 1 つであることは L5 が示す。

## L5 (対応する活性化は 1 対 1 である)

**言明**。D30 の共通接頭の上で、`DEF 対応する活性化` の全単射はただ 1 つ存在する。とくに共通接頭の各時点で
生きている `X` の活性化と `X'` の活性化は 1 対 1 である。

<1>1. 活性化を作る段は 5 種で尽きる -- (E1)、(E3)、(E7)、(E2) のうちオペランドを適用する `Llvm` の段、
      および (F) の解放が `Destructor` について作る段である。(F) の解放が作る活性化は 2 つである。
  BY D24

<1>2. (F) の解放が作る活性化を除く各活性化は、自分の段を 1 つ以上持ち、その最初の段はその本体の根の節点を
      実行する段である。活性化の位置は本体の根から始まり、節点を 1 つ実行するごとに進む (D23)。D2 より
      本体は節点を 1 つ以上持つ。(F) の解放が作る活性化だけが自分の段を持たない (D23、D24 の (F))。
  BY D2, D23, D24

<1>3. 1 つの活性化は自分の本体の根の節点を 1 度だけ実行する。D21 より活性化が節点を訪れる順序は D3 の意味の
      実行路の 1 つであり、D2 より本体は有限の木で節点が自分自身を含むことはないので、実行路は各位置を
      高々 1 回通る。
  BY D2, D3, D21

<1>4. (F) の解放が作る活性化を除く活性化は、D30 の段の対応で 1 対 1 に並ぶ。D30 は段を先頭から順に
      1 対 1 に並べ、並んだ 2 つの段が同じ位置の節点を実行することを共通接頭の条件に置く。`X` の活性化
      `a_in` が始まる段 `s` に対応する `X'` の段を `s'` とすると、`<1>2` より `s` は `a_in` の本体の根の
      節点を実行する段であり、`s'` は同じ位置の節点を実行するので、`<1>3` より `s'` は同じ本体のある
      活性化 `a` の最初の段である。逆向きも同じであり、`s` から `s'` を引くのは D30 の段の対応なので、
      この対応は全単射である。
  BY D30, <1>2, <1>3

<1>5. (F) の解放が作る活性化は、前提 (解放の一致) より対応する。共通接頭の上では 2 つの実行は対応する段で
      同じオブジェクトを解放するので、解放されるオブジェクトが `Std::FFI::Destructor` であるかどうかも
      一致し、その解放が作る 2 つの活性化 (`<1>1`) が順に対応する。
  BY 前提 (解放の一致), <1>1

<1>6. 対応する 2 つの活性化は対応する段で終わる。活性化が終わるのは、その位置が本体の終端の `Ret` に着き、
      その `Ret` の消費を行うときである (D23)。`<1>4` よりその段も 2 つの実行で対応する。
  BY D23, <1>4

<1>7. QED
  `<1>4` と `<1>5` が `<1>1` の 5 種を尽くし、`<1>6` が終わる段の対応を与える。生きている活性化とは
  始まって終わっていないものなので (D23)、共通接頭の各時点で生きている活性化はこの全単射で 1 対 1 で
  ある。(F) の解放が作る活性化はそれを起こした段の中で終わるので (D24 の (F))、D24 のどの時点でも
  生きていない。
  BY D23, D24, <1>1, <1>4, <1>5, <1>6
