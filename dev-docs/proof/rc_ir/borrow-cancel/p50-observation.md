...
あって、解放を行うならば対応するオブジェクトを解放する。共通接頭が終わる段については、対応は最初に
食い違う素動作の手前まで伸び、その食い違いは D30 の (X1)、(X2)、(X3) のいずれかである。

**これは仮定ではなく D30 が定めるものである。** D30 は共通接頭を「伸びる限りの部分」として定め、その
伸びる条件に素動作の列の対応を含める。細かくした分は (X3) の出口に落ちるので、果たす者は要らない。
