...
<1>5. 対応する 2 つの段は、同じ個数の活性化を同じ順に作る。作られた活性化のうち D24 の時点で生きて
  ...
  <2>5. (F) の解放が `Destructor` について作る段。D30 の (X3) より、共通接頭の上では 2 つの実行は対応する
        ...
        どうかも一致し、作られる活性化は 2 つずつである (`<1>2`)。**この 2 つについて言明が本体の対応を
        要らないのは、その活性化が D24 のどの時点でも生きていないからである** -- `<1>3` よりその活性化は
        それを起こした解放を含む段の中で終わり、`<1>2` よりその同じ段の中で作られる。段の中の点へ
        伸ばした全単射がこの 2 つの本体の対応を要り、L5a の `<2>5` がそこを扱う。
    BY D24, D30, <1>2, <1>3
  <2>6. QED
