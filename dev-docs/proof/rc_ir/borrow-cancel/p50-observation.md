...
<1>4a. 言明が述べる位置の対応が成り立つ。
      ...
      節点の対応 (D19 と P22) で並べ、節点を実行しない段 -- (E1)・(E5)・(E7)・(E8) -- は列の中の位置で
      ...
      述べる。(E5)・(E6)・(E8) はどの活性化の位置も動かさない。対を持たない段は一方の実行にしか無く
...
<1>5. 対応する 2 つの段は、同じ個数の活性化を同じ順に作る。作られた活性化のうち D24 の時点で生きて
  ...
  <2>3. (E7)。その段は 1 つの活性化を作る (D24)。その段が起きるのは、まだ初期化されていないグローバルを
        ...
        **本体も対応する。** (E7) の段は節点を実行しないので、D30 は「**節点を実行する段は節点の対応
        (D19 と P22) で並べ、節点を実行しない段 -- (E1)・(E5)・(E7)・(E8) -- は列の中の位置で並べる。**」
...
<1>3. DEF 共通接頭の段の中の対応 より、2 つの段の素動作は順序を保って 1 対 1 に対応する。対応する
  ...
  <2>1. **参照の受け渡し・生成・処分。** どの参照がどこへ動くかは、D9 の消費・移動の表と D10 の生成の表が
        ...
        **節点が 2 つの表の外で出す retain と release もこの場合に入る。** D24 は「**この網羅は段の境界に
        ...
        ある**」、「相殺しないもの」の持ち手は「**その参照の持ち手は、割り当てたオブジェクトの欄である**」で
        ある。対応する 2 つの `Llvm` 節点は同じ op を持ち (`borrow_ify` については L8 の (2)、`cancel` に
        ...
        **環境が呼ぶ retain と release もこの場合に入る。** A17 (ii-c) は「**その呼び出しは D24 の段で
        あり、その番地が指すオブジェクトへの参照を、retain なら 1 つ作って環境の持ち分に足し、release なら
        環境の持ち分から 1 つ処分する。**」と述べる。その段は節点を実行しないので、D30 の伸長の条件
        「並んだ 2 つの段が同じ位置の節点を実行するか同じ種の節点を持たない段であり」より対になった 2 つの
        段はどちらもこの種であり、番地を環境へ渡したのは `boxed_to_retained_ptr` の節点である
  ...
  <2>2a. **素動作に付随する書き込み。** D24 より書き込みはその値を渡した素動作に付随して起き、書き込みと
        ...
        書き込む内容は対応する。節点の実行以外の書き込みは 2 つ -- (F) の解放が `_value` の欄へ書き戻す
        分と、環境の書き込みの段 (E8) -- であり、`borrow_ify` については L8c の `<1>2a` がそれを尽くす。
        `cancel` については D29 の最後の行 (対応するオブジェクトが保持する値は対応する) がこの節を含む。
        よって (b) の「対応する値を保持する」が保たれる。
    BY A4, D21, D24, D29, D30, L8c, <2>1, <2>2, 帰納法の仮定
...
<1>4. `H` を動かす動作は次で尽きる。列挙は D24 の (E2) の 2 つの表 -- `Obl(a)` を離れる参照の行き先の表と
      ...
      - **op の生成コードが出す retain と release**。上の 4 つは段の境界についての網羅であり、D24 は
        ...
        **相殺しないもの** -- 「複製を作る腕が複製の欄へ retain する形である (`make_struct_union_unique` の
        ...
        上げる」。
      - **環境が呼ぶ retain と release**。A17 (ii-c) は「**環境は `get_funptr_retain` と
        `get_funptr_release` が渡した番地を呼びうる**」と述べ、「**その呼び出しは D24 の段であり、その番地が
        指すオブジェクトへの参照を、retain なら 1 つ作って環境の持ち分に足し、release なら環境の持ち分から
        1 つ処分する。**」と続ける。よって `H` はその 1 つ分だけ上下する。
      ...
      **A17 (ii-c) の呼び出しの段は (E1)-(E8) のどれとも名指されていないので、上の列挙は別の行としてそれを
      持つ** (第 12 節の項目 7)。
...
<1>5. `p` までに `X` が実行した削除される `Retain` の実行 (活性化と節点の対) の全体を `T(p)`、削除される
  ...
  <2>1. `<1>1` と `<1>3` より、削除された節点以外の各節点は 2 つの実行で同じ種類・同じ path であり、
        ...
        **環境が呼ぶ retain と release も同じである** (`<1>4`) -- その呼び出しの段は節点を実行しないので、
        D30 の伸長の条件「並んだ 2 つの段が同じ位置の節点を実行するか同じ種の節点を持たない段であり」より
        対になった 2 つの段はどちらもこの種であり、番地を環境へ渡したのは `boxed_to_retained_ptr` の節点で
        ある (D22 の第 3 の箇条、A17 (i-b))。その節点は `<1>1` より 2 つの実行の対応する位置に同じ op と
        して在り、`<1>2a` より対応する値を与えられるので、2 つの段は対応するオブジェクトへ同じ向きの
        1 つを作る / 処分する。
        環境の書き込みの段 (E8) は `H` を動かさない (`<1>4`)。よってこれらの動作は `H` を同じだけ動かす。
...
<1>2a. 出口は (X3) ではない。(X3) は「対応する 2 つの段の素動作の列が対応しない」であり、対応する 2 つの
  ...
  <2>2. 対になった段の素動作の列は、順序まで対応する。対になった段が実行するのは削除された節点ではない
        ...
        **節点を実行しない段も対になりうる。** D30 は「**節点を実行する段は節点の対応 (D19 と P22) で
        並べ、節点を実行しない段 -- (E1)・(E5)・(E7)・(E8) -- は列の中の位置で並べる。**」と定め、伸長の
        条件「並んだ 2 つの段が同じ位置の節点を実行するか同じ種の節点を持たない段であり」が対になった
        2 つを同じ種に揃える。A17 (ii-c) の呼び出しの段もこの形である (第 12 節の項目 7)。その段の
        素動作は `<2>3` が扱う。
  ...
  <2>3. 対になった段では、対応する 2 つの動作の一方だけが解放することも、対応しないオブジェクトを解放する
    ...
    <3>4. QED
      段の素動作の列は、その段が実行する節点ごとの列と、節点の実行に属さない動作からなる。後者は 4 つで
      ある -- (E1) が活性化の初期 `Obl` へ渡す参照、(E4) の返りが呼び出し元へ渡す参照、(E7) の返りが `E` へ
      渡す参照、そして (E5) のグローバル化である。どれも受け渡しかグローバル化であって `H` を動かさないので、
      解放を起こさない (D24 の (E1)、(E4)、(E5)、(E7))。
      **A17 (ii-c) の呼び出しの段は節点を実行しないので、その素動作の列は全体がこの後者である。**
      D30 の伸長の条件「並んだ 2 つの段が同じ位置の節点を実行するか同じ種の節点を持たない段であり」より、
      対になった 2 つの段はどちらもこの種である。番地を環境へ渡したのは `boxed_to_retained_ptr` の節点で
      ...
      BY A17, D7, D22, D24, D30, L5a, P19, P20, P21, <2>1, <2>2, <3>1, <3>3
  <2>4. QED
...
<1>0. 基底。実行の最初の時点で、`X` のオブジェクトと `X'` のオブジェクトは 1 対 1 に対応し、対応する 2 つは
      ...
      boxed leaf でない成分のうちスカラは等しい。funptr の番地については、A17 (i-d) より、そのオブジェクト
      から到達できるオブジェクトのクロージャの欄が名指す関数はそのプログラムの関数であり、L8 の (1) より
      `P'` の各関数は `P` のある関数の版である。環境がどちらの実行にも同じ入力を与え、その入力が名指す
      オブジェクトが 1 対 1 に対応する (D30) ので、対応する 2 つの欄は入力の同じ関数の版を名指す。
...
<1>2a. **帰納法の仮定 -- その段より前の各時点でこの補題の言明が成り立つこと -- の下で**、節点の実行以外に
      オブジェクトの記憶域へ書き込む動作は 2 つであり、共通接頭の上ではどちらも 2 つの実行の対応する段で
      起きて対応する内容を書き込む。
      ...
      オブジェクトの記憶域へ書き込まない。
  BY A4, A17, D22, D24, D30, <1>1, 帰納法の仮定,
...
<1>3. (E1) が (iv)、(E7) が (v) である。D22 は環境を 4 つに分け、そのうちグローバルのアクセサについて
      ...
      その動作は D24 の (E8) の段であり、D24 はそれを活性化を作る段に数えない。残る 2 つが (E1) である。
  BY D22, D24
...
<1>4. クロージャの値の funptr 欄 (`CLOSURE_FUNPTR_IDX`) に書き込む生成コードは 2 か所だけである --
      ...
      `InlineLLVMFixBody::generate_tail` が `gc.current_function()` の番地を入れる。環境の段 ((E8)) も
      この欄を書かない。
  ...
  <2>4a. 環境の書き込みの段 (E8) はこの欄を書かない。A17 (ii-b) は「**Fix の関数型の値の funptr の欄も
        書かない。**」と述べる。
    BY A17
  <2>5. QED
...
<1>1. 参照の持ち手 (D25) のうち、生きているオブジェクトが持つ分と環境が持つ分は、2 つの実行で
  ...
  <2>1. オブジェクトが持つ分。A5 より、生きているオブジェクトの inhabited な各 boxed leaf のうち計数下の
        ...
        leaf を 1 組ずつ持つ。**`<1>0` より生きているオブジェクトは 1 対 1 であり、L8c と L5a の (b) より
        対応する
  ...
  <2>2. 環境が持つ分。`<1>0a` より環境が作る活性化の本体は借用版ではないので、L8 の (1) よりそれは `f_own`
        ...
        **グローバルの初期化の段 (E7) も参照を `E` へ渡す** -- 初期化子の活性化の終端の `Ret` が消費する
        ...
        するのは計数下オブジェクト `O` である。
        **環境が自分で参照を作り、自分で処分する経路がもう 1 つある。** A17 (ii-c) は「**環境は
        `get_funptr_retain` と `get_funptr_release` が渡した番地を呼びうる**」と述べ、「**その呼び出しは
        D24 の段であり、その番地が指すオブジェクトへの参照を、retain なら 1 つ作って環境の持ち分に足し、
        release なら環境の持ち分から 1 つ処分する。**」と続ける。その段は節点を実行しないので、D30 の
        伸長の条件「並んだ 2 つの段が同じ位置の節点を実行するか同じ種の節点を持たない段であり」より
        対になった 2 つの段はどちらもこの種である。番地を環境へ渡したのは `boxed_to_retained_ptr` の節点で
...
<1>3. 対応する生きている活性化の対 `(a, a_in)` について、`B(a)` が借用版でないならば次が成り立つ。
  ...
  <2>3. L8 の (a)(b)(c) 以外の節点の段。L8 よりこの段は 2 つの実行で同じ種類・同じ path の節点であり、
        ...
        **op の生成コードが出す retain と release も `Obl(a)` を動かす。** D24 は「**素動作の粒度で勘定する
        ...
        「**その参照の持ち手は、割り当てたオブジェクトの欄である**」と書くので、そちらは `Obl(a)` を
        動かさない。よって `Obl'(a) − Obl(a_in)` は変わらない。
  ...
  <2>7. QED
    ...
    **A17 (ii-c) の呼び出しの段も環境の段であって `a` の段ではない** -- A17 (ii-c) はその段が作る参照を
    環境の持ち分に足し、処分する参照を環境の持ち分から取ると述べるので、どの活性化の `Obl` も動かない。`a` が
...
<1>3. (I2) は各節点の実行で保たれる。オブジェクトに参照を書き込む動作は 3 つある。
      ...
      2 つ目は **op の生成コードが、この段が割り当てたオブジェクトの欄へ出す retain** である。D24 は
      ...
      共有の腕、`clone_array_buf`)」を挙げ、「**その参照の持ち手は、割り当てたオブジェクトの欄である**」と
      書く。書き込む先はこの段が割り当てたオブジェクトであり、その割り当ては広がりの中の節点の実行で
      起きるので (`<1>1a`)、`t0` より後である。書き込まれる参照が指すのは、複製の元 -- その op の
      ...
      **A17 (ii-c) の呼び出しの段もここに数えない。** その段が作る参照も処分する参照も持ち手は環境で
      あって、どのオブジェクトの欄でもない (A17 (ii-c))。
  ...
  BY A8, A17, A18, D6, D9, D24, D25, D26, <1>1a, <1>2,
     CODE src/generator.rs: Generator::build_run_destructor
...
7. **A17 (ii-c) の呼び出しの段が段の種の一覧に無い (L5a、L6、L7a、L9、L11)。** A17 (ii-c) は
   「**その呼び出しは D24 の段であり、その番地が指すオブジェクトへの参照を、retain なら 1 つ作って環境の
   持ち分に足し、release なら環境の持ち分から 1 つ処分する。**」と述べる。ところが D24 は「段は (E1) から
   (E8) の 8 種である」と書いてこの段を (E1)-(E8) のどれとも名指さず、D30 も「**節点を実行する段は節点の
   対応 (D19 と P22) で並べ、節点を実行しない段 -- (E1)・(E5)・(E7)・(E8) -- は列の中の位置で並べる。**」で
   この段を挙げない。よって段の種を数え上げる段と、節点を実行しない段の対応を読む段は、この段を自分で
   足す。**この文書はそう書いた** -- L6 の `<1>4` が `H` を動かす行として、L9 の `<1>1` の `<2>2` が
   環境が持つ分を動かす経路として、L5a の `<1>3` の `<2>1` と L7a の `<1>2a` の `<2>2` が素動作の場合
   として、L11 の `<1>3` が参照を書き込まない段として挙げる。**足す先が 2 か所あるので、一覧に無いことは
   1 か所を直しても閉じない。** 果たす者: 無し (足す側の作業である)。
