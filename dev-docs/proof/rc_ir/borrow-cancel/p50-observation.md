...
L8 と L8c はこの補題を引かないので、`<2>1` から `<2>5` がその 2 つを引くことで循環は生じない。L8 の言明は
2 つの実行を読まない、節点についての静的な言明であり、その証明が引くのは README の定義・仮定・命題と、
L8a・L8b と、`borrow_ify` のコードだけである。L8c は D30 の共通接頭の各時点についての不変量であり、その
証明が引くのは README の定義・仮定・命題と、L8 と、`borrow_ify` のコードだけである。
...
<1>5. 対応する 2 つの段は、同じ個数の活性化を同じ順に作る。作られた活性化のうち D24 の時点で生きて
  ...
  <2>3. (E7)。その段は 1 つの活性化を作る (D24)。その段が起きるのは、まだ初期化されていないグローバルを
        ...
        **本体も対応する。** 読む者が生きている活性化であるときは、対応する 2 つの段が同じ位置の節点を
        実行する (D30) ので、2 つは同じ記号のグローバルを読む。読む者が環境であるときは、D30 が 2 つの
        実行を環境が与える入力を同じにして取るので、やはり同じ記号を読む (D24 の (E7) -- 「読む者は、
        `g` を読む節点の位置にある生きている活性化 `a` か、**環境**である」)。作られる活性化の本体は
    ...
    BY D22, D24, D30, L8, P22, P24, DEF 共通接頭の段の中の対応, DEF 対応する本体
...
<1>7. QED
  ...
  **対応する 2 つが対応する本体を持つことは `<1>5` が与える** -- 全単射が覆うのは D24 の時点で生きて
  いる活性化であり、`<1>5` はまさにその範囲について本体の対応を述べる。
