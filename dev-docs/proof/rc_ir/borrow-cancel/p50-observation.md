...
2. **A3 の `Unknown` の行が `InlineLLVMBoxedFromRetainedPtrIOS` を覆っていないので、L11 が仮定を負う。**
   ...
   運ぶ。**要るのは仮定ではなく命題である** -- 「`unsafe_get_retained_ptr_of_boxed_value` が渡した参照は
   環境 (D25) が持ち、`boxed_from_retained_ptr` はそれを活性化へ移す」を示せば、L12 の `<2>3` は
   「`E` の外の活性化は `O` への参照を持たない」を別の道で得る。果たす者: 誰も。
...
3. **環境について残る 3 つ (L9b、L10)。** D24 は「**`FFI_EXPORT` のエントリ点が boxed な
   ...
   - **(P-2)** L10 の `<1>6a` は `builds_a_destructor(prog)` を要る。環境が持ち込んだ `Destructor` の
     ...
     `mentions_a_destructor`)。**要る形は A17 に「環境が持ち込むオブジェクトは、このプログラムの
     生成コードが割り当てたものである」を足すこと**である。そこから `Destructor::_make` の `Let` が
     このプログラムに在ることが出る。
...
入った変更は、`borrow_ify` と `cancel` を `pub fn` から `pub(crate) fn` に替えたことと `// PROOF:` の
コメントだけであり、生成されるコードは同じである。同じ二値で第 6 節・第 7 節・第 8 節の
