...
**言明**。D30 の共通接頭の各時点で、`X` の生きている活性化と `X'` の生きている活性化の間に、対応する 2 つが
対応する本体 (`DEF 対応する本体`) を持ち、対応する段で作られ、対応する段で終わる全単射がちょうど 1 つ存在
する。これが `DEF 対応する活性化` の全単射である。
...
**言明**。`cancel` の入力 `P` を `borrow_ify` の出力とする。D30 の共通接頭の各時点 `p` と各計数下オブジェクト
