...
<1>3. 対応する生きている活性化の対 `(a, a_in)` について、`B(a)` が借用版でないならば次が成り立つ。
  ...
  <2>6b. デストラクタを走らせる動作 (L9a の (vi))。`a` の段が参照を処分して `Std::FFI::Destructor` の
        ...
        解放の走査が処分する ((F))。**L5a より共通接頭の段では 2 つの実行が対応する素動作で同じ
        オブジェクトを解放するので、この動作は 2 つの実行で対応する点で起こり、同じ参照を作り、同じ参照を
        渡し、同じ参照を消費する。**よって `Obl'(a) − Obl(a_in)` は変わらない。
    BY L5a, L9a, D9, D10, D24, <1>0, CODE src/generator.rs: Generator::build_run_destructor
  <2>7. QED
