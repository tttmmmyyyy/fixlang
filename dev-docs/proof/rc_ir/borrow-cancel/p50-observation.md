...
活性化 D29、対応する 2 つの実行と共通接頭 D30、実行の終わり方 D31、実行の読み D32 -- は README の
第 3.6 節にある。
...
第 6 節の反例を引く。この 5 節は、対象コミットに無いコードについての記録であるから、構造化証明の形 (`<k>n` の
...
<1>5. 対応する 2 つの段は、同じ個数の活性化を同じ順に作る。`<1>2` の 5 種で場合を分ける。
  ...
  <2>4. (E2) のうちオペランドを適用する `Llvm` の段。この段は活性化を 1 つ作るごとに区切られ、1 つの節点が
        ...
        いくつ在るかと、それぞれが渡す値は、その op の生成コードが決める (D24)。A3 と A4 より生成コードの
        振る舞いは、その段の節点とオペランドの値と D21 の 4 種の結果だけで決まり、D30 より対応する 2 つの
        段はその 3 つを共有する。
    BY A3, A4, D21, D24, D30
...
<1>2a. 共通接頭の各時点で、`X` の生きている活性化と `X'` の生きている活性化は D30 の対応で 1 対 1 であり、
  ...
  どうしの 1 対 1 を作るのは L5 である。値が対応することから、その値の
  各 unbox union の節のタグも対応し、D16 の inhabited は実行時のタグだけで決まる。
  BY D16, D21, D29, D30, L5, DEF 対応する活性化
...
<1>5. `p` までに `X` が実行した削除される `Retain` の実行 (活性化と節点の対) の全体を `T(p)`、削除される
  ...
  <2>1. `<1>1` と `<1>3` より、削除された節点以外の各段は 2 つの実行で同じ節点を同じ値に対して実行し
        ...
        書き込む先へ渡すか処分するかは、その op の生成コードが決める。A3 と A4 より、生成コードが
        書き込む内容も渡す値も、その段の節点とオペランドの値と D21 の 4 種の結果だけで決まり、`<1>2a`
        よりその 3 つは 2 つの実行で対応する。環境の書き込みの段 (E8) は `H` を動かさない (`<1>4`)。
        よってこれらの段は `H` を同じだけ動かす。
    BY A3, A4, D21, D30, <1>1, <1>2a, <1>3, <1>4
  <2>2. 実際に作る / 処分する参照の個数が inhabited な leaf の分であることは P6 である。
    BY P6
...
<1>2. (X2) について。分岐が違う点を `p`、その op が参照カウントを読むオブジェクトを `O` とする。
  ...
  <2>3. QED
    ...
    `X'` は一意の腕を取る。**腕によって何が違うかは op ごとである** (D30 の (X2))。`<2>2` より、
...
4. **引用と `// PROOF:` の対応。** この文書が引く記号はすべて実在し、
