...
L8 はこの補題を引かないので、`<1>4a` と `<2>2`・`<2>3` がそれを引くことで循環は生じない。L8 の言明は
...
<1>4a. 対応する 2 つの生きている活性化は、**対を持つ節点の位置にあるときは対応する位置 (D23) にある**。
      位置の対応は、`borrow_ify` については L8 の (2) の節点の対応、`cancel` については P22 と P24 が
      与えるものである。**対を持たない節点 -- `π` が消した節点と入れた節点、すなわち `Retain` と
      `Release` -- の位置にあるあいだは、一方の位置だけがその節点の列の分だけ先へ出ている。**
...
<1>5. 対応する 2 つの段は、同じ個数の活性化を同じ順に作る。作られた活性化のうち D24 の時点で生きて
  ...
  <2>2. (E3)。その段は 1 つの活性化を作る (D24)。呼び出し先は callee の値が指す関数であり、callee の値が
        ...
        2 つの値の
        funptr の番地は互いの版を名指し、2 つの呼び出し先は入力の同じ関数についての `cancel` の
        像どうしである。`borrow_ify` については、書き換えが `App` の callee に
...
L8c はこの補題を引かないので、`<1>2`・`<2>2a`・`<2>3`・`<2>5` が L8c を引くことで循環は生じない。
