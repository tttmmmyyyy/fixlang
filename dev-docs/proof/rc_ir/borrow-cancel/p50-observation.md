...
<1>4. `H'(q, O) ≤ 1` である。`<1>1a` より `O` は計数下であり、`q` の観測の点は共通接頭の段の段内の点で
      ある。
  BY L6, <1>1a, <1>3
...
<1>1. (X1) について。観測値が違い、かつ `X` の観測値が真であるとすると、L7 より `X'` の観測値も真になり、
      違うことに反する。よって `X` の観測値は偽であり、違うのだから `X'` の観測値は真である。**観測の
      事象が段の中で起きる場合 ((O-b)) も同じである** -- L7 はその事象も量化し、`<1>0` よりその観測の点は
      対応の上にある。
