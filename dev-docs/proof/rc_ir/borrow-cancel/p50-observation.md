...
<1>2. `Δ(p, O) := H'(p, O) − H(p, O)` は、対応する生きている活性化の対 `(a, a_in)` を渡る和
  ...
  **P28 の持ち手の分割は段内の点についても読む。** P28 の言明は D24 の時点を量化するが、D24 は
  「(F) が作る活性化の中で起きる割り当て・`Retain`・`Release`・入れ子の解放も、この段の一部である。
  … 追うには段を不可分な動作の有限列へ分解する -- 参照の受け渡し、生成、割り当て、処分、解放、
  グローバル化の 6 種であり、`p05-holders.md` がその形で P28 を示す」と書き、P28 の証明がその粒度で
  進むことを述べる。よって分割は素動作と素動作のあいだの各点で成り立つ。第 12 節の項目 7 が報告する。
...
<1>6. CASE 道が (v) を持つ。この場合 `Δ(q, O) = 0` である。
  ...
  <2>3. `E` の外の活性化 `a` について `Obl'(a)(O) = Obl(a_in)(O) = 0` である。`t0` で `O` はまだ
    ...
    BY D11, D12, D23, D24, D25, L9, L9a, <1>1a, <2>2
