...
<1>5. 対応する 2 つの段は、同じ個数の活性化を同じ順に作る。作られた活性化のうち D24 の時点で生きて
  ...
  <2>1. (E1)。D30 は 2 つの実行を、環境が与える入力を同じにし、複数の制御の流れがある場合は段の並び方も
        ...
        対応する 2 つの (E7) の段であり (D24 の (E7))、D30 の「その段が作る値が等しく」と帰納法の仮定より
...
<1>5. `H'(q, O) ≥ 1` である。
  ...
  `o` への参照を持つ活性化は 1 つも無く、どの節点も `o` を名指せない」と述べる。この節は、`H(o) = 0` で
  ...
  合わせて `H'(q, O) ≥ 1` である。
  BY D7, D8, D24, D25, D26, L0, <1>1, <1>1a
...
<1>2. (X2) について。分岐が違う段内の点を `p`、その op が参照カウントを読むオブジェクトを `O` とする。
  ...
  <2>2a. `H'(p, O) ≥ 1` である。`<2>0a` より `O` はその分岐を含む op のオペランドの inhabited な boxed
        ...
        時点で `o` への参照を持つ活性化は 1 つも無く、どの節点も `o` を名指せない」と述べる。この節は、
        ...
        ある。
    BY D7, D8, D24, D25, D26, L0, <2>0, <2>0a, <2>1
  <2>3. QED
