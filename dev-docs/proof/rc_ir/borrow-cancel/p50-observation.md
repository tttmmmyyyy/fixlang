...
<1>3. QED
  **基底は `<1>0`** -- 実行の最初の時点である。**帰納段**は次のとおりである。1 つの段の対が進む間に対応を
  ...
  対応する段は同じ値を与えられ、同じオブジェクトを名指す。`<1>1` よりその 2 つの段が書き込む内容は、帰納法の
  仮定の下で等しく、これは boxed leaf の指す先についても、boxed leaf でないスカラの成分についても言う。`<1>2` と
  ...
  BY A4, A13, A21, D26, D30, L8, P12, P24, <1>0, <1>1, <1>2, <1>2a, <1>2b, 帰納法の仮定,
