...
較正表は P26 の節を較正するバグとして #551 を挙げ、第 6 節がその反証である。D18 が第 6 節の反例を名指す。この 5 節は、対象コミットに無いコードについての記録であるから、構造化証明の形 (`<k>n` の
...
読むのは L5 の `<2>4`、L5a の `<2>5`、L6 の `<1>5` の `<2>1` である。第 12 節の項目 4 が報告する。
...
共通接頭の各段について、その段の素動作 (`p51 の DEF 段の素動作と段内の点`) は、**`π` が消した節点と
...
L8 はこの補題を引かないので、`<2>2` が L8 を引くことで循環は生じない -- L8 の言明は 2 つの実行を読まない、
節点についての静的な言明であり、その証明が引くのは README の定義・仮定・命題と、L8a・L8b と、
`borrow_ify` のコードだけである。
...
<1>5. 対応する 2 つの段は、同じ個数の活性化を同じ順に作る。共通接頭が終わる段については、対応が伸びる
      素動作の接頭の上でそう言う (DEF 共通接頭の段の中の対応)。`<1>2` の 5 種で場合を分ける。
  ...
  <2>3. (E7)。その段は 1 つの活性化を作る (D24)。その段が起きるのは、まだ初期化されていないグローバルを
    ...
    BY D22, D24, D30, DEF 共通接頭の段の中の対応
...
**言明**。前提 (段が作る活性化の決定性) と DEF 共通接頭の段の中の対応 の下で、D30 の共通接頭の段の
...
<1>1a. D30 の共通接頭の各段の最初の段内の点において、対応する 2 つの活性化の対応する位置で、対応する変数
  ...
  <2>3. 活性化の入力の束縛 (D23)。活性化を作る段は 5 種である (D24 の「活性化の林」)。(E1) では、D30 が
        ...
        `Llvm` の段では、前提 (段が作る活性化の決定性) より渡る値はその段の節点とオペランドの値と D21 の
        4 種の結果だけで決まり、オペランドの値は帰納法の仮定が、4 種の結果は対応する段が作る値なので
        D30 が与える。(E7) が作る活性化は入力の束縛を持たない (D1)。(F) の解放が作る活性化は自分の段を
        持たないので (`<1>1`)、次の段の最初の点では生きていない。
    BY D1, D21, D23, D24, D30, 前提 (段が作る活性化の決定性), <1>1, 帰納法の仮定
  <2>4. QED
    BY <2>1, <2>2, <2>3

<1>2. 段の最初の段内の点で (a)(b)(c) が成り立つ。その点は D24 の時点である
      (`p51 の DEF 段の素動作と段内の点`)。共通接頭が終わる段についても、その手前の各段は対応しているので
...
<1>3. DEF 共通接頭の段の中の対応 より、2 つの段の素動作は順序を保って 1 対 1 に対応する。対応する
      動作の対についての帰納で (a)(b)(c) を運ぶ。以下 `<2>1` から `<2>4` が、`p51 の DEF 段の素動作と
      段内の点` が挙げる 6 種の素動作を尽くし、`<2>5` が活性化の生成と終了を、`<2>6` が (c) を扱う。
  ...
  <2>1. **参照の受け渡し・生成・処分。** どの参照がどこへ動くかは、D9 の消費・移動の表と D10 の生成の表が
        ...
        が、`cancel` については P22 と P24 が与える。** 帰納法の仮定
  ...
  <2>3. **解放。** 解放は、処分によって計数下オブジェクトの `H` が
        ...
        2 つずつである (D24 の「活性化の林」)。渡る値は `_dtor` 欄と `_value` 欄の値であり、(b) より
        対応する。
    BY D24, D26, D30, DEF 共通接頭の段の中の対応, 帰納法の仮定
  ...
  <2>5. **活性化の生成と終了。** 段の中で活性化を作る動作は、`<2>3` の解放と、`<1>1` よりその活性化と
        ...
        **オペランドを適用する `Llvm` の動作。** 前提 (段が作る活性化の決定性) より、その個数と順序と
        渡す値は、その節点とオペランドの値と D21 の 4 種の結果だけで決まる。節点と値は帰納法の仮定 (a)(b)
        が、**D21 の 4 種の結果は D30 が**対応させる -- D30 は対応が次の段へ伸びる条件に「その段が作る値が
        等しく、その段が名指すオブジェクトが同じであり」を含める。
        ...
        **グローバルの初期化の動作。** 1 つの実行が 1 つのグローバルについて作る初期化子の活性化は
        ...
        DEF 共通接頭の段の中の対応 よりそこで対応が終わる。

        活性化が終わるのは本体の終端の `Ret` の消費であり (D23)、帰納法の仮定 (a) より
        対応する 2 つは対応する位置にあるので同時に終わる。よって (a) が保たれる。
    BY A13, D21, D22, D23, D24, D29, D30, L8, P12, P22, P24, 前提 (段が作る活性化の決定性),
       DEF 共通接頭の段の中の対応, <1>1, 帰納法の仮定
  <2>6. **1 本の道 (c)。** `<1>1` より段の中で作られる活性化はその段の中で終わり、それを作った活性化は
    ...
    BY D24, <1>1
  <2>7. QED
    `<2>1` から `<2>4` が `p51 の DEF 段の素動作と段内の点` の 6 種の素動作を、`<2>5` が活性化の生成と
...
**言明**。`cancel` の入力 `P` を `borrow_ify` の出力とする。前提 (段が作る活性化の決定性) と
DEF 共通接頭の段の中の対応 の下で、D30 の共通接頭の段の各段内の点 `p`
(`p51 の DEF 段の素動作と段内の点`。共通接頭が終わる段のうち対応が伸びる段内の点を含む) と各計数下
...
<1>4. `H` を動かす動作は次で尽きる。列挙は D24 の (E2) の 2 つの表 -- `Obl(a)` を離れる参照の行き先の表と
      ...
      **この列挙は段の中の動作にも当たる。** `p51 の DEF 段の素動作と段内の点` は 1 つの段を参照の
      ...
      定まる。
  BY A17, D9, D10, D24, p51 の DEF 段の素動作と段内の点
...
<1>5. `p` までに `X` が実行した削除される `Retain` の実行 (活性化と節点の対) の全体を `T(p)`、削除される
  ...
  <2>1. `<1>1` と `<1>3` より、削除された節点以外の各節点は 2 つの実行で同じ種類・同じ path であり、
        ...
        コードが決める。A4 より書き込む内容は、前提 (段が作る活性化の決定性) より作る活性化に渡す値は、
    ...
    BY A3, A4, D21, D30, L5a, <1>1, <1>2a, <1>3, <1>4, 前提 (段が作る活性化の決定性)
...
**言明**。`cancel` の入力 `P` を `borrow_ify` の出力とする。前提 (段が作る活性化の決定性) と
DEF 共通接頭の段の中の対応 の下で、2 つの実行の対になった観測の事象 `q`
...
<1>5. `H'(q, O) ≥ 1` である。
  `q` の節点は D7 の読む構文の表の `Llvm` の行であり、`O` は `q` のオペランドの inhabited な leaf が指す
  オブジェクトである。この命題の前提より `cancel` の入力は `borrow_ify` の出力なので、P23 より `P'` は
  D12 を満たす。(S-c) の接頭条件 -- その活性化がその時点まで解放について閉じている (D11a) こと -- は、
  D11a が「実行 (D24) の各時点と各段内の点で閉じていることは `p51-runs.md` の `L3` が示す」と書くことによる。その
  `L3` が要る「すべての本体が D11 を満たす」は P23 が与える。**この条件は段内の点についても読める** --
  `p51 の DEF 段の素動作と段内の点` が D11a の 2 つの語を段内の点について読み直し、その読みが D11a の
  読みを狭めないことを述べる。よって D11 の (S-c) より、`O` は `q` の観測の点 -- その読みの直前の点 --
  で解放されていない。**そこから `H'(q, O) ≥ 1` を出すのは D7 の第 2 文 --「`H(o)` が 0 になった
  オブジェクトは**解放される**」-- の対偶である。** その向きが段内の点の粒度で読めるのは、D24 の (F) が
  「**解放は、それを起こした処分に付随して起きる。** 処分とその処分が起こす解放のあいだに段内の点は
  挟まらない」と述べるからである -- `H` が 0 でありながらまだ解放されていない段内の点は無い。
  BY P23, D7, D11, D11a, D12, D24, p51 の DEF 段の素動作と段内の点, <1>1
...
**言明**。`cancel` の入力を `borrow_ify` の出力とする。前提 (段が作る活性化の決定性) と
DEF 共通接頭の段の中の対応 の下で、D30 の共通接頭が終わるとき、その出口は (X1) か (X2)
...
<1>2. (X2) について。分岐が違う段内の点を `p`、その op が参照カウントを読むオブジェクトを `O` とする。
  ...
  <2>2a. `H'(p, O) ≥ 1` である。`<2>0a` より `O` はその分岐を含む op のオペランドの inhabited な boxed
        ...
        D11a を段内の点について読み直すので、`p` が段の中の点であってもこの条件は読める。よって D11 の
        (S-c) より `O` は `p` で解放されていない。**そこから `H'(p, O) ≥ 1` を出すのは D7 の第 2 文 --
        「`H(o)` が 0 になったオブジェクトは**解放される**」-- の対偶である。** その向きが段内の点の
...
<1>2a. 出口は (X3) ではない。(X3) は「対応する 2 つの段の素動作の列が対応しない」であり、対応する 2 つの
  ...
  <2>2. 対になった段の素動作の列は、順序まで対応する。対になった段が実行するのは削除された節点ではない
        ...
        leaf のものである。書き込む内容は A4 の決定性の節が、作る活性化の個数・順序・渡す値は
        前提 (段が作る活性化の決定性) が一致させる。
    BY A4, D9, D10, D16, D29, D30, P22, P24, 前提 (段が作る活性化の決定性)
  <2>3. 対になった段では、対応する 2 つの動作の一方だけが解放することも、対応しないオブジェクトを解放する
    ...
    <3>3. 1 つの節点の実行について、その素動作の列の接頭の上の帰納で次の 2 つを同時に示す。(i) その点で
      ...
      BY D7, D24, D25, L5a, p51 の DEF 段の素動作と段内の点, <2>2, <3>1, <3>2
    <3>4. QED
      段の素動作の列は、段の中で実行される節点ごとの列の連結である (`p51 の DEF 段の素動作と段内の点`)。
      各節点の実行について `<3>3` が結論を与え、節点と節点のあいだでは `<3>1` が (i) を置き直す。対を
      持たない節点の実行 -- `cancel` が消した `Retain`/`Release` -- は `<2>1` より解放を起こさない。
      BY p51 の DEF 段の素動作と段内の点, <2>1, <3>1, <3>3
  <2>4. QED
...
- **(2)** 出力の各本体と、それが作られた入力の本体との違いは次の 4 つだけである。
  ...
  `Llvm` 節点は同じ op を持ち、その op は `result_prov`・`borrows_operand`・`applies_a_function_operand` に
  同じ答えを返す。**
...
<1>3. QED
  **基底は `<1>0`** -- 実行の最初の時点である。**帰納段**は次のとおりである。1 つの段の対が進む間に対応を
  ...
  仮定の下で等しく、これは boxed leaf の指す先についても、boxed leaf でない成分についても言う。`<1>2` と
  ...
  計数下かグローバル状態かの区別が一致することは、
  その遷移を起こすのが (E5) の段だけであり (D26)、対応する 2 つの (E5) の段が印を付けるのが、対応する
  初期化子の活性化が返した値の到達するグラフだからである -- 返す値は D30 より対応し、そこから到達する
  グラフが対応することは帰納法の仮定が与える。
...
<1>5. **`G` を名指すクロージャの値であって、その funptr 欄の内容をこの実行のどの段も書いていないものが
  ...
  **クロージャの値は unbox の対であり、その funptr の欄はレジスタに在りうる** -- D4 の規則 2 はクロージャの
  ...
  この実行が始まる前から在った記憶域から読み出されたものである。その記憶域を持つオブジェクトは実行の最初の時点に生きて
  ...
  BY A17, D4, D9, D10, D22, D24, D25, D26, 前提 (P-1), <1>4,
...
**言明**。前提 (段が作る活性化の決定性) と DEF 共通接頭の段の中の対応 の下で、D30 の共通接頭の段の
各段内の点 `p` (`p51 の DEF 段の素動作と段内の点`。共通接頭が終わる段の
...
<1>2. `Δ(p, O) := H'(p, O) − H(p, O)` は、対応する生きている活性化の対 `(a, a_in)` を渡る和
  ...
  **P28 の持ち手の分割は段内の点についても読む。** P28 の言明は「その実行 (D24) の**各時点と各段内の点**
  (`p51-runs.md` の `DEF 段の素動作と段内の点`) について」と量化するので、分割は素動作と素動作のあいだの
  各点で成り立つ。第 12 節の項目 7 が同じことを書く。
  BY A1, A20, D8, D12, D24, D25, P14, P14b, P28, p51 の DEF 段の素動作と段内の点, <1>0, <1>1
...
<1>7. QED
  ...
  位置と定めるので、`h` の本体にも観測点がある。よって `<1>1a` と
...
前提 (段が作る活性化の決定性)、DEF 共通接頭の段の中の対応 の下で、
...
また L11 の仮定、前提 (環境について残る 3 つ)、前提 (段が作る活性化の決定性)、
DEF 共通接頭の段の中の対応 の下で、2 つの実行の対になった観測の事象 `q`
...
5. **A23 が名指す数え上げはこの文書に在る (穴ではない)。** README の A23 と「検証状況」の節はどちらも、
...
7. **D30 と P28 は段内の点まで届いている (L5a、L6、L9、L12、L13)。** README の P26 は「範囲に入るのは、
