...
<1>0. 基底。実行の最初の時点で、`X` のオブジェクトと `X'` のオブジェクトは 1 対 1 に対応し、対応する 2 つは
      ...
      boxed leaf でない成分も等しい。その時点にグローバル状態のオブジェクトは無いので (D26 -- 計数下から
...
<1>3. QED
  **基底は `<1>0`** -- 実行の最初の時点である。**帰納段**は次のとおりである。1 つの段の対が進む間に対応を
  ...
  書くのは `build_rc_closure` と `InlineLLVMFixBody` であり (A21)、前者が入れるのは `Closure` 節点が名指す
  関数の番地で、その `FuncRef` は書き換えが変えない (P24)。計数下かグローバル状態かの区別が一致することは、
  ...
  BY A4, A21, D26, D30, L8, P24, <1>0, <1>1, <1>2, <1>2a, <1>2b, 帰納法の仮定
