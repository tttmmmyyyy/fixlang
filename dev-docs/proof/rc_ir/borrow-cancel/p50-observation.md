...
<1>2a. 出口は (X3) ではない。(X3) は「対応する 2 つの段の素動作の列が対応しない」であり、対応する 2 つの
  ...
  <2>3. 対になった段では、対応する 2 つの動作の一方だけが解放することも、対応しないオブジェクトを解放する
        こともない。`<2>2` よりその段は削除された `Retain`/`Release` を実行しないので、その段のあいだ
        P21 (a) の欠損 `k(O)` は動かず、その段の各点で `H'(・, O) = H(・, O) − k(O)` である。P18a より
        `k(O) ≥ 1` である位置では `H(・, O) ≥ 1 + k(O)`、すなわち `H'(・, O) ≥ 1` である。よって `X'` の
        側で `H'` が 0 になる処分では `k(O) = 0` であり、その点で `H' = H` である。逆に `X` の側で `H` が
        0 になる処分では、`H' = −k(O)` と `H' ≥ 0` (D7) から `k(O) = 0` であり、やはり `H' = H` である。
        よって `<2>2` の同じ順に並ぶ処分の列の上で、`X` の第 `j` 番目の処分が解放を起こすことと `X'` の
        第 `j` 番目が起こすことは同値であり、`<2>2` よりその 2 つは対応する leaf の処分なので、解放される
        のは対応するオブジェクトである。P21 (b) が同じことを節点の粒度で述べる。
    BY D7, D24, P18a, P21, <2>2
  <2>4. QED
