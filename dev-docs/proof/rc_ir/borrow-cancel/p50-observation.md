...
<1>5. 対応する 2 つの段は、同じ個数の活性化を同じ順に作る。作られた活性化のうち D24 の時点で生きて
  ...
  <2>1. (E1)。D30 は 2 つの実行を、環境が与える入力を同じにし、複数の制御の流れがある場合は段の並び方も
        ...
        長さで第 `i` 要素の `symbol` と `ty` は入力のものに等しい (P24)。その値の funptr の成分に
        ついては、`cancel` では D29 の「**funptr の番地は等号では読めない** -- 2 つのプログラムは別々に
        コンパイルされるので … **その成分については、対応する 2 つの番地が対応する版を名指す**」が、
        `borrow_ify` では L8c の同じ形の節が、対応する版を名指すことを与える。よって作られる 2 つの
        活性化の本体は `DEF 対応する本体` の意味で対応する。
    BY D22, D29, D30, L8c, P24, DEF 対応する本体
  ...
  <2>4. (E2) のうちオペランドを適用する `Llvm` の段。この段は活性化を 1 つ作るごとに区切られ、1 つの節点が
        ...
        **本体も対応する。** この段が適用するのは Fix の関数型の値であり (D24 の (E2))、渡す値がその
        3 つで決まってその 3 つが 2 つの実行で対応するので、適用される 2 つの値も対応する。その funptr の
        成分については、`cancel` では D29 の「**funptr の番地は等号では読めない** -- 2 つのプログラムは
        別々にコンパイルされるので … **その成分については、対応する 2 つの番地が対応する版を名指す**」
        が、`borrow_ify` では L8c の同じ形の節が、対応する版を名指すことを与える。よって作られる 2 つの
        活性化の本体は `DEF 対応する本体` の意味で対応する。
    BY A3, A4, D21, D24, D29, D30, L8c, DEF 対応する本体
...
<1>3. DEF 共通接頭の段の中の対応 より、2 つの段の素動作は順序を保って 1 対 1 に対応する。対応する
  ...
  <2>5. **活性化の生成と終了。** 段の中で活性化を作る動作は、`<2>3` の解放と、`<1>1` よりその活性化と
        ...
        **オペランドを適用する `Llvm` の動作。** A4 の決定性の節 (「**…その段が作る子の活性化の
        ...
        Fix の関数型の値であり (D24 の (E2))、渡す値がその 3 つで決まってその 3 つが対応するので 2 つの
        値は対応し、その funptr の成分については D29 の「**funptr の番地は等号では読めない** …
        **その成分については、対応する 2 つの番地が対応する版を名指す**」と L8c の同じ形の節が、対応する
        版を名指すことを与える。
