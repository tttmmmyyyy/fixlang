...
<1>5. 対応する 2 つの段は、同じ個数の活性化を同じ順に作る。作られた活性化のうち D24 の時点で生きて
  ...
  <2>1. (E1)。D30 は 2 つの実行を、環境が与える入力を同じにし、複数の制御の流れがある場合は段の並び方も
        ...
        その 2 つの値は対応する。値の funptr の成分については、`cancel` では D29 の「**funptr の番地は
        等号では読めない** -- 2 つのプログラムは別々にコンパイルされるので … **その成分については、
        対応する 2 つの番地が対応する版を名指す**」が、`borrow_ify` では L8c の同じ形の節が、対応する版を
        名指すことを与える。よって作られる 2 つの活性化の本体は `DEF 対応する本体` の意味で対応する。
    BY D22, D24, D29, D30, L8c, P24, DEF 対応する本体, 帰納法の仮定
  <2>2. (E3)。その段は 1 つの活性化を作る (D24)。呼び出し先は callee の値が指す関数であり、callee の値が
        ...
        P24)、D29 の「**funptr の番地は等号では読めない** -- 2 つのプログラムは別々にコンパイルされるので
        … **その成分については、対応する 2 つの番地が対応する版を名指す**」より、対応する 2 つの値の
        funptr の番地は互いの版を名指す。よって 2 つの呼び出し先は入力の同じ関数についての `cancel` の
  ...
  <2>4. (E2) のうちオペランドを適用する `Llvm` の段。この段は活性化を 1 つ作るごとに区切られ、1 つの節点が
        ...
        **本体も対応する。** この段が適用するのは Fix の関数型の値であり、それはこの節点のオペランドの
        ...
        帰納法の仮定の下で対応する。値の funptr の成分については、`cancel` では D29 の「**funptr の番地は
        等号では読めない** -- 2 つのプログラムは別々にコンパイルされるので … **その成分については、
        対応する 2 つの番地が対応する版を名指す**」が、`borrow_ify` では L8c の同じ形の節が、対応する版を
        名指すことを与える。よって作られる 2 つの活性化の本体は `DEF 対応する本体` の意味で対応する。
    BY A3, A4, D21, D24, D29, D30, L8c, DEF 対応する本体, 帰納法の仮定
...
<1>3. DEF 共通接頭の段の中の対応 より、2 つの段の素動作は順序を保って 1 対 1 に対応する。対応する
  ...
  <2>3. **解放。** 解放は、処分によって計数下オブジェクトの `H` が
        ...
        **その成分については、対応する 2 つの番地が対応する版を名指す**」と L8c の同じ形の節が、対応する
        ...
        が渡す値は `<2>1` の受け渡しの場合であって帰納法の仮定 (b) より対応し、その funptr の成分にも
        同じ 2 つの節が当たる。よって 2 つ目の活性化の本体も `DEF 対応する本体` の意味で対応する。
  ...
  <2>5. **活性化の生成と終了。** 段の中で活性化を作る動作は、`<2>3` の解放と、`<1>1` よりその活性化と
        ...
        **呼び出しの動作。** 呼び出し先は callee の値が指す関数であり、callee の値がクロージャなら
        ...
        `Closure` 節点の `FuncRef` も変えず (P22、P24)、D29 の「**funptr の番地は等号では読めない** …
        **その成分については、対応する 2 つの番地が対応する版を名指す**」より、対応する 2 つの値の funptr の
        ...
        **オペランドを適用する `Llvm` の動作。** A4 の決定性の節 (「**…その段が作る子の活性化の
        ...
        対応させる。値の funptr の成分については D29 の「**funptr の番地は等号では読めない** …
        **その成分については、対応する 2 つの番地が対応する版を名指す**」と L8c の同じ形の節が、対応する
        版を名指すことを与える。
    ...
    BY A4, A13, D21, D22, D23, D24, D29, D30, L8, L8c, P12, P22, P24,
       DEF 共通接頭の段の中の対応, DEF 対応する本体, <1>1, 帰納法の仮定
...
6. **D30 と P28 は段内の点まで届いている (L5a、L6、L9、L12、L13)。** README の P26 は「範囲に入るのは、
   ...
   - **P28 の量化。** P28 の言明は「その実行 (D24) の**各時点と各段内の点**について」であり、L9 の
