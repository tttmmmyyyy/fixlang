...
- **数え落としを直した形は A19 (ii) を果たさない。** 直した形は `p13-disposals-and-pending.md` の反例 `C2`
  が満たすので、`C2` が破る A19 (ii) を導けない (第 5 節)。すなわち `(P-insert)` は、強すぎる読みでは
...
- **R2** (第 5 節)。`Fut` から未実行の `Retain` の分を引いた形 `(P-insert-net)` は、
  `p13-disposals-and-pending.md` の反例 `C2` が満たす。よって `(P-insert-net)` から A19 (ii) は出ない。
...
**R2**。`(P-insert-net)` から A19 (ii) は出ない。
...
ある。`L5` よりその同じ `C2` が `(P-insert-net)` を満たす。よって `(P-insert-net)` を満たしながら
A19 (ii) を破る入力が在り、前者から後者は出ない。∎

**この 2 つが挟んでいるもの。** `(P-insert)` は `insert_rc` の出力について偽であり (R1)、数え落としを
直した `(P-insert-net)` は `C2` を弾かない (R2)。すなわち、この形の言明には A19 (ii) の
`insert_rc` の側を担わせる強さと正しさが同時には無い。第 6 節が、担わせるべき形を書く。
...
**この不等式は `held` の推移だけからは決まらない。** `U` と `X` は、走査が持つ `outstanding` の鍵 --
`origin` の `identity` -- と、処分の構文が渡す `acted_on` が名前を共有するかで決まる。`C2` は `held` の
推移としては `(P-insert-net)` を満たしながら (`L5`) この不等式を破るので、`held` だけを主語にする言明が
A19 (ii) を導かないことは `L5` と `L7` の 2 つが示している。
