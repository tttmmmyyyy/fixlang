...
- speedtest 51 本 合計 277.829 G -> 275.055 G (**-1.00%**)。最大は cp_lib_bipartite の -3.30%
- 実プロジェクトのコーパス 17 本 合計 452.319 G -> 445.708 G (**-1.46%**)、退行 0 本。
  最大は `fix` 自身の 312.607 G -> 306.416 G (**-1.98%**)

見込んだ 1-2 割より小さい。LLVM の `-O3` パイプラインは早い段階で自前の大域デッドコード除去を
通すので、死んだ関数は高い最適化に届く前に消えていた。減ったのは **Fix が LLVM IR を組み立てる分と、
LLVM がそれを読んで検証する分**である。
