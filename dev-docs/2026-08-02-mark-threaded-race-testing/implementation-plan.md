...
x86-64 では**この 3 形が同じ命令に落ちる**。`lock` 前置の read-modify-write は `release` でも `acq_rel` でも同一の命令で、acquire load は plain `mov`、acquire fence は命令を生まない（アセンブリに出る `#MEMBARRIER` はコメントで、コード生成器への並べ替え禁止の印にすぎない）。単離した release 経路と is_unique 経路は、逆アセンブルしたバイト列が完全に一致した。
...
**プログラム全体では、fence を消したことで空になる基本ブロックの削除が別に効く。** release の生成箇所ごとに分岐が 1 本消えるので、`--threaded` ビルドの静的命令数は `-O none` で 84 減り、`-O basic` と `-O max` では最適化器の判断がずれて両方向に動く（それぞれ +80、+8。バイナリサイズは 3 レベルとも同一）。実行時コストは測っていない。

**`--threaded` を付けないビルドは影響を受けない。** ordering の変更も空ブロックの削除も threaded 経路の生成箇所の内側にあるためで、speedtest の 7 ケースを 2 つの最適化レベルでビルドして `.text` がバイト単位で一致することを確認した。

arm64 は実機がないため、確認は命令列の突き合わせまでとする。
